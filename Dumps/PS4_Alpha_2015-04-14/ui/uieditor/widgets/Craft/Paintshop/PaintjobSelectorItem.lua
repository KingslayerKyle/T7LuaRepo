...
CoD.PaintjobSelectorItem.new = function ( menu, controller )
	...
	attachmentImage:setRGB( 1, 1, 1 )
	attachmentImage:setAlpha( 0 )
	attachmentImage:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_normal" ) )
	self:addElement( attachmentImage )
	self.attachmentImage = attachmentImage
	
	local drawPaintjobLeftView = LUI.UIElement.new()
	drawPaintjobLeftView:setLeftRight( true, true, 4, -4 )
	drawPaintjobLeftView:setTopBottom( true, true, 4, -4 )
	drawPaintjobLeftView:setRGB( 1, 1, 1 )
	drawPaintjobLeftView:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_normal" ) )
	drawPaintjobLeftView:subscribeToGlobalModel( controller, "Customization", "type", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			drawPaintjobLeftView:setupPaintjobCustomizationType( modelValue )
		end
	end )
	drawPaintjobLeftView:linkToElementModel( self, "paintjobSlot", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			drawPaintjobLeftView:setupPaintjobSlot( modelValue )
		end
	end )
	drawPaintjobLeftView:linkToElementModel( self, "paintjobIndex", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			drawPaintjobLeftView:setupPaintjobIndex( modelValue )
		end
	end )
	self:addElement( drawPaintjobLeftView )
	self.drawPaintjobLeftView = drawPaintjobLeftView
	...
	self.clipsPerState = {
		DefaultState = {
			DefaultClip = function ()
				...
				drawPaintjobLeftView:completeAnimation()
				self.drawPaintjobLeftView:setAlpha( 1 )
				self.clipFinished( drawPaintjobLeftView, {} )
		...
		EmptySlot = {
			...
			GainFocus = function ()
				...
				local plusIconFrame2 = function ( plusIcon, event )
					if not event.interrupted then
						plusIcon:beginAnimation( "keyframe", 59, false, false, CoD.TweenType.Linear )
					end
					plusIcon:setAlpha( 0.85 )
					if event.interrupted then
						self.clipFinished( plusIcon, event )
	...
	self.close = function ( self )
		...
		self.drawPaintjobLeftView:close()
		CoD.PaintjobSelectorItem.super.close( self )
	end
