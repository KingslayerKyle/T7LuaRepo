...
CoD.PaintshopWeaponListButtonNew.new = function ( menu, controller )
	...
	FEFocusBarContainer0:setRGB( 1, 1, 1 )
	...
	itemImageBg:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_normal" ) )
	...
	itemName.itemName:setRGB( 0.82, 0.85, 0.88 )
	...
	local totalPaintjobs = LUI.UIText.new()
	totalPaintjobs:setLeftRight( true, false, 213, 248 )
	totalPaintjobs:setTopBottom( true, false, 21.33, 46.33 )
	totalPaintjobs:setRGB( 1, 1, 1 )
	totalPaintjobs:setTTF( "fonts/default.ttf" )
	totalPaintjobs:setAlignment( Enum.LUIAlignment.LUI_ALIGNMENT_CENTER )
	totalPaintjobs:setAlignment( Enum.LUIAlignment.LUI_ALIGNMENT_TOP )
	...
	self.totalPaintjobs:linkToElementModel( self, "itemIndex", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			totalPaintjobs:setText( Engine.Localize( GetPaintjobCountByWeaponIndex( modelValue ) ) )
		end
	end )
	self.clipsPerState = {
		DefaultState = {
	...
	self.close = function ( self )
		...
		CoD.PaintshopWeaponListButtonNew.super.close( self )
	end
