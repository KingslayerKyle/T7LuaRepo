CoD.FriendsTabWidget = InheritFrom( LUI.UIElement )
CoD.FriendsTabWidget.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.FriendsTabWidget )
	self.id = "FriendsTabWidget"
	self.soundSet = "FriendsMenu"
	self:setLeftRight( true, false, 0, 200 )
	self:setTopBottom( true, false, 0, 40 )
	
	local text = LUI.UIText.new()
	text:setLeftRight( true, true, 20, -20 )
	...
	buttonText:setTopBottom( true, false, 5, 35 )
	...
	self.close = function ( self )
		...
		CoD.FriendsTabWidget.super.close( self )
	end
