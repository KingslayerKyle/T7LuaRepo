...
CoD.AmmoWidget_HeroAbilityContainer.new = function ( menu, controller )
	...
	AbilityRingBack.ImgRingDeplete:setRGB( 1, 0.58, 0.02 )
	AbilityRingBack.ImgRingDeplete0:setRGB( 1, 0.81, 0 )
	...
	HeroWeaponPulse:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if not IsHeroWeaponGadgetAmmoEmpty( menu, controller ) and not IsHeroWeaponGadgetCurrentWeapon( menu, controller ) then
					f14_local0 = IsHeroWeaponOrSpecialGadgetAvailable( menu, controller )
				else
					f14_local0 = false
				end
				return f14_local0
	...
	self.clipsPerState = {
		...
		InUse = {
			...
			Ready = function ()
				...
				local ReadyImageFrame2 = function ( ReadyImage, event )
					if not event.interrupted then
						ReadyImage:beginAnimation( "keyframe", 400, false, false, CoD.TweenType.Linear )
					end
					ReadyImage:setRGB( 0.98, 0.67, 0 )
					ReadyImage:setAlpha( 1 )
					if event.interrupted then
	...
	self.close = function ( self )
		...
		CoD.AmmoWidget_HeroAbilityContainer.super.close( self )
	end
