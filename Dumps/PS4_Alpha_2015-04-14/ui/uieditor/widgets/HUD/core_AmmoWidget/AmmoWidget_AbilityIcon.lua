...
CoD.AmmoWidget_AbilityIcon.new = function ( menu, controller )
	...
	ImgIcon:setLeftRight( true, true, 0, -0.33 )
	ImgIcon:setTopBottom( true, true, 0, 0.33 )
	ImgIcon:setRGB( 1, 1, 1 )
	ImgIcon:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_normal" ) )
	ImgIcon:subscribeToGlobalModel( controller, "HeroWeapon", "imageAvailable", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			ImgIcon:setImage( RegisterImage( modelValue ) )
		end
	end )
	...
	self.close = function ( self )
		self.ImgIcon:close()
		CoD.AmmoWidget_AbilityIcon.super.close( self )
	end
