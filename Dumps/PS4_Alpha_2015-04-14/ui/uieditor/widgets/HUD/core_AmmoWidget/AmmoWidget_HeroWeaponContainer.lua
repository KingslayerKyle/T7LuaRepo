...
CoD.AmmoWidget_HeroWeaponContainer.new = function ( menu, controller )
	...
	HeroWeaponShader:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f8_local0
				if not IsHeroWeaponGadgetAmmoEmpty( menu, controller ) and not IsHeroWeaponGadgetCurrentWeapon( menu, controller ) then
					f8_local0 = IsHeroWeaponOrSpecialGadgetAvailable( menu, controller )
				else
					f8_local0 = false
				end
				return f8_local0
	...
	self.close = function ( self )
		self.HeroWeaponSolid:close()
		self.HeroWeaponShader:close()
		CoD.AmmoWidget_HeroWeaponContainer.super.close( self )
	end
