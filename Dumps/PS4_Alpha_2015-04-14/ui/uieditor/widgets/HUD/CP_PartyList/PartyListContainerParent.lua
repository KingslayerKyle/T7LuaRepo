...
CoD.PartyListContainerParent.new = function ( menu, controller )
	...
	PartyListContainer:setRGB( 1, 1, 1 )
	...
	if PartyListContainer.m_eventHandlers.hud_update_refresh then
		local currentEv = PartyListContainer.m_eventHandlers.hud_update_refresh
		PartyListContainer:registerEventHandler( "hud_update_refresh", function ( self, event )
			event.menu = event.menu or menu
			self:updateState( event )
			return currentEv( self, event )
		end )
	else
		PartyListContainer:registerEventHandler( "hud_update_refresh", LUI.UIElement.updateState )
	end
	...
	self.close = function ( self )
		self.PartyListContainer:close()
		CoD.PartyListContainerParent.super.close( self )
	end
