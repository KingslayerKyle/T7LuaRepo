...
local PostLoadFunc = function ( self )
	if Engine.DvarBool( nil, "ShakeHUD" ) or Engine.DvarBool( nil, "ParallaxHUD" ) then
		local scoreShaker_xOffset = 135
		local scoreShaker_yOffset = -25
		if Dvar.ParallaxHUD:get() then
			self.scoreShaker = CoD.HUDShaker.new( 10, 10 )
		else
			self.scoreShaker = CoD.HUDShaker.new( 0, 0 )
		end
		self.scoreShaker:setLeftRight( true, false, -132 + scoreShaker_xOffset, 482 + scoreShaker_xOffset )
		self.scoreShaker:setTopBottom( false, true, -218 + scoreShaker_yOffset, 32 + scoreShaker_yOffset )
		self:addElement( self.scoreShaker )
		self:removeElement( self.ScoreInfo )
		self.scoreShaker:addElement( self.ScoreInfo )
	end
end
...
CoD.ScoreInfoContainer.new = function ( menu, controller )
	...
	if ScoreInfo.m_eventHandlers.hud_update_refresh then
		local currentEv = ScoreInfo.m_eventHandlers.hud_update_refresh
		ScoreInfo:registerEventHandler( "hud_update_refresh", function ( self, event )
			event.menu = event.menu or menu
			self:updateState( event )
			return currentEv( self, event )
		end )
	else
		ScoreInfo:registerEventHandler( "hud_update_refresh", LUI.UIElement.updateState )
	end
	...
	self.close = function ( self )
		...
		CoD.ScoreInfoContainer.super.close( self )
	end
