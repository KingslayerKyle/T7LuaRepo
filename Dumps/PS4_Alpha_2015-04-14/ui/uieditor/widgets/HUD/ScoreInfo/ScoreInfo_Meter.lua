...
CoD.ScoreInfo_Meter.new = function ( menu, controller )
	...
	ImgMeterEnvBack:setLeftRight( true, true, 0, 0.33 )
	ImgMeterEnvBack:setTopBottom( true, true, 0, 0 )
	ImgMeterEnvBack:setRGB( 1, 1, 1 )
	ImgMeterEnvBack:setAlpha( 0.8 )
	ImgMeterEnvBack:setImage( RegisterImage( "uie_t7_cp_hud_damagewidget_hess_meterback" ) )
	ImgMeterEnvBack:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_normal" ) )
	...
	self.clipsPerState = {
		...
		Visible = {
			DefaultClip = function ()
				self:setupElementClipCounter( 2 )
				ImgMeterEnvBack:completeAnimation()
				self.ImgMeterEnvBack:setAlpha( 0.6 )
				self.clipFinished( ImgMeterEnvBack, {} )
				ImgMeterEnvFill:completeAnimation()
