...
CoD.ScoreInfo.new = function ( menu, controller )
	...
	PanelTimer:setRGB( 1, 1, 1 )
	PanelTimer:setZoom( -15 )
	PanelTimer:registerEventHandler( "countdown_low", function ( element, event )
		local retVal = nil
		SetElementState( self, element, controller, "TimeLow" )
		if not retVal then
			retVal = element:dispatchEventToChildren( event )
		end
		return retVal
	end )
	PanelTimer:registerEventHandler( "countdown_end", function ( element, event )
		local retVal = nil
	...
	PrimaryStatus:setLeftRight( true, false, 221, 511 )
	PrimaryStatus:setTopBottom( true, false, 193, 209 )
	...
	GameTimer:registerEventHandler( "countdown_low", function ( element, event )
		local retVal = nil
		SetElementState( self, element, controller, "TimeLow" )
		if not retVal then
			retVal = element:dispatchEventToChildren( event )
	...
	GameTimer:registerEventHandler( "countdown_end", function ( element, event )
		local retVal = nil
		SetElementState( self, element, controller, "DefaultState" )
		if not retVal then
			retVal = element:dispatchEventToChildren( event )
	...
	RoundsContainer:setRGB( 1, 1, 1 )
	RoundsContainer:mergeStateConditions( {
		{
			stateName = "Invisible",
			condition = function ( menu, element, event )
				return IsSingleRound( controller ) and not IsInOvertime( controller )
			end
		},
		{
			...
			condition = function ( menu, element, event )
				local f13_local0
				if not IsSingleRound( controller ) and not HasRoundLimit( controller ) then
					f13_local0 = not IsInOvertime( controller )
				else
					f13_local0 = false
				end
				return f13_local0
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f14_local0
				if not IsSingleRound( controller ) then
					f14_local0 = HasRoundLimit( controller )
					if f14_local0 then
						f14_local0 = not IsInOvertime( controller )
					end
				else
					f14_local0 = false
				end
	...
	if RoundsContainer.m_eventHandlers.hud_update_scores then
		local currentEv = RoundsContainer.m_eventHandlers.hud_update_scores
		RoundsContainer:registerEventHandler( "hud_update_scores", function ( self, event )
			event.menu = event.menu or menu
			self:updateState( event )
			return currentEv( self, event )
		end )
	else
		RoundsContainer:registerEventHandler( "hud_update_scores", LUI.UIElement.updateState )
	end
	if RoundsContainer.m_eventHandlers.hud_update_team_change then
		local currentEv = RoundsContainer.m_eventHandlers.hud_update_team_change
		RoundsContainer:registerEventHandler( "hud_update_team_change", function ( self, event )
			event.menu = event.menu or menu
			self:updateState( event )
			return currentEv( self, event )
		end )
	else
		RoundsContainer:registerEventHandler( "hud_update_team_change", LUI.UIElement.updateState )
	end
	if RoundsContainer.m_eventHandlers.hud_update_refresh then
		local currentEv = RoundsContainer.m_eventHandlers.hud_update_refresh
		RoundsContainer:registerEventHandler( "hud_update_refresh", function ( self, event )
			event.menu = event.menu or menu
			self:updateState( event )
			return currentEv( self, event )
		end )
	else
		RoundsContainer:registerEventHandler( "hud_update_refresh", LUI.UIElement.updateState )
	end
	self:addElement( RoundsContainer )
	self.RoundsContainer = RoundsContainer
	
	local currentEv = CoD.UserScoreContainer.new( menu, controller )
	currentEv:setLeftRight( true, false, 304.2, 527.2 )
	currentEv:setTopBottom( true, false, 121.12, 166.12 )
	currentEv:setRGB( 0.8, 0.8, 0.8 )
	currentEv:setZoom( 3 )
	currentEv:setRFTMaterial( LUI.UIImage.GetCachedMaterial( "uie_aberration_cheap_blur" ) )
	currentEv:setShaderVector( 0, 0, 0, 0, 0 )
	currentEv:setShaderVector( 1, 1, 0, 0, 0 )
	currentEv:setShaderVector( 2, 10, -3, 0, 0 )
	currentEv:setShaderVector( 3, 2, 2, 0, 0 )
	currentEv:setShaderVector( 4, 0, 0, 0, 0 )
	currentEv:subscribeToGlobalModel( controller, "GameScore", nil, function ( model )
		currentEv:setModel( model, controller )
	end )
	self:addElement( currentEv )
	self.UserScore = currentEv
	
	local EnemyScore = CoD.ScoreInfo_EnemyScoreContainer.new( menu, controller )
	EnemyScore:setLeftRight( true, false, 302.06, 461.06 )
	...
	self.close = function ( self )
		self.EnemyScorePanel:close()
		self.UserScorePanel:close()
		self.PanelTimer:close()
		self.PanelCircle:close()
		self.BackCircleInner:close()
		self.PrimaryStatus:close()
		self.GameTimer:close()
		self.RoundsContainer:close()
		self.UserScore:close()
		self.EnemyScore:close()
		self.FactionContainer:close()
		self.Dot3:close()
		self.Dot5:close()
		self.Dot6:close()
		self.Dot7:close()
		CoD.ScoreInfo.super.close( self )
	end
