...
require( "ui.uieditor.widgets.HUD.BombTimer.BombHeld" )
...
CoD.SDOverlay.new = function ( menu, controller )
	...
	BombTimerWidgetA:setLeftRight( true, false, 49, 97 )
	BombTimerWidgetA:setTopBottom( true, false, 221.28, 269.28 )
	BombTimerWidgetA:setRGB( 1, 1, 1 )
	BombTimerWidgetA.BombTimerBgImage:setImage( RegisterImage( "uie_hud_icon_bomb_a" ) )
	BombTimerWidgetA.timer:setupBombTimer( 0 )
	BombTimerWidgetA:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_HUD_VISIBLE ) and Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_WEAPON_HUD_VISIBLE ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_HUD_HARDCORE ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_GAME_ENDED ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_DEMO_CAMERA_MODE_MOVIECAM ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_DEMO_ALL_GAME_HUD_HIDDEN ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IN_KILLCAM ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IS_FLASH_BANGED ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_UI_ACTIVE ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IN_VEHICLE ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IN_GUIDED_MISSILE ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_SCOREBOARD_OPEN ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IN_REMOTE_KILLSTREAK_STATIC ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_EMP_ACTIVE ) then
					f2_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_BOMB_TIMER_A )
				else
					f2_local0 = false
				end
				return f2_local0
	...
	if BombTimerWidgetA.m_eventHandlers.hud_update_refresh then
		local currentEv = BombTimerWidgetA.m_eventHandlers.hud_update_refresh
		BombTimerWidgetA:registerEventHandler( "hud_update_refresh", function ( self, event )
			event.menu = event.menu or menu
			self:updateState( event )
			return currentEv( self, event )
		end )
	else
		BombTimerWidgetA:registerEventHandler( "hud_update_refresh", LUI.UIElement.updateState )
	end
	...
	local currentEv = CoD.BombTimerWidget.new( menu, controller )
	currentEv:setLeftRight( true, false, 112, 161 )
	currentEv:setTopBottom( true, false, 221.28, 269.28 )
	currentEv:setRGB( 1, 1, 1 )
	currentEv.BombTimerBgImage:setImage( RegisterImage( "uie_hud_icon_bomb_b" ) )
	currentEv.timer:setupBombTimer( 1 )
	currentEv:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f19_local0
				if Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_HUD_VISIBLE ) and Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_WEAPON_HUD_VISIBLE ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_HUD_HARDCORE ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_GAME_ENDED ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_DEMO_CAMERA_MODE_MOVIECAM ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_DEMO_ALL_GAME_HUD_HIDDEN ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IN_KILLCAM ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IS_FLASH_BANGED ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_UI_ACTIVE ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IN_VEHICLE ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IN_GUIDED_MISSILE ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_SCOREBOARD_OPEN ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IN_REMOTE_KILLSTREAK_STATIC ) and not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_EMP_ACTIVE ) then
					f19_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_BOMB_TIMER_B )
				else
					f19_local0 = false
				end
				return f19_local0
	...
	currentEv:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_BOMB_TIMER_B ), function ( model )
		menu:updateElementState( currentEv, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
	...
	self.BombTimerWidgetB = currentEv
	
	local currentEv = CoD.BombHeld.new( menu, controller )
	currentEv:setLeftRight( false, true, -201, -137 )
	currentEv:setTopBottom( false, true, -205.98, -143.98 )
	currentEv:setRGB( 1, 1, 1 )
	self:addElement( currentEv )
	self.BombHeld = currentEv
	
	self.close = function ( self )
		self.BombTimerWidgetA:close()
		self.BombTimerWidgetB:close()
		self.BombHeld:close()
		CoD.SDOverlay.super.close( self )
	end
