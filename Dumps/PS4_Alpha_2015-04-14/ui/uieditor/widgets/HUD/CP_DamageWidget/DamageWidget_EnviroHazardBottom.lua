...
CoD.DamageWidget_EnviroHazardBottom.new = function ( menu, controller )
	...
	EnviroHazardMeter:setRGB( 1, 1, 1 )
	...
	EnviroHazardMeter:linkToElementModel( self, "ratio", true, function ( model )
		...
		if modelValue then
			EnviroHazardMeter.ImgMeterEnvFill:setShaderVector( 0, CoD.GetVectorComponentFromString( modelValue, 1 ), CoD.GetVectorComponentFromString( modelValue, 2 ), CoD.GetVectorComponentFromString( modelValue, 3 ), CoD.GetVectorComponentFromString( modelValue, 4 ) )
		end
	end )
	...
	MeterText:setRGB( 1, 1, 1 )
	MeterText:setTTF( "fonts/Entovo.ttf" )
	...
	MeterText:linkToElementModel( self, "type", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			MeterText:setText( Engine.Localize( HessTypeToString( modelValue ) ) )
		end
	end )
	...
	AmmoWidgetBackCircleOutter0:setRGB( 1, 1, 1 )
	...
	DamageWidgetArrow1:setRGB( 1, 1, 1 )
	...
	self.close = function ( self )
		self.EnviroHazardMeter:close()
		self.IconEnviroHazard:close()
		self.AmmoWidgetBackCircleOutter0:close()
		self.DamageWidgetArrow0:close()
		self.DamageWidgetArrow1:close()
		self.MeterText:close()
		CoD.DamageWidget_EnviroHazardBottom.super.close( self )
	end
