...
CoD.ShockCharge.new = function ( menu, controller )
	...
	shockImageBottom:setRGB( 1, 1, 1 )
	shockImageBottom:setAlpha( 0 )
	shockImageBottom:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_normal" ) )
	shockImageBottom:subscribeToGlobalModel( controller, "HUDItems", "shockImageBottom", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			shockImageBottom:setImage( RegisterImage( modelValue ) )
		end
	end )
	...
	shockImageRight:setRGB( 1, 1, 1 )
	shockImageRight:setAlpha( 0 )
	shockImageRight:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_normal" ) )
	shockImageRight:subscribeToGlobalModel( controller, "HUDItems", "shockImageRight", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			shockImageRight:setImage( RegisterImage( modelValue ) )
		end
	end )
	...
	shockImageLeft:setRGB( 1, 1, 1 )
	shockImageLeft:setAlpha( 0 )
	shockImageLeft:setYRot( 180 )
	shockImageLeft:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_normal" ) )
	shockImageLeft:subscribeToGlobalModel( controller, "HUDItems", "shockImageLeft", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			shockImageLeft:setImage( RegisterImage( modelValue ) )
		end
	end )
	...
	self.close = function ( self )
		self.shockImageBottom:close()
		self.shockImageRight:close()
		self.shockImageLeft:close()
		self.shockImageTop:close()
		CoD.ShockCharge.super.close( self )
	end
