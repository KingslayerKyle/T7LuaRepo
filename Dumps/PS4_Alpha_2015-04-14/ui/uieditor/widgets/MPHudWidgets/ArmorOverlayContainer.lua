...
CoD.ArmorOverlayContainer.new = function ( menu, controller )
	...
	ArmorOverlayImage:setRGB( 1, 1, 1 )
	ArmorOverlayImage:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_normal" ) )
	ArmorOverlayImage:subscribeToGlobalModel( controller, "HUDItems", "armorOverlay", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			ArmorOverlayImage:setImage( RegisterImage( modelValue ) )
		end
	end )
	...
	self.close = function ( self )
		self.ArmorOverlayImage:close()
		CoD.ArmorOverlayContainer.super.close( self )
	end
