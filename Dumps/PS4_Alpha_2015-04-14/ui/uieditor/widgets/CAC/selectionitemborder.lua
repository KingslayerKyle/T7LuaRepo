CoD.SelectionItemBorder = InheritFrom( LUI.UIElement )
CoD.SelectionItemBorder.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.SelectionItemBorder )
	self.id = "SelectionItemBorder"
