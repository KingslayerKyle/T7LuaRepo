CoD.RightBumperWidget = InheritFrom( LUI.UIElement )
CoD.RightBumperWidget.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.RightBumperWidget )
	self.id = "RightBumperWidget"
	self.soundSet = "CAC_PrimaryWeapon"
	self:setLeftRight( true, false, 0, 32 )
	self:setTopBottom( true, false, 0, 32 )
	
	local bumper = LUI.UIImage.new()
	bumper:setLeftRight( false, true, -32, 0 )
	bumper:setTopBottom( true, false, 0, 32 )
	bumper:setRGB( 1, 1, 1 )
	bumper:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_normal" ) )
	bumper:subscribeToGlobalModel( controller, "Controller", "right_shoulder_button_image", function ( model )
	...
	self.close = function ( self )
		self.bumper:close()
		CoD.RightBumperWidget.super.close( self )
	end
