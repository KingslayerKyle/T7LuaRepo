...
CoD.CustomizationItem_ND.new = function ( menu, controller )
	...
	itemImage:subscribeToGlobalModel( controller, "Customization", "type", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			itemImage.paintjob:setupPaintjobCustomizationType( modelValue )
		end
	end )
	itemImage:linkToElementModel( self, nil, false, function ( model )
		itemImage:setModel( model, controller )
	end )
	itemImage:linkToElementModel( self, "paintjobSlot", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			itemImage.paintjob:setupPaintjobSlot( modelValue )
		end
	end )
	itemImage:linkToElementModel( self, "paintjobIndex", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			itemImage.paintjob:setupPaintjobIndex( modelValue )
		end
	end )
	...
	cacIconTokenGrid0:setRGB( 1, 1, 1 )
	...
	self.close = function ( self )
		...
		CoD.CustomizationItem_ND.super.close( self )
	end
