...
CoD.CustomizationListButton_NB = InheritFrom( LUI.UIElement )
CoD.CustomizationListButton_NB.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.CustomizationListButton_NB )
	self.id = "CustomizationListButton_NB"
	...
	LabelButton0:setRGB( 1, 1, 1 )
	LabelButton0.itemName:setRGB( 0.82, 0.85, 0.88 )
	...
	local itemImage0 = CoD.CustomizationListButtonImage.new( menu, controller )
	itemImage0:setLeftRight( true, false, 0, 65 )
	itemImage0:setTopBottom( false, false, -32, 32 )
	itemImage0:setRGB( 1, 1, 1 )
	itemImage0:subscribeToGlobalModel( controller, "Customization", "type", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			itemImage0.paintjob:setupPaintjobCustomizationType( modelValue )
		end
	end )
	itemImage0:linkToElementModel( self, nil, false, function ( model )
		itemImage0:setModel( model, controller )
	end )
	itemImage0:linkToElementModel( self, "paintjobSlot", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			itemImage0.paintjob:setupPaintjobSlot( modelValue )
		end
	end )
	itemImage0:linkToElementModel( self, "paintjobIndex", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			itemImage0.paintjob:setupPaintjobIndex( modelValue )
		end
	end )
	itemImage0:mergeStateConditions( {
		{
			stateName = "PaintJob",
			condition = function ( menu, element, event )
				return IsPaintJobCustomListButton( element, controller )
			end
		}
	} )
	...
	self.LabelButton0:linkToElementModel( self, "displayText", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
	...
	self.close = function ( self )
		...
		self.itemImage0:close()
		self.itemImageSolid:close()
		CoD.CustomizationListButton_NB.super.close( self )
	end
