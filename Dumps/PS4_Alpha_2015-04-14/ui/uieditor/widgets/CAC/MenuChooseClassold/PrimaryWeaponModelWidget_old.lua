...
CoD.PrimaryWeaponModelWidget_old = InheritFrom( LUI.UIElement )
CoD.PrimaryWeaponModelWidget_old.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.PrimaryWeaponModelWidget_old )
	self.id = "PrimaryWeaponModelWidget_old"
	self.soundSet = "CAC"
	self:setLeftRight( true, false, 0, 448 )
	self:setTopBottom( true, false, 0, 193 )
	...
	weapon:setLeftRight( false, false, -107, 107 )
	weapon:setTopBottom( true, false, 30, 137 )
	weapon:setRGB( 1, 1, 1 )
	...
	nameLabel:setLeftRight( true, false, 11, 49 )
	...
	headerLabel.text:setText( Engine.Localize( "MPUI_PRIMARY_CAPS" ) )
	...
	self.close = function ( self )
		...
		CoD.PrimaryWeaponModelWidget_old.super.close( self )
	end
