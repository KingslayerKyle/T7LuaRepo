...
CoD.LethalSlot = InheritFrom( LUI.UIElement )
CoD.LethalSlot.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.LethalSlot )
	self.id = "LethalSlot"
	...
	Label0:linkToElementModel( self, "modifiedName", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
		...
		Unequipped = {
			...
			GainFocus = function ()
				...
				local BorderBakedStroke0Frame2 = function ( BorderBakedStroke0, event )
					if not event.interrupted then
						BorderBakedStroke0:beginAnimation( "keyframe", 150, false, false, CoD.TweenType.Linear )
					end
					BorderBakedStroke0:setAlpha( 0 )
					if event.interrupted then
	...
	self.close = function ( self )
		...
		self.Image0:close()
		self.Label0:close()
		CoD.LethalSlot.super.close( self )
	end
