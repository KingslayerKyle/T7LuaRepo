...
CoD.PerkSlot = InheritFrom( LUI.UIElement )
CoD.PerkSlot.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.PerkSlot )
	self.id = "PerkSlot"
	...
	CustomClassCategoryHeader0.text:setText( Engine.Localize( "MPUI_PERK1_CAPS" ) )
	...
	Image0:linkToElementModel( self, "image", true, function ( model )
	...
	self.clipsPerState = {
		...
		Unequipped = {
			...
			Focus = function ()
				...
				self.BorderBakedSolid0:setLeftRight( true, true, -5.03, 5.03 )
				self.BorderBakedSolid0:setTopBottom( true, true, 21.83, 4.17 )
				self.BorderBakedSolid0:setAlpha( 0.8 )
				self.BorderBakedSolid0:setZoom( 30 )
				self.clipFinished( BorderBakedSolid0, {} )
				Image0:completeAnimation()
				self.Image0:setLeftRight( false, false, -103, 103 )
