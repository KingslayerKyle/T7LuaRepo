...
require( "ui.uieditor.widgets.CAC.CustomClassListScreen.Panes.Weapons.PrimaryWeaponSlotImage" )
...
CoD.PrimaryWeaponSlot = InheritFrom( LUI.UIElement )
CoD.PrimaryWeaponSlot.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.PrimaryWeaponSlot )
	self.id = "PrimaryWeaponSlot"
	...
	local Image0 = CoD.PrimaryWeaponSlotImage.new( menu, controller )
	Image0:setLeftRight( false, false, -220, 220 )
	Image0:setTopBottom( false, false, -97, 123 )
	Image0:setRGB( 1, 1, 1 )
	Image0:setAlpha( 0.85 )
	Image0:mergeStateConditions( {
		{
			stateName = "Extracam",
			condition = function ( menu, element, event )
				return IsExtracamCACDvarSet()
			end
		}
	} )
	...
	BorderFocusHeader:setLeftRight( true, true, 285, -285 )
	...
	CustomClassPlusSymbol0:setLeftRight( true, true, 321.5, -321.5 )
	CustomClassPlusSymbol0:setTopBottom( true, true, 145, -127 )
	...
	self.clipsPerState = {
		DefaultState = {
			DefaultClip = function ()
				...
				self.Image0:setLeftRight( false, false, -220, 220 )
				self.Image0:setTopBottom( false, false, -97, 123 )
				...
				self.BorderFocusHeader:setLeftRight( true, true, 285, -285 )
			...
			Focus = function ()
				...
				self.clipFinished( BorderBakedSolid0, {} )
				Image0:completeAnimation()
				self.Image0:setLeftRight( false, false, -226.6, 226.6 )
				self.Image0:setTopBottom( false, false, -100.87, 125.73 )
			...
			LoseFocus = function ()
				...
				CustomClassPlusSymbol0:completeAnimation()
				self.CustomClassPlusSymbol0:setLeftRight( true, true, 321.5, -321.5 )
				self.CustomClassPlusSymbol0:setTopBottom( true, true, 145, -127 )
				self.CustomClassPlusSymbol0:setAlpha( 0 )
				self.clipFinished( CustomClassPlusSymbol0, {} )
			end,
			GainFocus = function ()
				...
				local Image0Frame2 = function ( Image0, event )
					...
					Image0:setLeftRight( false, false, -226.6, 226.6 )
					Image0:setTopBottom( false, false, -100.87, 125.73 )
					Image0:setAlpha( 1 )
					Image0:setZoom( 30 )
					if event.interrupted then
						self.clipFinished( Image0, event )
		...
		Unequipped = {
			...
			LoseFocus = function ()
				...
				local CustomClassPlusSymbol0Frame2 = function ( CustomClassPlusSymbol0, event )
					if not event.interrupted then
						CustomClassPlusSymbol0:beginAnimation( "keyframe", 200, true, false, CoD.TweenType.Linear )
					end
					CustomClassPlusSymbol0:setRGB( 1, 1, 1 )
	...
	self.close = function ( self )
		...
		self.Label0:close()
		CoD.PrimaryWeaponSlot.super.close( self )
	end
