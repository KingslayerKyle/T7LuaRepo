...
CoD.LethalGrenadeSlotWidget = InheritFrom( LUI.UIElement )
CoD.LethalGrenadeSlotWidget.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.LethalGrenadeSlotWidget )
	self.id = "LethalGrenadeSlotWidget"
	...
	self.clipsPerState = {
		...
		Unequipped = {
			DefaultClip = function ()
				...
				self.clipFinished( currentEv, {} )
				currentEv:completeAnimation()
				self.plus:setLeftRight( false, false, -16, 16 )
				self.plus:setTopBottom( true, false, 55, 87 )
	...
	self.close = function ( self )
		...
		CoD.LethalGrenadeSlotWidget.super.close( self )
	end
