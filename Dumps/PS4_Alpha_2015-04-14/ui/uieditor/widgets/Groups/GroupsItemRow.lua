...
CoD.GroupsItemRow.new = function ( menu, controller )
	...
	Text:setTTF( "fonts/UnitedSansRgMd.ttf" )
	...
	GroupEmblemWidget:setLeftRight( true, false, 3, 43 )
	GroupEmblemWidget:setTopBottom( true, false, 3.42, 43.42 )
	GroupEmblemWidget:setRGB( 1, 1, 1 )
	...
	self.clipsPerState = {
		DefaultState = {
			DefaultClip = function ()
				...
				self.GroupEmblemWidget:setZoom( 0 )
			...
			GainFocus = function ()
				...
				local GroupEmblemWidgetFrame2 = function ( GroupEmblemWidget, event )
					if not event.interrupted then
						GroupEmblemWidget:beginAnimation( "keyframe", 150, true, false, CoD.TweenType.Back )
					end
					GroupEmblemWidget:setZoom( 60 )
					if event.interrupted then
						self.clipFinished( GroupEmblemWidget, event )
					else
						GroupEmblemWidget:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
				
				GroupEmblemWidget:completeAnimation()
				self.GroupEmblemWidget:setZoom( 0 )
				GroupEmblemWidgetFrame2( GroupEmblemWidget, {} )
			end,
			Focus = function ()
				self:setupElementClipCounter( 6 )
				LobbyMemberBackingMask0:completeAnimation()
				...
				self.clipFinished( LobbyMemberBackingFooter0, {} )
				GroupEmblemWidget:completeAnimation()
				self.GroupEmblemWidget:setZoom( 60 )
				self.clipFinished( GroupEmblemWidget, {} )
			end,
			LoseFocus = function ()
				self:setupElementClipCounter( 6 )
				...
				local GroupEmblemWidgetFrame2 = function ( GroupEmblemWidget, event )
					if not event.interrupted then
						GroupEmblemWidget:beginAnimation( "keyframe", 200, false, true, CoD.TweenType.Back )
					end
					GroupEmblemWidget:setZoom( 0 )
					if event.interrupted then
						self.clipFinished( GroupEmblemWidget, event )
					else
						GroupEmblemWidget:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
		...
		Header = {
			DefaultClip = function ()
				...
				self.clipFinished( LfocusLINE, {} )
				Text:completeAnimation()
				self.Text:setLeftRight( true, false, 24.5, 350.18 )
				self.Text:setTopBottom( false, false, -6.5, 10.5 )
				self.Text:setRGB( 1, 1, 1 )
				self.Text:setAlpha( 1 )
	...
	self.close = function ( self )
		...
		self.Text:close()
		CoD.GroupsItemRow.super.close( self )
	end
