CoD.vhud_ms_ModDNI = InheritFrom( LUI.UIElement )
CoD.vhud_ms_ModDNI.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.vhud_ms_ModDNI )
	self.id = "vhud_ms_ModDNI"
	...
	self.clipsPerState = {
		DefaultState = {
			DefaultClip = function ()
				...
				ModDNIFrame2( ModDNI, {} )
				local ModDNIArrowFrame2 = function ( ModDNIArrow, event )
					local ModDNIArrowFrame3 = function ( ModDNIArrow, event )
						local ModDNIArrowFrame4 = function ( ModDNIArrow, event )
							if not event.interrupted then
								ModDNIArrow:beginAnimation( "keyframe", 389, false, true, CoD.TweenType.Linear )
							end
							ModDNIArrow:setLeftRight( true, false, 18, 34 )
							ModDNIArrow:setTopBottom( true, false, 0, 16 )
							if event.interrupted then
								self.clipFinished( ModDNIArrow, event )
							else
								ModDNIArrow:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
							end
						end
						
						if event.interrupted then
							ModDNIArrowFrame4( ModDNIArrow, event )
							return 
						else
							ModDNIArrow:beginAnimation( "keyframe", 500, false, false, CoD.TweenType.Linear )
							ModDNIArrow:registerEventHandler( "transition_complete_keyframe", ModDNIArrowFrame4 )
						end
					end
					
					if event.interrupted then
						ModDNIArrowFrame3( ModDNIArrow, event )
						return 
					else
						ModDNIArrow:beginAnimation( "keyframe", 329, false, true, CoD.TweenType.Back )
						ModDNIArrow:setTopBottom( true, false, -10, 6 )
						ModDNIArrow:registerEventHandler( "transition_complete_keyframe", ModDNIArrowFrame3 )
					end
				end
				...
				ModDNIArrow:setLeftRight( true, false, 18, 34 )
				ModDNIArrow:setTopBottom( true, false, 0, 16 )
				ModDNIArrow:registerEventHandler( "transition_complete_keyframe", ModDNIArrowFrame2 )
