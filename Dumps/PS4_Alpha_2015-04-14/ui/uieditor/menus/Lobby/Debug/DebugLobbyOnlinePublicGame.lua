...
LUI.createMenu.DebugLobbyOnlinePublicGame = function ( controller )
	local self = CoD.Menu.NewForUIEditor( "DebugLobbyOnlinePublicGame" )
	if PreLoadFunc then
	...
	btnStart:setLeftRight( true, false, 96, 347 )
	btnStart:setTopBottom( true, false, 528.5, 558.5 )
	...
	btnStart:registerEventHandler( "button_action", function ( element, event )
		local retVal = nil
		LobbyOnlinePublicLaunchGame( self, controller )
		if not retVal then
	...
	btnCAC:setLeftRight( true, false, 96, 347 )
	btnCAC:setTopBottom( true, false, 558.5, 588.5 )
	btnCAC:setRGB( 1, 1, 1 )
