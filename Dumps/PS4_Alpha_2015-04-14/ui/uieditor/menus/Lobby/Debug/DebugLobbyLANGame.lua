...
LUI.createMenu.DebugLobbyLANGame = function ( controller )
	local self = CoD.Menu.NewForUIEditor( "DebugLobbyLANGame" )
	if PreLoadFunc then
	...
	btnStart:setLeftRight( true, false, 96, 346 )
	...
	btnStart:registerEventHandler( "button_action", function ( element, event )
		local retVal = nil
		LobbyLANLaunchGame( self, element, controller )
		if not retVal then
	...
	btnCAC:setLeftRight( true, false, 96, 346 )
	...
	btnChangeGameMode:setLeftRight( true, false, 96, 346 )
	...
	button0:setLeftRight( true, false, 96, 346 )
	button0:setTopBottom( true, false, 587, 617 )
	button0:setRGB( 1, 1, 1 )
	button0.buttoninternal0.Text0:setText( Engine.Localize( "MPUI_CHANGE_MAP_CAPS" ) )
	...
	self.buttonPromptAddFunctions.alt1 = function ( menu, element, event )
		menu:addButtonPrompt( "alt1", Engine.Localize( "MENU_FIND_GAME" ), "G", element )
		return true
	end
	...
	self:registerEventHandler( "gamepad_button", function ( element, event )
		...
		if not self.occludedBy and (event.button == "alt1" or event.button == "key_shortcut" and event.key == "G") then
