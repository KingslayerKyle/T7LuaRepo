...
LUI.createMenu.DOA_INGAME_PAUSE = function ( controller )
	...
	bevel:setLeftRight( true, false, 0, 1280 )
	bevel:setTopBottom( true, false, 0, 720 )
	bevel:setRGB( 1, 1, 1 )
	bevel:setImage( RegisterImage( "uie_menu_zom_bezel_overlay" ) )
	bevel:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_normal" ) )
	...
	control:setImage( RegisterImage( "uie_zombietron_controller_layout" ) )
	...
	self:registerEventHandler( "gamepad_button", function ( element, event )
		...
		elseif not self.occludedBy and (event.button == "secondary" or event.button == "key_shortcut" and event.key == "ESCAPE") then
			CloseAllInGameMenus( self, controller )
			ResumeGame( self, controller )
		end
