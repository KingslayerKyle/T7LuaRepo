...
LUI.createMenu.ScriptDebugBillboard = function ( controller )
	...
	BillboardBackground:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_normal" ) )
	...
	BillboardNameLabel:setRGB( 1, 1, 1 )
	...
	BillboardTypeLabel:setRGB( 1, 1, 1 )
	...
	BillboardSizeLabel:setRGB( 1, 1, 1 )
	...
	self.BillboardName:linkToElementModel( self, "name", true, function ( model )
		...
		if modelValue then
			BillboardName:setText( Engine.Localize( modelValue ) )
		end
	end )
	self.BillboardType:linkToElementModel( self, "type", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			BillboardType:setText( Engine.Localize( modelValue ) )
		end
	end )
	self.BillboardSize:linkToElementModel( self, "size", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			BillboardSize:setText( Engine.Localize( modelValue ) )
		end
	end )
	self.clipsPerState = {
		DefaultState = {
	...
	self.close = function ( self )
		self.BillboardName:close()
		self.BillboardType:close()
		self.BillboardSize:close()
		CoD.Menu.close( self )
	end
