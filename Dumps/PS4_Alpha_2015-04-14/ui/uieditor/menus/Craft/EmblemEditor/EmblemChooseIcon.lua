...
local GetSelectedDecalCategory = function ( self, controller )
	...
	if selectedIconID ~= CoD.emblem.INVALID_ICON_ID then
		for key, tabData in ipairs( CoD.emblem.DECALTABDATA ) do
			if Engine.GetEmblemIconIndexInCategory( controller, tabData.category, selectedIconID ) ~= CoD.emblem.INVALID_ICON_ID then
				return tabData.category
			end
		end
		
	else
		return CoD.emblem.DECALTABDATA[1].category
	end
end
...
local PreLoadFunc = function ( self, controller )
	self.restoreState = function ( self )
		...
		if selectedIconID ~= CoD.emblem.INVALID_ICON_ID then
			...
			self.tabList.grid:setActiveItem( self.tabList.grid:getItemAt( FindTabIDForSelectedDecal( controller ) ) )
...
LUI.createMenu.EmblemChooseIcon = function ( controller )
	...
	TempBackdrop:setRGB( 1, 1, 1 )
	TempBackdrop:setImage( RegisterImage( "uie_img_t7_menu_cacselection_tempbackdrop" ) )
	TempBackdrop:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_normal" ) )
	...
	emblemDraw:setLeftRight( true, false, 700.67, 1158.67 )
	emblemDraw:setTopBottom( true, false, 248, 528.5 )
	emblemDraw:setRGB( 1, 1, 1 )
	...
	cac3dTitleIntermediary0:setLeftRight( true, false, -71.33, 506.67 )
