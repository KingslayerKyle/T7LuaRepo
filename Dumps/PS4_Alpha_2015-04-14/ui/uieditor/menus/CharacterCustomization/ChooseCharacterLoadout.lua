...
LUI.createMenu.ChooseCharacterLoadout = function ( controller )
	...
	GenericMenuFrame0.titleLabel:setText( Engine.Localize( AppendString( "HEROES_CHOOSE_HERO_CAPS", "_CAPS" ) ) )
	...
	characterCarousel:registerEventHandler( "list_item_button_action", function ( element, event )
		local retVal = nil
		if not IsDisabled( element, controller ) and not IsHeroOnDisabledLoadoutItem( element ) then
	...
	bumperBacking:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_normal" ) )
