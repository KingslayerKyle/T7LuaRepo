...
LUI.UIElement.setMouseDisabled = function ( self, disabled )
	...
	self.handleMouseAnim = not disabled
	if CoD.isPC then
...
LUI.UIElement.setHandleMouseAnim = function ( self, canMouseAnim )
	self.handleMouseAnim = canMouseAnim
end
...
LUI.UIElement.MouseMoveEvent = function ( self, event )
	...
	elseif self.handleMouseMove and not event.waitingForKeyBind then
		...
		if inside then
			if self.m_focusable and not self:isInFocus() then
				if LUI.currentMouseFocus ~= nil and not LUI.UIElement.IsMouseInsideElement( LUI.currentMouseFocus, event ) then
	...
	if LUI.UIElement.IsMouseInsideElement( self, event ) then
...
LUI.UIElement.AcceptGamePadButtonInput = function ( self, event )
	if self.m_inputDisabled or self.occludedBy then
		return false
	elseif not self.m_ownerController or self.m_ownerController == event.controller or self.anyControllerAllowed then
		return true
...
local GetCorrespondingGamepadButtonForKey = function ( key )
...
LUI.UIElement.handleGamepadButton = function ( self, event, traversingUpFromElement )
	...
	elseif (self:isInFocus() or self.navigation and traversingUpFromElement) and event.down == true then
		...
		if button == "key_shortcut" then
			button = GetCorrespondingGamepadButtonForKey( event.key )
		end
...
LUI.UIElement.animationQueue = function ( self, queue, startWait, repeats )
	...
	local f90_local0 = startWait
	local wait = 0
	local f90_local1 = f90_local0 or wait
	if f90_local1 == 0 then
		f90_local1 = ProcessNextQueueItem( self, queue, repeats )
	end
	if #queue > 0 then
		self.queueTimer = LUI.UITimer.new( f90_local1, {
...
LUI.UIElement.NextListAnimation = function ( self, event )
	...
	if event._index <= event._lastIndex then
		...
		local f91_local0 = self:getParent()
		f91_local0:childClipFinished()
	end
end
...
LUI.UIElement.clipFinished = function ( self, event )
	if not event.interrupted and self:getParent() ~= nil then
		local f107_local0 = self:getParent()
		f107_local0:childClipFinished()
	end
end
