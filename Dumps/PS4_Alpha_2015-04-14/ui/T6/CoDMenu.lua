...
CoD.Menu.addFriendsButton = function ( self )
	self.friendsButton = CoD.ButtonPrompt.new( "alt2", Engine.Localize( "MENU_FRIENDS" ), self, "button_prompt_friends", false, nil, false, nil, "F" )
	self:addRightButtonPrompt( self.friendsButton )
end

CoD.Menu.addFriendsButtonSPMainMenu = function ( self )
	self.friendsButton = CoD.ButtonPrompt.new( "start", Engine.Localize( "MENU_FRIENDS" ), self, "button_prompt_friends", false, nil, false, nil, "F" )
	self:addRightButtonPrompt( self.friendsButton )
end

CoD.Menu.addJoinButton = function ( self )
	self.joinButton = CoD.ButtonPrompt.new( "alt1", Engine.Localize( "MENU_JOIN_GAME" ), self, "button_prompt_join", false, nil, false, nil, "J" )
...
CoD.Menu.addPartyPrivacyButton = function ( self )
	...
	if f17_local0 then
		self.partyPrivacyButton = CoD.ButtonPrompt.new( "alt1", Engine.Localize( "MPUI_LOBBY_PRIVACY" ), self, "button_prompt_partyprivacy", false, nil, false, nil, "P" )
...
CoD.Menu.addGlassesToggleButton = function ( self )
	self.glassesToggleButton = CoD.ButtonPrompt.new( "right_trigger", Engine.Localize( "MENU_TURN_OFF_HUD" ), nil, nil, false, nil, false, "space" )
	self:addRightButtonPrompt( self.glassesToggleButton )
end

CoD.Menu.addGlassesToggleOnButton = function ( self )
	self.glassesToggleButton = CoD.ButtonPrompt.new( "right_trigger", Engine.Localize( "MENU_TURN_ON_HUD" ), nil, nil, false, nil, false, "space" )
	self:addRightButtonPrompt( self.glassesToggleButton )
end
...
CoD.Menu.SetDebugMenuName = function ( menuName )
...
CoD.Menu.openMenu = function ( self, menuName, controller, userData )
	...
	local f45_local0 = self:getParent()
	f45_local0:addElement( newMenu )
	CoD.Menu.SetDebugMenuName( menuName )
	return newMenu
end
...
CoD.Menu.openPopup = function ( self, menuName, controller, userData, parentMenu )
	...
	local f47_local0 = parentMenu
	local parent = self:getParent()
	REG6.addElement( f47_local0 or parent, newMenu )
	...
	CoD.Menu.SetDebugMenuName( self.menuName .. ", " .. menuName )
	return newMenu
end

CoD.Menu.openOverlay = function ( self, menuName, controller, userData )
	local popup = CoD.Menu.openPopup( self, menuName, controller, userData )
...
CoD.Menu.returnToPreviousMenu = function ( self, controller )
	...
	if currentMenu then
		CoD.Menu.SetDebugMenuName( currentMenu.menuName )
...
CoD.Menu.animateOutAndGoBack = function ( self, event )
	...
	if Engine.CanPauseGame() and self.menuName == "class" then
		animDuration = 1
	end
	self.m_inputDisabled = true
	self:setAlpha( 1 )
...
CoD.Menu.OpenErrorPopup = function ( self, event )
	if Engine.DvarString( event.controller, "ui_errorMessage" ) ~= "" and not self.occludedBy then
		local f68_local0 = Engine.IsInGame()
		if not f68_local0 then
			local isInGame = CoD.isSinglePlayer
		end
		if not f68_local0 or not self.occludedMenu then
...
CoD.Menu.updateElementState = function ( self, element, event )
	...
	if element.anyChildUsesUpdateState then
		if not event.menu then
			event.menu = self
		end
...
CoD.Menu.OpenMenu = function ( self, event )
	...
	elseif self.menuName == event.menuName then
		...
		self:close()
		local f90_local0 = menu:getParent()
		f90_local0:processEvent( {
			name = "menu_changed",
			prevMenu = self,
			nextMenu = menu,
			controller = event.controller
