...
CoD.GameOptions.ShowStarForGametypeSettings = function ( button, gametypeSettings )
	for index, f7_local3 in ipairs( gametypeSettings ) do
		local settingInfo = CoD.GameOptions.GameSettings[f7_local3]
		if not settingInfo.shouldShow or settingInfo.shouldShow() then
			if settingInfo.setting then
				f7_local3 = settingInfo.setting
			end
...
CoD.GameOptions.AddSelectorButtons = function ( self, controller, options, editable )
	...
	for i = 1, #options, 1 do
		...
		if options[i].hintText ~= nil then
			button = self.buttonList:addGametypeSettingLeftRightSelector( controller, Engine.Localize( options[i].label ), options[i].settingName, Engine.Localize( options[i].hintText ) )
		else
			button = self.buttonList:addGametypeSettingLeftRightSelector( controller, Engine.Localize( options[i].label ), options[i].settingName )
		end
...
CoD.GameOptions.Button_EnabledDisabled_AddChoices = function ( self, controller, label, settingName )
	...
	local button = self.buttonList:addGametypeSettingLeftRightSelector( controller, Engine.Localize( label ), settingName )
	CoD.GameOptions.Button_AddChoices( controller, button, strings, values )
	return button
end
...
CoD.GameOptionsMenu.New = function ( controller, menuName )
	...
	self.infoPaneTitle:setTTF( "fonts/Entovo.ttf" )
	...
	self.infoPaneDefaultText:setTTF( "fonts/Entovo.ttf" )
	...
	self.infoPaneDescription:setTTF( "fonts/Entovo.ttf" )
...
CoD.GameOptionsMenu.addGametypeSetting = function ( self, controller, setting, forceCreation )
	...
	local button = self.buttonList:addGametypeSettingLeftRightSelector( controller, Engine.Localize( settingInfo.name ), setting, hintText )
	for index, value in ipairs( settingInfo.values ) do
		local f18_local3 = value
		if settingInfo.labels then
			f18_local3 = settingInfo.labels[index]
			if not f18_local3 then
				f18_local3 = settingInfo.labels[#settingInfo.labels]
			end
			f18_local3 = Engine.Localize( f18_local3, value )
		end
		button:addChoice( controller, f18_local3, value, nil, settingInfo.callback )
	end
	return button
end
