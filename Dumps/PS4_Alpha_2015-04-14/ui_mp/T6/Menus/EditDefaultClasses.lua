...
ClassButton_AddClassOptionsButton = function ( self, event )
	local buttonSize = 20
	if self.classOptionsButton == nil then
		local buttonLeft = 200
		self.classOptionsButton = CoD.MouseButton.new( {
			leftAnchor = true,
			rightAnchor = false,
			left = buttonLeft,
			...
			top = -buttonSize / 2,
			bottom = buttonSize / 2
		}, CoD.buttonStringsShortCut.mouse_edit )
		self.classOptionsButton:setActionEventName( "open_class_options" )
	end
	self:addElement( self.classOptionsButton )
end
...
ClassButton_GainFocus = function ( self, event )
	...
	if CoD.useMouse then
		ClassButton_AddClassOptionsButton( self )
	end
...
Close = function ( self )
	...
	if not f9_local0[self:getOwner()].editingPresetClass then
		Engine.StopEditingPresetClass()
		f9_local0 = CoD.perController
		f9_local1 = self
		f9_local0[self:getOwner()].isPreset = nil
	end
	CoD.Menu.close( self )
end
...
CreateClassButtons = function ( self )
	...
	for i = 0, 4, 1 do
		...
		classButton:setActionEventName( "class_chosen" )
...
CreateTeamButtons = function ( self )
	...
	for i = 1, numTeams, 1 do
		...
		teamButton:setActionEventName( "team_chosen" )
...
TeamChosen = function ( self, event )
	...
	if event then
		...
		local f22_local0 = CoD.perController
		local f22_local1 = self
		self.team = f22_local0[self:getOwner()].editingPresetClass
		for index, button in ipairs( self.teamButtons ) do
