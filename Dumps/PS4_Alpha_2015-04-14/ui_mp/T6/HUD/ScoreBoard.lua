...
function CreateScoreBoardBody( self, controller, frontEndOnly )
	...
	if not frontEndOnly then
		...
		gameTimer:setLeftRight( true, false, 10, 10 )
		gameTimer:setTopBottom( false, false, -CoD.textSize[HeaderFontName] / 2, CoD.textSize[HeaderFontName] / 2 )
		gameTimer:setFont( CoD.fonts[HeaderFontName] )
		header:addElement( gameTimer )
...
LUI.createMenu.Scoreboard = function ( controller )
	...
	if IsZodMap == true then
		local persistentItem = CoD.BuildableIcon.new( self, controller )
		persistentItem:setLeftRight( true, false, 0, 48 )
		persistentItem:setTopBottom( true, false, -80, -32 )
		persistentItem:setRGB( 1, 1, 1 )
		persistentItem:setAlpha( 1 )
		persistentItem.iconImage:setImage( RegisterImage( CoD.Zombie.PersistentItemInfo[CoD.Zombie.MAP_ZM_ZOD].image ) )
		...
		questItemList:setLeftRight( true, false, 0 + leftOffset, 256 + leftOffset )
		questItemList:setTopBottom( true, false, -80, -32 )
		...
		leftOffset = leftOffset + 256
		
		local shieldCraftableList = CoD.QuestItemList.new( menu, controller )
		shieldCraftableList:setLeftRight( true, false, 0 + leftOffset, 256 + leftOffset )
		shieldCraftableList:setTopBottom( true, false, -80, -32 )
		shieldCraftableList:setRGB( 1, 1, 1 )
		shieldCraftableList.questList:setDataSource( "ZombieCraftableList" )
		self:addElement( shieldCraftableList )
		self.shieldCraftableList = shieldCraftableList
		
		self:registerEventHandler( "update_zod_item_display_scoreboard", CoD.Zombie.ZodScoreboardUpdate )
	end
				...
				if CoD.isZombie == true then
					...
					elseif gameTypeGroup == CoD.Zombie.GAMETYPEGROUP_ZSURVIVAL then
						...
						elseif CoD.Zombie.IsSurvivalUsingCIAModel == true then
							factionR, factionG, factionB = Engine.GetFactionColor( "cia" )
						end
					end
				end
