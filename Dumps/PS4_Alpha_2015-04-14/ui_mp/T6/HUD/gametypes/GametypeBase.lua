...
CoD.GametypeBase.Refresh = function ( self, event )
	for index = 0, CoD.GametypeBase.maxObjectives - 1, 1 do
		self:createObjectiveIfNeeded( event, index )
		local objective_name = Engine.GetObjectiveName( event.controller, index )
		local objective = self.objectives[index]
		if objective then
...
CoD.ObjectiveBombSite.update = function ( self, event )
	...
	local f17_local0 = Engine.GetObjectiveGamemodeFlags( controller, self.index ) == CoD.ObjectiveBombSite.OBJECTIVE_FLAG_PLANTED
	local f17_local1 = Engine.GetGametypeSetting( "multiBomb" ) == 1
