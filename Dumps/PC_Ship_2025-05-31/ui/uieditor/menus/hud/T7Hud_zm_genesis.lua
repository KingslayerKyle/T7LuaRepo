...
LUI.createMenu.T7Hud_zm_genesis = function ( controller )
	...
	local WearablePerksContainerGenesis0 = CoD.WearablePerksContainer_Genesis.new( self, controller )
	...
	MPScore:subscribeToGlobalModel( controller, "PerController", "scriptNotify", function ( model )
		...
		if IsParamModelEqualToString( model, "score_event" ) and PropertyIsTrue( self, "menuLoaded" ) then
			...
			SetMPScoreText( self, f37_local0, controller, model )
		end
	end )
	...
	local TrialWidgetGenesis0 = CoD.TrialWidget_Genesis.new( self, controller )
	...
	TrialWidgetGenesis0:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "trialWidget.visible" ), function ( model )
		self:updateElementState( TrialWidgetGenesis0, {
			name = "model_validation",
			menu = self,
			modelValue = Engine.GetModelValue( model ),
	...
	if IsPC() then
		f4_local19 = CoD.IngameChatClientContainer.new( self, controller )
	else
	...
	local InventoryWidgetGenesis = CoD.InventoryWidgetGenesis.new( self, controller )
	...
	local ApothiconGatewormWidget = CoD.ApothiconGatewormWidget_Genesis.new( self, controller )
	...
	ApothiconGatewormWidget:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_SCOREBOARD_OPEN ), function ( model )
		self:updateElementState( ApothiconGatewormWidget, {
			name = "model_validation",
			menu = self,
			modelValue = Engine.GetModelValue( model ),
			modelName = "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_SCOREBOARD_OPEN
	...
	local KeeperProtectorWidgetsGenesis = CoD.KeeperProtectorWidgets_Genesis.new( self, controller )
	...
	KeeperProtectorWidgetsGenesis:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "zmInventory.widget_keeper_protector_parts" ), function ( model )
		self:updateElementState( KeeperProtectorWidgetsGenesis, {
			name = "model_validation",
			menu = self,
			modelValue = Engine.GetModelValue( model ),
	...
	local RocketShieldWidgetGenesis = CoD.RocketShieldWidget_Genesis.new( self, controller )
	...
	RocketShieldWidgetGenesis:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "zmInventory.widget_shield_parts" ), function ( model )
		self:updateElementState( RocketShieldWidgetGenesis, {
			name = "model_validation",
			menu = self,
			modelValue = Engine.GetModelValue( model ),
