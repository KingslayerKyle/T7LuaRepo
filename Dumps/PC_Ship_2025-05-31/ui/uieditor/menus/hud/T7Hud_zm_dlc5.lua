...
LUI.createMenu.T7Hud_zm_dlc5 = function ( controller )
	local self = CoD.Menu.NewForUIEditor( "T7Hud_zm_dlc5" )
	if PreLoadFunc then
	...
	self.buttonModel = Engine.CreateModel( Engine.GetModelForController( controller ), "T7Hud_zm_dlc5.buttonPrompts" )
	...
	MPScore:subscribeToGlobalModel( controller, "PerController", "scriptNotify", function ( model )
		...
		if IsParamModelEqualToString( model, "score_event" ) and PropertyIsTrue( self, "menuLoaded" ) then
			...
			SetMPScoreText( self, f36_local0, controller, model )
		end
	end )
	...
	local BubbleGumPackInGame = CoD.BubbleGumPackInGame.new( self, controller )
	...
	local SidequestIconInventoryWidget = CoD.SidequestIconInventoryWidget.new( self, controller )
	SidequestIconInventoryWidget:setLeftRight( false, false, -201.5, 201.5 )
	SidequestIconInventoryWidget:setTopBottom( false, true, -106, -12 )
	self:addElement( SidequestIconInventoryWidget )
	self.SidequestIconInventoryWidget = SidequestIconInventoryWidget
	
	local SidequestNotificationList = LUI.GridLayout.new( self, controller, false, 0, 0, 2, 0, nil, nil, false, false, 0, 0, false, false )
	...
	SidequestNotificationList:setVerticalCount( 4 )
	SidequestNotificationList:setDataSource( "ZMSidequestIconList" )
	SidequestNotificationList:mergeStateConditions( {
		{
	...
	self:addElement( SidequestNotificationList )
	self.SidequestNotificationList = SidequestNotificationList
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( element )
		...
		element.SidequestIconInventoryWidget:close()
		element.SidequestNotificationList:close()
		Engine.UnsubscribeAndFreeModel( Engine.GetModel( Engine.GetModelForController( controller ), "T7Hud_zm_dlc5.buttonPrompts" ) )
	end )
