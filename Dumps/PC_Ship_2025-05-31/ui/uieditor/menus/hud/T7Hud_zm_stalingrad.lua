...
LUI.createMenu.T7Hud_zm_stalingrad = function ( controller )
	...
	local ZMPrematchCountdown = CoD.ZM_PrematchCountdown.new( self, controller )
	...
	MPScore:subscribeToGlobalModel( controller, "PerController", "scriptNotify", function ( model )
		...
		if IsParamModelEqualToString( model, "score_event" ) then
			...
			SetMPScoreText( self, f50_local0, controller, model )
		end
	end )
	...
	local TimerListStalingrad = CoD.TimerList_Stalingrad.new( self, controller )
	...
	if IsPC() then
		f4_local17 = CoD.IngameChatClientContainer.new( self, controller )
	else
	...
	local InventoryWidgetStalingrad = CoD.InventoryWidgetStalingrad.new( self, controller )
	...
	local PodControlWidget = CoD.PodControlWidget.new( self, controller )
	...
	PodControlWidget:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "zmInventory.widget_cylinder" ), function ( model )
		self:updateElementState( PodControlWidget, {
			name = "model_validation",
			menu = self,
			modelValue = Engine.GetModelValue( model ),
	...
	local DragonCollarControllerWidgets = CoD.DragonCollarControllerWidgets_stalingrad.new( self, controller )
	...
	DragonCollarControllerWidgets:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "zmInventory.widget_dragonride_parts" ), function ( model )
		self:updateElementState( DragonCollarControllerWidgets, {
			name = "model_validation",
			menu = self,
			modelValue = Engine.GetModelValue( model ),
	...
	local DragonShieldWidgets = CoD.DragonShieldWidgets_Stalingrad.new( self, controller )
	...
	DragonShieldWidgets:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "zmInventory.widget_shield_parts" ), function ( model )
		self:updateElementState( DragonShieldWidgets, {
			name = "model_validation",
			menu = self,
			modelValue = Engine.GetModelValue( model ),
	...
	local DragonEggWidget = CoD.DragonEggWidget.new( self, controller )
	...
	DragonEggWidget:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_SCOREBOARD_OPEN ), function ( model )
		self:updateElementState( DragonEggWidget, {
			name = "model_validation",
			menu = self,
			modelValue = Engine.GetModelValue( model ),
			modelName = "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_SCOREBOARD_OPEN
	...
	local WearableBenefitsWidget = CoD.WearableBenefitsWidget.new( self, controller )
	...
	local DragonStrikeWidgetsStalingrad = CoD.DragonStrikeWidgets_Stalingrad.new( self, controller )
	...
	DragonStrikeWidgetsStalingrad:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "zmInventory.widget_dragon_strike" ), function ( model )
		self:updateElementState( DragonStrikeWidgetsStalingrad, {
			name = "model_validation",
			menu = self,
			modelValue = Engine.GetModelValue( model ),
