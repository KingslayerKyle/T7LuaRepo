...
LUI.createMenu.ChooseShowcaseWeapon = function ( controller )
	...
	self:AddButtonCallbackFunction( weaponList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( element, menu, controller, model )
		if not IsCACItemLocked( menu, element, controller ) and IsCACItemPurchased( element, controller ) and IsCACWeaponVariantAvailable( element, controller ) and not IsSelfModelValueTrue( element, controller, "isBMClassified" ) and not IsCACItemContractLocked( element, controller ) then
			CallCustomElementFunction_Self( self, "updateMode", Enum.ShowcaseWeaponMode.SHOWCASEWEAPON_VARIANTS )
			return true
		elseif not IsCACItemLocked( menu, element, controller ) and IsCACItemPurchased( element, controller ) and not IsCACWeaponVariantAvailable( element, controller ) and not IsSelfModelValueTrue( element, controller, "isBMClassified" ) and not IsCACItemContractLocked( element, controller ) then
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "ShowcaseWeapon.Mode" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f68_local0, self, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE )
	end )
	...
	self:registerEventHandler( "menu_opened", function ( self, event )
		...
		if not f69_local0 then
			f69_local0 = self:dispatchEventToChildren( event )
		end
		return f69_local0
	end )
