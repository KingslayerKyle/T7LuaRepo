...
LUI.createMenu.PlaylistSelection = function ( controller )
	...
	PlaylistDLC1CRM:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "LastInput" ), function ( model )
		self:updateElementState( PlaylistDLC1CRM, {
			name = "model_validation",
			menu = self,
			modelValue = Engine.GetModelValue( model ),
			modelName = "LastInput"
	...
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, "P", function ( element, menu, controller, model )
		if ShouldSquareOpenStore( menu, controller, "PlaylistDLC1CRMContextual" ) and IsPC() then
			OpenSteamStore( self, element, controller, "", menu )
			return true
		elseif ShouldSquareOpenStore( menu, controller, "PlaylistDLC1CRMContextual" ) then
