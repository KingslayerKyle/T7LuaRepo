...
LUI.createMenu.BlackMarket = function ( controller )
	...
	self:AddButtonCallbackFunction( cryptokeyList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, nil, function ( element, menu, controller, model )
		if IsElementInState( element, "OutfitShop" ) then
			ProcessListAction( self, element, controller )
			return true
		elseif IsCryptokeyWidgetAvailable( self, element, controller ) then
			ProcessListAction( self, element, controller )
			return true
		else
		...
		if IsElementInState( element, "OutfitShop" ) then
			CoD.Menu.SetButtonLabel( menu, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "MENU_GO_TO_STORE" )
			return true
		elseif IsCryptokeyWidgetAvailable( self, element, controller ) then
	...
	self:AddButtonCallbackFunction( buttonList0, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, nil, function ( element, menu, controller, model )
		ProcessListAction( self, element, controller )
		return true
	end, function ( element, menu, controller )
		CoD.Menu.SetButtonLabel( menu, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "MENU_SELECT" )
	...
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, "S", function ( element, menu, controller, model )
		OpenOutfitStore( self, element, controller, "BlackMarket", menu )
		return true
	end, function ( element, menu, controller )
		CoD.Menu.SetButtonLabel( menu, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, "MENU_VIEW_STORE_OUTFITS" )
		return true
	end, false )
