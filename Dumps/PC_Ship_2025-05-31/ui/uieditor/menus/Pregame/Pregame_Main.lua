...
LUI.createMenu.Pregame_Main = function ( controller )
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "lobbyRoot.Pregame.state" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f275_local0, self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	self:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "lobbyRoot.Pregame.stage" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f276_local0, self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	self:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "factions.isCoDCaster" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f277_local0, self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	...
	self:registerEventHandler( "menu_loaded", function ( self, event )
		...
		if not f278_local0 then
			f278_local0 = self:dispatchEventToChildren( event )
		end
		return f278_local0
	end )
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, "E", function ( element, menu, controller, model )
		if ItemRestrictionCACEditActive() and IsGlobalModelValueEqualTo( element, controller, "lobbyRoot.Pregame.stage", 102 ) and not IsCodCaster( controller ) then
			SetPregameEditIncomplete( controller )
			return true
		elseif ItemRestrictionScorestreakEditActive() and IsGlobalModelValueEqualTo( element, controller, "lobbyRoot.Pregame.stage", 102 ) and not IsCodCaster( controller ) then
			SetPregameEditIncomplete( controller )
			return true
		else
		...
		if ItemRestrictionCACEditActive() and IsGlobalModelValueEqualTo( element, controller, "lobbyRoot.Pregame.stage", 102 ) and not IsCodCaster( controller ) then
			CoD.Menu.SetButtonLabel( menu, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, "MENU_OPEN_SCORESTREAK_EDIT" )
			return true
		elseif ItemRestrictionScorestreakEditActive() and IsGlobalModelValueEqualTo( element, controller, "lobbyRoot.Pregame.stage", 102 ) and not IsCodCaster( controller ) then
			CoD.Menu.SetButtonLabel( menu, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, "MENU_OPEN_EDIT_CAC" )
			return true
		else
			return false
		end
