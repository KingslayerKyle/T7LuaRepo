...
LUI.createMenu.SecondaryWeaponSelect = function ( controller )
	...
	self:AddButtonCallbackFunction( weaponList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( element, menu, controller, model )
		if not IsCACItemLocked( menu, element, controller ) and not IsCACItemPurchased( element, controller ) and not IsCACHaveTokens( controller ) and not IsSelfModelValueTrue( element, controller, "isBMClassified" ) and not IsCACItemContractLocked( element, controller ) then
			SetUnlockConfirmationInfo( element, controller )
			OpenPopup( self, "OutOfUnlockTokens", controller, "", "" )
			return true
		elseif not IsCACItemLocked( menu, element, controller ) and not IsCACItemPurchased( element, controller ) and IsCACHaveTokens( controller ) and not IsSelfModelValueTrue( element, controller, "isBMClassified" ) and not IsCACItemContractLocked( element, controller ) then
			OpenUnlockClassItemDialog( menu, element, controller )
			return true
		elseif not IsCACItemLocked( menu, element, controller ) and IsCACItemPurchased( element, controller ) and IsCACWeaponVariantAvailable( element, controller ) and not IsSelfModelValueTrue( element, controller, "isBMClassified" ) and not IsCACItemContractLocked( element, controller ) then
			...
			return true
		elseif not IsCACItemLocked( menu, element, controller ) and not IsSelfModelValueTrue( element, controller, "isBMClassified" ) and not IsCACItemContractLocked( element, controller ) then
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "WGSV_Category" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f60_local0, self, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE )
	end )
