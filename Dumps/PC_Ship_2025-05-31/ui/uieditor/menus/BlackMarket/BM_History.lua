...
LUI.createMenu.BM_History = function ( controller )
	...
	local decryptionList = LUI.UIList.new( self, controller, 50, 210, nil, false, false, 0, 0, false, false )
	...
	decryptionList:linkToElementModel( decryptionList, "category", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f4_local0, self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	...
	local BMLogo = CoD.BM_LogoRecently.new( self, controller )
	...
	local horizontalCounter = CoD.BM_HorizontalCounter.new( self, controller )
