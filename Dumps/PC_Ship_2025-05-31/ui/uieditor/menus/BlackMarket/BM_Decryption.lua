...
LUI.createMenu.BM_Decryption = function ( controller )
	...
	decryptionList:linkToElementModel( decryptionList, "category", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f37_local0, self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	...
	local TradeAgainList = LUI.UIList.new( self, controller, 15, 0, nil, false, false, 0, 0, false, false )
	...
	TradeAgainList:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "CryptoKeyProgress.codPoints" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f47_local0, self, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS )
	end )
	TradeAgainList:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "CryptoKeyProgress.keyCount" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f48_local0, self, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS )
	end )
	TradeAgainList:linkToElementModel( TradeAgainList, "itemCODPointCost", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f49_local0, self, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS )
	end )
	...
	self:AddButtonCallbackFunction( TradeAgainList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, nil, function ( element, menu, controller, model )
		...
		elseif IsSelfModelValueGreaterThan( element, controller, "itemCODPointCost", 0 ) and IsMenuInState( menu, "Results" ) then
			OpenCodPointsStore( self, element, controller, "BMTradeAgainCPStore", menu )
			return true
		else
			return true
		end
		...
		if CanAffordTradeAgain( element, controller ) and IsMenuInState( menu, "Results" ) then
			...
			return true
		elseif IsSelfModelValueGreaterThan( element, controller, "itemCODPointCost", 0 ) and IsMenuInState( menu, "Results" ) then
			CoD.Menu.SetButtonLabel( menu, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "MPUI_PURCHASE_CODPOINTS" )
			return true
		else
	...
	local BMDecryptionNoBundleUses = CoD.BM_DecryptionNoBundleUses.new( self, controller )
	...
	local DebugLootItemStats = CoD.BM_Debug_LootItemStats.new( self, controller )
	...
	local BMGunMeter0 = CoD.BM_GunMeter.new( self, controller )
	...
	self:registerEventHandler( "menu_opened", function ( self, event )
		...
		if not f99_local0 then
			f99_local0 = self:dispatchEventToChildren( event )
		end
		return f99_local0
	end )
	...
	LUI.OverrideFunction_CallOriginalFirst( self, "setState", function ( element, controller )
		if IsSelfInState( self, "Results" ) then
			...
			CheckAndShowGunMeter( self, controller, self )
		end
	end )
