...
CoD.CombatRecordSummaryPanel.new = function ( menu, controller )
	...
	menu:AddButtonCallbackFunction( PrimaryProfileSnapshot, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( element, menu, controller, model )
		if not IsCombatRecordForRemotePlayer() and IsControllerModelValueDefaultID64Value( element, controller, "CombatRecordProfileSnapshot.fileId" ) then
			CombatRecordOpenProfileSnapshot( menu, controller )
			return true
		elseif not IsControllerModelValueDefaultID64Value( element, controller, "CombatRecordProfileSnapshot.fileId" ) then
			CombatRecordProfileSnapshotFullscreenView( self, controller )
			return true
		else
	...
	menu:AddButtonCallbackFunction( PrimaryProfileSnapshot, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, "C", function ( element, menu, controller, model )
		if not IsCombatRecordForRemotePlayer() and not IsControllerModelValueDefaultID64Value( element, controller, "CombatRecordProfileSnapshot.fileId" ) then
			CombatRecordClearProfileSnapshot( controller )
			return true
		else
		...
		if not IsCombatRecordForRemotePlayer() and not IsControllerModelValueDefaultID64Value( element, controller, "CombatRecordProfileSnapshot.fileId" ) then
			CoD.Menu.SetButtonLabel( menu, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, "MENU_CLEAR" )
			return true
		else
			return false
		end
	...
	menu:AddButtonCallbackFunction( CallingCardShowcaseList, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, "C", function ( element, menu, controller, model )
		if not IsCombatRecordForRemotePlayer() and not IsSelfModelValueEqualTo( element, controller, "backgroundID", 0 ) then
			CombatRecordClearCallingCard( menu, element, controller )
			return true
		else
	...
	self:registerEventHandler( "gain_focus", function ( self, event )
		if self.m_focusable and self.PrimaryProfileSnapshot:processEvent( event ) then
			return true
		else
