...
CoD.Store_NonFeaturedFrame.new = function ( menu, controller )
	...
	menu:AddButtonCallbackFunction( productList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, nil, function ( element, menu, controller, model )
		if IsSelfModelValueEqualToEnum( element, controller, "purchasestatus", Enum.StoreProductPurchaseStatus.STORE_PRODUCT_NOT_PURCHASED ) and not IsPerControllerTablePropertyValue( controller, "selectedStoreCategory", "EXTRASLOTS" ) and not IsPerControllerTablePropertyValue( controller, "selectedStoreCategory", "ESPORTS" ) and not IsPerControllerTablePropertyValue( controller, "selectedStoreCategory", "EXTRAPACKS" ) and not IsPerControllerTablePropertyValue( controller, "selectedStoreCategory", "OUTFITS" ) then
			PurchaseProduct( self, element, controller )
			PlaySoundSetSound( self, "action" )
			return true
		elseif IsSelfModelValueEqualToEnum( element, controller, "purchasestatus", Enum.StoreProductPurchaseStatus.STORE_PRODUCT_PURCHASED ) and not IsPerControllerTablePropertyValue( controller, "selectedStoreCategory", "EXTRASLOTS" ) and not IsPerControllerTablePropertyValue( controller, "selectedStoreCategory", "ESPORTS" ) and not IsPerControllerTablePropertyValue( controller, "selectedStoreCategory", "EXTRAPACKS" ) and not IsPerControllerTablePropertyValue( controller, "selectedStoreCategory", "OUTFITS" ) then
			PurchaseProduct( self, element, controller )
			return true
		...
		elseif IsPerControllerTablePropertyValue( controller, "selectedStoreCategory", "ESPORTS" ) and not DoesPlayerHaveEsportsPackItem( element, controller ) then
			...
			return true
		elseif IsPerControllerTablePropertyValue( controller, "selectedStoreCategory", "EXTRAPACKS" ) and not DoesPlayerHaveEsportsPackItem( element, controller ) then
			OpenBuyInventoryItemDialog( self, element, controller, "Store" )
			return true
		elseif IsPerControllerTablePropertyValue( controller, "selectedStoreCategory", "OUTFITS" ) and not DoesPlayerHaveOutfitItem( element, controller ) then
			OpenBuyInventoryItemDialog( self, element, controller, "Store" )
			return true
		else
