...
CoD.AdminTabGroupAdminsFrame.new = function ( menu, controller )
	...
	menu:AddButtonCallbackFunction( AdminsList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, nil, function ( element, menu, controller, model )
		if ListHasElements( element ) then
			SetSelectedFriendXUID( self, element, controller )
			OpenAdminPrivilegesPopup( self, element, controller )
			return true
		else
	...
	self:registerEventHandler( "gain_focus", function ( self, event )
		if self.m_focusable and self.AdminsList:processEvent( event ) then
			return true
		else
