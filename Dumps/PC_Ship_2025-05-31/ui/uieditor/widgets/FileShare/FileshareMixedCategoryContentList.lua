...
CoD.FileshareMixedCategoryContentList.new = function ( menu, controller )
	...
	menu:AddButtonCallbackFunction( contentList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, nil, function ( element, menu, controller, model )
		if IsFileshareCategoryScreenshot( self, element, controller ) and FileshareHasCurrentScreenshot( controller ) then
			FileshareSetSelectedItem( self, element, controller, "true" )
			FileshareOpenFullscreenView( self, element, controller, menu )
			return true
		elseif not IsFileshareCategoryScreenshot( self, element, controller ) then
	...
	menu:AddButtonCallbackFunction( contentList, controller, Enum.LUIButton.LUI_KEY_START, nil, function ( element, menu, controller, model )
		if FileshareCanShowOptionsMenu( controller ) and IsFileshareCategoryScreenshot( self, element, controller ) and FileshareHasCurrentScreenshot( controller ) then
			OpenPopup( self, "FileshareOptions", controller, "", "" )
			PreserveThumbnails( controller, true )
			return true
		elseif FileshareCanShowOptionsMenu( controller ) and not IsFileshareCategoryScreenshot( self, element, controller ) then
			OpenPopup( self, "FileshareOptions", controller, "", "" )
			PreserveThumbnails( controller, true )
			return true
		else
