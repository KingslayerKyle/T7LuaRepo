...
LUI.createMenu.Loading = function ( controller )
	...
	if Engine.IsMultiplayerGame() then
		Engine.PlayMenuMusic( "load_" .. mapName )
	end
	if true == Dvar.ui_useloadingmovie:get() or CoD.isCampaign then
		if true == Engine.IsCampaignModeZombies() then
			mapName = string.gsub( mapName, "mi", "nightmares_mi" )
		end
		local movie = CoD.GetMapValue( mapName, "introMovie" )
		if mapName == "cp_sh_mobile" and Dvar.cp_queued_level:get() == "cp_mi_eth_prologue" then
			movie = "cp_prologue_load_loadingmovie"
		elseif mapName == "cp_sh_singapore" and Dvar.cp_queued_level:get() == "cp_mi_sing_blackstation" then
	...
	if Engine.IsLevelPreloaded( mapName ) then
	...
	loadingMenu:addElement( loadingMenu.mapImage )
	...
	if not Engine.IsMultiplayerGame() and not Engine.IsZombiesGame() then
		loadingMenu.cinematicSubtitles = CoD.MovieSubtitles.new( self, controller )
		loadingMenu.cinematicSubtitles:setLeftRight( false, false, -640, 640 )
		loadingMenu.cinematicSubtitles:setTopBottom( false, false, -360, 360 )
		loadingMenu:addElement( loadingMenu.cinematicSubtitles )
	end
	if showMovie == true then
		loadingMenu.mapImage:registerEventHandler( "loading_updateimage", f0_local0 )
		loadingMenu.mapImage.id = "loadingMenu.mapImage"
...
CoD.Loading.AddNewLoadingScreen = function ( self )
	...
	menu.loadingScreenOverlay = loadingScreenOverlay
	...
	LUI.OverrideFunction_CallOriginalSecond( menu, "close", function ( self )
		self.loadingScreenOverlay:close()
	end )
end
