...
CoD.DemoHUD.DollyCamPictureWindowUpdateVisibility = function ( self, event )
	local controller = event.controller
	if not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_DEMO_ALL_GAME_HUD_HIDDEN ) and Engine.IsDemoPlaying() and Engine.IsDemoCameraEditMode() and Engine.GetDollyCamMarkerCount() > 0 and not Engine.IsDemoClipPreviewRunning() then
