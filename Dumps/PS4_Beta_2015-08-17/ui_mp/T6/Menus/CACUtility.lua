...
CoD.CACUtility.UpdateEmptySlotImages = function ( controller, classNum, classModel )
	...
	local isUsingOverkill = CoD.CACUtility.IsBonusCardEquippedByName( class, "bonuscard_overkill" )
	local isUsingTwoTacticals = CoD.CACUtility.IsBonusCardEquippedByName( class, "bonuscard_two_tacticals" )
	for slotName, f82_local3 in pairs( CoD.CACUtility.emptyImageSlotList ) do
		local slotModel = Engine.GetModel( classModel, slotName )
		if slotModel then
...
function AnyAttachmentBanned( weaponIndex, attachments )
	for attachmentIndex, attachment in ipairs( attachments ) do
		if Engine.GetAttachmentVote( attachment ) == Enum.VoteType.VOTE_TYPE_BAN then
			return true
		elseif Engine.GetItemAttachmentVote( weaponIndex, attachment ) == Enum.VoteType.VOTE_TYPE_BAN then
			return true
		end
	end
	return false
					...
					if itemIndex > CoD.CACUtility.EmptyItemIndex then
						if LUI.startswith( slotName, "primarygadgetattachment" ) then
							...
							if gadgetIndexModel then
								...
								table.insert( completeItemList, getItemModels( slotName, customSlotModel, categorySpacing, f124_local5 ) )
							end
						end
						...
						if not LUI.startswith( weaponCategory, "secondaryattachment" ) or itemName ~= "bonuscard_overkill" then
							...
							if f124_local4 and (not (f124_local4 ~= "primary" or not LUI.startswith( slotName, "primaryattachment" )) or not (not LUI.startswith( slotName, "primaryattachment" ) or not LUI.startswith( slotName, "primaryattachment" )) or not (f124_local4 ~= "secondary" or not LUI.startswith( slotName, "secondaryattachment" )) or not (not LUI.startswith( slotName, "secondaryattachment" ) or not LUI.startswith( slotName, "secondaryattachment" )) or not (f124_local4 ~= "specialty1" or slotName ~= "specialty4") or not (f124_local4 ~= "specialty2" or slotName ~= "specialty5") or not (f124_local4 ~= "specialty3" or slotName ~= "specialty6") or LUI.startswith( f124_local4, "bonuscard" ) and LUI.startswith( slotName, "bonuscard" )) then
								completeItemList[itemListPrevIndex].properties.spacing = itemSpacing
								f124_local5 = false
							end
							table.insert( completeItemList, getItemModels( slotName, slotModel, spacing, f124_local5 ) )
						end
					end
				end
			end
...
f0_local4 = function ( controller, weaponVariantInfo )
	...
	infoTable.models.listDataSource = f0_local3( controller, infoTable, weaponVariantInfo )
	return infoTable
end
