	...
	for index, gameMode in pairs( gameModes ) do
		if gameMode.category == CoD.perController[controller].gameModeCategory then
			...
			local f3_local5 = {
				models = {
					text = gameMode.name,
					image = Engine.StructTableLookupString( CoD.gametypesStructTable, "name", gameMode.gametype, "image" ),
					description = Engine.StructTableLookupString( CoD.gametypesStructTable, "name", gameMode.gametype, "description" )
				}
			}
			local f3_local6 = {
			...
			if f3_local7 then
				f3_local7 = not isGameModeInDemo( gameMode.gametype )
			end
