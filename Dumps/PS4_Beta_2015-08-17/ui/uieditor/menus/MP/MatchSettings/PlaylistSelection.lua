	...
	if CoD.perController[controller].playlistCategory then
		...
		for index, playlist in ipairs( CoD.perController[controller].playlistCategory.playlists ) do
			...
			if CoD.perController[controller].playlistCategory.playerCount > 0 then
				if Engine.DvarBool( nil, "groupCountsVisible" ) == true then
					f4_local3 = Engine.Localize( "MENU_PLAYLIST_USER_COUNT", CoD.separateNumberWithCommas( playlist.playerCount ), CoD.separateNumberWithCommas( CoD.perController[controller].playlistCategory.playerCount ) )
				else
					f4_local3 = Engine.Localize( "MENU_PLAYLIST_USER_PERCENT", math.floor( playlist.playerCount / CoD.perController[controller].playlistCategory.playerCount * 100 + 0.5 ) )
				end
			end
