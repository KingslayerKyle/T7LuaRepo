...
local GetEquippedSecondaryAttachmentLoudoutSlot = function ( self, controller, itemIndex )
	for index = 1, CoD.CACUtility.maxSecondaryAttachments, 1 do
		local slotName = "secondaryattachment" .. index
...
local PostLoadFunc = function ( self, controller )
	...
	self.getEquippedLoadoutSlot = GetEquippedSecondaryAttachmentLoudoutSlot
end

LUI.createMenu.SecondaryAttachmentSelect = function ( controller )
	local self = CoD.Menu.NewForUIEditor( "SecondaryAttachmentSelect" )
	if PreLoadFunc then
	...
	self.buttonModel = Engine.CreateModel( Engine.GetModelForController( controller ), "SecondaryAttachmentSelect.buttonPrompts" )
	...
	selectMenuWidget.background.cac3dTitleIntermediary0.FE3dTitleContainer0.MenuTitle.TextBox1.Label0:setText( Engine.Localize( GetSecondaryAttachmentMenuTitle( controller, "MPUI_ATTACHMENTS_CAPS" ) ) )
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( self )
		...
		Engine.UnsubscribeAndFreeModel( Engine.GetModel( Engine.GetModelForController( controller ), "SecondaryAttachmentSelect.buttonPrompts" ) )
	end )
