...
Lobby.Events.GetScheduledEvents = function ( f4_arg0, f4_arg1 )
	...
	for f4_local4 = 0, f4_local2 - 1, 1 do
		...
		if f4_local7 == "all" or f4_local7 == Lobby.Events.GetPlatform() then
			...
			if Engine.IsInRange( tostring( Engine.GetServerUTCTimeStr() ), f4_local12, f4_local13 ) and (Engine.IsDedicatedServer() or f4_local8 == nil or f4_local8 == "" or CoDShared.IsInExperiment( f4_local8, f4_local9 )) and Lobby.Events.IsInCustomRule( f4_local10 ) then
				...
				local f4_local14 = Engine.StructTableLookupString( f4_local0, "name", f4_local11 .. "_ON", "timer_name" )
				if f4_local14 ~= nil and f4_local14 ~= "" then
					local f4_local15 = Engine.CreateModel( Engine.GetGlobalModel(), "autoevents" )
					Engine.SetModelValue( Engine.CreateModel( f4_local15, f4_local14 ), LuaUtils.SecondsToTimeRemainingString( Engine.GetSecondsRemainingServer( f4_local13 ) + 1 ) )
					Engine.SetModelValue( Engine.CreateModel( f4_local15, f4_local14 .. "_red" ), Engine.GetSecondsRemainingServer( f4_local13 ) < 300 )
				end
			end
