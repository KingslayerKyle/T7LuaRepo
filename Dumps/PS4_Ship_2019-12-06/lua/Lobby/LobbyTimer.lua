...
Lobby.Timer.ManualCPPump = function ()
	...
	local f26_local3 = 0
	local f26_local4 = 0
	local f26_local5 = Engine.GetLobbyClientCount( f26_local1 )
	local f26_local6 = f26_local5 == 0
	Lobby.MapVote.ShowLockedIn()
	local f26_local7 = Engine.CreateModel( Engine.GetGlobalModel(), "MapVote.timerActive" )
	local f26_local8 = Engine.CreateModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyTimeRemaining" )
	local f26_local9 = 0
	if f26_local6 then
		Lobby.Timer.lobbyTimer.status = Lobby.Timer.LOBBY_STATUS.IDLE
		Lobby.Timer.lobbyTimer.endTime = Lobby.Timer.MATCH_START_INVALID
	elseif Lobby.Timer.lobbyTimer.endTime ~= Lobby.Timer.MATCH_START_INVALID then
		local f26_local10 = Engine.CreateModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyStatus" )
		if f26_local5 == 1 then
			Engine.SetModelValue( f26_local10, "" )
			Lobby.Timer.lobbyTimer.endTime = Lobby.Timer.MATCH_START_INVALID
			...
			if f26_local4 > 0 then
				Engine.SetModelValue( f26_local10, Engine.Localize( "MENU_GAME_STARTING_IN", f26_local4 ) )
				f26_local9 = f26_local4
			else
				Engine.SetModelValue( f26_local10, "" )
			end
			if f26_local4 ~= Lobby.Timer.lastValue then
				...
				Engine.PlaySound( LobbyData.Sounds.TimerTick )
				Engine.SetModelValue( f26_local7, 1 )
				Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "Time left: " .. f26_local4 .. "\n" )
			end
			local f26_local11 = false
			if Engine.IsLobbyMigratActive then
				f26_local11 = Engine.IsLobbyMigratActive( Enum.LobbyType.LOBBY_TYPE_GAME )
			end
			if Lobby.ProcessQueue.IsQueueEmpty() and f26_local11 == false and f26_local4 <= 0 and LobbyVM.LaunchGameExec( f26_local0, f26_local1 ) == true then
				Engine.SetModelValue( f26_local10, "" )
				Lobby.Timer.lobbyTimer.endTime = Lobby.Timer.MATCH_START_INVALID
				return 
			end
			f26_local3 = math.max( f26_local4, 0 )
		end
	else
		Lobby.Timer.lastValue = 0
		Engine.SetModelValue( f26_local7, 0 )
	end
	Engine.SetModelValue( f26_local8, f26_local9 )
...
Lobby.Timer.ManualCPPumpClient = function ( f28_arg0 )
	...
	local f28_local5 = Engine.CreateModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyTimeRemaining" )
	local f28_local6 = 0
	...
	Engine.SetModelValue( f28_local5, f28_local6 )
end
...
Lobby.Timer.TestingPump = function ()
	...
	local f29_local3 = 0
	local f29_local4 = 0
	local f29_local5 = Engine.GetLobbyClientCount( f29_local1 ) == 0
	Lobby.MapVote.ShowLockedIn()
	local f29_local6 = Engine.CreateModel( Engine.GetGlobalModel(), "MapVote.timerActive" )
	local f29_local7 = Engine.CreateModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyTimeRemaining" )
	local f29_local8 = 0
	if f29_local5 then
		Lobby.Timer.lobbyTimer.status = Lobby.Timer.LOBBY_STATUS.IDLE
		Lobby.Timer.lobbyTimer.endTime = Lobby.Timer.MATCH_START_INVALID
	...
	elseif Lobby.Timer.lobbyTimer.endTime ~= Lobby.Timer.MATCH_START_INVALID then
		local f29_local9 = Engine.CreateModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyStatus" )
		...
		if f29_local4 > 0 then
			Engine.SetModelValue( f29_local9, Engine.Localize( "MENU_GAME_STARTING_IN", f29_local4 ) )
			f29_local8 = f29_local4
		else
			Engine.SetModelValue( f29_local9, "" )
		end
		if f29_local4 ~= Lobby.Timer.lastValue then
			Lobby.Timer.lastValue = f29_local4
			Engine.PlaySound( LobbyData.Sounds.TimerTick )
			Engine.SetModelValue( f29_local6, 1 )
			if f29_local4 <= 10 then
		...
		Lobby.Timer.lastValue = 0
		Engine.SetModelValue( f29_local6, 0 )
	end
	Engine.SetModelValue( f29_local7, f29_local8 )
...
Lobby.Timer.TestingPumpClient = function ( f31_arg0 )
...
Lobby.Timer.NonePumpClient = function ( f34_arg0 )
	
end

Lobby.Timer.UpdateLobbyStatusInfo = function ( f35_arg0 )
	...
	local f35_local4 = f35_arg0.statusValue
	if Engine.IsLobbyHost( f35_local1 ) then
		return 
	end
	local f35_local5 = LobbyData:UITargetFromId( Engine.GetLobbyUIScreen() )
	local f35_local6 = f35_local5.lobbyTimerType
	if f35_local6 == LuaEnums.TIMER_TYPE.NONE then
		
	elseif f35_local6 == LuaEnums.TIMER_TYPE.AUTO_CP then
		Lobby.Timer.AutoCPPumpClient( f35_arg0 )
	elseif f35_local6 == LuaEnums.TIMER_TYPE.AUTO_MP then
		Lobby.Timer.AutoMPPumpClient( f35_arg0 )
	elseif f35_local6 == LuaEnums.TIMER_TYPE.AUTO_ZM then
		Lobby.Timer.AutoZMPumpClient( f35_arg0 )
	elseif f35_local6 == LuaEnums.TIMER_TYPE.MANUAL then
		Lobby.Timer.ManualPumpClient( f35_arg0 )
	elseif f35_local6 == LuaEnums.TIMER_TYPE.MANUAL_CP then
		Lobby.Timer.ManualCPPumpClient( f35_arg0 )
	elseif f35_local6 == LuaEnums.TIMER_TYPE.READYUP then
		Lobby.Timer.ManualPumpClient( f35_arg0 )
	elseif f35_local6 == LuaEnums.TIMER_TYPE.THEATER then
		Lobby.Timer.ManualPumpClient( f35_arg0 )
	elseif f35_local6 == LuaEnums.TIMER_TYPE.TESTING then
		Lobby.Timer.TestingPumpClient( f35_arg0 )
	end
end

Lobby.Timer.HostingLobby = function ( f36_arg0 )
	local f36_local0 = f36_arg0.controller
	local f36_local1 = f36_arg0.lobbyType
	local f36_local2 = f36_arg0.mainMode
	local f36_local3 = f36_arg0.lobbyTimerType
	local f36_local4 = f36_arg0.matchEnded
	if f36_local1 ~= Enum.LobbyType.LOBBY_TYPE_GAME then
		return false
	end
	Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "Lobby.Timer.HostingLobby: Creating a lobby timer.\n" )
	Lobby.Pregame.Shutdown( f36_local0 )
	Lobby.Timer.Clear( true )
	Lobby.Timer.lobbyTimer = {}
	Lobby.Timer.lobbyTimer.controller = f36_local0
	Lobby.Timer.lobbyTimer.lobbyType = f36_local1
	Lobby.Timer.lobbyTimer.mainMode = f36_local2
	Lobby.Timer.lobbyTimer.lobbyTimerType = f36_local3
	if LuaUtils.IsArenaMode() and f36_local4 then
	...
	if f36_arg0.status ~= nil then
		if f36_arg0.status == Lobby.Timer.LOBBY_STATUS.RESET_TO_NEED then
			Lobby.Timer.lobbyTimer.status = f36_arg0.status
			Engine.SetGameLobbyStatusInfo( f36_arg0.status, 0 )
		elseif f36_arg0.status ~= Lobby.Timer.LOBBY_STATUS.IDLE then
			Lobby.Timer.lobbyTimer.status = f36_arg0.status
			if f36_arg0.status == Lobby.Timer.LOBBY_STATUS.STARTING and f36_arg0.statusValue < 5 then
				f36_arg0.statusValue = Dvar.lobbyTimerStatusStartInterval:get() / 1000
			end
			Lobby.Timer.lobbyTimer.endTime = Engine.milliseconds() + f36_arg0.statusValue * 1000
			Engine.SetGameLobbyStatusInfo( f36_arg0.status, f36_arg0.statusValue )
		end
	end
	if f36_local3 == LuaEnums.TIMER_TYPE.NONE then
		Lobby.Timer.lobbyTimer.pumpFunc = Lobby.Timer.NonePump
		Lobby.Timer.lobbyTimer.endFunc = Lobby.Timer.NonePumpEnd
	elseif f36_local3 == LuaEnums.TIMER_TYPE.AUTO_CP then
		Lobby.Timer.lobbyTimer.pumpFunc = Lobby.Timer.AutoCPPump
		Lobby.Timer.lobbyTimer.endFunc = Lobby.Timer.AutoCPPumpEnd
	elseif f36_local3 == LuaEnums.TIMER_TYPE.AUTO_MP then
		Lobby.Timer.lobbyTimer.pumpFunc = Lobby.Timer.AutoMPPump
		Lobby.Timer.lobbyTimer.endFunc = Lobby.Timer.AutoMPPumpEnd
	elseif f36_local3 == LuaEnums.TIMER_TYPE.AUTO_ZM then
		Lobby.Timer.lobbyTimer.pumpFunc = Lobby.Timer.AutoZMPump
		Lobby.Timer.lobbyTimer.endFunc = Lobby.Timer.AutoZMPumpEnd
	elseif f36_local3 == LuaEnums.TIMER_TYPE.MANUAL or f36_local3 == LuaEnums.TIMER_TYPE.READYUP or f36_local3 == LuaEnums.TIMER_TYPE.THEATER then
		Lobby.Timer.lobbyTimer.pumpFunc = Lobby.Timer.ManualPump
		Lobby.Timer.lobbyTimer.endFunc = Lobby.Timer.ManualPumpEnd
	elseif f36_local3 == LuaEnums.TIMER_TYPE.MANUAL_CP then
		Lobby.Timer.lobbyTimer.pumpFunc = Lobby.Timer.ManualCPPump
		Lobby.Timer.lobbyTimer.endFunc = Lobby.Timer.ManualCPPumpEnd
	elseif f36_local3 == LuaEnums.TIMER_TYPE.TESTING then
...
Lobby.Timer.HostingLobbyEnd = function ( f37_arg0 )
	if f37_arg0.lobbyType ~= Enum.LobbyType.LOBBY_TYPE_GAME or Lobby.Timer.lobbyTimer == nil then
		return true
	else
		Engine.SetModelValue( Engine.CreateModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyStatus" ), "" )
...
Lobby.Timer.LaunchGame = function ( f38_arg0 )
	if Lobby.Timer.lobbyTimer == nil then
		return 
	end
	local f38_local0 = Lobby.Timer.lobbyTimer.controller
	local f38_local1 = Lobby.Timer.lobbyTimer.lobbyType
	local f38_local2 = Lobby.Timer.lobbyTimer.lobbyTimerType
	Lobby.Timer.lobbyTimer.data = f38_arg0
	if f38_local2 == LuaEnums.TIMER_TYPE.MANUAL or f38_local2 == LuaEnums.TIMER_TYPE.READYUP or f38_local2 == LuaEnums.TIMER_TYPE.THEATER then
		if Lobby.Timer.lobbyTimer.endTime == Lobby.Timer.MATCH_START_INVALID then
			...
			if LuaEnums.TEAM_ASSIGNMENT.AUTO == Engine.GetGametypeSetting( "teamAssignment" ) and not LuaUtils.IsArenaMode() and f38_local2 ~= LuaEnums.TIMER_TYPE.THEATER then
				Lobby.TeamSelection.AutoSplitTeam( false )
			end
		end
	elseif f38_local2 == LuaEnums.TIMER_TYPE.MANUAL_CP then
		if Lobby.Timer.lobbyTimer.endTime == Lobby.Timer.MATCH_START_INVALID then
	...
	elseif f38_local2 == LuaEnums.TIMER_TYPE.NONE then
		Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "Launch Game: starting game.\n" )
		LobbyVM.LaunchGameExec( f38_local0, f38_local1 )
		Lobby.Timer.Clear( true )
		Lobby.Timer.UnlockLobby( Engine.GetPrimaryController(), false )
	end
end
...
Lobby.Timer.Pump = function ()
	...
	local f39_local0 = Engine.IsLobbyHostActive( Enum.LobbyType.LOBBY_TYPE_GAME )
	if not f39_local0 then
		f39_local0 = Engine.IsLobbyClientActive( Enum.LobbyType.LOBBY_TYPE_GAME )
	end
