...
function SelectLiveEventQuality_Internal( f71_arg0 )
	if f71_arg0 and f71_arg0 ~= Dvar.hls_quality:get() then
		Dvar.hls_quality:set( f71_arg0 )
		CoD.LiveEventUpdateCurrentFeed()
		local f71_local0 = DataSources.LiveEventViewer.getModel()
		if f71_local0 then
			local f71_local1 = Engine.GetModel( f71_local0, "stream" )
			if f71_local1 then
				Engine.ForceNotifyModelSubscriptions( f71_local1 )
			end
		end
		local f71_local1 = DataSources.VoDViewer.getModel()
		if f71_local1 then
			local f71_local2 = Engine.GetModel( f71_local1, "stream" )
			if f71_local2 then
				Engine.ForceNotifyModelSubscriptions( f71_local2 )
			end
		end
	end
end

function CallingCard_TrackCWLEquip( f72_arg0, f72_arg1 )
	if IsLive() and IsPerControllerTablePropertyValue( f72_arg0, "uploadProfile", true ) then
		local f72_local0 = Engine.GetEmblemBackgroundId()
		if CoD.StoreUtility.DoesInventoryItemExistsByName( Engine.TableLookup( nil, CoD.backgroundsTable, 1, f72_local0, 10 ) ) then
			LUI.CoDMetrics.InventoryItemEquipped( f72_arg0, Engine.TableLookup( nil, CoD.backgroundsTable, 1, f72_local0, 3 ), f72_local0, f72_arg1 )
		end
	end
end

function SetEmblemBackground_Internal( f73_arg0, f73_arg1 )
	if IsLive() then
		if f73_arg1 ~= Engine.GetEmblemBackgroundId() then
			Engine.ExecNow( f73_arg0, "emblemSelectBackground " .. f73_arg1 )
			CoD.perController[f73_arg0].uploadProfile = true
			Engine.ExecNow( f73_arg0, "emblemSetProfile" )
		end
	else
		Engine.ExecNow( f73_arg0, "emblemSelectBackground " .. f73_arg1 )
		Engine.SetProfileVar( f73_arg0, "default_background_index", f73_arg1 )
		Engine.ExecNow( f73_arg0, "invalidateEmblemComponent" )
	end
end

function FileshareCategorySelectorCategoryChanged( f74_arg0, f74_arg1, f74_arg2, f74_arg3 )
	if f74_arg1 ~= nil and f74_arg1 ~= "" then
		CoD.FileshareUtility.SetCurrentCategory( f74_arg1 )
	else
		CoD.FileshareUtility.SetCurrentCategory( "paintjob" )
	end
	if CoD.FileshareUtility.GetIsGroupsMode( f74_arg0 ) then
		local f74_local0 = Enum.FileshareGroupsDataType.FILESHARE_GROUPS_DATA_RECENT
		if f74_arg1 == "favorites" then
			f74_local0 = Enum.FileshareGroupsDataType.FILESHARE_GROUPS_DATA_FAVORITE
		elseif f74_arg1 == "featured" then
			f74_local0 = Enum.FileshareGroupsDataType.FILESHARE_GROUPS_DATA_FEATURED
		end
		Engine.FileshareFetchGroupContent( f74_arg0, f74_arg1, f74_local0 )
	elseif CoD.FileshareUtility.GetIsCommunityMode( f74_arg0 ) then
		Engine.FileshareFetchCommunityContent( f74_arg0, f74_arg1, f74_arg2 )
	elseif CoD.FileshareUtility.GetIsUserMode( f74_arg0 ) then
		local f74_local0 = CoD.FileshareUtility.GetCurrentUser()
		if f74_arg2 == Enum.FileshareCommunityDataType.FILESHARE_COMMUNITY_DATA_RECENT_GAMES then
			Engine.FileshareFetchCommunityContent( f74_arg0, CoD.FileshareUtility.RenameToFileshareTableCategory( f74_arg1 ), f74_arg2, f74_local0 )
		else
			CoD.FileshareUtility.FetchContentForUser( f74_arg0, f74_local0 )
		end
	end
	if f74_arg1 ~= CoD.FileshareUtility.GetPreviousCategory() or f74_arg3 == true then
		ResetThumbnailViewer( f74_arg0 )
		CoD.FileshareUtility.SetDirty()
	end
	CoD.FileshareUtility.ProcessUIIndex( f74_arg0, f74_arg1, 0 )
	CoD.FileshareUtility.SetPreviousCategory( f74_arg1 )
	if CoD.FileshareUtility.ShouldSetPreviewIconSizeShort( f74_arg1 ) then
...
function OpenShowcaseCustomGames( f75_arg0, f75_arg1, f75_arg2, f75_arg3, f75_arg4 )
	...
	if FileshareIsCurrentUserContext( f75_arg2 ) and CoD.FileshareUtility.GetIsCommunityMode( f75_arg2 ) == false then
		f75_local0 = true
	end
	CoD.FileshareUtility.SetShowCreateButton( f75_local0 )
	OpenOverlay( f75_arg0, "MyShowcase_CustomGames", f75_arg2 )
end

function OpenShowcaseScreenshots( f76_arg0, f76_arg1, f76_arg2, f76_arg3, f76_arg4 )
	...
	if FileshareIsCurrentUserContext( f76_arg2 ) then
		f76_local0 = true
	end
	CoD.FileshareUtility.SetShowCreateButton( f76_local0 )
	PreserveThumbnails( f76_arg2, true )
	OpenOverlay( f76_arg0, "MyShowcase_Screenshots", f76_arg2 )
	PreserveThumbnails( f76_arg2, false )
end

function OpenShowcaseRecentGames( f77_arg0, f77_arg1, f77_arg2, f77_arg3, f77_arg4 )
	...
	if FileshareIsCurrentUserContext( f77_arg2 ) then
		f77_local0 = true
	end
	CoD.FileshareUtility.SetShowCreateButton( false )
	OpenOverlay( f77_arg4, "MyShowcase_RecentGames", f77_arg2 )
end

function OpenShowcaseClips( f78_arg0, f78_arg1, f78_arg2, f78_arg3, f78_arg4 )
	...
	OpenOverlay( f78_arg4, "MyShowcase_Clips", f78_arg2 )
	PreserveThumbnails( f78_arg2, false )
end

function OpenShowcaseLocalClips( f79_arg0, f79_arg1, f79_arg2, f79_arg3, f79_arg4 )
	...
	CoD.FileshareUtility.SetShowCreateButton( false )
	PreserveThumbnails( f79_arg2, true )
	OpenOverlay( f79_arg4, "MyShowcase_Clips", f79_arg2 )
	PreserveThumbnails( f79_arg2, false )
end

function PublishClip( f80_arg0, f80_arg1 )
	local f80_local0 = CoD.FileshareUtility.GetSelectedItemProperty( "fileId" )
	local f80_local1 = CoD.FileshareUtility.GetPublishName()
	local f80_local2 = CoD.FileshareUtility.GetPublishDescription()
	if Engine.SetupTransferData( f80_arg0, "clip_private", f80_local0, CoD.FileshareUtility.GetIsCommunityMode( f80_arg0 ) ) then
		Engine.PublishClip( f80_arg0, "clip", f80_local0, f80_local1, f80_local2 )
	else
		GoBack( f80_arg1, f80_arg0 )
		FileshareShowErrorToast( f80_arg1, nil, f80_arg0 )
	end
end

function PublishScreenshot( f81_arg0, f81_arg1 )
	local f81_local0 = CoD.FileshareUtility.GetSelectedItemProperty( "fileId" )
	local f81_local1 = CoD.FileshareUtility.GetPublishName()
	local f81_local2 = CoD.FileshareUtility.GetPublishDescription()
	if Engine.SetupTransferData( f81_arg0, "screenshot_private", f81_local0, CoD.FileshareUtility.GetIsCommunityMode( f81_arg0 ) ) then
		Engine.PublishScreenshot( f81_arg0, "screenshot", f81_local0, f81_local1, f81_local2 )
	else
		GoBack( f81_arg1, f81_arg0 )
		FileshareShowErrorToast( f81_arg1, nil, f81_arg0 )
	end
end

function FileshareUpdateDetailsViewModel( f82_arg0, f82_arg1, f82_arg2, f82_arg3 )
	local f82_local0 = CoD.FileshareUtility.GetCurrentCategory()
	local f82_local1, f82_local2 = CoD.FileshareUtility.GetCurrentFilter()
	local f82_local3 = nil
	if CoD.FileshareUtility.GetIsCommunityMode( f82_arg2 ) or CoD.FileshareUtility.GetIsGroupsMode( f82_arg2 ) then
		f82_local3 = Engine.FileshareGetCommunitySlotData( f82_arg2, f82_local0, f82_local1, f82_local2, f82_arg3 )
	else
		f82_local3 = Engine.FileshareGetSlotData( f82_arg2, f82_local0, f82_local1, f82_local2, f82_arg3 )
	end
	if CoD.FileshareUtility.GetIsGroupsMode( f82_arg2 ) then
		f82_local0 = CoD.FileshareUtility.GetSelectedItemProperty( "fileCategory" )
	end
	local f82_local4 = Engine.CreateModel( Engine.GetGlobalModel(), "FileshareSelectedItem", false )
	for f82_local8, f82_local9 in ipairs( CoD.FileshareUtility.FileProperties ) do
		if f82_local3[f82_local9] ~= nil then
			Engine.SetModelValue( Engine.CreateModel( f82_local4, f82_local9 ), f82_local3[f82_local9] )
		end
	end
	Engine.SetModelValue( Engine.CreateModel( f82_local4, "codeIndex" ), f82_arg3 )
	f82_local6 = Engine.CreateModel( f82_local4, "showDetailsViewSpinner" )
	if Engine.GetModelValue( Engine.CreateModel( f82_local4, "renderFileId" ) ) == Engine.GetModelValue( Engine.CreateModel( f82_local4, "fileId" ) ) then
		Engine.SetModelValue( f82_local6, 0 )
	elseif f82_local0 ~= "emblem" then
		Engine.SetModelValue( f82_local6, 1 )
	end
	f82_local9 = ""
	if f82_local3.weaponIndex ~= nil then
		f82_local9 = Engine.ToUpper( Engine.Localize( Engine.GetItemName( f82_local3.weaponIndex ) ) )
	end
	Engine.SetModelValue( Engine.CreateModel( f82_local4, "weaponName" ), f82_local9 )
	CoD.FileshareUtility.SetSelectedItem( f82_local4 )
	f82_arg0:setModel( f82_local4, f82_arg2 )
	FileshareSetSelectedItem( f82_arg0, f82_arg1, f82_arg2, "true" )
	CoD.FileshareUtility.ProcessUIIndex( f82_arg2, f82_local0, f82_arg3 )
end

function MediaManagerGetQuotaForCategory( f83_arg0, f83_arg1 )
	local f83_local0 = nil
	if f83_arg1 == "screenshot_private" or f83_arg1 == "clip_private" then
		f83_local0 = Engine.FileshareGetQuota( f83_arg0, f83_arg1 )
	else
		f83_local0 = Engine.MediaManagerGetQuota( f83_arg0, f83_arg1 )
	end
	return f83_local0
end

function MediaManagerUpdateSlotsUsed( f84_arg0, f84_arg1 )
	if f84_arg1 then
		local f84_local0 = MediaManagerGetQuotaForCategory( f84_arg0, f84_arg1 )
		Engine.SetModelValue( Engine.CreateModel( Engine.CreateModel( Engine.GetGlobalModel(), "MediaManager", false ), "slotsUsed", false ), f84_local0.categorySlotsUsed )
	end
end

function MediaManagerSetSlotModel( f85_arg0, f85_arg1, f85_arg2, f85_arg3 )
	local f85_local0 = Engine.CreateModel( Engine.GetGlobalModel(), "MediaManager", false )
	local f85_local1 = Engine.CreateModel( f85_local0, "isBuyMoreSlot", false )
	local f85_local2 = Engine.CreateModel( f85_local0, "createTime", false )
	local f85_local3 = Engine.CreateModel( f85_local0, "authorName", false )
	Engine.SetModelValue( f85_local1, f85_arg1 )
	Engine.SetModelValue( f85_local2, f85_arg2 )
	Engine.SetModelValue( f85_local3, f85_arg3 )
	ForceNotifyGlobalModel( f85_arg0, "MediaManager.createTime" )
	ForceNotifyGlobalModel( f85_arg0, "MediaManager.authorName" )
end

function MediaManagerFetchPaintjobsData( f86_arg0 )
	Engine.SetModelValue( Engine.CreateModel( Engine.GetModelForController( f86_arg0 ), "Paintshop.Mode" ), Enum.PaintshopMode.PAINTSHOPMODE_WEAPONLIST )
	local f86_local0 = CoD.CraftUtility.Paintjobs.ParseDDL( f86_arg0, Enum.StorageFileType.STORAGE_PAINTJOBS )
end

function MediaManagerFetchVariantsData( f87_arg0 )
	Engine.SetModelValue( Engine.CreateModel( Engine.GetModelForController( f87_arg0 ), "Gunsmith.Mode" ), Enum.GunsmithMode.GUNSMITHMODE_WEAPONLIST )
	local f87_local0 = CoD.CraftUtility.Gunsmith.ParseDDL( f87_arg0, Enum.StorageFileType.STORAGE_GUNSMITH )
end

function ShowcaseSetSelectedFriendXUID( f88_arg0, f88_arg1, f88_arg2, f88_arg3 )
	local f88_local0 = CoD.FileshareUtility.GetSelectedItemProperty( "fileAuthorName" )
	local f88_local1 = CoD.FileshareUtility.GetSelectedItemProperty( "fileAuthorXuid" )
	local f88_local2 = Engine.GetPlayerInfo( f88_arg2, f88_local1 )
	local f88_local3 = Engine.GetModelForController( f88_arg2 )
	Engine.SetModelValue( Engine.CreateModel( f88_local3, "Social.selectedFriendXUID" ), f88_local1 )
	Engine.SetModelValue( Engine.CreateModel( f88_local3, "Social.selectedFriendGamertag" ), f88_local0 )
	local f88_local4 = Engine.CreateModel( f88_local3, "Social.hideSelectedFriendShowcase" )
	if f88_arg3 ~= nil then
		Engine.SetModelValue( f88_local4, f88_arg3 )
	else
		Engine.SetModelValue( f88_local4, false )
	end
end

function AddGroupShowcaseItemToUnfeaturedList( f89_arg0, f89_arg1 )
	if not CoD.groupUnfeaturedList then
		CoD.groupUnfeaturedList = {}
	end
	local f89_local0 = tostring( f89_arg0 )
	if not CoD.groupUnfeaturedList[f89_local0] then
		CoD.groupUnfeaturedList[f89_local0] = {}
	end
	for f89_local4, f89_local5 in ipairs( CoD.groupUnfeaturedList[f89_local0] ) do
		if f89_local5 == f89_arg1 then
			return 
		end
	end
	table.insert( CoD.groupUnfeaturedList[f89_local0], f89_arg1 )
end

function RemoveGroupShowcaseItemFromUnfeaturedList( f90_arg0, f90_arg1 )
	local f90_local0 = tostring( f90_arg0 )
	if CoD.groupUnfeaturedList and CoD.groupUnfeaturedList[f90_local0] then
		for f90_local4, f90_local5 in ipairs( CoD.groupUnfeaturedList[f90_local0] ) do
			if f90_local5 == f90_arg1 then
				table.remove( CoD.groupUnfeaturedList[f90_local0], f90_local4 )
				break
			end
		end
	end
...
function LoginRewardConfirmationAction( f91_arg0 )
	if not DoesPlayerHaveCurrentLoginReward( f91_arg0 ) then
		local f91_local0 = 1
		local f91_local1 = CoD.attachmentTable
		local f91_local2 = "overgrowth_camo"
		local f91_local3 = 21
		local f91_local4 = 6
		local f91_local5 = 4
		local f91_local6 = 7
		local f91_local7 = Engine.StorageGetBuffer( f91_arg0, Enum.StorageFileType.STORAGE_MP_STATS_ONLINE )
		for f91_local11, f91_local12 in pairs( CoD.LoginRewardReceivedStatPath ) do
			f91_local7 = f91_local7[f91_local12]
		end
		f91_local7:set( f91_local0 )
		Engine.StorageWrite( f91_arg0, Enum.StorageFileType.STORAGE_MP_STATS_ONLINE )
		CoD.OverlayUtility.ShowToast( "LootBonusCamo", Engine.Localize( Engine.TableLookup( nil, f91_local1, f91_local3, f91_local2, f91_local6 ) ), Engine.Localize( "MPUI_CAMO" ), Engine.TableLookup( nil, f91_local1, f91_local3, f91_local2, f91_local4 ), nil, nil, nil )
	end
end

function SetNextPreStartPopup( f92_arg0, f92_arg1 )
	if f92_arg1 ~= nil then
		Engine.SetModelValue( Engine.CreateModel( Engine.GetModelForController( f92_arg0 ), "PreStart.NextPopup" ), f92_arg1 )
	end
end

function ProceedToNextPreStartPopup( f93_arg0, f93_arg1, f93_arg2 )
	local f93_local0 = Engine.CreateModel( Engine.GetModelForController( f93_arg1 ), "PreStart.NextPopup" )
	local f93_local1 = Engine.GetModelValue( f93_local0 )
	Engine.SetModelValue( f93_local0, "" )
	if f93_local1 == "LoginReward" then
		OpenLoginRewardPopup( GoBack( f93_arg2, f93_arg1 ), f93_arg1 )
		return 
	elseif f93_local1 == "ZMHDPurchaseReward" then
		OpenZMHDPurchaseRewardPopup( GoBack( f93_arg2, f93_arg1 ), f93_arg1 )
		return 
	elseif f93_local1 == "ZMHDGobbleGumPS4" then
		OpenZMHDGobbleGumPopup( GoBack( f93_arg2, f93_arg1 ), f93_arg1, CoD.BubbleGumBuffUtility.ZMHD_RELEASE_PROMO_PS4_INDEX_ID )
		return 
	elseif f93_local1 == "ZMHDGobbleGumXB1" then
		OpenZMHDGobbleGumPopup( GoBack( f93_arg2, f93_arg1 ), f93_arg1, CoD.BubbleGumBuffUtility.ZMHD_RELEASE_PROMO_XB1_INDEX_ID )
		return 
	elseif f93_local1 == "URMFirstTime" then
		OpenURMFirstTimePopup( GoBack( f93_arg2, f93_arg1 ), f93_arg1 )
		return 
	else
		Close( f93_arg0, f93_arg1 )
	end
end
