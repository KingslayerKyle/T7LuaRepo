...
CoD.WeaponItemModelWidgetNew.new = function ( menu, controller )
	...
	if IsPC() then
		f4_local19 = CoD.RemoveItemButton.new( menu, controller )
	else
		f4_local19 = LUI.UIElement.createFake()
	end
	f4_local19:setLeftRight( false, true, -22, -3 )
	f4_local19:setTopBottom( true, false, 3, 23 )
	f4_local19:setAlpha( 0 )
	f4_local19:linkToElementModel( self, nil, false, function ( model )
		f4_local19:setModel( model, controller )
	end )
	if IsPC() then
		self:addElement( f4_local19 )
	end
	self.removeButton = f4_local19
	local f4_local20 = nil
	if IsPC() then
		f4_local20 = CoD.PersonalizeButton.new( menu, controller )
	else
		f4_local20 = LUI.UIElement.createFake()
	end
	f4_local20:setLeftRight( true, false, 3, 23 )
	f4_local20:setTopBottom( true, false, 3.75, 23.75 )
	f4_local20:setAlpha( 0 )
	f4_local20.itemHintText.textCenterAlign:setText( Engine.Localize( "HEROES_PERSONALIZE" ) )
	if IsPC() then
		self:addElement( f4_local20 )
	end
	self.personalizeButton = f4_local20
	...
	self.clipsPerState = {
		DefaultState = {
			DefaultClip = function ()
				...
				f4_local19:completeAnimation()
				self.removeButton:setAlpha( 0 )
				self.clipFinished( f4_local19, {} )
				f4_local20:completeAnimation()
				self.personalizeButton:setAlpha( 0 )
				self.clipFinished( f4_local20, {} )
			end,
			GainFocus = function ()
		...
		Equipped = {
			...
			GainFocus = function ()
				...
				self.glitch2:setAlpha( 1 )
				glitch2Frame2( glitch2, {} )
				local f4_local19Frame2 = function ( f4_local19, event )
					if not event.interrupted then
						f4_local19:beginAnimation( "keyframe", 140, false, false, CoD.TweenType.Linear )
					end
					f4_local19:setLeftRight( false, true, -16, -2 )
					f4_local19:setTopBottom( true, false, 3, 17 )
					f4_local19:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( f4_local19, event )
					else
						f4_local19:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
		...
		WeaponEquipped = {
			...
			GainFocus = function ()
				...
				self.removeButton:setAlpha( 0 )
				f4_local19Frame2( f4_local19, {} )
				local f4_local20Frame2 = function ( f4_local20, event )
					if not event.interrupted then
						f4_local20:beginAnimation( "keyframe", 140, false, false, CoD.TweenType.Linear )
					end
					f4_local20:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( f4_local20, event )
					else
						f4_local20:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
				
				f4_local20:completeAnimation()
				self.personalizeButton:setAlpha( 0 )
				f4_local20Frame2( f4_local20, {} )
			end,
			Focus = function ()
				self:setupElementClipCounter( 18 )
				BoxButtonLrgInactiveStroke:completeAnimation()
			...
			LoseFocus = function ()
				...
				local f4_local19Frame2 = function ( f4_local19, event )
					if not event.interrupted then
						f4_local19:beginAnimation( "keyframe", 159, false, false, CoD.TweenType.Linear )
					end
					f4_local19:setAlpha( 0 )
					if event.interrupted then
						self.clipFinished( f4_local19, event )
				...
				local f4_local20Frame2 = function ( f4_local20, event )
					if not event.interrupted then
						f4_local20:beginAnimation( "keyframe", 159, false, false, CoD.TweenType.Linear )
					end
					f4_local20:setAlpha( 0 )
					if event.interrupted then
						self.clipFinished( f4_local20, event )
