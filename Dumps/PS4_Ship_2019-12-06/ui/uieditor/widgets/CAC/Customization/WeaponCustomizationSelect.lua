...
CoD.WeaponCustomizationSelect.new = function ( menu, controller )
	...
	menu:AddButtonCallbackFunction( selectionList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( element, menu, controller, model )
		if IsSelfModelValueTrue( element, controller, "isPackage" ) then
			OpenBuyInventoryItemDialog( self, element, controller, "CamoCAC" )
			return true
		elseif not IsItemWeaponOptionLocked( menu, element, controller ) and not IsSelfModelValueTrue( element, controller, "isBMClassified" ) then
