...
local PostLoadFunc = function ( self, controller, menu )
	self.GalleryList:registerEventHandler( "list_item_gain_focus", function ( element, event )
		local f2_local0 = Engine.GetModel( element:getModel(), "image" )
		if f2_local0 then
			self.Image0:setImage( RegisterImage( Engine.GetModelValue( f2_local0 ) ) )
		end
		return nil
	end )
end
...
CoD.GalleryViewerWidget.new = function ( menu, controller )
	...
	GalleryList:setLeftRight( true, false, 17.77, 107.77 )
	...
	self:registerEventHandler( "gain_focus", function ( self, event )
		if self.m_focusable and self.GalleryList:processEvent( event ) then
			return true
		else
