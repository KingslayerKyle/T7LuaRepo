...
local PostLoadFunc = function ( self, controller, menu )
	menu.PostCodCasterLoad = function ( f7_arg0 )
		...
		menu:AddButtonCallbackFunction( f7_arg0, controller, Enum.LUIButton.LUI_KEY_NONE, "LEFTARROW", function ( element, menu, controller, model )
			element = self
			if not menu.m_disableNavigation then
				f0_local2( self, controller )
			end
		end, nil, false, true )
		menu:AddButtonCallbackFunction( f7_arg0, controller, Enum.LUIButton.LUI_KEY_NONE, "RIGHTARROW", function ( element, menu, controller, model )
			element = self
			if not menu.m_disableNavigation then
				f0_local3( self, controller )
			end
