...
DataSources.CallingCardsDefault = DataSourceHelpers.ListSetup( "CallingCardsDefault", function ( f1_arg0 )
	...
	for f1_local10, f1_local11 in ipairs( f1_local1 ) do
		local f1_local8 = Engine.Localize( f1_local11.description )
		local f1_local9 = ""
		if f1_local11.isContractBg and f1_local11.isBGLocked then
		...
		if not f1_local11.isBGLocked or f1_local11.isContractBg and IsLive() then
			table.insert( f1_local0, {
				models = {
					title = f1_local8,
					description = f1_local9,
					iconId = f1_local11.id,
					isLocked = f1_local11.isBGLocked,
					isContractBg = f1_local11.isContractBg,
					isPackage = false
				},
				properties = {
					action = f1_local3
	...
	if InFrontend() and AreCodPointsEnabled( f1_arg0 ) then
		for f1_local10, f1_local11 in ipairs( CoD.StoreUtility.CWLPackages ) do
			if not CoD.StoreUtility.IsCWLV2Package( f1_local11 ) and CoD.StoreUtility.IsInventoryItemVisible( f1_arg0, f1_local11 ) and not CoD.StoreUtility.IsInventoryItemPurchased( f1_arg0, f1_local11 ) then
				local f1_local8 = CoD.StoreUtility.GetCWLPackageCallingCardModel( f1_arg0, f1_local11 )
				f1_local8.properties.packageSortIndex = f1_local10
...
CoD.CallingCards_Stickerbook_Default.new = function ( menu, controller )
	...
	menu:AddButtonCallbackFunction( CallingCardGrid, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( element, menu, controller, model )
		if not IsElementInState( element, "Locked" ) and not IsElementInState( element, "ContractClassified" ) then
