...
CoD.ChooseCharacterLoadoutCarouselItem_BlackJack_Frame.new = function ( menu, controller )
	...
	local f1_local6 = nil
	if IsPC() then
		f1_local6 = CoD.CharacterMiniSelector.new( menu, controller )
	else
		f1_local6 = LUI.UIElement.createFake()
	end
	f1_local6:setLeftRight( false, false, -152, 148 )
	f1_local6:setTopBottom( true, false, 6.25, 36.25 )
	f1_local6.leftButton.FETitleNumBrdr0:setAlpha( 0 )
	f1_local6.rightButton.FETitleNumBrdr0:setAlpha( 0 )
	f1_local6.FETitleNumBrdr0:setAlpha( 0 )
	if IsPC() then
		self:addElement( f1_local6 )
	end
	self.charMiniSelector = f1_local6
	self.clipsPerState = {
		DefaultState = {
