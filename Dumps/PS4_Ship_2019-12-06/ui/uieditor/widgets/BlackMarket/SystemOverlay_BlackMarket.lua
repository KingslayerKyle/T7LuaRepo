...
CoD.SystemOverlay_BlackMarket.new = function ( menu, controller )
	...
	menu:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, nil, function ( element, menu, controller, model )
		if not IsMenuModelValueEqualTo( menu, "title", "MPUI_BM_NO_CURRENCY" ) and AreCodPointsEnabled( controller ) and not IsPC() and not IsElementPropertyValue( element, "isBurningDuplicates", true ) and not IsSelfInState( self, "PurchasingBundle" ) and PurchaseExperimentCanShowStoreButton( controller, self ) then
