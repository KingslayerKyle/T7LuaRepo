...
local PostLoadFunc = function ( self, controller, menu )
	local f11_local0 = self.m_ownerController
	EmblemEditor_SetupEditorBasedOnMode( self, self, f11_local0 )
	self.isRightStickRepeated = false
	f0_local4( self, f11_local0 )
	if IsPaintshop( f11_local0 ) and Dvar.ui_disablePSControlWidgets:get() == true then
	...
	if CoD.isPC then
		f0_local5( self, f11_local0, menu )
	end
end
...
LUI.createMenu.EmblemEditor = function ( controller )
	...
	layerCarousel:setLeftRight( true, false, -20, 2525 )
	...
	layerCarousel:linkToElementModel( layerCarousel, "isGrouped", true, function ( model )
		local f15_local0 = layerCarousel
		local f15_local1 = {
		...
		CoD.Menu.UpdateButtonShownState( f15_local0, self, controller, Enum.LUIButton.LUI_KEY_RTRIG )
		CoD.Menu.UpdateButtonShownState( f15_local0, self, controller, Enum.LUIButton.LUI_KEY_LB )
	end )
	layerCarousel:linkToElementModel( layerCarousel, "isLinked", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f16_local0, self, controller, Enum.LUIButton.LUI_KEY_RB )
	end )
	...
	if self.m_eventHandlers.input_source_changed then
		local f13_local18 = self.m_eventHandlers.input_source_changed
		self:registerEventHandler( "input_source_changed", function ( self, event )
			event.menu = event.menu or self
			self:updateState( event )
			return f13_local18( self, event )
		end )
	else
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "Emblem.EmblemProperties.editorMode" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f66_local0, self, controller, Enum.LUIButton.LUI_KEY_START )
	end )
	...
	self:registerEventHandler( "menu_loaded", function ( self, event )
		...
		if not f67_local0 then
			f67_local0 = self:dispatchEventToChildren( event )
		end
		return f67_local0
	end )
	...
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_LEFT, nil, function ( element, menu, controller, model )
		...
		if IsEditMode( controller ) and IsDpadButton( f91_local0 ) and not IsRepeatButtonPress( f91_local0 ) then
	...
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_RIGHT, nil, function ( element, menu, controller, model )
		...
		if IsEditMode( controller ) and IsDpadButton( f93_local0 ) and not IsRepeatButtonPress( f93_local0 ) then
