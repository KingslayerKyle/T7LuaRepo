...
LUI.createMenu.WeaponBuildKits = function ( controller )
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "Gunsmith.Mode" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f51_local0, self, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE )
	end )
	...
	self:registerEventHandler( "menu_loaded", function ( self, event )
		...
		if not f52_local0 then
			f52_local0 = self:dispatchEventToChildren( event )
		end
		return f52_local0
	end )
	self:registerEventHandler( "menu_opened", function ( self, event )
		...
		if not f53_local0 then
			f53_local0 = self:dispatchEventToChildren( event )
		end
		return f53_local0
	end )
