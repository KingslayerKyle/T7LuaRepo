...
LUI.createMenu.NumericKeypad = function ( controller )
	...
	CancelButton:setLeftRight( true, false, 635, 739.5 )
	CancelButton:setTopBottom( true, false, 419, 449 )
	...
	if self.m_eventHandlers.input_source_changed then
		local f4_local33 = self.m_eventHandlers.input_source_changed
		self:registerEventHandler( "input_source_changed", function ( self, event )
			event.menu = event.menu or self
			self:updateState( event )
			return f4_local33( self, event )
		end )
	else
