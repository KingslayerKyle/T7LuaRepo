...
LUI.createMenu.Pregame_Main = function ( controller )
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "lobbyRoot.Pregame.state" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f272_local0, self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	self:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "lobbyRoot.Pregame.stage" ), function ( model )
		local f273_local0 = self
		local f273_local1 = {
			controller = controller,
			name = "model_validation",
			modelValue = Engine.GetModelValue( model ),
		...
		CoD.Menu.UpdateButtonShownState( f273_local0, self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	self:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "factions.isCoDCaster" ), function ( model )
		local f274_local0 = self
		local f274_local1 = {
			controller = controller,
			name = "model_validation",
			modelValue = Engine.GetModelValue( model ),
		...
		CoD.Menu.UpdateButtonShownState( f274_local0, self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	CoD.Menu.AddNavigationHandler( self, self, controller )
	self:registerEventHandler( "menu_loaded", function ( self, event )
		local f275_local0 = nil
		SendClientScriptMenuStateChangeNotify( controller, self, true, "mp" )
		if not f275_local0 then
			f275_local0 = self:dispatchEventToChildren( event )
		end
		return f275_local0
	end )
