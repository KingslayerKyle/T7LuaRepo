...
LUI.createMenu.ChangeGameMode = function ( controller )
	...
	gamesList:linkToElementModel( gamesList, "isOfficial", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f19_local0, self, controller, Enum.LUIButton.LUI_KEY_START )
	end )
	gamesList:linkToElementModel( gamesList, "isCommunityOption", true, function ( model )
		local f20_local0 = gamesList
		local f20_local1 = {
			controller = controller,
			name = "model_validation",
			modelValue = Engine.GetModelValue( model ),
		...
		CoD.Menu.UpdateButtonShownState( f20_local0, self, controller, Enum.LUIButton.LUI_KEY_START )
	end )
	...
	gamesList:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f30_local0
				if not IsParentListInFocus( element ) then
					f30_local0 = IsDisabled( element, controller )
				else
					f30_local0 = false
				end
				return f30_local0
	...
	CustomGameOfficial:mergeStateConditions( {
		{
			stateName = "Visible",
			condition = function ( menu, element, event )
				local f34_local0 = IsCustomMPLobby()
				if f34_local0 then
					f34_local0 = IsSelfModelValueTrue( element, controller, "isOfficial" )
					if f34_local0 then
						f34_local0 = IsWidgetInFocus( self, "gamesList", event )
					end
				end
				return f34_local0
			end
