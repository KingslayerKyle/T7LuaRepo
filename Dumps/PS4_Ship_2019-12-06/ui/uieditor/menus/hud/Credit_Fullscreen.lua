...
local PostLoadFunc = function ( f3_arg0, f3_arg1 )
	...
	local f3_local8 = function ( f11_arg0, f11_arg1, f11_arg2 )
		local f11_local0 = Engine.GetModelValue( f11_arg1 )
		local f11_local1 = f11_local0 and math.abs( f11_local0 ) > 0.2
		if f11_local1 and not f3_local7[f11_arg2] then
			if f3_arg0.skipHideTimer then
				f3_arg0.skipHideTimer:close()
				f3_arg0.skipHideTimer = nil
			elseif f3_arg0.sticksDown == 0 then
				f3_local1()
			end
			f3_arg0.sticksDown = f3_arg0.sticksDown + 1
		elseif not f11_local1 and f3_local7[f11_arg2] then
			f3_arg0.sticksDown = f3_arg0.sticksDown - 1
			if f3_arg0.sticksDown == 0 then
				f3_local3()
			end
		end
		f3_local7[f11_arg2] = f11_local1
	end
	
	f3_arg0:subscribeToGlobalModel( f3_arg1, "PerController", "RightStick.X", function ( model )
		f3_local8( f3_arg1, model, "x" )
	end )
	f3_arg0:subscribeToGlobalModel( f3_arg1, "PerController", "RightStick.Y", function ( model )
		f3_local8( f3_arg1, model, "y" )
	end )
end
...
LUI.createMenu.Credit_Fullscreen = function ( controller )
	...
	if self.m_eventHandlers.input_source_changed then
		local f14_local6 = self.m_eventHandlers.input_source_changed
		self:registerEventHandler( "input_source_changed", function ( self, event )
			event.menu = event.menu or self
			self:updateState( event )
			return f14_local6( self, event )
		end )
	else
