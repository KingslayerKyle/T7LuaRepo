...
CoD.ScoreboardUtility.GetScoreboardTeamTable = function ( f1_arg0, f1_arg1 )
	...
	local f1_local4 = math.max( CoD.ScoreboardUtility.MinRowsToShowOnEachTeam, math.max( f1_local2, f1_local3 ) )
	local f1_local5 = {}
...
CoD.ScoreboardUtility.SetScoreboardUIModels = function ( f5_arg0 )
	...
	if Engine.IsMultiplayerGame() then
		...
		local f5_local3 = Engine.GetTeamPositions( f5_arg0, f5_local0 )
		local f5_local4 = {}
		for f5_local5 = 1, f5_local0, 1 do
			local f5_local8 = {}
			local f5_local9 = f5_local3[f5_local5].team
			f5_local8.FactionName = ""
			f5_local8.FactionIcon = ""
			f5_local8.Score = f5_local3[f5_local5].score
			...
			table.insert( f5_local4, f5_local8 )
		end
		for f5_local13, f5_local8 in ipairs( f5_local4 ) do
			for f5_local10, f5_local11 in pairs( f5_local8 ) do
				Engine.SetModelValue( Engine.CreateModel( f5_local2, "team" .. f5_local13 .. f5_local10 ), f5_local11 )
			end
		end
		Engine.SetModelValue( Engine.CreateModel( f5_local2, "gameType" ), Engine.StructTableLookupString( CoD.gametypesStructTable, "name", Engine.GetCurrentGameType(), "name_ref_caps" ) )
		Engine.SetModelValue( Engine.CreateModel( f5_local2, "mapName" ), CoD.GetMapValue( Engine.GetCurrentMapName(), "mapNameCaps", "" ) )
		if f5_local0 < 2 then
			f5_local7 = Engine.GetModel( f5_local1, "gameScore.playerScore" )
			if f5_local7 then
				Engine.SetModelValue( Engine.CreateModel( f5_local2, "team1Score" ), Engine.GetModelValue( f5_local7 ) )
			else
				local f5_local3 = 5
				for f5_local4 = 1, f5_local3, 1 do
					Engine.SetModelValue( Engine.CreateModel( f5_local2, "column" .. f5_local4 .. "Header" ), Engine.GetScoreboardColumnHeader( f5_arg0, f5_local4 - 1 ) )
				end
			end
		else
			local f5_local3 = 5
