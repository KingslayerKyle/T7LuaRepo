...
CoD.Menu.NewFromState = function ( f4_arg0, f4_arg1, f4_arg2 )
	...
	if f4_arg0 ~= "SystemOverlay_MessageDialog" then
		local f4_local1 = Engine.GetPrimaryController()
		self:subscribeToGlobalModel( f4_local1, "MessageDialog", "messagePending", function ( model )
			if ShouldOpenMessageDialog( self ) then
				local f5_local0 = Engine.IsInGame()
				if not f5_local0 then
					f5_local0 = CoD.isSinglePlayer
				end
				...
				if f5_local1 and f5_local1.menuName == "MessageDialogBox" then
					Close( f5_local1, f4_local1 )
					f5_local1 = nil
				end
				if f5_local0 then
					if self.id == "Menu.HUD" then
						local f5_local3 = CoD.OverlayUtility.CreateOverlay( f4_local1, self, "MessageDialogBox", f4_local1 )
						f5_local3:setOwner( f4_local1 )
						Engine.LockInput( f4_local1, true )
						Engine.SetUIActive( f4_local1, true )
					end
				elseif not f5_local1 then
					local f5_local3 = CoD.OverlayUtility.CreateOverlay( f4_local1, self, "MessageDialogBox", f4_local1 )
					f5_local3:registerEventHandler( "unused_durango_gamepad_button", function ( element, event )
						element:close()
					end )
				end
			end
...
CoD.Menu.close = function ( f9_arg0 )
	...
	if f9_arg0.occludedBy ~= nil and f9_arg0.occludedBy.occludedMenu == f9_arg0 then
		f9_arg0.occludedBy.occludedMenu = nil
	end
...
CoD.Menu.UpdateButtonShownState = function ( f50_arg0, f50_arg1, f50_arg2, f50_arg3 )
	...
	for f50_local6, f50_local7 in ipairs( f50_local0 ) do
		...
		if f50_local5 then
			...
			break
		elseif not (not f50_local7.element or not f50_local7.element.showButtonDisabled or not f50_local7.element.showButtonDisabled[f50_arg3]) or f50_arg1.showButtonDisabled and f50_arg1.showButtonDisabled[f50_arg3] then
			f50_local1 = Enum.LUIButtonPromptStates.FLAG_DISABLE_PROMPTS
		else
			f50_local1 = Enum.LUIButtonPromptStates.FLAG_HIDE_PROMPTS
		end
	end
...
CoD.Menu.AddButtonCallbackFunction = function ( f64_arg0, f64_arg1, f64_arg2, f64_arg3, f64_arg4, f64_arg5, f64_arg6, f64_arg7 )
	...
	if f64_arg3 ~= Enum.LUIButton.LUI_KEY_NONE then
		...
		f64_arg0.showButtonDisabled[f64_arg3] = f64_arg7
			...
			for f64_local2 = f64_local0, f64_local1, 1 do
				...
				table.insert( f64_arg0.controllerSubscriptions, {
					...
					subscription = f64_arg0:subscribeToModel( Engine.GetModel( Engine.GetModelForController( f64_local5 ), "ButtonBits." .. f64_arg3 ), function ( f65_arg0 )
						if not f64_arg0.occludedBy and CoD.BitUtility.IsBitwiseAndNonZero( Engine.GetModelValue( f65_arg0 ), Enum.LUIButtonFlags.FLAG_DOWN ) then
	...
	if CoD.isPC and not f64_arg0.hasButtonActionHandler then
		...
		f64_arg0:registerEventHandler( "mouse_left_click", function ( element, event )
			if element.handleMouseButton ~= false and f64_arg0.buttonModel then
				local f66_local0 = Engine.GetModel( Engine.GetModelForController( f64_arg2 ), "ButtonBits." .. Enum.LUIButton.LUI_KEY_XBA_PSCROSS )
				if f66_local0 then
					Engine.SetModelValue( f66_local0, Enum.LUIButtonFlags.FLAG_DOWN )
					return true
				end
			end
		end )
	end
	if CoD.isPC and f64_arg4 ~= nil then
		...
		if not f64_arg0.keyPressSubscriptions[f64_arg4] then
			...
			f64_arg0:subscribeToModel( Engine.GetModel( Engine.GetModelForController( f64_arg2 ), "KeyPressBits." .. f64_arg4 ), function ( model )
				if not f64_arg0.occludedBy and CoD.BitUtility.IsBitwiseAndNonZero( Engine.GetModelValue( model ), Enum.LUIButtonFlags.FLAG_DOWN ) then
					local f67_local0 = f64_arg0:GetElementAndFunctionTableForButton( f64_arg3, "buttonFunctions" )
					for f67_local4, f67_local5 in ipairs( f67_local0 ) do
						if f67_local5.fn( f67_local5.element, f64_arg0, f64_arg2, model ) then
...
CoD.Menu.OpenModalDialog = function ( f75_arg0, f75_arg1, f75_arg2, f75_arg3, f75_arg4, f75_arg5, f75_arg6 )
	CoD.Menu.ModalDialogInfo.Title = f75_arg3
	CoD.Menu.ModalDialogInfo.Description = f75_arg4
	CoD.Menu.ModalDialogInfo.Options = f75_arg5
	CoD.Menu.ModalDialogInfo.Callback = f75_arg6
	f75_arg0:openPopup( "ModalDialog", f75_arg2 )
end
...
CoD.Menu.updateIngamePopupBg = function ( f82_arg0 )
	if f82_arg0.occludedMenu then
		...
		if not f82_arg0.darkenElement and not f82_arg0.disableDarkenElement then
...
CoD.Menu.updateBlur = function ( f91_arg0 )
	if f91_arg0.disableBlur then
		return 
	end
	local f91_local0 = f91_arg0:getOwner()
	if f91_local0 == nil then
		f91_local0 = 0
	end
	if f91_arg0.occludedMenu then
		if not Engine.IsInGame() then
...
CoD.Menu.ResolutionChanged = function ( f95_arg0, f95_arg1 )
	if f95_arg1.resolutionChanged and not f95_arg0.occludedBy then
...
CoD.Menu.OpenDemoErrorPopup = function ( f99_arg0, f99_arg1 )
	if f99_arg1.titleText ~= nil and not f99_arg0.occludedBy then
		local f99_local0 = Engine.IsInGame()
		local f99_local1 = f99_arg0:openPopup( "Demo_Error", f99_arg1.controller )
		f99_local1.title:setText( f99_arg1.titleText )
		f99_local1.msg:setText( f99_arg1.messageText )
		if f99_local0 then
			f99_local1:setOwner( f99_arg1.controller )
			Engine.LockInput( f99_arg1.controller, true )
			Engine.SetUIActive( f99_arg1.controller, true )
		end
	end
end

CoD.Menu.OpenWiiUControllerSelectorPopup = function ( f100_arg0, f100_arg1 )
	if not f100_arg0.occludedBy then
		local f100_local0 = f100_arg0:openPopup( f100_arg1.popupName, f100_arg1.controller )
		f100_arg1.newPlayerOption = false
		f100_local0:init( f100_arg1 )
		return true
	else
...
CoD.Menu.OpenKeyboardTextFieldPopup = function ( f101_arg0, f101_arg1 )
	if CoD.isPC and not f101_arg0.occludedBy then
		local f101_local0 = f101_arg0:openPopup( "KeyboardTextField", f101_arg1.controller )
	end
end

CoD.Menu.OpenPopupEventHandler = function ( f102_arg0, f102_arg1 )
	if f102_arg0.occludedBy then
		if f102_arg1.recursive then
			return 
		else
			return 
		end
	elseif f102_arg0.menuName ~= f102_arg1.popupName then
		f102_arg0:openPopup( f102_arg1.popupName, f102_arg1.controller )
	end
	return true
end

CoD.Menu.GetOnlinePlayerCountText = function ( f103_arg0 )
	if CoD.isOnlineGame() then
		local f103_local0 = "live"
		if not CoD.isZombie then
			if f103_arg0 ~= nil then
				f103_local0 = "playlist/" .. Engine.GetPlaylistID()
			elseif Engine.GameModeIsMode( CoD.GAMEMODE_PUBLIC_MATCH ) == true then
				f103_local0 = "matchtype/public"
			elseif Engine.GameModeIsMode( CoD.GAMEMODE_LEAGUE_MATCH ) == true then
				f103_local0 = "matchtype/league"
			elseif Engine.GameModeIsMode( CoD.GAMEMODE_PRIVATE_MATCH ) == true then
				f103_local0 = "matchtype/private"
			elseif Engine.GameModeIsMode( CoD.GAMEMODE_THEATER ) == true then
				f103_local0 = "matchtype/theater"
			end
		end
		local f103_local1 = Engine.GetPlayerGroupCount( f103_local0 )
		if f103_local1 ~= "" then
			return Engine.Localize( "MENU_PLAYERS_ONLINE", f103_local1 )
		end
	end
	return ""
end

CoD.Menu.OpenPlayercard = function ( f104_arg0, f104_arg1 )
	if f104_arg0.occludedBy then
		return 
	elseif Engine.IsGuest( f104_arg1.controller ) then
		f104_arg0:openPopup( "popup_guest_contentrestricted", f104_arg1.controller )
		return 
	elseif f104_arg0.menuName ~= "FriendPopup" and f104_arg0.menuName ~= "MainMenu" then
		CoD.FriendPopup.SelectedPlayerXuid = f104_arg1.selectedPlayerXuid
		CoD.FriendPopup.SelectedPlayerName = f104_arg1.selectedFriendName
		f104_arg0:openPopup( "FriendPopup", f104_arg1.controller )
	end
	return true
end

CoD.Menu.COD9ButtonIsEnabled = function ( f105_arg0, f105_arg1 )
	if f105_arg0.customActionButtonPrompt then
		f105_arg0.customActionButtonPrompt:close()
		f105_arg0.customActionButtonPrompt = nil
	end
	if f105_arg0.selectButton then
		f105_arg0:addLeftButtonPrompt( f105_arg0.selectButton )
	end
end

CoD.Menu.COD9ButtonIsDisabled = function ( f106_arg0, f106_arg1 )
	f106_arg0:closeSelectButton()
end

CoD.Menu.SetActionPromptString = function ( f107_arg0, f107_arg1 )
	f107_arg0:closeSelectButton()
	f107_arg0.customActionButtonPrompt = CoD.ButtonPrompt.new( "primary", f107_arg1.promptString )
	f107_arg0.customActionButtonPrompt:setPriority( -10 )
	f107_arg0:addLeftButtonPrompt( f107_arg0.customActionButtonPrompt )
end

CoD.Menu.LeftRightSelectorGainFocus = function ( f108_arg0, f108_arg1 )
	f108_arg0:closeSelectButton()
end

CoD.Menu.LeftRightSelectorLoseFocus = function ( f109_arg0, f109_arg1 )
	if f109_arg0.selectButton then
		f109_arg0:addLeftButtonPrompt( f109_arg0.selectButton )
	end
end

CoD.Menu.LeftRightSilderGainFocus = function ( f110_arg0, f110_arg1 )
	f110_arg0:closeSelectButton()
end

CoD.Menu.LeftRightSilderLoseFocus = function ( f111_arg0, f111_arg1 )
	if f111_arg0.selectButton then
		f111_arg0:addLeftButtonPrompt( f111_arg0.selectButton )
	end
end

CoD.Menu.updateTitleForSplitscreen = function ( f112_arg0 )
	f112_arg0.titleElement:close()
	local f112_local0 = CoD.SplitscreenScaler.new( nil, 1.5 )
	f112_local0:setLeftRight( true, false, 0, 0 )
	f112_local0:setTopBottom( true, false, 0, 0 )
	f112_arg0:addElement( f112_local0 )
	f112_arg0.titleElement:setLeftRight( true, false, CoD.Menu.SplitscreenSideOffset, CoD.Menu.Width )
	f112_local0:addElement( f112_arg0.titleElement )
end

CoD.Menu.updateButtonPromptBarsForSplitscreen = function ( f113_arg0 )
	f113_arg0.leftButtonPromptBar:close()
	local f113_local0 = CoD.SplitscreenScaler.new( nil, 1.5 )
	f113_local0:setLeftRight( true, false, 0, 0 )
	f113_local0:setTopBottom( false, true, 0, 0 )
	f113_arg0:addElement( f113_local0 )
	f113_arg0.leftButtonPromptBar:setLeftRight( true, true, -10 + CoD.Menu.SplitscreenSideOffset, 0 )
	f113_local0:addElement( f113_arg0.leftButtonPromptBar )
	f113_arg0.rightButtonPromptBar:close()
	local f113_local1 = CoD.SplitscreenScaler.new( nil, 1.5 )
	f113_local1:setLeftRight( false, true, 0, -CoD.Menu.SplitscreenSideOffset )
	f113_local1:setTopBottom( false, true, 0, 0 )
	f113_arg0:addElement( f113_local1 )
	f113_arg0.rightButtonPromptBar:setLeftRight( true, true, 0, -CoD.Menu.SplitscreenSideOffset )
	f113_local1:addElement( f113_arg0.rightButtonPromptBar )
end

CoD.Menu.OpenFrontendSaveGameWarningPopup = function ( f114_arg0, f114_arg1 )
	f114_arg0:openPopup( "FrontendSaveGameWarningPopup", Engine.GetPrimaryController(), f114_arg1 )
end

CoD.Menu.OpenFrontendSaveGameErrorPopup = function ( f115_arg0, f115_arg1 )
	f115_arg0:openPopup( "FrontendSaveGameErrorPopup", Engine.GetPrimaryController(), f115_arg1 )
end

CoD.Menu.updateElementState = function ( f116_arg0, f116_arg1, f116_arg2 )
	local f116_local0 = nil
	if f116_arg1.stateConditions then
		for f116_local4, f116_local5 in ipairs( f116_arg1.stateConditions ) do
			if not f116_local5.condition or f116_local5.condition( f116_arg0, f116_arg1, f116_arg2 ) then
				f116_local0 = f116_local5.stateName
				break
			end
		end
	end
	if not f116_local0 then
		f116_local0 = "DefaultState"
	end
	if f116_local0 ~= f116_arg1.currentState then
		local f116_local1 = f116_arg1:isInFocus()
		f116_arg1:setState( f116_local0 )
		if f116_local1 then
			f116_arg1:playClip( "Focus" )
		end
		f116_arg1.currentState = f116_local0
	end
end

CoD.Menu.addButtonPrompt = function ( f117_arg0, f117_arg1, f117_arg2, f117_arg3, f117_arg4 )
	local f117_local0 = "buttonPrompt_" .. f117_arg1
	if not f117_arg0[f117_local0] then
		f117_arg0[f117_local0] = CoD.ButtonPrompt.addUIEditorButtonPrompt( f117_arg0, f117_arg1, f117_arg2, f117_arg3 )
	end
	local f117_local1 = f117_arg0[f117_local0]
	if not f117_local1:getParent() then
		if f117_arg1 == "primary" or f117_arg1 == "secondary" or f117_arg1 == "start" then
			f117_local1:setPriority( 100 )
			if f117_arg1 == "start" then
				f117_local1:setPriority( -200 )
			elseif f117_arg1 == "primary" then
				f117_local1:setPriority( -100 )
				if CoD.isPC and not Engine.LastInput_Gamepad() then
					f117_local1:setAlpha( 0 )
					f117_local1:setPriority( 1000 )
				end
			end
			f117_arg0:addLeftButtonPrompt( f117_local1 )
		else
			f117_arg0:addRightButtonPrompt( f117_local1 )
		end
	end
	f117_local1:SetText( f117_arg2 )
	f117_local1.elementThatAddedButtonPrompt = f117_arg4
end

CoD.Menu.removeButtonPrompt = function ( f118_arg0, f118_arg1, f118_arg2 )
	local f118_local0 = f118_arg0["buttonPrompt_" .. f118_arg1]
	if f118_local0 then
		if f118_local0.elementThatAddedButtonPrompt and f118_local0.elementThatAddedButtonPrompt ~= f118_arg2 then
			return 
		end
		f118_local0:close()
	end
...
CoD.Menu.setupForUI3D = function ( f119_arg0, f119_arg1, f119_arg2, f119_arg3 )
	f119_arg0:setLeftRight( true, false, 0, f119_arg2 )
	f119_arg0:setTopBottom( true, false, 0, f119_arg3 )
	Engine.SetupUI3DWindow( f119_arg1, f119_arg2, f119_arg3 )
	f119_arg0:setUI3DWindow( f119_arg1 )
end

CoD.Menu.OpenMenu = function ( f120_arg0, f120_arg1 )
	if f120_arg0.occludedMenu then
		return 
	elseif f120_arg0.menuName == f120_arg1.menuName then
		return 
	else
		local f120_local0 = f120_arg0.userData
		local f120_local1 = f120_arg0.menuName
		local f120_local2 = f120_arg0.occludedMenu
		local f120_local3 = f120_arg0:getParent()
		f120_arg0:close()
		local f120_local4 = f120_arg0:replaceMenu( f120_arg1.menuName, f120_arg1.controller, nil, oldUserData, f120_local1, f120_local2, f120_local3 )
		local f120_local5 = f120_local4:getParent()
		f120_local5:processEvent( {
			name = "menu_changed",
			prevMenu = f120_arg0,
			nextMenu = f120_local4,
			controller = f120_arg1.controller
		} )
	end
end

CoD.Menu.GetNavigationForMenu = function ( f121_arg0, f121_arg1 )
	if not CoD.MenuNavigation then
		return 
	end
	local f121_local0 = CoD.MenuNavigation[f121_arg0]
	if not f121_local0 then
		return 
	end
	for f121_local1 = #f121_local0, 1, -1 do
		local f121_local4 = f121_local0[f121_local1]
		for f121_local5 = #f121_local4, 1, -1 do
			if f121_local4[f121_local5] == f121_arg1 then
				return f121_local1, f121_local5
			end
		end
	end
end

CoD.Menu.GetNavigationForMenuToMenu = function ( f122_arg0, f122_arg1, f122_arg2 )
	if not CoD.MenuNavigation then
		return 
	end
	...
	for f122_local1 = #f122_local0, 1, -1 do
		...
		for f122_local5 = #f122_local4, 1, -1 do
			if f122_local4[f122_local5] == f122_arg1 then
				for f122_local8 = f122_local5 - 1, 1, -1 do
					if f122_local4[f122_local8] == f122_arg2 then
						return f122_local1, f122_local8
					end
				end
			end
		end
...
CoD.Menu.RatGetMenuInfo = function ( f123_arg0, f123_arg1 )
	if Rat ~= nil then
		Rat.HandleGetRatMenuInfo( f123_arg0, f123_arg1 )
	end
end

CoD.Menu.UpdateAllButtonPrompts = function ( f124_arg0, f124_arg1 )
	if f124_arg0.buttonSubscriptions ~= nil then
		for f124_local3, f124_local4 in pairs( f124_arg0.buttonSubscriptions ) do
			CoD.Menu.UpdateButtonShownState( nil, f124_arg0, f124_arg1, f124_local3 )
		end
	end
end

CoD.Menu.RecordCurrFocusedElemID = function ( f125_arg0, f125_arg1 )
	if f125_arg1.idStack then
		table.insert( f125_arg1.idStack, 1, f125_arg0.id )
	end
	f125_arg0.lastFocusedElemEvent = f125_arg1
	if LUI.DEV ~= nil and Dvar.ui_currFocusList:exists() then
		local f125_local0 = ""
		if f125_arg0.lastFocusedElemEvent and f125_arg0.lastFocusedElemEvent.idStack then
			local f125_local1 = f125_arg0.lastFocusedElemEvent
			local f125_local2 = f125_arg0
			local f125_local3 = 2
			f125_local0 = f125_arg0.id
			while f125_local1 ~= nil do
				for f125_local4 = f125_local3, #f125_local1.idStack, 1 do
					f125_local2 = f125_local2[f125_local1.idStack[f125_local4]]
					if not f125_local2 then
						f125_local0 = f125_local0 .. ".<not found>"
						break
					end
					f125_local0 = f125_local0 .. "." .. f125_local2.id
				end
				if not f125_local2 then
					
				end
				f125_local1 = f125_local1.originalEvent
				f125_local3 = 1
			end
		end
		Dvar.ui_currFocusList:set( f125_local0 )
	end
	if f125_arg1.controller ~= nil then
		CoD.Menu.UpdateAllButtonPrompts( f125_arg0, f125_arg1.controller )
	end
	return true
end

CoD.Menu.subscribeToCurrAnimationModel = function ( f126_arg0 )
	f126_arg0:subscribeToModel( Engine.CreateModel( f126_arg0:getModel(), "current_animation" ), function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			if modelValue == "" and f126_arg0.lastAnimPlayed then
				modelValue = f126_arg0.lastAnimPlayed
			elseif modelValue ~= "" then
				f126_arg0.lastAnimPlayed = modelValue
			end
			f126_arg0:playClip( modelValue )
		end
	end )
end

CoD.Menu.subscribeToCloseMenuModel = function ( f128_arg0 )
	local f128_local0 = f128_arg0:getModel( Engine.GetPrimaryController(), "close_current_menu" )
	if f128_local0 then
		f128_arg0:subscribeToModel( f128_local0, function ( model )
			local modelValue = Engine.GetModelValue( model )
