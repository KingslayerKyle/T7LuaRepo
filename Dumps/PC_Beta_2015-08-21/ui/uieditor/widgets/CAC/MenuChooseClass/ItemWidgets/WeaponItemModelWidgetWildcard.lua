...
CoD.WeaponItemModelWidgetWildcard.new = function ( menu, controller )
	...
	itemHintText:setLeftRight( false, false, -100, 100 )
	itemHintText:setTopBottom( false, true, 13, 33 )
	...
	hintArrow:setTopBottom( false, true, 3, 13 )
	...
	newNumber:setTopBottom( true, false, 3, 19 )
	self:addElement( newNumber )
	self.newNumber = newNumber
	
	local RestrictedItemWarning = CoD.RestrictedItemWarning.new( menu, controller )
	RestrictedItemWarning:setLeftRight( true, false, -7.63, 134.63 )
	RestrictedItemWarning:setTopBottom( true, false, 7, 129.85 )
	...
	removeButton:setLeftRight( false, true, -23, -3 )
	removeButton:setTopBottom( true, false, 3.75, 23.75 )
	...
	removeButton:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if f10_local0 then
					f10_local0 = IsCurrentMenu( menu, "CustomClass" )
					if f10_local0 then
						if not IsGamepad( controller ) then
							f10_local0 = IsCACSlotEquipped( menu, element, controller )
						else
							f10_local0 = false
						end
					end
	...
	if removeButton.m_eventHandlers.input_source_changed then
		local f2_local23 = removeButton.m_eventHandlers.input_source_changed
		removeButton:registerEventHandler( "input_source_changed", function ( element, event )
			event.menu = event.menu or menu
			element:updateState( event )
			return f2_local23( element, event )
		end )
	else
