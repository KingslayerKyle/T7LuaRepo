...
CoD.GridItemButtonNew.new = function ( menu, controller )
	...
	tokenUnlock:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if not IsCACItemLocked( menu, element, controller ) and not IsCACItemPurchased( element, controller ) then
					f4_local0 = not IsCurrentMenuAttachmentMenu( menu )
				else
					f4_local0 = false
				end
				return f4_local0
	...
	ConsumableLabel:setLeftRight( true, false, 7, 29 )
	...
	ConsumableLabel:linkToElementModel( self, "itemIndex", true, function ( model )
		...
		if itemIndex then
			ConsumableLabel.ConsumableCountLabel:setText( Engine.Localize( GetConsumableCountFromIndex( controller, itemIndex ) ) )
		end
	end )
	...
	RestrictedItemWarning:mergeStateConditions( {
		{
			stateName = "Visible",
			condition = function ( menu, element, event )
				local f15_local0 = ItemIsBanned( menu, element, controller )
				if f15_local0 then
					f15_local0 = PregameActive()
					if f15_local0 then
						f15_local0 = not ItemRestrictionVoteActive()
					end
				end
				return f15_local0
			end
	...
	self:registerEventHandler( "gain_focus", function ( self, event )
		if self.m_focusable and self.ConsumableLabel:processEvent( event ) then
			return true
		else
