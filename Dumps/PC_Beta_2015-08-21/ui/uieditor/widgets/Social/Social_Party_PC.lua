...
DataSources.SocialPartyPrivacyPresets = DataSourceHelpers.ListSetup( "PC.SocialPartyPrivacyPresets", function ( f3_arg0 )
	...
	table.insert( f3_local0, {
		models = {
			...
			valueDisplay = "MENU_PARTY_PRIVACY_INVITE_ONLY"
		}
	} )
	table.insert( f3_local0, {
		models = {
...
local PostLoadFunc = function ( self, controller, menu )
	...
	self:registerEventHandler( "dropdown_triggered", function ( element, event )
		for f14_local0 = 1, element.DropdownList.requestedRowCount, 1 do
			local f14_local3 = element.DropdownList:getItemAtPosition( f14_local0, 1 )
			if event.inUse then
	...
	self.DropdownList:registerEventHandler( "dropdown_triggered", function ( element, event )
		if not event.inUse then
			local f15_local0 = event.widget.dropDownCurrentValue( controller, element )
			local f15_local1 = event.widget:getModel()
			if f15_local1 then
				local f15_local2 = Engine.GetModelValue( Engine.GetModel( f15_local1, "profileVarName" ) )
				if f15_local2 == "party_privacyStatus" then
					f0_local0( f15_local0 )
				elseif f15_local2 == "party_maxplayers" then
					f0_local1( f15_local0 )
				end
			end
		end
