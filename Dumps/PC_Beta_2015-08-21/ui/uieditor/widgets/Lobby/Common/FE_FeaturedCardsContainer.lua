...
CoD.FE_FeaturedCardsContainer.new = function ( menu, controller )
	...
	CardsList:setTopBottom( true, false, 36, 123 )
	...
	Action:setLeftRight( true, false, 28, 254.76 )
	...
	if Action.m_eventHandlers.input_source_changed then
		local f7_local16 = Action.m_eventHandlers.input_source_changed
		Action:registerEventHandler( "input_source_changed", function ( element, event )
			event.menu = event.menu or menu
			element:updateState( event )
			return f7_local16( element, event )
		end )
	else
	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f126_local0 = FeaturedCards_IsEnabled( element, controller )
				if f126_local0 then
					if not IsGameLobby() then
						f126_local0 = not IsLobbyNetworkModeLAN()
					else
						f126_local0 = false
					end
				end
