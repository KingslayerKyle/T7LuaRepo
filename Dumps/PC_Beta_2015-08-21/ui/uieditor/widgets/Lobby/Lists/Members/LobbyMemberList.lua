...
CoD.LobbyMemberList.new = function ( menu, controller )
	...
	lobbyList:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyNav" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f10_local0, menu, controller, Enum.LUIButton.LUI_KEY_LB )
		CoD.Menu.UpdateButtonShownState( f10_local0, menu, controller, Enum.LUIButton.LUI_KEY_RB )
	end )
	...
	if FEListAdditonal0.m_eventHandlers.on_client_added then
		local FEListHeaderFull0 = FEListAdditonal0.m_eventHandlers.on_client_added
		FEListAdditonal0:registerEventHandler( "on_client_added", function ( element, event )
			event.menu = event.menu or menu
			element:updateState( event )
			return FEListHeaderFull0( element, event )
		end )
	else
	...
	if FEListAdditonal0.m_eventHandlers.on_client_removed then
		local FEListHeaderFull0 = FEListAdditonal0.m_eventHandlers.on_client_removed
		FEListAdditonal0:registerEventHandler( "on_client_removed", function ( element, event )
			event.menu = event.menu or menu
			element:updateState( event )
			return FEListHeaderFull0( element, event )
		end )
	...
	FEListHeaderFull0:setTopBottom( true, false, 584, 613 )
	...
	friendsAndGroups:linkToElementModel( friendsAndGroups, "isJoinable", true, function ( model )
		local f31_local0 = friendsAndGroups
		local f31_local1 = {
			controller = controller,
			name = "model_validation",
			modelValue = Engine.GetModelValue( model ),
		...
		CoD.Menu.UpdateButtonShownState( f31_local0, menu, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
