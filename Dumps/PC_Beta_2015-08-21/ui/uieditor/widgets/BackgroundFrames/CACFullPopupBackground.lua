...
CoD.CACFullPopupBackground.new = function ( menu, controller )
	...
	if selectButtonPrompt.m_eventHandlers.input_source_changed then
		local backButtonPrompt = selectButtonPrompt.m_eventHandlers.input_source_changed
		selectButtonPrompt:registerEventHandler( "input_source_changed", function ( element, event )
			event.menu = event.menu or menu
			element:updateState( event )
			return backButtonPrompt( element, event )
		end )
	else
	...
	local backButtonPrompt = CoD.FooterButtonPrompt.new( menu, controller )
	backButtonPrompt:setLeftRight( true, false, 171, 271 )
	backButtonPrompt:setTopBottom( true, false, 544.5, 576.5 )
	backButtonPrompt.label:setText( Engine.Localize( "Select" ) )
	...
	backButtonPrompt:linkToElementModel( self, "" .. Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE, false, function ( model )
		backButtonPrompt:setModel( model, controller )
	end )
