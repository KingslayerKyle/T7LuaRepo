...
CoD.FileshareSlotsAvailable.new = function ( menu, controller )
	...
	LblSlotsAvailable:subscribeToGlobalModel( controller, "FileshareRoot", "currentCategoryQuota.categorySlotsAvailable", function ( model )
		local currentCategoryQuotaCategorySlotsAvailable = Engine.GetModelValue( model )
		if currentCategoryQuotaCategorySlotsAvailable then
			LblSlotsAvailable:setText( Engine.Localize( FileshareGetSlotString( currentCategoryQuotaCategorySlotsAvailable ) ) )
		end
	end )
	...
	TxtSlots:subscribeToGlobalModel( controller, "FileshareRoot", "currentCategoryQuota.categorySlotsAvailable", function ( model )
		local currentCategoryQuotaCategorySlotsAvailable = Engine.GetModelValue( model )
		if currentCategoryQuotaCategorySlotsAvailable then
			TxtSlots:setText( Engine.Localize( currentCategoryQuotaCategorySlotsAvailable ) )
		end
	end )
