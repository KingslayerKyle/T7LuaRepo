local PreLoadFunc = function ( self, controller )
	local f1_local0 = Engine.CreateModel( Engine.GetGlobalModel(), "NetworkImage", true )
	Engine.CreateModel( f1_local0, "image1", true )
	Engine.CreateModel( f1_local0, "image2", true )
end
...
CoD.StartMenu_ConnectionMeter.new = function ( menu, controller )
	...
	Image0:subscribeToGlobalModel( controller, "NetworkImage", "image1", function ( model )
		local image1 = Engine.GetModelValue( model )
		if image1 then
			Image0:setImage( RegisterImage( image1 ) )
		end
	end )
	...
	Image00:subscribeToGlobalModel( controller, "NetworkImage", "image2", function ( model )
		local image2 = Engine.GetModelValue( model )
		if image2 then
			Image00:setImage( RegisterImage( image2 ) )
		end
	end )
