...
CoD.WeaponBuildKitsVariantAttachments.new = function ( menu, controller )
	...
	Optic:registerEventHandler( "gain_focus", function ( element, event )
		...
		Gunsmith_SetSelectedItemName( self, element, "attachment", "1", controller )
	...
	Attachment3:linkToElementModel( Attachment3, "attachment4", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f43_local0, menu, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	...
	Attachment4:linkToElementModel( Attachment4, "attachment5", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f56_local0, menu, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	Attachment4:registerEventHandler( "gunsmith_remove_item", function ( element, event )
		...
		if not f57_local0 then
			f57_local0 = element:dispatchEventToChildren( event )
		end
		return f57_local0
	end )
	...
	Attachment5:linkToElementModel( Attachment5, "attachment6", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f69_local0, menu, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	...
	Attachment5:registerEventHandler( "lose_focus", function ( element, event )
		...
		if element.loseFocus then
			f72_local0 = element:loseFocus( event )
		elseif element.super.loseFocus then
			f72_local0 = element.super:loseFocus( event )
		end
		return f72_local0
	end )
	...
	paintjob:linkToElementModel( paintjob, "paintjobSlot", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f82_local0, menu, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	paintjob:registerEventHandler( "gunsmith_remove_item", function ( element, event )
		...
		if not f83_local0 then
			f83_local0 = element:dispatchEventToChildren( event )
		end
		return f83_local0
	end )
	...
	paintjob:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if not IsSelfModelValueEqualTo( element, controller, "paintjobSlot", 15 ) then
					f90_local0 = not IsSelfModelValueEqualTo( element, controller, "paintjobIndex", 15 )
				else
					f90_local0 = false
				end
				return f90_local0
	...
	Camo:registerEventHandler( "gain_focus", function ( element, event )
		...
		if element.gainFocus then
			f97_local0 = element:gainFocus( event )
		elseif element.super.gainFocus then
			f97_local0 = element.super:gainFocus( event )
		end
	...
	Camo:registerEventHandler( "lose_focus", function ( element, event )
		...
		if element.loseFocus then
			f98_local0 = element:loseFocus( event )
		elseif element.super.loseFocus then
			f98_local0 = element.super:loseFocus( event )
		end
		return f98_local0
	end )
	...
	self.clipsPerState = {
		DefaultState = {
			DefaultClip = function ()
				...
				self.AttachmentsHeader:setLeftRight( true, true, 112.67, -245.9 )
				...
				self.PaintjobHeader:setLeftRight( true, false, 501.77, 609.77 )
				...
				self.CamoHeader:setLeftRight( true, false, 611.44, 719.44 )
		...
		SpecialWeapon = {
			DefaultClip = function ()
				...
				self.clipFinished( PaintjobHeader, {} )
				CamoHeader:completeAnimation()
				self.CamoHeader:setLeftRight( true, false, 110.67, 218.67 )
				self.CamoHeader:setTopBottom( true, false, 0, 17 )
				self.CamoHeader:setAlpha( 1 )
				self.clipFinished( CamoHeader, {} )
				paintjob:completeAnimation()
		...
		Handguns = {
			DefaultClip = function ()
				...
				self.clipFinished( PaintjobHeader, {} )
				CamoHeader:completeAnimation()
				self.CamoHeader:setLeftRight( true, false, 402.1, 510.1 )
				self.CamoHeader:setTopBottom( true, false, 0, 17 )
				self.CamoHeader:setAlpha( 1 )
				self.clipFinished( CamoHeader, {} )
				paintjob:completeAnimation()
