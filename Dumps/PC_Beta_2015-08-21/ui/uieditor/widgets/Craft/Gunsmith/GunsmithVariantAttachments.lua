...
CoD.GunsmithVariantAttachments.new = function ( menu, controller )
	...
	Attachment1:linkToElementModel( Attachment1, "attachment2", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f16_local0, menu, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	...
	Attachment3:linkToElementModel( Attachment3, "attachment4", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f40_local0, menu, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	...
	Attachment4:linkToElementModel( Attachment4, "attachment5", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f52_local0, menu, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	...
	paintjob:linkToElementModel( paintjob, "paintjobSlot", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f76_local0, menu, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	paintjob:registerEventHandler( "gunsmith_remove_item", function ( element, event )
		...
		if not f77_local0 then
			f77_local0 = element:dispatchEventToChildren( event )
		end
		return f77_local0
	end )
	...
	Camo:linkToElementModel( Camo, "camoIndex", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f89_local0, menu, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	Camo:registerEventHandler( "gunsmith_remove_item", function ( element, event )
		...
		if not f90_local0 then
			f90_local0 = element:dispatchEventToChildren( event )
		end
		return f90_local0
	end )
	Camo:registerEventHandler( "gain_focus", function ( element, event )
		...
		if element.gainFocus then
			f91_local0 = element:gainFocus( event )
		elseif element.super.gainFocus then
			f91_local0 = element.super:gainFocus( event )
		end
	...
	Camo:registerEventHandler( "lose_focus", function ( element, event )
		...
		if element.loseFocus then
			f92_local0 = element:loseFocus( event )
		elseif element.super.loseFocus then
			f92_local0 = element.super:loseFocus( event )
		end
		return f92_local0
	end )
