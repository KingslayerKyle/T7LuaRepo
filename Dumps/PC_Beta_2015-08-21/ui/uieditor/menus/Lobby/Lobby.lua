...
local f0_local2 = function ( f4_arg0, f4_arg1 )
	...
	while f4_local0 ~= nil do
		...
		if f4_local0.disableLeaderChangePopupShutdown ~= nil then
			return 
		elseif f4_local0.menuName == "Spinner" then
			return 
		end
		Engine.SendClientScriptNotify( f4_arg1, "menu_change", "Main", "closeToMenu" )
...
local PostLoadFunc = function ( f11_arg0, f11_arg1 )
	f11_arg0.tryOpenFirstTimeFlow = f0_local6
	f11_arg0.LeftContainer.navigation = nil
	f11_arg0.LeftContainer.navigation = {}
	f11_arg0.LeftContainer.navigation.right = f11_arg0.ClientList
	f11_arg0.LeftContainer.navigation.down = f11_arg0.MapVote
	f11_arg0.LeftContainer.navigation.up = f11_arg0.MapVote
	f11_arg0.ClientList.navigation = nil
	f11_arg0.ClientList.navigation = {}
	f11_arg0.ClientList.navigation.left = f11_arg0.LeftContainer
	f11_arg0.MapVote.navigation = nil
	f11_arg0.MapVote.navigation = {}
	f11_arg0.MapVote.navigation.up = f11_arg0.LeftContainer
	f11_arg0.MapVote.navigation.down = f11_arg0.LeftContainer
	f11_arg0.MapVote.navigation.right = f11_arg0.ClientList
	f11_arg0:clearSavedState()
	CoD.LobbyBase.RegisterEventHandlers( f11_arg0 )
	Engine.CreateModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyNav" )
	f11_arg0:subscribeToModel( Engine.CreateModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyNav" ), function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue == LobbyData.UITargets.UI_MAIN.id then
			local f12_local1 = CoD.Menu.safeCreateMenu( "Main", f11_arg1 )
			local f12_local2 = f11_arg0:getParent()
		...
		if modelValue == LobbyData.UITargets.UI_MODESELECT.id then
			CoD.PlayFrontendMusic( "titlescreen" )
		end
		f0_local4( f11_arg1 )
	end, false )
	local f11_local0 = Engine.CreateModel( Engine.GetGlobalModel(), "lobbyRoot.Pregame" )
	local f11_local1 = Engine.CreateModel( f11_local0, "Update" )
	f11_arg0:subscribeToModel( Engine.CreateModel( f11_local0, "state" ), function ( model )
		local modelValue = Engine.GetModelValue( model )
		...
		elseif modelValue ~= Enum.LobbyPregameState.LOBBY_PREGAME_STATE_IDLE then
			...
			f0_local2( f11_arg0, f11_arg1 )
			NavigateToMenu( f11_arg0, "Pregame_Main", false, f11_arg1 )
		end
	end )
	if CoD.isPC then
	...
	if Dvar.ui_execdemo_cp:get() then
		f11_arg0.Vignette:setAlpha( 0 )
	end
	f11_arg0:subscribeToModel( Engine.CreateModel( Engine.GetGlobalModel(), "lobbyRoot.room" ), function ( model )
		RefreshLobbyRoom( f11_arg0, f11_arg1 )
	end )
	f11_arg0:subscribeToModel( Engine.CreateModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyGameMode" ), function ( model )
		f0_local2( f11_arg0, f11_arg1 )
	end, false )
	f11_arg0:subscribeToModel( Engine.CreateModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyMainMode" ), function ( model )
		f0_local2( f11_arg0, f11_arg1 )
		Engine.MarkPSDataDirty( f11_arg1, true )
	end, false )
	if CoD.isPC then
		LUI.OverrideFunction_CallOriginalSecond( f11_arg0, "handleGamepadButtonModelCallback", function ( element, f17_arg1, f17_arg2, f17_arg3, f17_arg4 )
			local f17_local0 = LUI.UIElement.GetCorrespondingGamepadButtonString( f17_arg3 )
			if f17_local0 == "left" or f17_local0 == "right" then
				element.defaultNav = f17_local0
			end
		end )
	end
	f11_arg0:subscribeToModel( Engine.CreateModel( Engine.GetGlobalModel(), "MapVote.timerActive" ), function ( model )
		if model and Engine.GetModelValue( model ) and Engine.GetModelValue( model ) ~= 0 then
			local f18_local0 = Dvar.ui_mapname:get()
			local f18_local1 = CoD.GetMapValue( f18_local0, "loadingImage", "black" )
			if not f11_arg0.mapImage or f18_local0 ~= f11_arg0.mapName then
				f11_arg0.mapName = f18_local0
				f11_arg0.mapImage = LUI.UIStreamedImage.new()
				f11_arg0.mapImage:setLeftRight( true, false, 0, 1 )
				f11_arg0.mapImage:setTopBottom( true, false, 0, 1 )
				f11_arg0.mapImage:setAlpha( 0 )
				f11_arg0.mapImage:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_normal" ) )
				f11_arg0.mapImage:setImage( RegisterImage( f18_local1 ) )
				f11_arg0:addElement( f11_arg0.mapImage )
			end
		end
	end )
	f11_arg0.occluded = false
	f11_arg0:subscribeToModel( Engine.CreateModel( Engine.GetGlobalModel(), "pubstorageFilesChanged" ), function ( model )
		if Engine.IsLobbyActive( Enum.LobbyType.LOBBY_TYPE_GAME ) then
			return 
		elseif Engine.GetModelValue( model ) == true and f11_arg0.occluded ~= nil and f11_arg0.occluded == false then
			CoD.OverlayUtility.CreateOverlay( f11_arg1, f11_arg0, "PublisherFilesChangedPopup" )
		end
	end, false )
	f11_arg0:registerEventHandler( "occlusion_change", function ( element, event )
...
LUI.createMenu.Lobby = function ( controller )
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "lobbyRoot.hideMenusForGameStart" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f97_local0, self, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE )
	end )
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyNav" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f99_local0, self, controller, Enum.LUIButton.LUI_KEY_XBY_PSTRIANGLE )
		CoD.Menu.UpdateButtonShownState( f99_local0, self, controller, Enum.LUIButton.LUI_KEY_LB )
		CoD.Menu.UpdateButtonShownState( f99_local0, self, controller, Enum.LUIButton.LUI_KEY_RB )
		CoD.Menu.UpdateButtonShownState( f99_local0, self, controller, Enum.LUIButton.LUI_KEY_BACK )
		CoD.Menu.UpdateButtonShownState( f99_local0, self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyMainMode" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f101_local0, self, controller, Enum.LUIButton.LUI_KEY_BACK )
	end )
	self:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "FeaturedCardsRoot.Enabled" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f102_local0, self, controller, Enum.LUIButton.LUI_KEY_RB )
		CoD.Menu.UpdateButtonShownState( f102_local0, self, controller, Enum.LUIButton.LUI_KEY_LB )
	end )
	...
	self:registerEventHandler( "record_curr_focused_elem_id", function ( self, event )
		...
		if self.RecordCurrFocusedElemID then
			f104_local0 = self:RecordCurrFocusedElemID( event )
		elseif self.super.RecordCurrFocusedElemID then
			f104_local0 = self.super:RecordCurrFocusedElemID( event )
		end
	...
	self:registerEventHandler( "menu_loaded", function ( self, event )
		...
		if not f105_local0 then
			f105_local0 = self:dispatchEventToChildren( event )
		end
		return f105_local0
	end )
	self:registerEventHandler( "menu_opened", function ( self, event )
		...
		if not f106_local0 then
			f106_local0 = self:dispatchEventToChildren( event )
		end
		return f106_local0
	end )
	...
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_LB, "Q", function ( element, menu, controller, model )
		...
		if not IsRepeatButtonPress( nil ) and IsLobbyWithTeamAssignment() and not IsTeamAssignment( LuaEnums.TEAM_ASSIGNMENT.HOST ) and IsMenuInState( menu, "Left" ) then
	...
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_RB, "W", function ( element, menu, controller, model )
		if not IsRepeatButtonPress( model ) and IsLobbyWithTeamAssignment() and not IsTeamAssignment( LuaEnums.TEAM_ASSIGNMENT.HOST ) and IsMenuInState( menu, "Left" ) then
