...
local PreLoadFunc = function ( self, controller )
	...
	self.restoreState = function ( f2_arg0 )
		if Paintjobs_IsOccupied( f2_arg0, controller ) then
			f2_arg0.save:processEvent( {
				name = "gain_focus",
				controller = controller
			} )
		else
			f2_arg0.paintjobNameInput:processEvent( {
				name = "gain_focus",
				controller = controller
			} )
		end
...
LUI.createMenu.PaintjobSaveChanges = function ( controller )
	...
	local paintjobNameInput = CoD.GunsmithInputButton.new( self, controller )
	...
	self:AddButtonCallbackFunction( paintjobNameInput, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( element, menu, controller, model )
	...
	LUI.OverrideFunction_CallOriginalFirst( paintjobNameInput, "setState", function ( element, controller )
	...
	paintjobNameInput:mergeStateConditions( {
		{
			stateName = "Disabled",
			condition = function ( menu, element, event )
				return Paintjobs_IsOccupied( element, controller )
			end
		}
	} )
	...
	self:AddButtonCallbackFunction( save, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( element, menu, controller, model )
		PaintjobEditor_SavePaintjob( self, element, controller )
	...
	LUI.OverrideFunction_CallOriginalFirst( save, "setState", function ( element, controller )
		if IsElementInState( element, "Disabled" ) then
			MakeNotFocusable( element )
		else
			MakeFocusable( element )
	...
	save:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f21_local0
				if not Paintjobs_ValidPaintjobNameEntered( element, controller ) then
					f21_local0 = not Paintjobs_IsOccupied( element, controller )
				else
					f21_local0 = false
				end
				return f21_local0
	...
	save:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "Paintshop.validPaintjobNameEntered" ), function ( model )
		self:updateElementState( save, {
			name = "model_validation",
			menu = self,
			modelValue = Engine.GetModelValue( model ),
			modelName = "Paintshop.validPaintjobNameEntered"
		} )
	end )
	...
	self:AddButtonCallbackFunction( discard, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( element, menu, controller, model )
		PaintjobEditor_DiscardChanges( self, element, controller )
		return true
	end, function ( element, menu, controller )
		CoD.Menu.SetButtonLabel( menu, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "MENU_SELECT" )
		return true
	...
	self:registerEventHandler( "ui_keyboard_input", function ( self, event )
		...
		PaintjobEditor_HandleKeyboardComplete( self, self, controller, event )
	...
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE, nil, function ( element, menu, controller, model )
		...
		PaintjobEditor_SaveChangesBack( self, element, controller )
		return true
	end, function ( element, menu, controller )
	...
	if not self:restoreState() then
		self.paintjobNameInput:processEvent( {
			name = "gain_focus",
			controller = controller
		} )
	end
