...
LUI.createMenu.WeaponBuildKitsCustomizeVariant = function ( controller )
	...
	cacWpnLvl0:linkToElementModel( self, "weaponIndex", true, function ( model )
		local weaponIndex = Engine.GetModelValue( model )
		if weaponIndex then
			cacWpnLvl0.nextLevel:setText( Engine.Localize( GetNextWeaponLevel( controller, weaponIndex ) ) )
		end
	end )
	cacWpnLvl0:linkToElementModel( self, "weaponIndex", true, function ( model )
		local weaponIndex = Engine.GetModelValue( model )
		if weaponIndex then
			cacWpnLvl0.currentLevelBacking:setText( Engine.Localize( GetCurrentWeaponLevel( controller, weaponIndex ) ) )
		end
	end )
	cacWpnLvl0:linkToElementModel( self, "weaponIndex", true, function ( model )
		local weaponIndex = Engine.GetModelValue( model )
		if weaponIndex then
			cacWpnLvl0.currentLevel:setText( Engine.Localize( GetCurrentWeaponLevel( controller, weaponIndex ) ) )
		end
	end )
	cacWpnLvl0:linkToElementModel( self, "weaponIndex", true, function ( model )
		local weaponIndex = Engine.GetModelValue( model )
		if weaponIndex then
			cacWpnLvl0.cacWpnLvlMeter0.Meter:setShaderVector( 0, GetCurrentWeaponXP( controller, CoD.GetVectorComponentFromString( weaponIndex, 1 ), CoD.GetVectorComponentFromString( weaponIndex, 2 ), CoD.GetVectorComponentFromString( weaponIndex, 3 ), CoD.GetVectorComponentFromString( weaponIndex, 4 ) ) )
		end
	end )
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( element )
		...
		element.GunsmithCustomizeGraphics0:close()
