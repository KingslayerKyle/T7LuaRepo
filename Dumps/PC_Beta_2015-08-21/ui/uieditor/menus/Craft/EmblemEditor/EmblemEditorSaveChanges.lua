...
local PreLoadFunc = function ( self, controller )
	...
	self.restoreState = function ( f2_arg0 )
		if Emblem_IsOccupied( f2_arg0, controller ) then
			f2_arg0.save:processEvent( {
				name = "gain_focus",
				controller = controller
			} )
		else
			f2_arg0.emblemNameInput:processEvent( {
				name = "gain_focus",
				controller = controller
			} )
		end
...
LUI.createMenu.EmblemEditorSaveChanges = function ( controller )
	...
	LUI.OverrideFunction_CallOriginalFirst( emblemNameInput, "setState", function ( element, controller )
	...
	local savePlayerCard = CoD.List1ButtonLarge_Left_ND.new( self, controller )
	...
	self:AddButtonCallbackFunction( savePlayerCard, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( element, menu, controller, model )
	...
	LUI.OverrideFunction_CallOriginalFirst( savePlayerCard, "setState", function ( element, controller )
		if IsElementInState( element, "Disabled" ) then
			MakeNotFocusable( element )
		else
			MakeFocusable( element )
	...
	savePlayerCard:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "Emblem.validEmblemNameEntered" ), function ( model )
		self:updateElementState( savePlayerCard, {
			name = "model_validation",
			menu = self,
			modelValue = Engine.GetModelValue( model ),
			modelName = "Emblem.validEmblemNameEntered"
