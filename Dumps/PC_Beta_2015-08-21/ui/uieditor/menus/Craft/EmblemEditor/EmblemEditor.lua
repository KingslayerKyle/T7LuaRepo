...
local PostLoadFunc = function ( f5_arg0 )
	...
	EmblemEditor_SetupEditorBasedOnMode( f5_arg0, f5_arg0, f5_local0 )
	f5_arg0.isRightStickRepeated = false
	f0_local4( f5_arg0, f5_local0 )
	if IsPaintshop( f5_local0 ) and Dvar.ui_disablePSControlWidgets:get() == true then
		f5_arg0.editorControlsGuide:setAlpha( 0 )
		f5_arg0.layerProperties:setAlpha( 0 )
		f5_arg0.paintshopControls:setAlpha( 0 )
		f5_arg0.choosePaintSide:setAlpha( 0 )
	end
end
...
LUI.createMenu.EmblemEditor = function ( controller )
	...
	layerCarousel:linkToElementModel( layerCarousel, "isGrouped", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f8_local0, self, controller, Enum.LUIButton.LUI_KEY_RTRIG )
		CoD.Menu.UpdateButtonShownState( f8_local0, self, controller, Enum.LUIButton.LUI_KEY_LB )
	end )
	layerCarousel:linkToElementModel( layerCarousel, "isLinked", true, function ( model )
		local f9_local0 = layerCarousel
		local f9_local1 = {
			controller = controller,
			name = "model_validation",
			modelValue = Engine.GetModelValue( model ),
	...
	self:AddButtonCallbackFunction( layerCarousel, controller, Enum.LUIButton.LUI_KEY_RTRIG, nil, function ( element, menu, controller, model )
		...
		if IsBrowseMode( controller ) and not IsLayerEmpty( self, element, controller ) and not Emblem_IsLayerGrouped( element, controller ) and not IsRepeatButtonPress( nil ) and not IsDemoBuildBeta() then
			return false
		else
			return false
		end
	...
	self:AddButtonCallbackFunction( layerCarousel, controller, Enum.LUIButton.LUI_KEY_RB, nil, function ( element, menu, controller, model )
		...
		if IsBrowseMode( controller ) and not IsLayerEmpty( self, element, controller ) and Emblems_IsLayerLinked( element, controller ) and not IsRepeatButtonPress( nil ) and not IsDemoBuildBeta() then
			return false
		else
			return false
		end
	...
	browseControlBar:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "Emblem.EmblemProperties.editorMode" ), function ( model )
		self:updateElementState( browseControlBar, {
			name = "model_validation",
			menu = self,
			modelValue = Engine.GetModelValue( model ),
			modelName = "Emblem.EmblemProperties.editorMode"
	...
	browseControlBar:linkToElementModel( browseControlBar, "isLinked", true, function ( model )
		self:updateElementState( browseControlBar, {
			name = "model_validation",
			menu = self,
			modelValue = Engine.GetModelValue( model ),
	...
	browseControlBar:linkToElementModel( browseControlBar, "isGrouped", true, function ( model )
		self:updateElementState( browseControlBar, {
			name = "model_validation",
			menu = self,
			modelValue = Engine.GetModelValue( model ),
	...
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_NONE, "R", function ( element, menu, controller, model )
	...
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_BACK, "F", function ( element, menu, controller, model )
		if IsEditMode( controller ) then
	...
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_DOWN, nil, function ( element, menu, controller, model )
		...
		if IsBrowseMode( controller ) and not IsLayerEmpty( self, element, controller ) and IsDpadButton( nil ) then
			return false
		else
			return false
		end
	end, false )
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_UP, "E", function ( element, menu, controller, model )
	...
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_LEFT, nil, function ( element, menu, controller, model )
		...
		if IsEditMode( controller ) and IsDpadButton( f78_local0 ) and not IsRepeatButtonPress( f78_local0 ) then
			return false
		else
			return false
		end
	...
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_RIGHT, nil, function ( element, menu, controller, model )
		...
		if IsEditMode( controller ) and IsDpadButton( f80_local0 ) and not IsRepeatButtonPress( f80_local0 ) then
			return false
		else
			return false
		end
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( element )
		...
		element.paintjobTitle:close()
		element.emblemEditorTitle:close()
