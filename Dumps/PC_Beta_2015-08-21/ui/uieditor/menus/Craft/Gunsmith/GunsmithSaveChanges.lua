...
LUI.createMenu.GunsmithSaveChanges = function ( controller )
	...
	subTitle:subscribeToGlobalModel( controller, "Customization", "weapon_index", function ( model )
		...
		if weaponIndex then
			subTitle:setText( Engine.Localize( LocalizeWeaponNameIntoString( "MENU_GUNSMITH_SAVE_CHANGES_SUBTITLE", controller, weaponIndex ) ) )
		end
	end )
	...
	self.StartMenuTitlePopup:linkToElementModel( self, "variantName", true, function ( model )
		...
		if variantName then
			StartMenuTitlePopup.StartMenuTitleContainerLOC.StartMenu3dTitle.StartMenu3dTitleLabel.Label0:setText( Engine.Localize( LocalizeIntoString( "MENU_GUNSMITH_SAVE_CHANGES_TITLE", variantName ) ) )
		end
	end )
