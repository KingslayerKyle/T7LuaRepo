...
LUI.createMenu.Gunsmith = function ( controller )
	...
	itemName:setLeftRight( true, false, 449.5, 830.5 )
	...
	cacElemsSideListBoxes0:setTopBottom( true, false, -11, 659 )
	...
	self.clipsPerState = {
		...
		ShowVariants = {
			DefaultClip = function ()
				...
				self.clipFinished( weaponAttributes, {} )
				variantSelector:completeAnimation()
				self.variantSelector:setLeftRight( true, false, 72, 428 )
				self.variantSelector:setTopBottom( true, false, 129, 655 )
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "Gunsmith.Mode" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f46_local0, self, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE )
	end )
