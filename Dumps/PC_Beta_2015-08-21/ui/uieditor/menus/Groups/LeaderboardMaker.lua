...
LUI.createMenu.LeaderboardMaker = function ( controller )
	...
	LeaderboardMakerChosenInformation:linkToElementModel( ButtonList, "baseLeaderboard", true, function ( model )
		...
		if baseLeaderboard then
			LeaderboardMakerChosenInformation.BaseLeaderboard:setText( Engine.Localize( baseLeaderboard ) )
		end
	end )
	LeaderboardMakerChosenInformation:linkToElementModel( ButtonList, "sortColumn", true, function ( model )
		local sortColumn = Engine.GetModelValue( model )
		if sortColumn then
			LeaderboardMakerChosenInformation.DefaultSort:setText( Engine.Localize( sortColumn ) )
		end
	end )
	LeaderboardMakerChosenInformation:linkToElementModel( ButtonList, "timeFrame", true, function ( model )
		...
		if timeFrame then
			LeaderboardMakerChosenInformation.TimeFrame:setText( Engine.Localize( timeFrame ) )
		end
	end )
	self.clipsPerState = {
		DefaultState = {
	...
	self:registerEventHandler( "list_item_gain_focus", function ( self, event )
		...
		SetHintText( self, self, controller )
		return f19_local0
	end )
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( element )
		element.GroupsBackground:close()
		element.ButtonList:close()
		element.HintText:close()
		element.LeaderboardMakerProgressBar:close()
		element.LeaderboardMakerChosenInformation:close()
