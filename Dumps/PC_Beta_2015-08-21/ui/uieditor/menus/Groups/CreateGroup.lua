...
LUI.createMenu.CreateGroup = function ( controller )
	...
	PrivacyInput:subscribeToGlobalModel( controller, "CreateGroup", "privacy", function ( model )
		...
		if privacy then
			PrivacyInput:setText( Engine.Localize( privacy ) )
		end
	end )
	...
	NameInput:subscribeToGlobalModel( controller, "CreateGroup", "default_name", function ( model )
		local defaultName = Engine.GetModelValue( model )
		if defaultName then
			NameInput.DefaultText:setText( Engine.Localize( defaultName ) )
		end
	end )
	NameInput:subscribeToGlobalModel( controller, "CreateGroup", "name", function ( model )
		local name = Engine.GetModelValue( model )
		if name then
			NameInput.Text:setText( Engine.Localize( name ) )
		end
	end )
	...
	DescriptionInput:subscribeToGlobalModel( controller, "CreateGroup", "default_description", function ( model )
		local defaultDescription = Engine.GetModelValue( model )
		if defaultDescription then
			DescriptionInput.DefaultText:setText( Engine.Localize( defaultDescription ) )
		end
	end )
	DescriptionInput:subscribeToGlobalModel( controller, "CreateGroup", "description", function ( model )
		local description = Engine.GetModelValue( model )
		if description then
			DescriptionInput.Text:setText( Engine.Localize( description ) )
		end
	end )
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( element )
		...
		element.BorderBaked1:close()
		element.BorderBaked2:close()
