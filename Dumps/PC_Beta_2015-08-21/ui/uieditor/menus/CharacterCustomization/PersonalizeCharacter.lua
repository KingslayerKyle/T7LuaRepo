...
local PostLoadFunc = function ( f3_arg0, f3_arg1 )
	f3_arg0.personalizeList:clearNavigationTable()
	f3_arg0.optionsList:clearNavigationTable()
	f3_arg0.colorsList:clearNavigationTable()
	local f3_local0 = function ( f4_arg0 )
		if CoD.CCUtility.Heroes.selectionTable and CoD.CCUtility.PersonalizeHeroData.EdittingArea then
			...
			if CoD.CCUtility.PersonalizeHeroData.EdittingArea == Enum.CharacterItemType.CHARACTER_ITEM_TYPE_BODY then
				f4_local0 = CoD.CCUtility.Heroes.selectionTable.selectedBody + 1
			elseif CoD.CCUtility.PersonalizeHeroData.EdittingArea == Enum.CharacterItemType.CHARACTER_ITEM_TYPE_HELMET then
				f4_local0 = CoD.CCUtility.Heroes.selectionTable.selectedHelmet + 1
			end
			local f4_local1 = f4_arg0:findItem( nil, {
				optionIndex = f4_local0
			} )
			if f4_local1 then
				f4_arg0.savedActiveIndex = f4_local1.gridInfoTable.zeroBasedIndex
			end
		end
	end
	
	local f3_local1 = nil
	f3_arg0.updateMode = function ( f5_arg0, f5_arg1, f5_arg2 )
		...
		if f5_arg0 == CoD.CCUtility.PersonalizeHeroData.Modes.ExploringOptions then
			SendClientScriptNotify( f3_arg1, "camera_change", "exploring" )
			f3_arg0.reloadSelectionList()
			if f5_arg1 then
				f3_arg0.personalizeList.customizationList:updateDataSource()
				f3_arg0.personalizeList.customizationList:findItem( nil, {
					categorySize = CoD.CCUtility.Heroes.LargeCategory,
					customizationArea = f5_arg1
				}, true )
			end
			CoD.SwapFocusableElements( f3_arg1, f3_local1, f3_arg0.personalizeList )
			f3_local1 = f3_arg0.personalizeList
		...
		elseif f5_arg0 == CoD.CCUtility.PersonalizeHeroData.Modes.EdittingOption then
			if f5_arg2.customizationArea == Enum.CharacterItemType.CHARACTER_ITEM_TYPE_HELMET then
				SendClientScriptNotify( f3_arg1, "camera_change", "inspecting_helmet" )
			elseif f5_arg2.customizationArea == Enum.CharacterItemType.CHARACTER_ITEM_TYPE_BODY then
				SendClientScriptNotify( f3_arg1, "camera_change", "inspecting_body" )
			else
				SendClientScriptNotify( f3_arg1, "camera_change", "exploring" )
			end
			...
			f3_arg0.optionsList.optionsList:updateDataSource()
			f3_local0( f3_arg0.optionsList.optionsList )
			CoD.SwapFocusableElements( f3_arg1, f3_local1, f3_arg0.optionsList )
			f3_local1 = f3_arg0.optionsList
			f3_arg0:setState( "EdittingItem" )
		elseif f5_arg0 == CoD.CCUtility.PersonalizeHeroData.Modes.EdittingColor then
			...
			CoD.CCUtility.PersonalizeHeroData.EdittingArea = f5_arg2.customizationArea
			CoD.CCUtility.PersonalizeHeroData.EdittingElement = f5_arg2
			f3_arg0.colorsList.colorsList:updateDataSource()
			CoD.SwapFocusableElements( f3_arg1, f3_local1, f3_arg0.colorsList )
			f3_local1 = f3_arg0.colorsList
			f3_arg0:setState( "EdittingColor" )
		end
		f3_arg0.currentMode = f5_arg0
	end
	
	f3_arg0.updateMode( CoD.CCUtility.PersonalizeHeroData.Modes.ExploringOptions )
end
...
LUI.createMenu.PersonalizeCharacter = function ( controller )
	...
	optionsList:linkToElementModel( personalizeList.customizationList, "fullCategoryName", true, function ( model )
		...
		if fullCategoryName then
			optionsList.categoryName:setText( Engine.Localize( fullCategoryName ) )
		end
	end )
	colorsList:linkToElementModel( personalizeList.customizationList, "fullCategoryName", true, function ( model )
		local fullCategoryName = Engine.GetModelValue( model )
		if fullCategoryName then
			colorsList.categoryName:setText( Engine.Localize( fullCategoryName ) )
		end
	end )
	categoryInfoBlock:linkToElementModel( personalizeList.customizationList, "currentSelectionName", true, function ( model )
		...
		if currentSelectionName then
			categoryInfoBlock.currentlyEquipped:setText( Engine.Localize( currentSelectionName ) )
		end
	end )
	categoryInfoBlock:linkToElementModel( personalizeList.customizationList, "fullCategoryName", true, function ( model )
		local fullCategoryName = Engine.GetModelValue( model )
		if fullCategoryName then
			categoryInfoBlock.WeaponNameWidget0.weaponNameLabel:setText( Engine.Localize( fullCategoryName ) )
		end
	end )
	colorInfoBlock:linkToElementModel( personalizeList.customizationList, "currentSelectionName", true, function ( model )
		local currentSelectionName = Engine.GetModelValue( model )
		if currentSelectionName then
			colorInfoBlock.currentlyEquipped:setText( Engine.Localize( currentSelectionName ) )
		end
	end )
	colorInfoBlock:linkToElementModel( colorsList.colorsList, "colorIndex", true, function ( model )
		...
		if colorIndex then
			colorInfoBlock.WeaponNameWidget0.weaponNameLabel:setText( Engine.Localize( LocalizeIntoString( "HEROES_COLOR", colorIndex ) ) )
		end
	end )
	optionInfoBlock:linkToElementModel( optionsList.optionsList, "name", true, function ( model )
		local name = Engine.GetModelValue( model )
		if name then
			optionInfoBlock.WeaponNameWidget0.weaponNameLabel:setText( Engine.Localize( name ) )
		end
	end )
	...
	self.clipsPerState = {
		...
		EdittingItem = {
			...
			DefaultState = function ()
				...
				self.optionsList:setAlpha( 1 )
				optionsListFrame2( optionsList, {} )
				local f27_local2 = function ( f30_arg0, f30_arg1 )
					if not f30_arg1.interrupted then
						f30_arg0:beginAnimation( "keyframe", 70, false, true, CoD.TweenType.Linear )
					end
					f30_arg0:setAlpha( 1 )
				...
				categoryInfoBlock:setAlpha( 0 )
				categoryInfoBlock:registerEventHandler( "transition_complete_keyframe", f27_local2 )
				local optionInfoBlockFrame2 = function ( optionInfoBlock, event )
					if not event.interrupted then
		...
		EdittingColor = {
			...
			DefaultState = function ()
				...
				self.colorsList:setAlpha( 1 )
				colorsListFrame2( colorsList, {} )
				local f33_local2 = function ( f36_arg0, f36_arg1 )
					if not f36_arg1.interrupted then
						f36_arg0:beginAnimation( "keyframe", 129, false, true, CoD.TweenType.Linear )
					end
					f36_arg0:setAlpha( 1 )
				...
				categoryInfoBlock:beginAnimation( "keyframe", 29, false, false, CoD.TweenType.Linear )
				categoryInfoBlock:setAlpha( 0 )
				categoryInfoBlock:registerEventHandler( "transition_complete_keyframe", f33_local2 )
				local colorInfoBlockFrame2 = function ( colorInfoBlock, event )
					if not event.interrupted then
