...
local f0_local1 = function ( f2_arg0, f2_arg1, f2_arg2, f2_arg3 )
	...
	if f2_local4 and f2_local4 ~= "" then
		if LUI.startswith( f2_local4, "specialty" ) then
			f2_local3 = CoD.CACUtility.GetPerkXModel( Engine.GetClassItem( f2_arg0, f2_arg1, f2_local4 ) )
		elseif LUI.startswith( f2_local4, "bonuscard" ) then
			f2_local3 = CoD.CACUtility.GetBonuscardXModel( Engine.GetClassItem( f2_arg0, f2_arg1, f2_local4 ) )
		elseif f2_local4 == "cybercom" then
			f2_local3 = CoD.CACUtility.GetPerkXModel( Engine.GetClassItem( f2_arg0, f2_arg1, f2_arg3 ) )
		else
			f2_local3 = Engine.GetWeaponString( f2_arg0, f2_arg1, f2_local4 )
		end
	end
	if f2_local3 and f2_local3 ~= "" then
		Engine.SendClientScriptNotify( f2_arg0, "CustomClass_update", f2_local4, f2_local3, CoD.CACUtility.GetCameraNameForAttachments( f2_local3 ), CoD.CACUtility.GetWeaponOptionsFromClass( f2_arg0, f2_arg2, f2_local4 ), CoD.CACUtility.GetAttachmentCosmeticVariantsFromClass( f2_arg0, f2_arg2, f2_local4 ) )
	else
		Engine.SendClientScriptNotify( f2_arg0, "CustomClass_remove" )
	end
end
...
local PostLoadFunc = function ( f6_arg0, f6_arg1 )
	...
	f6_arg0.navigateTo = function ( f11_arg0, f11_arg1, f11_arg2 )
		...
		if f11_arg0.currentActiveSlot then
			...
			if f11_local0 and f11_local0.widget then
				...
				if f11_local0.parentWidget then
					...
					if f11_local0.parentWidget.id == "Wildcards" then
						f11_local0.parentWidget:setPriority( 0 )
						f11_arg0.feFooterContainer:setPriority( 1 )
					end
				end
			end
		end
		...
		if f11_local1 and f11_local1.widget then
			...
			if f11_local2.id == "Wildcards" then
				f11_arg0.feFooterContainer:setPriority( 0 )
				while f11_local2 ~= f11_arg0 do
	...
	if CoD.CACUtility.CustomClass_LastClassNum ~= f6_local1 then
...
LUI.createMenu.CustomClass = function ( controller )
	...
	CACBackground0:linkToElementModel( self, "customClassName", true, function ( model )
		...
		if customClassName then
			CACBackground0.cac3dTitleIntermediary0.FE3dTitleContainer0.MenuTitle.TextBox1.Label0:setText( Engine.Localize( customClassName ) )
		end
	end )
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "LastInput" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f40_local0, self, controller, Enum.LUIButton.LUI_KEY_PCKEY_0 )
	end )
	...
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBY_PSTRIANGLE, "C", function ( element, menu, controller, model )
		if not IsGamesComBuild() and not IsUserContentRestricted( controller ) and not ShouldHideForPCBeta( element, controller ) then
			NavigateToPersonalizeMenu( self, controller )
			PlaySoundSetSound( self, "cac_enter_wpn_cust_sub" )
			return true
		else
