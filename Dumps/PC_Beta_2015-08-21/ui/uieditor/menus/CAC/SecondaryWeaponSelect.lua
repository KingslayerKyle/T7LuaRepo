	...
	self.restoreState = function ( f2_arg0 )
		...
		if f2_local4 then
			...
			if f2_local6 then
				...
				f2_arg0.weaponCategoryList.grid:findItem( nil, {
					filter = f2_local5
				}, true, nil )
			...
			f2_arg0.weaponCategoryList.grid:setActiveItem( f2_arg0.weaponCategoryList.grid:getItemAt( 2 ) )
			f2_arg0.weaponList:updateDataSource( true )
...
local PostLoadFunc = function ( f3_arg0, f3_arg1 )
	f3_arg0.updateMode = function ( f4_arg0, f4_arg1 )
		...
		if f4_arg0.currentMode == CoD.WGSV_Category.WEAPON then
			f4_arg0.weaponCategoryList.m_disableNavigation = false
		...
		elseif f4_arg0.currentMode == CoD.WGSV_Category.GUNSMITH_VARIANT then
			f4_arg0.weaponCategoryList.m_disableNavigation = true
...
LUI.createMenu.SecondaryWeaponSelect = function ( controller )
	...
	self.clipsPerState = {
		...
		ShowVariants = {
			...
			DefaultState = function ()
				...
				self.LeftPanel:setTopBottom( true, false, 78, 667 )
				LeftPanelFrame2( LeftPanel, {} )
				local f44_local1 = function ( f46_arg0, f46_arg1 )
					if not f46_arg1.interrupted then
						f46_arg0:beginAnimation( "keyframe", 100, false, false, CoD.TweenType.Linear )
					end
					f46_arg0:setAlpha( 1 )
				...
				CategoryListPanel:beginAnimation( "keyframe", 100, false, false, CoD.TweenType.Linear )
				CategoryListPanel:setAlpha( 0 )
				CategoryListPanel:registerEventHandler( "transition_complete_keyframe", f44_local1 )
				local f44_local2 = function ( f47_arg0, f47_arg1 )
				...
				weaponList:beginAnimation( "keyframe", 100, false, false, CoD.TweenType.Linear )
				weaponList:setAlpha( 0 )
				weaponList:registerEventHandler( "transition_complete_keyframe", f44_local2 )
				local variantListFrame2 = function ( variantList, event )
					local variantListFrame3 = function ( variantList, event )
						if not event.interrupted then
							variantList:beginAnimation( "keyframe", 100, false, false, CoD.TweenType.Linear )
				...
				local f44_local4 = function ( f50_arg0, f50_arg1 )
					if not f50_arg1.interrupted then
						f50_arg0:beginAnimation( "keyframe", 100, false, false, CoD.TweenType.Linear )
					end
					f50_arg0:setAlpha( 1 )
				...
				weaponCategoryList:beginAnimation( "keyframe", 100, false, false, CoD.TweenType.Linear )
				weaponCategoryList:setAlpha( 0 )
				weaponCategoryList:registerEventHandler( "transition_complete_keyframe", f44_local4 )
				local BlackHeaderFrame2 = function ( BlackHeader, event )
					if not event.interrupted then
						BlackHeader:beginAnimation( "keyframe", 100, false, false, CoD.TweenType.Linear )
					end
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "WGSV_Category" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f53_local0, self, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE )
	end )
	...
	self:registerEventHandler( "menu_opened", function ( self, event )
		...
		if not f54_local0 then
			f54_local0 = self:dispatchEventToChildren( event )
		end
		return f54_local0
	end )
