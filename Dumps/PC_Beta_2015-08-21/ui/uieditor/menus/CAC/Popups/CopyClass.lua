...
local PostLoadFunc = function ( f1_arg0 )
	...
	f1_arg0:setModel( CoD.perController[f1_local0].classModel )
	local f1_local1 = f1_arg0:getModel()
	if f1_local1 then
		for f1_local2 = 1, #f1_arg0.customClassList.layoutItems, 1 do
			local f1_local5 = f1_arg0.customClassList.layoutItems[f1_local2][1]
			if f1_local5 and f1_local1 == f1_local5:getModel() then
				f1_local5:setState( "Disabled" )
				break
			end
		end
	end
	f1_arg0.customClassList:processEvent( {
		name = "gain_focus",
...
LUI.createMenu.CopyClass = function ( controller )
	...
	self:AddButtonCallbackFunction( customClassList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( element, menu, controller, model )
