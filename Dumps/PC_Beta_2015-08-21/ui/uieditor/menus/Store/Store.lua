...
local PreLoadFunc = function ( self, controller )
	local f1_local0 = Engine.CreateModel( Engine.GetGlobalModel(), "StoreRoot" )
	Engine.CreateModel( f1_local0, "ready" )
	Engine.CreateModel( f1_local0, "storeSource" )
	Engine.CreateModel( f1_local0, "storeLocation" )
	Engine.FetchStoreProductsByCategory( controller, "BETA", false )
...
LUI.createMenu.Store = function ( controller )
	...
	rightPane:mergeStateConditions( {
		{
			stateName = "Hide",
			condition = function ( menu, element, event )
				return IsBooleanDvarSet( "live_store_hidedesc" ) and IsDurango()
			end
		}
	} )
	...
	self:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, nil, function ( element, menu, controller, model )
		if IsSelfModelValueEqualTo( element, controller, "purchasestatus", 0 ) and IsDurango() and not HidePurchaseAndPrice() then
			PurchaseProduct( self, element, controller )
			return true
		elseif IsSelfModelValueEqualTo( element, controller, "purchasestatus", 1 ) and IsDurango() and not HidePurchaseAndPrice() then
			PurchaseProduct( self, element, controller )
			return true
		elseif HidePurchaseAndPrice() then
			PurchaseProduct( self, element, controller )
			return true
		else
