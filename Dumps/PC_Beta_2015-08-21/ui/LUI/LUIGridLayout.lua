...
LUI.GridLayout.setHorizontalCount = function ( f8_arg0, f8_arg1 )
	f8_arg0.hCount = f8_arg1
	f8_arg0:updateDataSource( nil, nil, true )
end

LUI.GridLayout.setVerticalCount = function ( f9_arg0, f9_arg1 )
	f9_arg0.vCount = f9_arg1
	f9_arg0:updateDataSource( nil, nil, true )
end
...
local f0_local5 = function ( f31_arg0, f31_arg1 )
	if f31_arg1 then
		...
		elseif f31_arg0.currentStartRow == 1 and not f31_arg0.loopEdges then
			f31_local0 = "AtTop"
		elseif f31_arg0.requestedRowCount < f31_arg0.currentStartRow + f31_arg0.vCount and not f31_arg0.loopEdges then
			f31_local0 = "AtBottom"
		end
		...
		if f31_arg1.count then
			...
			if f31_local2 or f31_local2.getCount then
...
local f0_local6 = function ( f32_arg0, f32_arg1 )
	if f32_arg1 then
		...
		elseif f32_arg0.currentStartColumn == 1 and not f32_arg0.loopEdges then
			f32_local0 = "AtLeft"
		elseif f32_arg0.requestedColumnCount < f32_arg0.currentStartColumn + f32_arg0.hCount and not f32_arg0.loopEdges then
			f32_local0 = "AtRight"
		end
		...
		if f32_arg1.count then
			...
			if f32_local2 or f32_local2.getCount then
...
LUI.GridLayout.updateLayout = function ( f64_arg0, f64_arg1 )
	...
	if 0 < f64_arg0.firstElementYOffset then
	...
	if 0 < f64_arg0.firstElementXOffset then
	...
	f64_local9 = function ( f66_arg0, f66_arg1, f66_arg2, f66_arg3, f66_arg4 )
		...
		elseif not f64_local1[f66_arg0.gridInfoTable.gridRowIndex] or not f64_local2[f66_arg0.gridInfoTable.gridColIndex] then
			return false
		elseif f66_arg1 <= f0_local0( f66_arg0 ) and f66_arg0.gridInfoTable.gridRowIndex < f66_arg1 + f64_arg0.vCount and f66_arg2 <= f0_local1( f66_arg0 ) and f66_arg0.gridInfoTable.gridColIndex < f66_arg2 + f64_arg0.hCount then
			return true
		elseif f66_arg1 + f64_arg0.vCount <= f66_arg0.gridInfoTable.gridRowIndex or f66_arg2 + f64_arg0.hCount <= f66_arg0.gridInfoTable.gridColIndex then
			return false
	...
	f64_local11 = 0
	f64_local12 = 0
	for f64_local15 = 1, f64_arg0.hCount, 1 do
	...
	for f64_local15 = 1, f64_arg0.vCount, 1 do
...
LUI.GridLayout.createWidgetForPosition = function ( f75_arg0, f75_arg1, f75_arg2, f75_arg3, f75_arg4 )
	local f75_local0 = f75_arg0:getDataSource()
	if not f75_local0 or not f75_local0.getCount then
		return nil
	end
	local f75_local1 = f75_local0.getCount( f75_arg0 )
	local f75_local2 = f75_arg0:GetTableIndexForPosition( f75_arg1, f75_arg2 )
	if f75_local2 <= f75_local1 then
		local f75_local3 = f75_arg0:getModelForIndex( f75_local2 )
		local f75_local4 = nil
		if f75_local0.getWidgetTypeForItem then
			f75_local4 = f75_local0.getWidgetTypeForItem( f75_arg0, f75_local3, f75_arg1 )
		end
		local f75_local5 = f75_arg0:createWidgetFromDataSource( f75_arg0.controller, f75_local3, f75_local4 )
		if f75_local5 ~= nil then
			if f75_local3 then
				f75_local5:setModel( f75_local3 )
			end
			f75_arg0:setItemAtPosition( f75_local5, f75_arg1, f75_arg2, false, f75_arg3, f75_arg4 )
			if f75_local0.getCustomPropertiesForItem then
				local f75_local6 = f75_local0.getCustomPropertiesForItem( f75_arg0, f75_local2 )
				if f75_local6 then
					for f75_local10, f75_local11 in pairs( f75_local6 ) do
						if f75_local10 == "id" then
							f0_local9( f75_arg0.itemStencil, f75_local5, f75_local11 )
						else
							f75_local5[f75_local10] = f75_local11
						end
					end
				end
			end
			if f75_local3 then
				local f75_local6 = Engine.GetModel( f75_local3, "customId" )
				if f75_local6 then
					f0_local9( f75_arg0.itemStencil, f75_local5, Engine.GetModelValue( f75_local6 ) )
				end
			end
...
LUI.GridLayout.updateDataSource = function ( f76_arg0, f76_arg1, f76_arg2, f76_arg3 )
	local f76_local0 = f76_arg0:getDataSource()
	if not f76_local0 or not f76_local0.getCount or not f76_local0.getItem then
		return 
	elseif not f76_arg0.widgetType then
		return 
	elseif f76_local0.prepare and (not f76_arg0.prepared or not f76_arg3) then
		f76_arg0.prepared = true
		f76_local0.prepare( f76_arg0.controller, f76_arg0, f76_arg0.filter )
	end
	if f76_arg0.hCount > 1 and f76_arg0.vCount > 1 then
		local f76_local1 = f76_local0.getCount( f76_arg0 )
		f76_arg0.requestedColumnCount = f76_arg0.hCount
		f76_arg0.requestedRowCount = math.floor( math.max( 0, f76_local1 - 1 ) / f76_arg0.hCount ) + 1
	elseif f76_arg0.vCount > 1 then
		f76_arg0.requestedRowCount = f76_local0.getCount( f76_arg0 )
		f76_arg0.requestedColumnCount = 1
	elseif f76_arg0.hCount >= 1 then
		local f76_local1 = f76_local0.getCount( f76_arg0 )
		f76_arg0.requestedRowCount = 1
		f76_arg0.requestedColumnCount = f76_local1
	else
		return 
	end
	if not f76_arg2 then
		f76_arg0:clearLayout( true, false )
	end
	f76_arg0:processEvent( {
		name = "update_state",
		menu = f76_arg0.menu
	} )
	f76_arg0.activeWidget = nil
	local f76_local1, f76_local2, f76_local3 = nil
	for f76_local4 = 1, f76_arg0.requestedRowCount, 1 do
		for f76_local7 = 1, f76_arg0.requestedColumnCount, 1 do
			local f76_local10 = f76_arg0:getPropertiesForPosition( f76_local4, f76_local7 )
			if f76_local10 and f76_local10.selectIndex then
				f76_local10.selectIndex = false
				f76_local2 = f76_local4
	...
	if f76_local2 then
		f76_local1 = f76_arg0:getItemAtPosition( f76_local2, f76_local3, true )
	end
	if not f76_local1 and f76_arg0.savedActiveIndex and not f76_arg1 then
		local f76_local4, f76_local5 = f76_arg0:getRowAndColumnForIndex( f76_arg0.savedActiveIndex )
		f76_local1 = f76_arg0:getItemAtPosition( f76_local4, f76_local5, true )
	end
	if not f76_local1 and (f76_arg0.hasListFocus or f76_arg0.setActiveOnUpdate) and not f76_arg1 then
		f76_local1 = f76_arg0:getFirstSelectableItem()
	end
	if f76_local1 then
		f76_local4 = nil
		if f76_local2 ~= nil then
			f76_local4 = 0
		end
		f76_arg0:setActiveItem( f76_local1, f76_local4 )
	end
...
LUI.GridLayout.close = function ( f77_arg0 )
	f77_arg0:unsubscribeFromAllModels()
	f77_arg0:clearLayout( true, false )
	local f77_local0 = f77_arg0:getDataSource()
	LUI.GridLayout.super.close( f77_arg0 )
	if f77_local0 and f77_local0.cleanup then
		f77_local0.cleanup( f77_arg0 )
	end
end

LUI.GridLayout.updateState = function ( f78_arg0, f78_arg1 )
	LUI.GridLayout.super.updateState( f78_arg0, f78_arg1 )
	f78_arg0:updateLayout( 0 )
end
