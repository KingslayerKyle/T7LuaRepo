...
LUI.createMenu.HUD = function ( f12_arg0 )
	...
	if CoD.isWIIU then
		f12_local0:registerEventHandler( "occlusion_change", HUD_OcclusionChange )
	end
	if CoD.isPC then
...
function HUD_OcclusionChange( f18_arg0, f18_arg1 )
	CoD.Menu.OcclusionChange( f18_arg0, f18_arg1 )
end

f0_local1 = function ( f19_arg0 )
	HUD_SetupEventHandlers_Common( f19_arg0 )
	if CoD.isZombie == false then
		HUD_SetupEventHandlers_Multiplayer( f19_arg0 )
	end
end

function UpdateHUDVisibility( f20_arg0, f20_arg1 )
	if Engine.IsVisibilityBitSet( f20_arg1.controller, Enum.UIVisibilityBit.BIT_CAMERA_ANIM_HIDING_HUD ) then
		if f20_arg0.T7HudMenu then
			f20_arg0.T7HudMenu:setAlpha( 0 )
		end
		if f20_arg0.T7HudMenuGameMode then
			f20_arg0.T7HudMenuGameMode:setAlpha( 0 )
		end
	else
		if f20_arg0.T7HudMenu then
			f20_arg0.T7HudMenu:setAlpha( 1 )
		end
		if f20_arg0.T7HudMenuGameMode then
			f20_arg0.T7HudMenuGameMode:setAlpha( 1 )
		end
	end
end

function HUD_Hide( f21_arg0, f21_arg1 )
	f21_arg0:setAlpha( 0 )
end

function HUD_Show( f22_arg0, f22_arg1 )
	f22_arg0:setAlpha( 1 )
end

f0_local18 = function ( f23_arg0, f23_arg1 )
	local f23_local0 = LUI.roots
	local f23_local1 = "UIRoot"
	if f23_local0["UIRoot" .. f23_arg1.controller].ingameFriendsList then
		...
		f23_arg0:openPopup( "FriendsList", f23_arg1.controller )
	end
end

f0_local19 = function ( f24_arg0, f24_arg1 )
	Engine.SetActiveMenu( f24_arg1.controller, CoD.UIMENU_NONE )
	f24_arg0:processEvent( {
		name = "close_scoreboard_menu",
		controller = f24_arg0:getOwner()
	} )
end
...
function HUD_SetupEventHandlers_Common( f25_arg0 )
	...
	f25_arg0:subscribeToModel( Engine.GetModel( Engine.GetModelForController( f25_arg0.controller ), "scriptNotify" ), function ( model )
		...
		elseif modelValue == "force_scoreboard" then
			if Engine.IsDemoPlaying() == false or CoD.isZombie == false then
				Engine.SetModelValue( Engine.CreateModel( Engine.GetModelForController( f25_arg0.controller ), "forceScoreboard" ), 1 )
			end
	...
	if Engine.GameModeIsMode( CoD.GAMEMODE_LOCAL_SPLITSCREEN ) == false then
		f25_arg0:registerEventHandler( "activate_demo_information_screen", CoD.DemoHUD.ActivateInformationScreen )
		f25_arg0:registerEventHandler( "open_lightman_marker_options", CoD.DemoPopup.OpenLightmanMarkerOptionsPopup )
		f25_arg0:registerEventHandler( "open_demo_manage_segments", HUD_OpenManageSegments )
	end
	CoD.DemoUtility.RegisterEventHandlers( f25_arg0, f25_local0 )
end

function HUD_SetupEventHandlers_Multiplayer( f32_arg0 )
	f32_arg0:registerEventHandler( "hud_force_kill_killstreak_hud", f0_local3 )
	local f32_local0 = f32_arg0:getOwner()
	local f32_local1 = function ( f33_arg0 )
		HUD_UpdateRefresh( f32_arg0, {
			controller = f32_local0
		} )
	end
	
	local f32_local2 = Engine.GetModelForController( f32_local0 )
	f32_arg0:subscribeToModel( Engine.GetModel( f32_local2, "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_SPECTATING_CLIENT ), f32_local1 )
	f32_arg0:subscribeToModel( Engine.GetModel( f32_local2, "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_TEAM_SPECTATOR ), f32_local1 )
	f32_arg0:subscribeToModel( Engine.GetModel( f32_local2, "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_UI_ACTIVE ), f32_local1 )
	f32_arg0:subscribeToModel( Engine.GetModel( f32_local2, "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_GAME_ENDED ), f32_local1 )
	f32_arg0:subscribeToModel( Engine.GetModel( f32_local2, "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_IN_KILLCAM ), f32_local1 )
	f32_arg0:subscribeToModel( Engine.GetModel( f32_local2, "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_FINAL_KILLCAM ), f32_local1 )
	f32_arg0:subscribeToModel( Engine.GetModel( f32_local2, "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_ROUND_END_KILLCAM ), f32_local1 )
	f32_arg0:subscribeToModel( Engine.GetModel( f32_local2, "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_MIGRATING_HOST ), f32_local1 )
	f32_arg0:subscribeToModel( Engine.GetModel( f32_local2, "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_SELECTING_LOCATION ), function ( model )
		f0_local4( f32_arg0, {
			controller = f32_local0
		} )
	end )
	if Engine.GetModel( f32_local2, "team" ) == nil then
		Engine.SetModelValue( Engine.CreateModel( f32_local2, "team" ), 0 )
	end
	if CoD.isPC then
		f32_arg0:registerEventHandler( "chooseclass_hotkey", HUD_Handle_ChooseClass_HotKey )
	end
	local f32_local3 = Engine.CreateModel( Engine.GetModelForController( f32_arg0:getOwner() ), "forceScoreboard" )
	Engine.SetModelValue( f32_local3, 0 )
	f32_arg0:subscribeToModel( f32_local3, function ( model )
		if Engine.GetModelValue( model ) > 0 then
			f32_arg0:processEvent( {
				name = "open_scoreboard_menu",
				controller = f32_arg0:getOwner()
			} )
		else
			f32_arg0:processEvent( {
				name = "close_scoreboard_menu",
...
function HUD_FirstSnapshot( f36_arg0, f36_arg1 )
	...
	f36_arg0.bootOnOpen = false
	if f36_arg0.T7HudMenu ~= nil then
		if f36_arg1.debugReload then
			CoD.Menu.DebugCloseMenu( f36_arg0.T7HudMenu )
			f36_arg0.T7HudMenu = nil
		else
			f36_arg0.bootOnOpen = f36_arg0.T7HudMenu.hasBooted
		end
	end
	if f36_arg0.T7HudMenuGameMode ~= nil and f36_arg1.debugReload then
		CoD.Menu.DebugCloseMenu( f36_arg0.T7HudMenuGameMode )
		f36_arg0.T7HudMenuGameMode = nil
	end
	for f36_local3, f36_local4 in ipairs( LUI.ShallowCopy( f36_arg0.forceCloseChildren ) ) do
		f36_local4:close()
	end
	f36_arg0.forceCloseChildren = {}
	f36_arg0.loadingMenu = nil
	f36_arg0:removeAllChildren()
	f36_arg0:setOwner( f36_arg1.controller )
	f36_arg0.controller = f36_arg1.controller
	HUD_FirstSnapshot_Common( f36_arg0, f36_arg1 )
	if CoD.isZombie == false then
		HUD_FirstSnapshot_Multiplayer( f36_arg0, f36_arg1 )
		if CoD.isCampaign == true then
			HUD_FirstSnapshot_Campaign( f36_arg0, f36_arg1 )
		end
	else
		...
		Engine.SetModelValue( Engine.CreateModel( Engine.GetModelForController( f36_arg1.controller ), "forceScoreboard" ), 0 )
		HUD_FirstSnapshot_Zombie( f36_arg0, f36_arg1 )
	end
	Engine.ForceHUDRefresh( f36_arg1.controller )
	HUD_UpdateRefresh( f36_arg0, f36_arg1 )
	f36_arg0.firstSnapshotProcessed = true
	if f36_arg0.pendingInGameMenuEvents then
		for f36_local0 = 1, #f36_arg0.pendingInGameMenuEvents, 1 do
			f0_local0( f36_arg0, f36_arg0.pendingInGameMenuEvents[f36_local0] )
		end
		f36_arg0.pendingInGameMenuEvents = nil
	end
	return true
end

function HUD_StartMigration( f37_arg0, f37_arg1 )
	f37_arg0:removeAllChildren()
	f37_arg0:addElement( LUI.createMenu.migration_ingame( f37_arg1.controller, f37_arg0 ) )
end

function HUD_FirstSnapshot_Common( f38_arg0, f38_arg1 )
	if not CoD.isMultiplayer then
		f38_arg0.cinematicSubtitles = CoD.MovieSubtitles.new( f38_arg0, f38_arg1.controller )
		f38_arg0.cinematicSubtitles:setLeftRight( false, false, -640, 640 )
		f38_arg0.cinematicSubtitles:setTopBottom( false, false, -360, 360 )
		f38_arg0:addElement( f38_arg0.cinematicSubtitles )
	end
	local f38_local0 = f38_arg1.controller
	HUD_AddHintTextElements( f38_arg0 )
	local f38_local1 = CoD.Menu.NewSafeAreaFromState( "hud_safearea", f38_arg1.controller )
	f38_local1:setPriority( 100 )
	f38_arg0:addElement( f38_local1 )
	if f38_arg0.safeArea and f38_arg0.safeArea.buttonModel then
		Engine.UnsubscribeAndFreeModel( f38_arg0.safeArea.buttonModel )
		f38_arg0.safeArea:close()
	end
	f38_arg0.safeArea = f38_local1
	f38_arg0.safeArea.buttonModel = Engine.CreateModel( Engine.GetModelForController( f38_local0 ), "HUD.buttonPrompts" )
	local f38_local2 = CoD.GrenadeEffect.new( f38_local0 )
	f38_local2:setLeftRight( true, true, 0, 0 )
	f38_local2:setTopBottom( true, true, 0, 0 )
	f38_arg0:addForceClosedSafeAreaChild( f38_local2 )
	if CoD.isZombie == true then
		CoD.Zombie.SoloQuestMode = false
		local f38_local3 = Engine.GetLobbyClientCount( Enum.LobbyType.LOBBY_TYPE_GAME )
		if f38_local3 == 1 and (CoD.isOnlineGame() == false or Engine.GameModeIsMode( CoD.GAMEMODE_PRIVATE_MATCH ) == false) then
			CoD.Zombie.SoloQuestMode = true
		end
		if Engine.GameModeIsMode( CoD.GAMEMODE_LOCAL_SPLITSCREEN ) == true and f38_local3 > 2 then
			CoD.Zombie.LocalSplitscreenMultiplePlayers = true
		end
	end
	HUD_CloseScoreBoard( f38_arg0, f38_arg1 )
	if Dvar.ui_gametype:get() ~= "fr" and not CoD.ShowNewScoreboard() then
		f38_arg0.scoreBoard = LUI.createMenu.Scoreboard( f38_arg1.controller )
		f38_arg0.scoreboardUpdateTimer = LUI.UITimer.new( 1000, {
...
function HUD_AddHintTextElements( f39_arg0 )
	...
	local f39_local1 = LUI.UIElement.new()
	f39_local1:setLeftRight( true, true, 0, 0 )
	f39_local1:setTopBottom( false, false, 180, 180 + CoD.textSize.Default )
	f39_arg0:addElement( f39_local1 )
	f39_local1:setupCursorHint( 64 )
	local f39_local2 = LUI.UIElement.new()
	f39_local2:setLeftRight( true, true, 0, 0 )
	f39_local2:setTopBottom( true, true, 0, 0 )
	f39_local2:setImage( RegisterMaterial( "directional_damage_feedback" ) )
	f39_arg0:addElement( f39_local2 )
	f39_local2:setupDirectionalHitIndicator()
	local f39_local3 = LUI.UIElement.new()
	f39_local3:setLeftRight( true, true, 0, 0 )
	f39_local3:setTopBottom( false, true, -180, -180 + CoD.textSize.Default )
	f39_arg0:addElement( f39_local3 )
	f39_local3:setupMantleHint()
	local f39_local4 = LUI.UIElement.new()
	f39_local4:setLeftRight( true, true, 0, 0 )
	f39_local4:setTopBottom( false, true, -320, -320 + CoD.textSize.ExtraSmall )
	f39_arg0:addElement( f39_local4 )
	f39_local4:setupPlayerStance()
	local f39_local5 = LUI.UIElement.new()
	f39_local5:setLeftRight( true, true, 0, 0 )
	f39_local5:setTopBottom( true, false, 180, 180 + CoD.textSize.Default )
	f39_arg0:addElement( f39_local5 )
	f39_local5:setupHoldBreathHint()
	local f39_local6 = LUI.UIElement.new()
	f39_local6:setLeftRight( true, true, 0, 0 )
	f39_local6:setTopBottom( true, false, 140, 140 + CoD.textSize.Default )
	f39_arg0:addElement( f39_local6 )
	f39_local6:setupHoldBreathZoomHint()
	local f39_local7 = LUI.UIElement.new()
	f39_local7:setLeftRight( true, true, 0, 0 )
	f39_local7:setTopBottom( true, false, 180, 180 + CoD.textSize.Default )
	f39_arg0:addElement( f39_local7 )
	f39_local7:setupCancelChargeShotHint()
	local f39_local8 = LUI.UIElement.new()
	f39_local8:setLeftRight( true, true, 0, 0 )
	f39_local8:setTopBottom( false, false, 90, 90 + CoD.textSize.Default )
	f39_arg0:addElement( f39_local8 )
	f39_local8:setupMeleeChargeHint()
	local f39_local9 = LUI.UIElement.new()
	f39_local9:setLeftRight( true, true, 0, 0 )
	f39_local9:setTopBottom( false, false, 90, 90 + CoD.textSize.Default )
	f39_arg0:addElement( f39_local9 )
	f39_local9:setupMeleeCounterHint()
	local f39_local10 = LUI.UIElement.new()
	f39_local10:setLeftRight( true, true, 0, 0 )
	f39_local10:setTopBottom( false, false, 45, 45 + CoD.textSize.ExtraSmall )
	f39_arg0:addElement( f39_local10 )
	f39_local10:setupInvalidCmdHint()
	local f39_local11 = LUI.UIElement.new()
	f39_local11:setLeftRight( true, true, 0, 0 )
	f39_local11:setTopBottom( true, true, 0, 0 )
	f39_local11:setImage( RegisterMaterial( "compassping_enemy_diamond_bottom" ) )
	f39_local11:setRGB( 1, 0, 0 )
	f39_arg0:addElement( f39_local11 )
	f39_local11:setupTargetHighlights_Enemy()
	local f39_local12 = LUI.UIElement.new()
	f39_local12:setLeftRight( true, true, 0, 0 )
	f39_local12:setTopBottom( true, true, 0, 0 )
	f39_local12:setImage( RegisterMaterial( "compassping_enemysatellite_diamond" ) )
	f39_local12:setRGB( 1, 0, 0 )
	f39_arg0:addElement( f39_local12 )
	f39_local12:setupPlayerTargetHighlightsLineOfSight_Enemy()
	local f39_local13 = LUI.UIElement.new()
	f39_local13:setLeftRight( true, true, 0, 0 )
	f39_local13:setTopBottom( true, true, 0, 0 )
	f39_local13:setImage( RegisterMaterial( "compassping_enemy_diamond_bottom" ) )
	f39_local13:setRGB( 0, 1, 0 )
	f39_arg0:addElement( f39_local13 )
	f39_local13:setupTargetHighlights_Friendly()
	local f39_local14 = LUI.UIElement.new()
	f39_local14:setLeftRight( true, true, 0, 0 )
	f39_local14:setTopBottom( true, true, 0, 0 )
	f39_local14:setImage( RegisterMaterial( "compassping_enemysatellite_diamond" ) )
	f39_local14:setRGB( 0, 1, 0 )
	f39_arg0:addElement( f39_local14 )
	f39_local14:setupTargetHighlights_Friendly()
	local f39_local15 = LUI.UIElement.new()
	f39_local15:setLeftRight( true, true, 0, 0 )
	f39_local15:setTopBottom( true, true, 0, 0 )
	f39_local15:setImage( RegisterMaterial( "compassping_enemysatellite_diamond" ) )
	f39_local15:setRGB( 1, 0, 0 )
	f39_arg0:addElement( f39_local15 )
	f39_local15:setupPlayerPivotTargetting()
	local f39_local16 = LUI.UIElement.new()
	f39_local16:setLeftRight( true, true, 0, 0 )
	f39_local16:setTopBottom( true, true, 0, 0 )
	f39_local16:setImage( RegisterMaterial( "compassping_enemysatellite_diamond" ) )
	f39_local16:setRGB( 1, 0, 0 )
	f39_arg0:addElement( f39_local16 )
	f39_local16:setupPlayerGrappleTargetting()
	local f39_local17 = LUI.UIElement.new()
	f39_local17:setLeftRight( true, true, 0, 0 )
	f39_local17:setTopBottom( true, true, 0, 0 )
	f39_local17:setImage( RegisterMaterial( "compassping_enemysatellite_diamond" ) )
	f39_local17:setRGB( 1, 0, 0 )
	f39_arg0:addElement( f39_local17 )
	f39_local17:setupLockonHighlights()
	if CoD.isZombie == true or CoD.isCampaign == true then
		local f39_local18 = LUI.UIElement.new()
		f39_local18:setLeftRight( true, true, 0, 0 )
		f39_local18:setTopBottom( true, true, 0, 0 )
		f39_arg0:addElement( f39_local18 )
		f39_local18:setupAmITalking()
	end
	if CoD.isZombie == true then
		local f39_local18 = LUI.UIElement.new()
		f39_local18:setLeftRight( true, true, 0, 0 )
		f39_local18:setTopBottom( true, true, 0, 0 )
		f39_arg0:addElement( f39_local18 )
		f39_local18:setupFriendlyArrows()
	end
end

f0_local20 = function ( f40_arg0, f40_arg1 )
	if Engine.IsDemoPlaying() then
		return 
	else
		local f40_local0 = Engine.GetModel( Engine.GetModelForController( f40_arg1 ), "team" )
		local f40_local1 = function ( f41_arg0 )
			if Engine.GetModelValue( f41_arg0 ) == Enum.team_t.TEAM_SPECTATOR then
				if f40_arg0.safeArea and f40_arg0.safeArea.CodCaster == nil then
					f40_arg0.safeArea.CodCaster = CoD.CodCaster.new( f40_arg0.safeArea, f40_arg1 )
					f40_arg0.safeArea.CodCaster:setLeftRight( true, true, 0, 0 )
					f40_arg0.safeArea.CodCaster:setTopBottom( true, true, 0, 0 )
					f40_arg0:addForceClosedSafeAreaChild( f40_arg0.safeArea.CodCaster )
					f40_arg0.safeArea.CodCaster:processEvent( {
						name = "gain_focus",
						controller = f40_arg1
					} )
					f40_arg0.safeArea.CodCaster:gainFocus( {
						controller = f40_arg1
					} )
					if f40_arg0.safeArea.PostCodCasterLoad then
						f40_arg0.safeArea.PostCodCasterLoad( f40_arg0.safeArea.CodCaster )
						f40_arg0.safeArea.PostCodCasterLoad = nil
					end
					LUI.OverrideFunction_CallOriginalSecond( f40_arg0.safeArea.CodCaster, "close", function ( element )
						CoD.Menu.UnsubscribeFromControllerSubscriptionsForElement( f40_arg0.safeArea, element )
					end )
					UpdateState( f40_arg0.safeArea.CodCaster )
				end
			elseif f40_arg0.safeArea and f40_arg0.safeArea.CodCaster ~= nil then
				f40_arg0.safeArea.CodCaster:close()
				f40_arg0.safeArea.CodCaster = nil
			end
		end
		
		if f40_arg0.hasTeamModelSubscription then
			f40_local1( f40_local0 )
			return 
		else
			f40_arg0.hasTeamModelSubscription = true
			f40_arg0:subscribeToModel( f40_local0, f40_local1 )
		end
	end
end

f0_local21 = function ( f43_arg0, f43_arg1 )
	if Engine.IsDemoPlaying() then
		if f43_arg0.safeArea and f43_arg0.safeArea.demoHud == nil then
			f43_arg0.safeArea.Demo = CoD.Demo.new( f43_arg0.safeArea, f43_arg1 )
			f43_arg0.safeArea.Demo:setLeftRight( true, true, 0, 0 )
			f43_arg0.safeArea.Demo:setTopBottom( true, true, 0, 0 )
			f43_arg0:addForceClosedSafeAreaChild( f43_arg0.safeArea.Demo )
			f43_arg0.safeArea.Demo:processEvent( {
				name = "gain_focus",
				controller = f43_arg1
			} )
			f43_arg0.safeArea.Demo:gainFocus( {
				controller = f43_arg1
			} )
			LUI.OverrideFunction_CallOriginalSecond( f43_arg0.safeArea.Demo, "close", function ( element )
				CoD.Menu.UnsubscribeFromControllerSubscriptionsForElement( f43_arg0.safeArea, element )
			end )
			UpdateState( f43_arg0.safeArea.Demo )
		end
	elseif f43_arg0.safeArea and f43_arg0.safeArea.Demo ~= nil then
		f43_arg0.safeArea.Demo:close()
		f43_arg0.safeArea.Demo = nil
	end
end

function HUD_FirstSnapshot_Multiplayer( f45_arg0, f45_arg1 )
	if not Engine.IsSplitscreen() then
		CoD.GameMessages.AddObituaryWindow( f45_arg0, {
			leftAnchor = true,
			rightAnchor = false,
			left = 13,
			right = 277,
	...
	CoD.GameMessages.BoldGameMessagesWindow( f45_arg0, {
		leftAnchor = false,
		rightAnchor = false,
		left = 0,
		right = 0,
	...
	if Engine.GameModeIsMode( CoD.GAMEMODE_LOCAL_SPLITSCREEN ) == false then
		CoD.LiveStream.AddInGameStatusWidget( f45_arg0, f45_arg1.controller, {
			leftAnchor = false,
			rightAnchor = true,
			left = -200,
			right = 0,
		...
		CoD.DemoHUD.AddHUDWidgets( f45_arg0, f45_arg1 )
	end
	CoD.CodCasterUtility.SetupUIModels( f45_arg1.controller )
	f0_local10()
	local f45_local0 = Engine.GetModelForController( f45_arg1.controller )
	if f45_local0 then
		f45_local0 = Engine.GetModel( f45_local0, "hudItems.playerSpawned" )
	end
	local f45_local1 = f45_local0 and Engine.GetModelValue( f45_local0 )
	if f45_local1 then
		Engine.SetModelValue( f45_local0, false )
	end
	if f45_arg0.T7HudMenu then
		f45_arg0.T7HudMenu.SafeAreaContainerBack.Notifications.NotificationQueueWidget:clearNotificationQueue()
		f45_arg0.T7HudMenu.hasBooted = false
		f45_arg0.T7HudMenu:setAlpha( 1 )
	elseif Dvar.ui_gametype:get() ~= "doa" and Dvar.ui_gametype:get() ~= "codfu" then
		local f45_local2 = nil
		f45_local2 = LUI.createMenu.T7Hud( f45_arg1.controller )
		if f45_arg0.bootOnOpen then
			f45_arg0.bootOnOpen = false
			TryBootHUD( f45_local2, 0 )
		end
		f45_arg0.T7HudMenu = f45_local2
	end
	if f45_arg0.T7HudMenu then
		f45_arg0:addElement( f45_arg0.T7HudMenu )
	end
	local f45_local2 = nil
	if CoD.isCampaign == true then
		f45_local2 = "CP"
	...
	if not f45_arg0.T7HudMenuGameMode then
		local f45_local3 = nil
		f45_local3 = LUI.createMenu["T7Hud_" .. f45_local2]( f45_arg1.controller )
		if f45_local2 == "CP" then
			f45_local3:sizeToSafeArea()
			if not f45_arg0.InGameTalkersWidget then
				local f45_local4 = CoD.InGameTalkersWidget.new( f45_local3, f45_arg1.controller )
				if IsCPAndInSafehouse() then
					f45_local4:setLeftRight( false, true, -213, -23 )
					f45_local4:setTopBottom( false, true, -123, -36 )
				else
					f45_local4:setLeftRight( true, false, 64, 254 )
					f45_local4:setTopBottom( true, false, 36, 123 )
				end
				f45_arg0:addElement( f45_local4 )
				f45_arg0.InGameTalkersWidget = f45_local4
			end
		end
		if f45_local1 then
			Engine.SetModelValue( f45_local0, true )
		end
		f45_arg0.T7HudMenuGameMode = f45_local3
	else
		f45_arg0.T7HudMenuGameMode:setAlpha( 1 )
		if f45_arg0.T7HudMenuGameMode.Outcome then
			f45_arg0.T7HudMenuGameMode.Outcome:setAlpha( 0 )
		end
		if f45_local1 then
			Engine.SetModelValue( f45_local0, true )
		end
	end
	f45_arg0:addElement( f45_arg0.T7HudMenuGameMode )
	f45_arg0:setAlpha( 1 )
	f0_local20( f45_arg0, f45_arg1.controller )
	f0_local21( f45_arg0, f45_arg1.controller )
	LuaUtils.MessageDialogForceSubscriptionFire()
end

function HUD_FirstSnapshot_Campaign( f46_arg0, f46_arg1 )
	CoD.GameMessages.AddSubtitlesWindow( f46_arg0, {
		leftAnchor = true,
		rightAnchor = true,
		left = 0,
		right = 0,
...
function HUD_FirstSnapshot_Zombie( menu, controller )
	...
	if menu.T7HudMenuGameMode == nil then
		...
		local f47_local2 = LUI.createMenu
		local f47_local3
		if powerupsArea == "zm_zod" then
			...
			if not f47_local3 then
				...
				f47_local2 = f47_local2[f47_local3]
				if f47_local2 then
					menu.T7HudMenuGameMode = f47_local2( controller.controller )
				else
					menu.T7HudMenuGameMode = LUI.createMenu.T7Hud_zm_factory( controller.controller )
				end
			end
		end
		f47_local3 = "T7Hud_" .. powerupsArea
	else
		menu.T7HudMenuGameMode:setAlpha( 1 )
		if playerSpawnedValue then
	...
	f0_local21( menu, controller.controller )
end

function HUD_ToggleZombieHudContainer( f49_arg0, f49_arg1 )
	if f49_arg1.newValue == 0 then
		f49_arg0:beginAnimation( "fade_in", 500 )
		f49_arg0:setAlpha( 1 )
	else
		f49_arg0:beginAnimation( "fade_out", 500 )
		f49_arg0:setAlpha( 0 )
	end
end

function HasAnyChildWithID( f50_arg0, f50_arg1 )
	local f50_local0 = f50_arg0:getFirstChild()
	while f50_local0 ~= nil do
		if f50_local0.id == f50_arg1 then
			return true
		end
		f50_local0 = f50_local0:getNextSibling()
	end
	return false
end

f0_local0 = function ( f51_arg0, f51_arg1 )
	if not f51_arg0.firstSnapshotProcessed then
		if not f51_arg0.pendingInGameMenuEvents then
			f51_arg0.pendingInGameMenuEvents = {}
		end
		table.insert( f51_arg0.pendingInGameMenuEvents, f51_arg1 )
		return 
	elseif f51_arg1.menuName == "StartMenu_Main" then
		if Engine.IsMigrating( f51_arg1.controller ) == true then
			return 
		end
		local f51_local0 = f51_arg0:getParent()
		if f51_local0 ~= nil and HasAnyChildWithID( f51_local0, "Menu.StartMenu_Main" ) then
			return 
		end
	end
	if Engine.CanPauseGame() then
		...
		CoD.InGameMenu.m_unpauseDisabled[f51_arg1.controller + 1] = 0
		if f51_arg1.unpausable ~= nil and f51_arg1.unpausable == 0 then
			CoD.InGameMenu.m_unpauseDisabled[f51_arg1.controller + 1] = 1
		end
	end
	if f51_arg1.data ~= nil then
		f51_arg1.menuName = Engine.GetIString( f51_arg1.data[1], "CS_SCRIPT_MENUS" )
	end
	local f51_local0 = LUI.createMenu[f51_arg1.menuName]
	local f51_local1 = nil
	CoD.Menu.ModelToUse = f51_arg1.model
	if f51_local0 ~= nil then
		f0_local8 = true
		if f51_arg1.menuName == "ChooseClass_InGame" then
			f51_arg0:dispatchEventToRoot( {
				name = "close_menu",
				menuName = f51_arg1.menuName,
				controller = f51_arg1.controller
			} )
		end
		if f51_arg1.isMenu or LUI.hudMenuType[f51_arg1.menuName] == "hud" then
			f51_local1 = f51_arg0:openMenu( f51_arg1.menuName, f51_arg1.controller )
		else
			f51_local1 = f51_arg0:openPopup( f51_arg1.menuName, f51_arg1.controller )
		end
	else
		f51_local1 = CoD.OverlayUtility.CreateOverlay( f51_arg1.controller, f51_arg0, f51_arg1.menuName )
		if f51_local1 then
			f0_local8 = true
		end
	end
	if f51_local1 and f51_local1:getModel() then
		Engine.SetModelValue( Engine.CreateModel( f51_local1:getModel(), "close_current_menu" ), 0 )
		f51_local1:subscribeToCloseMenuModel()
		f51_local1:subscribeToCurrAnimationModel()
	end
	CoD.Menu.ModelToUse = nil
	return f51_local1
end

function HUD_CloseSpecificInGameMenu( f52_arg0, f52_arg1 )
	if f52_arg1.data ~= nil then
		if f52_arg0.pendingInGameMenuEvents then
			for f52_local0 = #f52_arg0.pendingInGameMenuEvents, 1, -1 do
				if f52_arg0.pendingInGameMenuEvents[f52_local0].controller == f52_arg1.controller and (not (not f52_arg0.pendingInGameMenuEvents[f52_local0].data or f52_arg0.pendingInGameMenuEvents[f52_local0].data[1] ~= f52_arg1.data[1]) or f52_arg0.pendingInGameMenuEvents[f52_local0].menuName and f52_arg0.pendingInGameMenuEvents[f52_local0].menuName == f52_arg1.menuName) then
					table.remove( f52_arg0.pendingInGameMenuEvents, f52_local0 )
				end
			end
		end
		f52_arg0:dispatchEventToRoot( {
			name = "close_menu",
			menuName = Engine.GetIString( f52_arg1.data[1], "CS_SCRIPT_MENUS" ),
			controller = f52_arg1.controller
		} )
	end
end

function HUD_OpenHudMenu( f53_arg0, f53_arg1 )
	f53_arg1.isMenu = true
	f0_local0( f53_arg0, f53_arg1 )
	return true
end

function HUD_CloseHudMenu( f54_arg0, f54_arg1 )
	local f54_local0 = f54_arg1.menuName
	if f54_local0 then
...
function HUD_Close( f55_arg0, f55_arg1 )
	if f55_arg0.safeArea and f55_arg0.safeArea.buttonModel then
		Engine.UnsubscribeAndFreeModel( f55_arg0.safeArea.buttonModel )
	end
	f55_arg0.safeArea.buttonModel = nil
	f55_arg0:close()
end

function HUD_OpenHudPopup( f56_arg0, f56_arg1 )
	f0_local0( f56_arg0, f56_arg1 )
	return true
end

function HUD_CloseInGameMenu( f57_arg0, f57_arg1 )
	if Engine.CanPauseGame() then
		if CoD.InGameMenu.m_unpauseDisabled == nil then
			CoD.InGameMenu.m_unpauseDisabled = {}
		end
		CoD.InGameMenu.m_unpauseDisabled[f57_arg1.controller + 1] = 0
	end
	f0_local8 = nil
	if Engine.CanPauseGame() then
		Engine.SetActiveMenu( f57_arg1.controller, CoD.UIMENU_NONE )
	end
end

function HUD_OpenScoreBoard( f58_arg0, f58_arg1 )
	if CoD.ShowNewScoreboard() then
		return true
	elseif Engine.CanPauseGame() and CoD.InGameMenu.m_unpauseDisabled ~= nil and CoD.InGameMenu.m_unpauseDisabled[f58_arg1.controller + 1] ~= nil and CoD.InGameMenu.m_unpauseDisabled[f58_arg1.controller + 1] > 0 then
		return 
	elseif f58_arg0.scoreBoard and (f58_arg0.safeArea.CodCaster == nil or not f58_arg0.safeArea.CodCaster.m_controlsOpen) then
		f58_arg0:addElement( f58_arg0.scoreBoard )
		f58_arg0.scoreBoard:processEvent( {
			name = "update_scoreboard",
			controller = f58_arg1.controller
		} )
		if f58_arg0.scoreboardUpdateTimer ~= nil then
			f58_arg0:addElement( f58_arg0.scoreboardUpdateTimer )
		end
		if CoD.isZombie == true then
			if f58_arg0.scoreBoard.questItemDisplay then
				f58_arg0.scoreBoard.questItemDisplay:processEvent( {
					name = "update_quest_item_display_scoreboard",
					controller = f58_arg1.controller
				} )
			end
			if f58_arg0.scoreBoard.persistentItemDisplay then
				f58_arg0.scoreBoard.persistentItemDisplay:processEvent( {
					name = "update_persistent_item_display_scoreboard",
					controller = f58_arg1.controller
				} )
			end
			if f58_arg0.scoreBoard.craftableItemDisplay then
				f58_arg0.scoreBoard.craftableItemDisplay:processEvent( {
					name = "update_craftable_item_display_scoreboard",
					controller = f58_arg1.controller
				} )
			end
			if f58_arg0.scoreBoard.captureZoneWheelDisplay then
				f58_arg0.scoreBoard.captureZoneWheelDisplay:processEvent( {
					name = "update_capture_zone_wheel_display_scoreboard",
					controller = f58_arg1.controller
				} )
			end
			if f58_arg0.scoreBoard.questItemList then
				f58_arg0.scoreBoard:processEvent( {
...
function HUD_CloseScoreBoard( f59_arg0, f59_arg1 )
	if CoD.ShowNewScoreboard() then
		return 
	elseif f59_arg0.scoreBoard then
		if f59_arg0.scoreBoard:getParent() == f59_arg0 then
			f59_arg0:removeElement( f59_arg0.scoreBoard )
		end
		if f59_arg0.scoreboardUpdateTimer then
			f59_arg0.scoreboardUpdateTimer:close()
			f59_arg0.scoreboardUpdateTimer:reset()
		end
	end
end

function HUD_DebugReload( f60_arg0, f60_arg1 )
	if f60_arg0.m_eventHandlers.debug_reload ~= HUD_DebugReload then
		f60_arg0:registerEventHandler( "debug_reload", HUD_DebugReload )
		...
		f0_local1( f60_arg0 )
		HUD_FirstSnapshot( f60_arg0, {
			controller = f60_arg0.controller,
			debugReload = true
		} )
		Engine.ForceHUDRefresh( f60_arg0.controller )
		f60_arg0:processEvent( {
			name = "hud_boot",
			controller = f60_arg0.controller
		} )
		Engine.SetModelValue( Engine.GetModel( Engine.GetModelForController( f60_arg0.controller ), "scriptNotify" ), "player_spawned" )
	end
end

function HUD_UpdateRefresh( f61_arg0, f61_arg1 )
	local f61_local0 = CoD.SafeGetModelValue( DataSources.PerController.getModel( f61_arg1.controller ), "vehicle.vehicleType" )
	if f61_local0 then
		f0_local14( f61_arg0, f61_arg1.controller, f61_local0, true )
	end
	local f61_local1 = CoD.SafeGetModelValue( DataSources.PerController.getModel( f61_arg1.controller ), "currentWeapon.weaponReticle" )
	if f61_local1 then
		f0_local16( f61_arg0, f61_arg1.controller, f61_local1, true )
	end
	f0_local4( f61_arg0, f61_arg1 )
	f0_local2( f61_arg0, f61_arg1 )
end

function HUD_FullscreenStart( f62_arg0, f62_arg1 )
	if f62_arg0.scoreBoard and not CoD.ShowNewScoreboard() then
		f62_arg0.scoreBoard:processEvent( f62_arg1 )
	end
	f62_arg0:dispatchEventToChildren( f62_arg1 )
end

function HUD_FullscreenStop( f63_arg0, f63_arg1 )
	if f63_arg0.scoreBoard and not CoD.ShowNewScoreboard() then
		f63_arg0.scoreBoard:processEvent( f63_arg1 )
	end
	f63_arg0:dispatchEventToChildren( f63_arg1 )
end

function HUD_UpdateScoreboardColumns( f64_arg0, f64_arg1 )
	if not CoD.ShowNewScoreboard() then
		if f64_arg0.scoreBoard ~= nil then
			f64_arg0.scoreBoard:processEvent( f64_arg1 )
		end
		f64_arg0:dispatchEventToChildren( f64_arg1 )
	end
end

function HUD_StartKillcamHud( f65_arg0, f65_arg1 )
	if f65_arg0.T7HudMenu and not f65_arg0.killcamHUD and not CoD.isCampaign then
		if Engine.UpdateKillcamUIModels then
			Engine.UpdateKillcamUIModels( f65_arg1.controller, Engine.GetPredictedClientNum( f65_arg1.controller ) )
		end
		f65_arg0.killcamHUD = LUI.createMenu.KillcamMenu( f65_arg1.controller )
		f65_arg0.killcamHUD:addElementBefore( f65_arg0.T7HudMenu )
	end
end

function HUD_StopKillcamHud( f66_arg0, f66_arg1 )
	if f66_arg0.killcamHUD then
		f66_arg0.killcamHUD:close()
		f66_arg0.killcamHUD = nil
	end
end

f0_local2 = function ( f67_arg0, f67_arg1 )
	if Engine.IsVisibilityBitSet( f67_arg1.controller, Enum.UIVisibilityBit.BIT_IN_KILLCAM ) then
		HUD_StartKillcamHud( f67_arg0, f67_arg1 )
	else
		HUD_StopKillcamHud( f67_arg0, f67_arg1 )
	end
end

f0_local22 = function ( f68_arg0 )
	local f68_local0
	if Engine.IsSplitscreen() == false then
		if Engine.IsDemoShoutcaster() ~= true then
			f68_local0 = Engine.IsVisibilityBitSet( f68_arg0, Enum.UIVisibilityBit.BIT_SPECTATING_CLIENT )
			if f68_local0 then
				f68_local0 = Engine.IsVisibilityBitSet( f68_arg0, Enum.UIVisibilityBit.BIT_TEAM_SPECTATOR )
				if f68_local0 then
					if not Engine.IsVisibilityBitSet( f68_arg0, Enum.UIVisibilityBit.BIT_GAME_ENDED ) then
						f68_local0 = not Engine.IsVisibilityBitSet( f68_arg0, Enum.UIVisibilityBit.BIT_UI_ACTIVE )
					else
						f68_local0 = false
					end
				end
...
f0_local23 = function ( f69_arg0 )
	return Engine.IsVisibilityBitSet( f69_arg0, Enum.UIVisibilityBit.BIT_SELECTING_LOCATION ) and not Engine.IsVisibilityBitSet( f69_arg0, Enum.UIVisibilityBit.BIT_IS_DEMO_PLAYING )
end

if not CoD.isZombie and not CoD.isCampaign then
	require( "ui.uieditor.widgets.Tablet.TabletLocationSelector" )
end
f0_local4 = function ( f70_arg0, f70_arg1 )
	if f0_local23( f70_arg1.controller ) then
		if f70_arg0.newLocationSelectorMap == nil then
			f70_arg0.newLocationSelectorMap = CoD.TabletLocationSelector.new( f70_arg0, f70_arg1.controller )
			f70_arg0.newLocationSelectorMap:processEvent( {
				name = "update_state",
				menu = f70_arg0
			} )
			LUI.OverrideFunction_CallOriginalSecond( f70_arg0.newLocationSelectorMap, "close", function ()
				f70_arg0.newLocationSelectorMap = nil
			end )
			f70_arg0:addForceClosedChild( f70_arg0.newLocationSelectorMap )
		end
	elseif f70_arg0.newLocationSelectorMap ~= nil then
		f70_arg0.newLocationSelectorMap:close()
		f70_arg0.newLocationSelectorMap = nil
	end
end

f0_local3 = function ( f72_arg0, f72_arg1 )
	Engine.DisableSceneFilter( f72_arg0:getOwner(), 4 )
end

f0_local5 = function ( f73_arg0, f73_arg1 )
	local f73_local0 = f73_arg1.data[1]
	local f73_local1 = f73_arg1.data[2]
	Engine.SetModelValue( Engine.CreateModel( Engine.CreateModel( Engine.GetModelForController( f73_arg1.controller ), "Attacker" ), "kills" ), f73_local0 )
	Engine.SetModelValue( Engine.CreateModel( Engine.CreateModel( Engine.GetModelForController( f73_arg1.controller ), "Victim" ), "kills" ), f73_local1 )
end

function HUD_IsFFA()
	if f0_local6 == nil then
		if CoD.isZombie == true then
			local f74_local0 = Engine.DvarString( nil, "ui_zm_gamemodegroup" )
			local f74_local1
			if f74_local0 ~= CoD.Zombie.GAMETYPEGROUP_ZCLASSIC and f74_local0 ~= CoD.Zombie.GAMETYPEGROUP_ZSURVIVAL then
				f74_local1 = false
			else
				f74_local1 = true
			end
			f0_local6 = f74_local1
		else
			local f74_local0
			...
			f0_local6 = f74_local0
		end
	end
	return f0_local6
end

function HUD_Handle_ChooseClass_HotKey( f75_arg0, f75_arg1 )
	if UIExpression.Team( f75_arg1.controller, "name" ) ~= "TEAM_SPECTATOR" and CoD.IsWagerMode() == false and not (Engine.GetGametypeSetting( "disableClassSelection" ) == 1) then
		f0_local0( f75_arg0, {
...
function HUD_OpenManageSegments( f76_arg0, f76_arg1 )
	StartMenuOpenManageSegments( f76_arg0, nil, f76_arg1.controller, nil, f76_arg0 )
end
