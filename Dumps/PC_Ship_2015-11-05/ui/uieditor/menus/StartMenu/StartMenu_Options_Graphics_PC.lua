...
DataSources.OptionGraphicsGame = DataSourceHelpers.ListSetup( "PC.OptionGraphicsGame", function ( f3_arg0 )
	local f3_local0 = {}
...
DataSources.OptionGraphicsGame.getWidgetTypeForItem = function ( f4_arg0, f4_arg1, f4_arg2 )
	if f4_arg1 then
...
DataSources.OptionPCGraphicsDisplayMode = DataSourceHelpers.ListSetup( "PC.OptionPCGraphicsDisplayMode", function ( f5_arg0 )
	local f5_local0 = {}
...
DataSources.OptionPCGraphicsMonitor = DataSourceHelpers.ListSetup( "PC.OptionPCGraphicsMonitor", function ( f6_arg0 )
	local f6_local0 = {}
	local f6_local1 = Dvar.r_monitorCount:get()
	for f6_local2 = 1, f6_local1, 1 do
		table.insert( f6_local0, {
			models = {
				value = f6_local2,
				valueDisplay = f6_local2
			}
		} )
	end
	return f6_local0
end, true )
DataSources.OptionPCGraphicsScreenResolution = DataSourceHelpers.ListSetup( "PC.OptionPCGraphicsScreenResolution", function ( f7_arg0 )
	local f7_local0 = {}
	local f7_local1 = nil
	if Dvar.r_fullscreen:get() == 0 then
		f7_local1 = Engine.GetAllResolutions()
	else
		local f7_local2 = tonumber( Engine.GetHardwareProfileValueAsString( "r_monitor" ) )
		if Dvar.r_monitorCount:get() < f7_local2 then
			f7_local2 = 0
		end
		if f7_local2 == 0 then
			f7_local2 = Dvar.r_monitor:get()
		end
		if f7_local2 == 0 then
			f7_local2 = 1
		end
		f7_local1 = Engine.GetAvailableResolutions( f7_local2 )
	end
	local f7_local2 = Dvar.r_mode:get()
	if f7_local2 then
		table.insert( f7_local1, f7_local2 )
	end
	table.sort( f7_local1, function ( f8_arg0, f8_arg1 )
		local f8_local0, f8_local1 = string.match( f8_arg0, "([%d]+)x([%d]+)" )
		local f8_local2, f8_local3 = string.match( f8_arg1, "([%d]+)x([%d]+)" )
		if f8_local0 == f8_local2 then
			return tonumber( f8_local3 ) < tonumber( f8_local1 )
		else
			return tonumber( f8_local2 ) < tonumber( f8_local0 )
		end
	end )
	local f7_local3 = {}
	for f7_local7, f7_local8 in ipairs( f7_local1 ) do
		if not f7_local3[f7_local8] then
			table.insert( f7_local0, {
				models = {
					value = f7_local8,
					valueDisplay = f7_local8
				}
			} )
			f7_local3[f7_local8] = true
		end
	end
	return f7_local0
end, true )
DataSources.OptionPCGraphicsSceneResolution = DataSourceHelpers.ListSetup( "PC.OptionPCGraphicsSceneResolution", function ( f9_arg0 )
	...
	local f9_local3 = Engine.GetMaxSceneResolutionMultiplier( f9_local1, f9_local2 )
	local f9_local4 = {}
	for f9_local5 = 50, f9_local3, 10 do
		local f9_local8 = f9_local5 * f9_local1 / 100
		local f9_local9 = f9_local5 * f9_local2 / 100
		if f9_local5 ~= 100 then
			f9_local8 = math.floor( f9_local8 / 8 ) * 8
			f9_local9 = math.floor( f9_local9 / 8 ) * 8
		end
		table.insert( f9_local0, {
			models = {
				value = f9_local5,
				valueDisplay = string.format( "%d%% - %dx%d", f9_local5, f9_local8, f9_local9 )
			}
		} )
	end
	return f9_local0
...
DataSources.OptionPCGraphicsRefreshRate = DataSourceHelpers.ListSetup( "PC.OptionPCGraphicsRefreshRate", function ( f10_arg0 )
	...
	if f10_local3 > Dvar.r_monitorCount:get() then
		f10_local3 = 0
	end
	...
	for f10_local7, f10_local8 in ipairs( Engine.GetAvailableRefreshRates( f10_local1, f10_local2, f10_local3 ) ) do
		table.insert( f10_local0, {
			models = {
				value = tonumber( f10_local8 ),
				valueDisplay = f10_local8 .. "hz"
			}
		} )
	end
	return f10_local0
...
DataSources.OptionPCGraphicsDisplayGamma = DataSourceHelpers.ListSetup( "PC.OptionPCGraphicsDisplayGamma", function ( f11_arg0 )
	...
	for f11_local3 = 1, 2, 1 do
		table.insert( f11_local0, {
			models = {
				value = f11_local1[f11_local3],
				valueDisplay = f11_local2[f11_local3]
			}
		} )
	end
	return f11_local0
...
DataSources.OptionGraphicsVideo.getWidgetTypeForItem = function ( f17_arg0, f17_arg1, f17_arg2 )
	if f17_arg1 then
		local f17_local0 = Engine.GetModelValue( Engine.GetModel( f17_arg1, "widgetType" ) )
		if f17_local0 == "dropdown" then
			return CoD.OptionDropdown
		elseif f17_local0 == "checkbox" then
			return CoD.StartMenu_Options_CheckBoxOption
		elseif f17_local0 == "slider" then
			return CoD.StartMenu_Options_SliderBar
		elseif f17_local0 == "spacer" then
			return CoD.VerticalListSpacer
		end
	end
	return nil
...
DataSources.OptionPCGraphicsTextureQuality = DataSourceHelpers.ListSetup( "PC.OptionPCGraphicsTextureQuality", function ( f18_arg0 )
	local f18_local0 = {}
...
DataSources.OptionPCGraphicsTextureFiltering = DataSourceHelpers.ListSetup( "PC.OptionPCGraphicsTextureFiltering", function ( f19_arg0 )
	local f19_local0 = {}
...
DataSources.OptionPCGraphicsMeshQuality = DataSourceHelpers.ListSetup( "PC.OptionPCGraphicsMeshQuality", function ( f20_arg0 )
	local f20_local0 = {}
...
DataSources.OptionPCGraphicsShadowQuality = DataSourceHelpers.ListSetup( "PC.OptionPCGraphicsShadowQuality", function ( f21_arg0 )
	local f21_local0 = {}
...
DataSources.OptionPCGraphicsVolumetricQuality = DataSourceHelpers.ListSetup( "PC.OptionPCGraphicsVolumetricQuality", function ( f22_arg0 )
	local f22_local0 = {}
...
DataSources.OptionPCGraphicsAntiAliasing = DataSourceHelpers.ListSetup( "PC.OptionPCGraphicsAntiAliasing", function ( f23_arg0 )
	local f23_local0 = {}
	local f23_local1 = Engine.GetGPUCount()
	local f23_local2 = {
		None = "PLATFORM_NONE",
		["Filmic SMAA 1x"] = "PLATFORM_FILMIC_SMAA_1x",
		["Filmic SMAA T2x"] = "PLATFORM_FILMIC_SMAA_T2x"
	}
	for f23_local6, f23_local7 in ipairs( Dvar.r_aaTechnique:getDomainEnumStrings() ) do
		table.insert( f23_local0, {
			models = {
				value = f23_local7,
				valueDisplay = f23_local2[f23_local7] or f23_local7
			}
		} )
		if f23_local1 > 1 and f23_local6 >= 3 then
			break
		end
	end
	return f23_local0
end, true )
DataSources.OptionPCGraphicsOIT = DataSourceHelpers.ListSetup( "PC.OptionPCGraphicsOIT", function ( f24_arg0 )
	local f24_local0 = {}
...
local f0_local0 = function ( f25_arg0, f25_arg1, f25_arg2 )
	local f25_local0 = ""
	local f25_local1 = f25_arg2:getModel()
	if f25_local1 then
		local f25_local2 = Engine.GetModel( f25_local1, "valueDisplay" )
		if f25_local2 then
			f25_local0 = Engine.GetModelValue( f25_local2 )
		end
		local f25_local3 = nil
		local f25_local4 = Engine.GetModel( f25_local1, "value" )
		if f25_local4 then
			f25_local3 = Engine.GetModelValue( f25_local4 )
		end
		...
		CoD.PCUtil.SetOptionValue( f25_arg1:getModel(), f25_arg0, f25_local3 )
		CoD.PCUtil.RefreshAllOptions( f25_arg1, f25_arg0 )
	end
	return f25_local0
end

local f0_local1 = function ( f26_arg0, f26_arg1, f26_arg2 )
	local f26_local0 = CoD.PCUtil.OptionsDropdownRefresh( f26_arg0, f26_arg1, f26_arg2 )
	if f26_local0 == "PLATFORM_MEDIUM" and not (Engine.GetHardwareProfileValueAsString( "r_oit" ) == "1") then
		f26_local0 = "PLATFORM_NONE"
	end
	return f26_local0
end

DataSources.OptionPCGraphicsMaxFPS = DataSourceHelpers.ListSetup( "PC.OptionPCGraphicsMaxFPS", function ( f27_arg0 )
	local f27_local0 = {}
...
DataSources.OptionPCGraphicsAmbientOcclusion = DataSourceHelpers.ListSetup( "PC.OptionPCGraphicsAmbientOcclusion", function ( f28_arg0 )
	local f28_local0 = {}
	local f28_local1 = Engine.GetGPUCount()
...
DataSources.OptionGraphicsAdvanced = DataSourceHelpers.ListSetup( "PC.OptionGraphicsAdvanced", function ( f29_arg0 )
	local f29_local0 = {}
	...
	table.insert( f29_local0, {
		...
		properties = {
			checkboxAction = function ( f30_arg0, f30_arg1 )
				Engine.SetHardwareProfileValue( "r_deferredForceShadowNeverUpdate", CoD.PCUtil.OptionsCheckboxAction( f30_arg0, f30_arg1 ) )
			end
		}
	} )
	table.insert( f29_local0, {
...
DataSources.OptionGraphicsAdvanced.getWidgetTypeForItem = function ( f32_arg0, f32_arg1, f32_arg2 )
	if f32_arg1 then
		local f32_local0 = Engine.GetModelValue( Engine.GetModel( f32_arg1, "widgetType" ) )
		if f32_local0 == "dropdown" then
			return CoD.OptionDropdown
		elseif f32_local0 == "checkbox" then
			return CoD.StartMenu_Options_CheckBoxOption
		elseif f32_local0 == "slider" then
			return CoD.StartMenu_Options_SliderBar
		elseif f32_local0 == "spacer" then
			return CoD.VerticalListSpacer
		end
	end
	return nil
...
DataSources.OptionGraphicsCategories = DataSourceHelpers.ListSetup( "PC.OptionGraphicsCategories", function ( f33_arg0 )
	local f33_local0 = {}
...
local PostLoadFunc = function ( f35_arg0, f35_arg1 )
	f35_arg0.categoryFrame:setForceMouseEventDispatch( true )
end
