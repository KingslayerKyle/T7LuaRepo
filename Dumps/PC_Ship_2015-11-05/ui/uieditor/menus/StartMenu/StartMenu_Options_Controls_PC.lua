...
DataSources.OptionGamepadSettingsPC = DataSourceHelpers.ListSetup( "PC.OptionGamepadSettingsPC", function ( f6_arg0 )
	...
	table.insert( f6_local0, {
		models = {
			...
			widgetType = "dropdown",
			getFunction = function ( f9_arg0 )
				return Engine.GamepadsConnectedPort( f9_arg0 )
			end,
			setFunction = function ( f10_arg0, f10_arg1 )
				Engine.GamepadsConnectedMap( f10_arg0, f10_arg1 )
				if not (Engine.GamepadsConnectedCount() <= 0 or Engine.GamepadsConnectedValidPort( Engine.GamepadsConnectedPort( f10_arg0 ) ) ~= false) or Engine.GamepadsConnectedCount() > 1 then
					local f10_local0 = 0
					if f10_arg0 == 0 then
						f10_local0 = 1
					end
					if Engine.GamepadsConnectedValidPort( Engine.GamepadsConnectedPort( f10_local0 ) ) == false then
						Engine.GamepadsConnectedMapAny( f10_local0 )
					end
				end
			end,
...
DataSources.OptionGamepadSettingsPC.getWidgetTypeForItem = function ( f12_arg0, f12_arg1, f12_arg2 )
	if f12_arg1 then
		local f12_local0 = Engine.GetModelValue( Engine.GetModel( f12_arg1, "widgetType" ) )
		if f12_local0 == "checkbox" then
			return CoD.StartMenu_Options_CheckBoxOption
		elseif f12_local0 == "dropdown" then
			return CoD.OptionDropdown
		elseif f12_local0 == "slider" then
			return CoD.StartMenu_Options_SliderBar
		elseif f12_local0 == "gamepadLayout" then
			return CoD.StartMenu_Options_GamepadLayoutOption
		elseif f12_local0 == "spacer" then
			return CoD.VerticalListSpacer
		end
	end
	return nil
...
local PostLoadFunc = function ( f14_arg0, f14_arg1 )
	f14_arg0.categoryFrame:setForceMouseEventDispatch( true )
	LUI.OverrideFunction_CallOriginalFirst( f14_arg0, "close", function ()
		Engine.ApplyHardwareProfileSettings()
		Engine.Exec( f14_arg1, "refreshLastInput" )
	end )
end
