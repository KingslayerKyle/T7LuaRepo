...
LUI.createMenu.EmblemSelect = function ( controller )
	...
	self:AddButtonCallbackFunction( emblemList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( element, menu, controller, model )
		if IsLive() and IsSelfModelValueEqualTo( element, controller, "isNonClickableEmblem", 0 ) and not IsElementInState( element, "BMClassified" ) and not CraftSlotsFullByStorageType( controller, Enum.StorageFileType.STORAGE_EMBLEMS ) then
	...
	self:AddButtonCallbackFunction( emblemList, controller, Enum.LUIButton.LUI_KEY_START, "O", function ( element, menu, controller, model )
		if Emblem_IsOccupied( element, controller ) and IsSelfModelValueEqualTo( element, controller, "isNonClickableEmblem", 0 ) and not IsPreBuiltEmblemTab( controller ) then
	...
	self:AddButtonCallbackFunction( emblemList, controller, Enum.LUIButton.LUI_KEY_XBY_PSTRIANGLE, "S", function ( element, menu, controller, model )
	...
	EmblemName:linkToElementModel( emblemList, "emblemTitle", true, function ( model )
		...
		if emblemTitle then
			EmblemName.weaponNameLabel:setText( Engine.Localize( emblemTitle ) )
		end
	end )
