...
local PreLoadFunc = function ( self, controller )
	...
	if CoD.CCUtility.customizationMode == Enum.eModes.MODE_CAMPAIGN then
		CoD.CCUtility.Heroes.HeroIndexForEdits = Engine.GetEquippedHero( controller, CoD.CCUtility.customizationMode )
	end
			...
			elseif f5_arg2.customizationArea == CoD.CCUtility.TauntCategoryIndex then
				...
				if f5_arg3 then
					...
					if Enum.eModes.MODE_MULTIPLAYER == Engine.CurrentSessionMode() then
						...
						for f5_local10, f5_local11 in ipairs( CoD.CCUtility.PersonalizeHeroData.HeroCustomizationTabCategories ) do
							local f5_local12 = f4_arg0.categoryTabs.Tabs.grid:findItem( nil, {
								filterName = f5_local11
							}, false, nil )
							if f5_local12 then
								local f5_local6 = f5_local12:getModel( f4_arg1, "breadcrumbCount" )
								if f5_local6 then
									local f5_local7 = 0
									if f5_local11 == "loot" then
										if type( f5_local2 ) == "table" then
											local f5_local8 = f5_local2.loot
										end
										f5_local7 = f5_local8 or 0
									else
										if type( f5_local2 ) == "table" then
											local f5_local9 = f5_local2.standard
										end
										f5_local7 = f5_local9 or 0
									end
...
LUI.createMenu.PersonalizeCharacter = function ( controller )
	...
	Frame.titleLabel:setText( Engine.Localize( ConvertToUpperString( LocalizeWithEdittingHeroName( controller, "HEROES_OUTFITS_CAPS" ) ) ) )
	Frame.cac3dTitleIntermediary0.FE3dTitleContainer0.MenuTitle.TextBox1.Label0:setText( Engine.Localize( ConvertToUpperString( LocalizeWithEdittingHeroName( controller, "HEROES_OUTFITS_CAPS" ) ) ) )
	...
	optionsList:setTopBottom( true, false, 109, 673 )
	...
	PersonalizeCharacterCPProgressionMessage0:setLeftRight( false, false, -205, 43 )
	...
	progressionInfo:setTopBottom( true, false, 545, 657 )
	...
	categoryInfoBlock:linkToElementModel( personalizeList.customizationList, "currentSelectionVariantName", true, function ( model )
		...
		if currentSelectionVariantName then
			categoryInfoBlock.CurrentlyEquippedInfo.weaponNameWithVariant.variantName.itemName:setText( Engine.Localize( currentSelectionVariantName ) )
		end
	end )
	...
	self.clipsPerState = {
		DefaultState = {
			...
			EdittingItem = function ()
				...
				local optionsListFrame2 = function ( optionsList, event )
					...
					optionsList:setTopBottom( true, true, 93, -63 )
				...
				self.optionsList:setTopBottom( true, true, 93, -63 )
				self.optionsList:setAlpha( 0 )
			...
			EdittingItemTabs = function ()
				...
				local optionsListFrame2 = function ( optionsList, event )
					...
					optionsList:setLeftRight( false, false, -573, -225 )
					optionsList:setTopBottom( true, true, 94, -62 )
					optionsList:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( optionsList, event )
					else
		...
		EdittingItem = {
			DefaultClip = function ()
				...
				self.optionsList:setTopBottom( true, false, 94, 658 )
		...
		EdittingItemTabs = {
			DefaultClip = function ()
				...
				optionsList:completeAnimation()
				self.optionsList:setLeftRight( false, false, -573, -225 )
				self.optionsList:setTopBottom( true, true, 94, -62 )
				self.optionsList:setAlpha( 1 )
				self.clipFinished( optionsList, {} )
				categoryInfoBlock:completeAnimation()
				self.categoryInfoBlock:setAlpha( 0 )
	...
	self:registerEventHandler( "menu_loaded", function ( self, event )
		...
		if not f59_local0 then
			f59_local0 = self:dispatchEventToChildren( event )
		end
		return f59_local0
	end )
