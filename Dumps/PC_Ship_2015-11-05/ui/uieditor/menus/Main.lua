...
local PostLoadFunc = function ( f3_arg0, f3_arg1 )
	...
	local f3_local0 = function ( f4_arg0, f4_arg1, f4_arg2, f4_arg3 )
		...
		elseif Engine.IsUserGuest( f4_arg2 ) then
			...
			return 
		elseif not LobbyBeginPlay( f3_arg0, f4_arg2 ) then
			return 
		else
	...
	f3_arg0:subscribeToModel( Engine.CreateModel( Engine.GetGlobalModel(), "lobbyRoot.beginPlay" ), function ( model )
		...
		if modelValue == Enum.LobbyNetworkMode.LOBBY_NETWORKMODE_LIVE then
			...
			CoD.LobbyBase.BeginLivePlay( f3_arg0, 0 )
		elseif modelValue == Enum.LobbyNetworkMode.LOBBY_NETWORKMODE_LAN then
			Engine.SetModelValue( model, Enum.LobbyNetworkMode.LOBBY_NETWORKMODE_INVALID )
			Engine.ActivatePrimaryLocalClient( 0 )
			CoD.LobbyBase.BeginLANPlay( f3_arg0, 0 )
		end
	end, false )
	...
	Engine.SendClientScriptNotify( f3_arg1, "menu_change" .. CoD.GetLocalClientAdjustedNum( f3_arg1 ), "Main", "closeToMenu" )
	...
	local f3_local3 = function ()
		f3_arg0.launchInvitesHotJoinTimer = nil
		Engine.LivePlatformSessionEnableInviteHotJoin()
	end
	
	if not f3_arg0.launchInvitesHotJoinTimer and not Engine.LivePlatformSessionCanProcessLaunchData() then
		f3_arg0.launchInvitesHotJoinTimer = LUI.UITimer.newElementTimer( 0, true, f3_local3 )
		f3_arg0:addElement( f3_arg0.launchInvitesHotJoinTimer )
	end
end
