...
CoD.LANServerBrowserDetails.new = function ( menu, controller )
	...
	PlayerList:setLeftRight( true, false, 2, 452 )
	PlayerList:setTopBottom( true, false, 232, 489 )
	...
	PlayerListTitle:setLeftRight( true, false, 8, 155 )
	PlayerListTitle:setTopBottom( true, false, 202, 224 )
	...
	MapImage:setLeftRight( true, false, 0, 446 )
	MapImage:setTopBottom( true, false, -57.73, 195 )
	self:addElement( MapImage )
	self.MapImage = MapImage
	
	local MapBackImage = LUI.UIImage.new()
	MapBackImage:setLeftRight( true, false, 0, 446 )
	MapBackImage:setTopBottom( true, false, 145.25, 195 )
	...
	GameType:setLeftRight( true, false, 8, 122 )
	GameType:setTopBottom( true, false, 172.25, 192.25 )
	...
	MapName:setLeftRight( true, false, 6, 176 )
	MapName:setTopBottom( true, false, 148.25, 178.25 )
	...
	self.GameType:linkToElementModel( self, "gameType", true, function ( model )
		...
		if gameType then
			GameType:setText( Engine.Localize( GameTypeToLocalizedGameType( gameType ) ) )
		end
	end )
	self.MapName:linkToElementModel( self, "mapName", true, function ( model )
		local mapName = Engine.GetModelValue( model )
		if mapName then
			MapName:setText( Engine.Localize( MapNameToLocalizedMapName( mapName ) ) )
		end
	end )
	self.clipsPerState = {
		DefaultState = {
