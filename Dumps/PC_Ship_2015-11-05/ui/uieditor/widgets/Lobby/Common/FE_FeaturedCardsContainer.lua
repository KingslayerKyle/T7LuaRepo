...
local PostLoadFunc = function ( self, controller, menu )
	...
	self.CardsList:subscribeToGlobalModel( controller, "FeaturedCardsRoot", "CardSequence", function ( model )
		if self.CardsList.activeWidget ~= nil then
			...
			local f5_local3 = Engine.GetFeaturedCard( f5_local2 )
			if f5_local3.actionTitle and f5_local3.actionTitle:len() > 0 then
	...
	if f4_local0 ~= nil and f4_local0.enabled == true then
		local f4_local1 = LUI.UITimer.newElementTimer( f4_local0.timer, false, function ()
			local f6_local0 = 1
			if self.CardsList.activeWidget ~= nil then
				FeaturedCardsGetNextCard()
			end
		end, self )
		self.CarouselTimer = f4_local1
		self:addElement( f4_local1 )
		local f4_local2 = Engine.GetFeaturedCard( 0 )
		if f4_local2.actionTitle and f4_local2.actionTitle:len() > 0 and f4_local2.valid == true then
			self.Action:setAlpha( 1 )
		else
			self.Action:setAlpha( 0 )
		end
...
CoD.FE_FeaturedCardsContainer.new = function ( menu, controller )
	...
	if Action.m_eventHandlers.input_source_changed then
		local FeaturedOfflineText0 = Action.m_eventHandlers.input_source_changed
		Action:registerEventHandler( "input_source_changed", function ( element, event )
			event.menu = event.menu or menu
			element:updateState( event )
			return FeaturedOfflineText0( element, event )
		end )
	else
	...
	self:mergeStateConditions( {
			...
			condition = function ( menu, element, event )
				...
				if f118_local0 then
					if not IsGameLobby() then
						f118_local0 = IsLive()
						if f118_local0 then
							f118_local0 = not PlayGoIsStillDownloading( controller )
						end
					else
						f118_local0 = false
					end
		...
		},
		{
			...
			condition = function ( menu, element, event )
				...
				if not IsGameLobby() and not IsLive() then
					f119_local0 = not PlayGoIsStillDownloading( controller )
				else
					f119_local0 = false
				end
				return f119_local0
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f120_local0
				if not IsGameLobby() then
					f120_local0 = PlayGoIsStillDownloading( controller )
				else
					f120_local0 = false
				end
				return f120_local0
