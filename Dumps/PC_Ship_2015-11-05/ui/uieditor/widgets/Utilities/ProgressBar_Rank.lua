...
CoD.ProgressBar_Rank.new = function ( menu, controller )
	...
	curRank:setLeftRight( true, false, 69.13, 104.13 )
	...
	nextRank:setLeftRight( false, true, -104, -74 )
	...
	nextRankDesc:subscribeToGlobalModel( controller, "XPProgressionBar", "xpToNextRank", function ( model )
		...
		if xpToNextRank then
			nextRankDesc:setText( Engine.Localize( LocalizeIntoString( "MPUI_N_XP", xpToNextRank ) ) )
		end
	end )
	...
	self:mergeStateConditions( {
		...
		},
		{
			stateName = "MaxLevel",
			condition = function ( menu, element, event )
				return IsModelValueGreaterThan( controller, "XPProgressionBar.nextRank", CoD.MAX_RANK + 1 )
			end
		}
	} )
