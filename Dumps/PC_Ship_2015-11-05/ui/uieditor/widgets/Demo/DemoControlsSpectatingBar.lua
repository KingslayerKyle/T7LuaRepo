...
CoD.DemoControlsSpectatingBar.new = function ( menu, controller )
	...
	PlayerName:setTopBottom( true, false, 2, 24 )
	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f14_local0
				if not IsDemoClipPlaying() then
					f14_local0 = not IsGlobalModelValueEqualToEnum( element, controller, "demo.cameraMode", Enum.demoCameraMode.DEMO_CAMERA_MODE_FREECAM )
				else
					f14_local0 = false
				end
				return f14_local0
