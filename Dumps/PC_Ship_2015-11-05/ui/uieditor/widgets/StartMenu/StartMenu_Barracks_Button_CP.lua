...
CoD.StartMenu_Barracks_Button_CP.new = function ( menu, controller )
	...
	RankNameBackground:setLeftRight( false, false, -141, 147 )
	...
	RankBackground:setLeftRight( false, false, -141, 147 )
	...
	TotalKills:subscribeToGlobalModel( controller, "StorageGlobal", "stats_cp", function ( model )
		local statsCp = Engine.GetModelValue( model )
		if statsCp then
			TotalKills.Title:setText( Engine.Localize( StorageLookup( controller, "playerstatslist.kills.statvalue", statsCp ) ) )
		end
	end )
	...
	KD:setLeftRight( true, false, 189, 364 )
	KD:setTopBottom( true, false, 375.94, 436.94 )
	KD.BorderThin0:setAlpha( 0.3 )
	KD.Value:setText( Engine.Localize( "MENU_PERCENT_COMPLETE_CAPS" ) )
	KD:subscribeToGlobalModel( controller, "CombatRecordCPPercentComplete", "percent", function ( model )
		local percent = Engine.GetModelValue( model )
		if percent then
			KD.Title:setText( Engine.Localize( percent ) )
		end
	end )
	self:addElement( KD )
	self.KD = KD
