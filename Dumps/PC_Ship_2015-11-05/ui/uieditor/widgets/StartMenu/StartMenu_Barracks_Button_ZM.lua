...
CoD.StartMenu_Barracks_Button_ZM.new = function ( menu, controller )
	...
	Rank:setTopBottom( true, false, 208, 236 )
	...
	KD:setLeftRight( true, false, 116, 244 )
	KD:setTopBottom( true, false, 375.94, 436.94 )
	KD.BorderThin0:setAlpha( 0.3 )
	KD.Value:setText( Engine.Localize( "MENU_ROUNDS_SURVIVED_CAPS" ) )
	KD:subscribeToGlobalModel( controller, "StorageGlobal", "stats_zm", function ( model )
		local statsZm = Engine.GetModelValue( model )
		if statsZm then
			KD.Title:setText( Engine.Localize( StorageLookup( controller, "playerstatslist.total_rounds_survived.statvalue", statsZm ) ) )
		end
	end )
	self:addElement( KD )
	self.KD = KD
	...
	SPM:subscribeToGlobalModel( controller, "StorageGlobal", "stats_zm", function ( model )
		local statsZm = Engine.GetModelValue( model )
		if statsZm then
			SPM.Title:setText( Engine.Localize( StorageLookupTwoStatRatioRounded( controller, "playerstatslist.total_rounds_survived.statvalue", "playerstatslist.total_games_played.statvalue", statsZm ) ) )
		end
	end )
	self:addElement( SPM )
	self.SPM = SPM
