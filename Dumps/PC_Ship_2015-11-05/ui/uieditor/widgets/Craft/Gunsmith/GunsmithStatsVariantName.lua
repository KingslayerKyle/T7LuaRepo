...
CoD.GunsmithStatsVariantName.new = function ( menu, controller )
	...
	variantName:setLeftRight( false, true, -147, 0 )
	variantName:setTopBottom( true, false, 30.5, 54.5 )
	...
	variantName:linkToElementModel( self, "variantName", true, function ( model )
		...
		if _variantName then
			variantName:setText( Engine.Localize( _variantName ) )
		end
	end )
	...
	weaponName:linkToElementModel( self, "weaponIndex", true, function ( model )
		...
		if weaponIndex then
			weaponName:setText( Engine.Localize( GetItemNameFromIndex( weaponIndex ) ) )
		end
	end )
