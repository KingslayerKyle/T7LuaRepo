...
CoD.Leaderboard_GameModeFrame.new = function ( menu, controller )
	...
	PlayerProfileInfoWidget:setAlpha( 0.26 )
	PlayerProfileInfoWidget:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if not IsLeaderboardUpdating() then
					f3_local0 = not IsLeaderboardEmpty()
				else
					f3_local0 = false
				end
				return f3_local0
	...
	RankAndStatsWidget:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if not IsLeaderboardUpdating() and not IsLeaderboardEmpty() then
					f6_local0 = not IsFreerunLeaderboard( controller )
				else
					f6_local0 = false
				end
				return f6_local0
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f7_local0
				if not IsLeaderboardUpdating() and not IsLeaderboardEmpty() then
					f7_local0 = IsFreerunLeaderboard( controller )
				else
					f7_local0 = false
				end
				return f7_local0
	...
	PlayerProfileInfoWidget:linkToElementModel( Leaderboard.LeaderboardRows, "backgroundId", true, function ( model )
		local backgroundId = Engine.GetModelValue( model )
		if backgroundId then
			PlayerProfileInfoWidget.GamerCard:setImage( RegisterImage( GetBackgroundByID( backgroundId ) ) )
		end
	end )
	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f29_local0
				if not IsLeaderboardUpdating() then
					f29_local0 = not IsLeaderboardEmpty()
				else
					f29_local0 = false
				end
				return f29_local0
