...
local PostLoadFunc = function ( f1_arg0, f1_arg1 )
	...
	f1_arg0.CardIconFrame:linkToElementModel( f1_arg0, nil, true, function ( model )
		for f2_local0 = 1, #f1_local0, 1 do
			local f2_local3 = f1_local0[f2_local0]
			local f2_local4 = Engine.GetModel( model, f2_local3 )
			if f2_local4 then
				f1_arg0:subscribeToModel( f2_local4, function ( model )
					local modelValue = Engine.GetModelValue( model )
					if f2_local3 == "xuid" then
						modelValue = GetPlayerBackground( modelValue )
					end
					if modelValue then
						CoD.ChallengesUtility.SetCallingCardForWidget( f1_arg0.CardIconFrame, GetBackgroundByID( modelValue ) )
					end
				end )
				break
			end
