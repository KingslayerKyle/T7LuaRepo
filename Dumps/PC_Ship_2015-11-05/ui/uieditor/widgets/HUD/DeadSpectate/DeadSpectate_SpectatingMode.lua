	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if f6_local0 then
					...
					if f6_local0 then
						f6_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IS_THIRD_PERSON )
						if f6_local0 then
							f6_local0 = not IsSplitscreenAndInGame( controller )
						end
					end
				end
				return f6_local0
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f7_local0 = IsActiveLocalClientsCountEqualTo( 1 )
				if f7_local0 then
					f7_local0 = IsGametypeSettingsValue( "disableThirdPersonSpectating", 0 )
					if f7_local0 then
						f7_local0 = not IsSplitscreenAndInGame( controller )
					end
				end
				return f7_local0
			end
