		...
		DefaultState = {
			...
			Charging = function ()
				...
				local IconDuplicateShadowFrame2 = function ( IconDuplicateShadow, event )
					...
					IconDuplicateShadow:setAlpha( 0.9 )
					if event.interrupted then
	...
	self:mergeStateConditions( {
		{
			stateName = "Inactive",
			condition = function ( menu, element, event )
				local f170_local0 = IsSelfModelValueNonEmptyString( element, controller, "rewardImage" )
				if f170_local0 then
					f170_local0 = IsSelfModelValueEqualTo( element, controller, "rewardAmmo", 0 )
					if f170_local0 then
						f170_local0 = not EarningTowardsThisKillstreak( controller, element )
					end
				end
				return f170_local0
			end
