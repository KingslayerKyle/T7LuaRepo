	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f11_local0 = IsGameTypeEqualToString( "gun" )
				if f11_local0 then
					f11_local0 = WeaponUsesAmmo( controller )
					if f11_local0 then
						f11_local0 = IsModelValueGreaterThanOrEqualTo( controller, "currentWeapon.ammoInDWClip", 0 )
					end
				end
				return f11_local0
			end
		...
		},
		{
			...
			condition = function ( menu, element, event )
				...
				if not WeaponUsesAmmo( controller ) then
					f16_local0 = not IsHeroWeaponGadgetCurrentWeapon( menu, controller )
				else
					f16_local0 = false
				end
				return f16_local0
