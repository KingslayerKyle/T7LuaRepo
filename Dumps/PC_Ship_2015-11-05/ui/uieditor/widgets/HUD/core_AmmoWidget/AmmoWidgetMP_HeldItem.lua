	...
	AmmoWidgetBall:mergeStateConditions( {
		{
			stateName = "Hide",
			condition = function ( menu, element, event )
				local f20_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_DEMO_ALL_GAME_HUD_HIDDEN )
				if not f20_local0 then
					f20_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_DEMO_CAMERA_MODE_MOVIECAM )
					if not f20_local0 then
						f20_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_EMP_ACTIVE )
						if not f20_local0 then
							f20_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_GAME_ENDED )
							if not f20_local0 then
								if Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_HUD_VISIBLE ) then
									f20_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IN_GUIDED_MISSILE )
									if not f20_local0 then
										f20_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IN_KILLCAM )
										if not f20_local0 then
											f20_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IN_REMOTE_KILLSTREAK_STATIC )
											if not f20_local0 then
												f20_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IN_VEHICLE )
												if not f20_local0 then
													f20_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IS_FLASH_BANGED )
													if not f20_local0 then
														f20_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IS_SCOPED )
														if not f20_local0 then
															f20_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_SCOREBOARD_OPEN )
															if not f20_local0 then
																f20_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_UI_ACTIVE )
																if not f20_local0 then
																	f20_local0 = not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_WEAPON_HUD_VISIBLE )
																end
															end
														end
													end
	...
	self.clipsPerState = {
		...
		Hide = {
			...
			ShowAllyFlag = function ()
				...
				self.Glow:setAlpha( 0 )
				GlowFrame2( Glow, {} )
				local f125_local1 = function ( f131_arg0, f131_arg1 )
					if not f131_arg1.interrupted then
						f131_arg0:beginAnimation( "keyframe", 470, false, false, CoD.TweenType.Linear )
					end
				...
				AllyFlag:setAlpha( 1 )
				AllyFlag:registerEventHandler( "transition_complete_keyframe", f125_local1 )
			end,
			ShowAxisFlag = function ()
				self:setupElementClipCounter( 2 )
				local GlowFrame2 = function ( Glow, event )
				...
				self.Glow:setAlpha( 0 )
				GlowFrame2( Glow, {} )
				local f132_local1 = function ( f138_arg0, f138_arg1 )
					if not f138_arg1.interrupted then
						f138_arg0:beginAnimation( "keyframe", 450, false, false, CoD.TweenType.Linear )
					end
				...
				AxisFlag:setAlpha( 1 )
				AxisFlag:registerEventHandler( "transition_complete_keyframe", f132_local1 )
			end
		}
	}
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f139_local0 = AmICarryingEnemyFlag( controller )
				if f139_local0 then
					f139_local0 = IsModelValueEqualTo( controller, "factions.playerFactionName", "axis" )
					if f139_local0 then
						f139_local0 = IsModelValueGreaterThanOrEqualTo( controller, "currentWeapon.ammoInDWClip", 0 )
					end
				end
				return f139_local0
			end
		},
		{
			...
			condition = function ( menu, element, event )
				local f140_local0 = AmICarryingEnemyFlag( controller )
				if f140_local0 then
					f140_local0 = IsModelValueEqualTo( controller, "factions.playerFactionName", "allies" )
					if f140_local0 then
						f140_local0 = IsModelValueGreaterThanOrEqualTo( controller, "currentWeapon.ammoInDWClip", 0 )
					end
				end
				return f140_local0
			end
