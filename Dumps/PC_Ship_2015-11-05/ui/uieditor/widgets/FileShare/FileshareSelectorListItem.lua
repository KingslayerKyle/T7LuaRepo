...
CoD.FileshareSelectorListItem.new = function ( menu, controller )
	...
	LabelButton0.itemName:setAlignment( Enum.LUIAlignment.LUI_ALIGNMENT_LEFT )
	...
	self.LabelButton0:linkToElementModel( self, "fileName", true, function ( model )
		...
		if fileName then
			LabelButton0.itemName:setText( Engine.Localize( fileName ) )
		end
	end )
	self.WCFileshareIconExtraCamRender0:linkToElementModel( self, "showPlusImage", true, function ( model )
		local showPlusImage = Engine.GetModelValue( model )
		if showPlusImage then
			WCFileshareIconExtraCamRender0:setAlpha( Negate( showPlusImage ) )
		end
	end )
