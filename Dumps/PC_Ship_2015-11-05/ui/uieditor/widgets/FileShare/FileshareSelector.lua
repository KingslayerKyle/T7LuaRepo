...
CoD.FileshareSelector.new = function ( menu, controller )
	...
	itemList:linkToElementModel( itemList, "isPublishNew", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f2_local0, menu, controller, Enum.LUIButton.LUI_KEY_START )
	end )
	...
	Loading:setLeftRight( true, false, 149.5, 206.5 )
	Loading:setTopBottom( true, false, 244.35, 269.35 )
	...
	itemName:setLeftRight( true, false, 387, 768 )
	...
	itemName:linkToElementModel( itemList, "fileName", true, function ( model )
		...
		if fileName then
			itemName.weaponNameWithVariant.variantName.itemName:setText( Engine.Localize( fileName ) )
		end
	end )
	self.clipsPerState = {
		DefaultState = {
