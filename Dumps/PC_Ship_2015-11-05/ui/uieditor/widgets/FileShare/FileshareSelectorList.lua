...
CoD.FileshareSelectorList.new = function ( menu, controller )
	...
	BG:setLeftRight( true, false, 20, 330 )
	BG:setTopBottom( true, false, 1, 528 )
	BG:setRGB( 0, 0, 0 )
	BG:setAlpha( 0.6 )
	...
	itemList:setTopBottom( true, false, 6.35, 473.35 )
	...
	menu:AddButtonCallbackFunction( itemList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( element, menu, controller, model )
		if IsSelfModelValueEqualTo( element, controller, "isPublishNew", false ) then
			...
			OpenStore( self, element, controller, "ShowcaseManager", menu )
			return true
		end
		...
		if IsSelfModelValueEqualTo( element, controller, "isPublishNew", false ) then
			...
			CoD.Menu.SetButtonLabel( menu, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "MENU_STORE_CAPS" )
			return true
		end
	end, false )
	...
	verticalCounter0:setLeftRight( true, false, 71, 271 )
	verticalCounter0:setTopBottom( true, false, 485.35, 510.35 )
