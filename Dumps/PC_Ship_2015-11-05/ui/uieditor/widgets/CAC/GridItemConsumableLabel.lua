...
CoD.GridItemConsumableLabel.new = function ( menu, controller )
	...
	ComsumableCountLabel:linkToElementModel( self, "itemIndex", true, function ( model )
		local itemIndex = Engine.GetModelValue( model )
		if itemIndex then
			ComsumableCountLabel:setText( Engine.Localize( GetConsumableCountFromIndex( controller, itemIndex ) ) )
		end
	end )
