...
CoD.Social_InfoPane_Presence.new = function ( menu, controller )
	...
	cacItemTitleGlow0:setLeftRight( true, true, 3, -3 )
	...
	titleContextScroller:linkToElementModel( self, "context", true, function ( model )
		local context = Engine.GetModelValue( model )
		if context then
			titleContextScroller.textBox:setText( Engine.Localize( PresenceContextToLocalizedString( context ) ) )
		end
	end )
	...
	titleActivityScroller:linkToElementModel( self, "activity", true, function ( model )
		local activity = Engine.GetModelValue( model )
		if activity then
			titleActivityScroller.textBox:setText( Engine.Localize( PresenceActivityToLocalizedString( activity ) ) )
		end
	end )
	...
	presence:setTopBottom( true, false, 9, 34 )
	...
	self.clipsPerState = {
		...
		ShowPlatformPresence = {
			DefaultClip = function ()
				...
				self.clipFinished( titleActivityScroller, {} )
				presenceIcon:completeAnimation()
				self.presenceIcon:setLeftRight( true, false, 7, 37 )
				self.presenceIcon:setTopBottom( true, false, 6.5, 36.5 )
