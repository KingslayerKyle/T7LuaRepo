...
CoD.ScoreboardUtility.SetScoreboardUIModels = function ( f5_arg0 )
	...
	if Engine.IsMultiplayerGame() then
		...
		Engine.SetModelValue( Engine.CreateModel( f5_local2, "gameType" ), Engine.StructTableLookupString( CoDShared.gametypesStructTable, "name", Engine.GetCurrentGameType(), "name_ref_caps" ) )
		if f5_local0 < 2 then
			f5_local6 = Engine.GetModel( f5_local1, "gameScore.playerScore" )
			if f5_local6 then
				Engine.SetModelValue( Engine.CreateModel( f5_local2, "team1Score" ), Engine.GetModelValue( f5_local6 ) )
			else
				local f5_local3 = 5
