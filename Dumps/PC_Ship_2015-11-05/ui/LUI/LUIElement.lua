...
f0_local0 = function ( f40_arg0, f40_arg1 )
	if f40_arg0 and not f40_arg0.m_keepFocusAsParent and f40_arg0 ~= LUI.currentMouseFocus and f40_arg0 ~= LUI.currentNavFocus and not LUI.UIElement.IsMouseInsideElement( f40_arg0, f40_arg1 ) then
		if f40_arg0:isInFocus() or f40_arg0.hasListFocus then
...
local f0_local1 = function ( f41_arg0, f41_arg1 )
	if not f41_arg0:hasClip( "Focus" ) then
		return 
	end
...
LUI.UIElement.LeftMouseUp = function ( f47_arg0, f47_arg1 )
	if f47_arg1.inside and not f47_arg0.disabled then
...
LUI.UIElement.gainFocus = function ( f62_arg0, f62_arg1 )
	...
	if f62_arg0.m_focusable and not f62_arg0.onlyChildrenFocusable then
		...
		if f62_arg1.isMouse and not f62_arg0.m_preventFromBeingCurrentMouseFocus then
...
LUI.UIElement.dispatchEventToChildren = function ( f70_arg0, f70_arg1 )
	local f70_local0 = f70_arg0:getFirstChild()
	if f70_local0 == nil then
		return 
	end
	while f70_local0 ~= nil do
		local f70_local1 = f70_local0:getNextSibling()
		local f70_local2 = f70_local0:processEvent( f70_arg1 )
		if f70_local2 then
			return f70_local2
		end
...
LUI.UIElement.registerEventHandler = function ( f71_arg0, f71_arg1, f71_arg2 )
	f71_arg0.m_eventHandlers[f71_arg1] = f71_arg2
end

LUI.UIElement.isFocusable = function ( f72_arg0 )
	return f72_arg0.m_focusable
end

LUI.UIElement.clearNavigationTable = function ( f73_arg0 )
	f73_arg0.navigation = {}
end

LUI.UIElement.makeFocusable = function ( f74_arg0 )
	f74_arg0.m_focusable = true
	f74_arg0.navigation = {}
end

LUI.UIElement.makeFocusableWithoutResettingNavigation = function ( f75_arg0 )
	f75_arg0.m_focusable = true
end

LUI.UIElement.makeNotFocusable = function ( f76_arg0 )
	f76_arg0.m_focusable = false
end

LUI.UIElement.isIDNamed = function ( f77_arg0 )
	if string.find( f77_arg0.id, "." ) then
		return true
	else
		return false
	end
...
LUI.UIElement.getFirstInFocus = function ( f78_arg0 )
	if f78_arg0:isInFocus() then
		return f78_arg0
	end
	local f78_local0 = f78_arg0:getFirstChild()
	while f78_local0 do
		local f78_local1 = f78_local0:getFirstInFocus()
		if f78_local1 then
			return f78_local1
		end
		f78_local0 = f78_local0:getNextSibling()
	end
	local f78_local1 = f78_arg0:getNextSibling()
	if f78_local1 then
		return f78_local1:getFirstInFocus()
	end
end

LUI.UIElement.doesElementOrChildHaveFocus = function ( f79_arg0 )
	if f79_arg0:isInFocus() then
		return true
	end
	local f79_local0 = f79_arg0:getFirstChild()
	while f79_local0 do
		if f79_local0:doesElementOrChildHaveFocus() then
			return true
		end
...
LUI.UIElement.hasChildWithID = function ( f80_arg0, f80_arg1 )
	if f80_arg0.id == f80_arg1 then
		return true
	end
	local f80_local0 = f80_arg0:getFirstChild()
	while f80_local0 do
		if f80_local0:hasChildWithID( f80_arg1 ) then
			return true
		end
...
LUI.UIElement.saveState = function ( f81_arg0 )
	if not f81_arg0:isIDNamed() then
		error( "LUI Error: Tried to save menu state, but element has no name: " .. f81_arg0:getFullID() )
		return 
	elseif f81_arg0.lastFocusedElemEvent and f81_arg0.lastFocusedElemEvent.id then
		LUI.savedMenuStates[f81_arg0.id] = {
			id = f81_arg0.lastFocusedElemEvent.id,
			saveEvent = f81_arg0.lastFocusedElemEvent
		}
	else
		LUI.savedMenuStates[f81_arg0.id] = nil
	end
end

LUI.UIElement.clearSavedState = function ( f82_arg0 )
...
LUI.UIElement.restoreState = function ( f83_arg0 )
	if not f83_arg0:isIDNamed() then
		error( "LUI Error: Tried to restore menu state, but element has no name: " .. f83_arg0:getFullID() )
		return 
	elseif Engine.DvarBool( nil, "rat_playback_enabled" ) then
		return 
	else
		local f83_local0 = LUI.savedMenuStates[f83_arg0.id]
		if f83_local0 ~= nil and f83_local0.id then
			return f83_arg0:processEvent( {
				name = "restore_focus",
				id = f83_local0.id,
				data = f83_local0.data,
				saveEvent = f83_local0.saveEvent
			} )
		else
...
LUI.UIElement.restoreFocus = function ( f84_arg0, f84_arg1 )
	local f84_local0 = f84_arg1.saveEvent
	if f84_local0 and f84_local0.idStack and #f84_local0.idStack > 0 and f84_arg0.id == f84_local0.idStack[1] then
		if #f84_local0.idStack == 1 then
			return f84_arg0:processEvent( {
				name = "gain_focus",
				saveEvent = f84_local0
			} )
		else
			table.remove( f84_local0.idStack, 1 )
...
LUI.UIElement.animate = function ( f85_arg0, f85_arg1 )
	if f85_arg0.m_animationStates[f85_arg1.animationStateName] then
		f85_arg0:animateToState( f85_arg1.animationStateName, f85_arg1.duration, f85_arg1.easeIn, f85_arg1.easeOut )
		if f85_arg1.animateChildren then
			f85_arg0:dispatchEventToChildren( f85_arg1 )
		end
	else
		f85_arg0:dispatchEventToChildren( f85_arg1 )
	end
...
LUI.UIElement.hide = function ( f86_arg0 )
	f86_arg0:setAlpha( 0 )
end

LUI.UIElement.show = function ( f87_arg0 )
	f87_arg0:setAlpha( 1 )
end

LUI.UIElement.rotateRandomly = function ( f88_arg0, f88_arg1 )
	if not f88_arg0.m_eventHandlers.rotate_randomly then
		f88_arg0:registerEventHandler( "rotate_randomly", LUI.UIElement.rotateRandomly )
		f88_arg0:addElement( LUI.UITimer.new( 2500, "rotate_randomly", false, f88_arg0 ) )
	end
	f88_arg0:beginAnimation( "rotate_randomly", 2500, true, true )
	f88_arg0:setXRot( math.random( -45, 45 ) )
	f88_arg0:setYRot( math.random( -45, 45 ) )
end

LUI.UIElement.spinRandomly = function ( f89_arg0, f89_arg1 )
	if not f89_arg0.m_eventHandlers.spin_randomly then
		f89_arg0:registerEventHandler( "spin_randomly", LUI.UIElement.rotateRandomly )
		f89_arg0:addElement( LUI.UITimer.new( 2500, "spin_randomly", false, f89_arg0 ) )
	end
	f89_arg0:beginAnimation( "rotate_randomly", 2500, true, true )
	f89_arg0:setZRot( math.random( -90, 90 ) )
end

LUI.UIElement.flicker = function ( f90_arg0, f90_arg1, f90_arg2, f90_arg3, f90_arg4, f90_arg5, f90_arg6 )
	...
	if not f90_arg5 then
		f90_arg5 = 50
	end
	if not f90_arg6 then
		f90_arg6 = 150
	end
	f90_arg0.lowFlickerAlpha = f90_arg3
	f90_arg0.highFlickerAlpha = f90_arg4
	f90_arg0.endFlickerAlpha = f90_arg2
	f90_arg0:registerEventHandler( "close_flicker_timer", LUI.UIElement.CloseFlickerTimer )
	f90_arg0:alternateStates( f90_arg1, LUI.UIElement.LowFlickerAlternate, LUI.UIElement.HighFlickerAlternate, f90_arg5, f90_arg6, LUI.UIElement.EndFlickerAlternate )
end

LUI.UIElement.LowFlickerAlternate = function ( f91_arg0, f91_arg1 )
	f91_arg0:beginAnimation( "lower_alpha", f91_arg1 )
	f91_arg0:setAlpha( f91_arg0.lowFlickerAlpha )
end

LUI.UIElement.HighFlickerAlternate = function ( f92_arg0, f92_arg1 )
	f92_arg0:beginAnimation( "raise_alpha", f92_arg1 )
	f92_arg0:setAlpha( f92_arg0.highFlickerAlpha )
end

LUI.UIElement.EndFlickerAlternate = function ( f93_arg0, f93_arg1 )
	f93_arg0:beginAnimation( "end_alpha", f93_arg1 )
	f93_arg0:setAlpha( f93_arg0.endFlickerAlpha )
	f93_arg0.lowFlickerAlpha = nil
	f93_arg0.highFlickerAlpha = nil
	f93_arg0.endFlickerAlpha = nil
end

LUI.UIElement.CloseFlickerTimer = function ( f94_arg0, f94_arg1 )
	if f94_arg0.lowFlickerAlpha then
		f94_arg0:closeStateAlternator()
	end
end

LUI.UIElement.closeStateAlternator = function ( f95_arg0 )
	if f95_arg0.alternatorTimer then
		f95_arg0.alternatorTimer:close()
		f95_arg0.alternatorTimer = nil
	end
	if f95_arg0.alternatorEnd then
		f95_arg0:alternatorEnd()
		f95_arg0.alternatorEnd = nil
	end
end

LUI.UIElement.UpdateStateAlternater = function ( f96_arg0, f96_arg1 )
	if f96_arg1.next ~= "first" and f96_arg1.next ~= "second" then
		f96_arg0:closeStateAlternator()
		return 
	end
	local f96_local0 = "first"
	local f96_local1 = f96_arg1.func2
	if f96_arg1.next == "first" then
		f96_local0 = "second"
		f96_local1 = f96_arg1.func1
	end
	local f96_local2 = math.random( f96_arg1.lowTime, f96_arg1.highTime )
	if f96_arg1.timeLeft <= f96_local2 and not f96_arg1.isInfinite then
		f96_local2 = f96_arg1.timeLeft + 1
		f96_local0 = "final"
	end
	f96_local1( f96_arg0, f96_local2 )
	local f96_local3 = -1
	if not f96_arg1.isInfinite then
		f96_local3 = f96_arg1.timeLeft - f96_local2
	end
	f96_arg0.alternatorTimer = LUI.UITimer.new( f96_local2, {
		name = "update_state_alternate",
		isInfinite = f96_arg1.isInfinite,
		next = f96_local0,
		func1 = f96_arg1.func1,
		func2 = f96_arg1.func2,
		timeLeft = f96_local3,
		lowTime = f96_arg1.lowTime,
		highTime = f96_arg1.highTime
	}, true, f96_arg0 )
	f96_arg0:addElement( f96_arg0.alternatorTimer )
end

LUI.UIElement.alternateStates = function ( f97_arg0, f97_arg1, f97_arg2, f97_arg3, f97_arg4, f97_arg5, f97_arg6 )
	if f97_arg0.alternatorEnd then
		return 
	elseif not f97_arg1 then
		f97_arg1 = 1000
	end
	if f97_arg2 == nil or f97_arg3 == nil then
		return 
	elseif f97_arg6 == nil then
		f97_arg6 = f97_arg3
	end
	if not f97_arg4 then
		f97_arg4 = 50
	end
	if not f97_arg5 then
		f97_arg5 = 150
	end
	f97_arg0.alternatorEnd = f97_arg6
	f97_arg0:registerEventHandler( "update_state_alternate", LUI.UIElement.UpdateStateAlternater )
	f97_arg0:processEvent( {
		name = "update_state_alternate",
		isInfinite = f97_arg1 == 0,
		next = "first",
		func1 = f97_arg2,
		func2 = f97_arg3,
		timeLeft = f97_arg1,
...
LUI.UIElement.CloseAnimationQueue = function ( f98_arg0 )
	if f98_arg0.queueTimer then
		f98_arg0.queueTimer:close()
		f98_arg0.queueTimer = nil
	end
end

local f0_local2 = function ( f99_arg0, f99_arg1, f99_arg2 )
	if #f99_arg1 == 0 then
		return 0
	end
	local f99_local0 = f99_arg1[1].funct
	local f99_local1 = f99_arg1[1].time
	if f99_local1 == nil then
		f99_local1 = 0
	end
	if f99_local0 then
		f99_local0( f99_arg0, f99_local1, f99_arg1[1] )
	end
	if f99_arg2 then
		table.insert( f99_arg1, f99_arg1[1] )
	end
	table.remove( f99_arg1, 1 )
	return f99_local1
end

local f0_local3 = function ( f100_arg0 )
	if not f100_arg0.queueTimer then
		return 
	end
	local f100_local0 = nil
	if f100_arg0.queueTimer.timerEvent then
		f100_local0 = f100_arg0.queueTimer.timerEvent.queue
	end
	if f100_local0 then
		local f100_local1 = f100_local0[#f100_local0].funct
		if f100_local1 then
			f100_local1( f100_arg0, 0, f100_local0[#f100_local0] )
		end
	end
...
LUI.UIElement.QueueAnimation = function ( f101_arg0, f101_arg1 )
	if #f101_arg1.queue == 0 or not f101_arg0.queueTimer then
		LUI.UIElement.CloseAnimationQueue( f101_arg0 )
		return 
	else
		f101_arg0.queueTimer.interval = f0_local2( f101_arg0, f101_arg1.queue, f101_arg1.repeats )
	end
end

LUI.UIElement.animationQueue = function ( f102_arg0, f102_arg1, f102_arg2, f102_arg3 )
	if not f102_arg1 then
		return 
	elseif f102_arg0.queueTimer then
		f0_local3( f102_arg0 )
	end
	f102_arg0:registerEventHandler( "queue_animation", LUI.UIElement.QueueAnimation )
	local f102_local0 = f102_arg2 or 0
	if f102_local0 == 0 then
		f102_local0 = f0_local2( f102_arg0, f102_arg1, f102_arg3 )
	end
	if #f102_arg1 > 0 then
		f102_arg0.queueTimer = LUI.UITimer.new( f102_local0, {
			name = "queue_animation",
			repeats = f102_arg3,
			queue = f102_arg1
		}, false, f102_arg0 )
		f102_arg0:addElement( f102_arg0.queueTimer )
	end
end

LUI.UIElement.NextListAnimation = function ( f103_arg0, f103_arg1 )
	if f103_arg1._index == 1 and f103_arg0._queueTimer ~= nil then
		f103_arg0._queueTimer:close()
	end
	if f103_arg1.interrupted then
		f103_arg1._index = f103_arg1._lastIndex
	end
	if f103_arg1._index <= f103_arg1._lastIndex then
		local f103_local0 = f103_arg1._anims[f103_arg1._index]( f103_arg0, f103_arg1 )
		f103_arg1._index = f103_arg1._index + 1
		f103_arg0._queueTimer = LUI.UITimer.new( f103_local0, f103_arg1, true, f103_arg0 )
		f103_arg0:addElement( f103_arg0._queueTimer )
	else
		f103_arg0._queueTimer = nil
		f103_arg0:registerEventHandler( "anim_done", nil )
		local f103_local0 = f103_arg0:getParent()
		f103_local0:childClipFinished()
	end
end

LUI.UIElement.ViewportAnimation = function ( f104_arg0, f104_arg1 )
	if not f104_arg0.viewportScaleTime then
		f104_arg0.viewportScaleTime = f104_arg1.timeElapsed
	else
		f104_arg0.viewportScaleTime = f104_arg0.viewportScaleTime + f104_arg1.timeElapsed
	end
	local f104_local0 = f104_arg0.viewportScaleTime / f104_arg1.scaleDuration
	if f104_local0 > 1 then
		f104_local0 = 1
		f104_arg1.timer:close()
		f104_arg0.viewportScaleTime = nil
	end
	local f104_local1 = f104_arg1.startScale + (f104_arg1.endScale - f104_arg1.startScale) * f104_local0
	Engine.SetViewport( f104_arg1.controller, 0.1 - f104_local1 / 10, 0.5 - f104_local1 / 2, f104_local1 )
end

LUI.UIElement.animateViewport = function ( f105_arg0, f105_arg1, f105_arg2, f105_arg3, f105_arg4 )
	f105_arg0:addElement( LUI.UITimer.new( 1, {
		name = "viewport_animation",
		controller = f105_arg1,
		startScale = f105_arg2,
		endScale = f105_arg3,
		scaleDuration = f105_arg4
	}, false, f105_arg0 ) )
end

LUI.UIElement.animateToNextState = function ( f106_arg0, f106_arg1, f106_arg2, f106_arg3, f106_arg4 )
	return function ( f107_arg0, f107_arg1 )
		if not f107_arg1.interrupted then
			f107_arg0:animateToState( f106_arg1, f106_arg2, f106_arg3, f106_arg4 )
		end
	end
	
end

LUI.UIElement.CompleteAnimationEvent = function ( f108_arg0, f108_arg1 )
	f108_arg0:completeAnimation()
	f108_arg0:dispatchEventToChildren( f108_arg1 )
end

LUI.UIElement.UpdateSafeArea = function ( f109_arg0, f109_arg1, f109_arg2 )
	if f109_arg2 == nil and f109_arg1 ~= nil then
		f109_arg2 = f109_arg1.controller
	end
	local f109_local0, f109_local1, f109_local2, f109_local3 = Engine.GetUserSafeAreaForController( f109_arg2 )
	f109_arg0:setLeftRight( false, false, f109_local0, f109_local2 )
	f109_arg0:setTopBottom( false, false, f109_local1, f109_local3 )
end

LUI.UIElement.sizeToSafeArea = function ( f110_arg0, f110_arg1 )
	f110_arg0:UpdateSafeArea( nil, f110_arg1 )
	f110_arg0:registerEventHandler( "update_safe_area", f110_arg0.UpdateSafeArea )
end

LUI.UIElement.setState = function ( f111_arg0, f111_arg1, f111_arg2 )
	if f111_arg0.currentState == f111_arg1 then
		return 
	end
	local f111_local0 = f111_arg0.currentState
	local f111_local1
	if not f111_arg2 then
		f111_local1 = f111_arg0:hasClip( f111_arg1 )
	else
		f111_local1 = false
	end
	f111_arg0.currentState = f111_arg1
	local f111_local2 = f111_local1
	local f111_local3 = "DefaultClip"
	if f111_arg0:isInFocus() and f111_arg0:hasClip( "Focus" ) then
		f111_local3 = "Focus"
		f111_local2 = false
	end
	if f111_local1 then
		f111_arg0.nextClip = f111_local3
		f111_arg0.currentClipIsTransitionClip = f111_local2
		f111_arg0.clipsPerState[f111_local0][f111_arg1]()
	else
		f111_arg0:playClip( f111_local3 )
	end
end

LUI.UIElement.mergeStateConditions = function ( f112_arg0, f112_arg1, f112_arg2 )
	if not f112_arg2 then
		if not f112_arg0.stateConditions then
			f112_arg0.stateConditions = {}
		end
		f112_arg2 = f112_arg0.stateConditions
	end
	local f112_local0 = function ( f113_arg0 )
		for f113_local3, f113_local4 in ipairs( f112_arg2 ) do
			if f113_local4.stateName == f113_arg0 then
				return f113_local3
			end
		end
		return nil
	...
	for f112_local4, f112_local5 in ipairs( f112_arg1 ) do
		local f112_local6 = f112_local0( f112_local5.stateName )
		if f112_local6 == nil then
			table.insert( f112_arg2, LUI.ShallowCopy( f112_local5 ) )
		else
			f112_arg2[f112_local6] = LUI.ShallowCopy( f112_local5 )
		end
	end
end

LUI.UIElement.clipOver = function ( f114_arg0, f114_arg1 )
	if f114_arg0.nextClip then
		f114_arg0:playClip( f114_arg0.nextClip )
	end
end

LUI.UIElement.playClip = function ( f115_arg0, f115_arg1 )
	f115_arg0.nextClip = nil
	f115_arg0.currentClipIsTransitionClip = false
	if not f115_arg0.currentState then
		f115_arg0.currentState = "DefaultState"
	end
	if f115_arg0.clipsPerState and f115_arg0.clipsPerState[f115_arg0.currentState] and f115_arg0.clipsPerState[f115_arg0.currentState][f115_arg1] then
		f115_arg0.clipsPerState[f115_arg0.currentState][f115_arg1]()
		return true
	else
		return false
	end
...
LUI.UIElement.hasClip = function ( f116_arg0, f116_arg1 )
...
LUI.UIElement.setupElementClipCounter = function ( f117_arg0, f117_arg1 )
	f117_arg0.elementsPlayingClips = f117_arg1
	if f117_arg0.elementsPlayingClips == 0 then
...
LUI.UIElement.childClipFinished = function ( f118_arg0 )
	f118_arg0.elementsPlayingClips = f118_arg0.elementsPlayingClips - 1
...
LUI.UIElement.clipFinished = function ( f119_arg0, f119_arg1 )
	local f119_local0 = f119_arg0:getParent()
	if f119_local0 ~= nil and (not f119_arg1.interrupted or f119_local0.currentClipIsTransitionClip) then
		f119_local0:childClipFinished()
	end
end

LUI.UIElement.updateState = function ( f120_arg0, f120_arg1 )
	local f120_local0 = f120_arg1.menu or f120_arg0
	while f120_local0 do
		if f120_local0.updateElementState then
			f120_arg1.menu = f120_local0
			f120_local0:updateElementState( f120_arg0, f120_arg1 )
			if f120_arg0.anyChildUsesUpdateState then
				f120_arg0:dispatchEventToChildren( f120_arg1 )
			end
			return 
		end
		f120_local0 = f120_local0:getParent()
	end
end

LUI.UIElement.RecordCurrFocusedElemID = function ( f121_arg0, f121_arg1 )
	if not f121_arg1.idStack then
		error( "LUI Error: " .. f121_arg1.name .. " processed without event.idStack " )
	end
	table.insert( f121_arg1.idStack, 1, f121_arg0.id )
	return f121_arg0:dispatchEventToParent( f121_arg1 )
end

LUI.UIElement.setFontBasedOnSize = function ( f122_arg0, f122_arg1 )
	if CoD.GetFontBasedOnSize then
		f122_arg0:setFont( CoD.GetFontBasedOnSize( f122_arg1 ) )
	end
end

LUI.UIElement.playBitchinFX = function ( f123_arg0, f123_arg1, f123_arg2, f123_arg3 )
	f123_arg0:setShaderVector( 0, 0, 0, 0, 0 )
	f123_arg0:beginAnimation( "bitchin", f123_arg1, f123_arg2, f123_arg3 )
	f123_arg0:setShaderVector( 0, 1, 0, 0, 0 )
end

LUI.UIElement.playBitchinFXReverse = function ( f124_arg0, f124_arg1, f124_arg2, f124_arg3 )
...
LUI.UIElement.setClass = function ( f125_arg0, f125_arg1 )
	local f125_local0 = getmetatable( f125_arg0 )
	local f125_local1 = f125_local0.__newindex
	local f125_local2 = getmetatable( f125_local1 )
	if not f125_local2 then
		setmetatable( f125_local1, {
			__index = f125_arg1
		} )
	else
		f125_local2.__index = f125_arg1
	end
	local f125_local3 = getmetatable( f125_local1.m_eventHandlers )
	if not f125_local3 then
		setmetatable( f125_local1.m_eventHandlers, {
			__index = f125_arg1.m_eventHandlers
		} )
	else
		f125_local3.__index = f125_arg1.m_eventHandlers
	end
end

LUI.UIElement.getParentMenu = function ( f126_arg0 )
	local f126_local0 = f126_arg0
	while not f126_local0.menuName do
		f126_local0 = f126_local0:getParent()
	end
	return f126_local0
end
...
LUI.UIElement.new = function ( f127_arg0 )
	local f127_local0
	if f127_arg0 then
		f127_local0 = ConstructLUIElement()
		if not f127_local0 then
		
		else
			LUI.UIElement.setClass( f127_local0, LUI.UIElement )
			f127_local0:setLayoutCached( false )
			f127_local0.anyChildUsesUpdateState = false
			if f127_arg0 then
				f127_local0:registerAnimationState( "default", f127_arg0 )
				f127_local0:animateToState( "default" )
			end
			return f127_local0
		end
	end
	f127_local0 = ConstructLUIElement( LUI.UIElement.m_defaultAnimationState )
end

LUI.UIElement.showDebugTimer = function ( f128_arg0, f128_arg1 )
	local f128_local0 = f128_arg0.playClip
	f128_arg0.playClip = function ( f129_arg0, f129_arg1 )
		f128_local0( f129_arg0, f129_arg1 )
		if f129_arg0.elementsPlayingClips ~= nil and f129_arg0.elementsPlayingClips > 0 then
			f129_arg0._debugTimerIndex = Engine.SetDebugTimerInfo( f129_arg0.id .. ":" .. f129_arg0.currentState .. ":" .. f129_arg1, f129_arg0._debugTimerIndex )
		end
	end
	
	f128_arg0:registerEventHandler( "clip_over", function ( element, event )
	...
	local f128_local1 = f128_arg0.close
	f128_arg0.close = function ( f131_arg0 )
		f128_local1( f131_arg0 )
		if f131_arg0._debugTimerIndex ~= nil then
			Engine.SetDebugTimerInfo( "", f131_arg0._debugTimerIndex )
			f131_arg0._debugTimerIndex = nil
		end
	end
