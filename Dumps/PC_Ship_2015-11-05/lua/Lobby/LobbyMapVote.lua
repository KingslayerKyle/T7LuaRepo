...
Lobby.MapVote.UpdateMapVoteInfo = function ()
	...
	if Lobby.MapVote.VoteState == LuaEnums.MAP_VOTE_STATE.LOCKEDIN then
		local f3_local7 = Engine.LobbyGetGameData()
		...
		f3_local0 = f3_local7[1].nextMapName
		f3_local1 = f3_local7[1].previousMapName
		f3_local3 = Engine.StructTableLookupString( "game_types", "name", f3_local7[1].nextGametype, "name_ref_caps" )
		f3_local4 = Engine.StructTableLookupString( "game_types", "name", f3_local7[1].previousGametype, "name_ref_caps" )
		previousGametypeType = f3_local4
	end
	local f3_local7 = Engine.CreateModel( Engine.GetGlobalModel(), "MapVote" )
	if f3_local0 == "cp_doa_bo3" and not Dvar.ui_doa_unlocked:get() then
		f3_local0 = Engine.GetSavedMap()
		f3_local1 = Engine.GetSavedMap()
	end
	if f3_local0 ~= nil then
		...
		local f3_local8 = Engine.LobbyGetGameData()
		if Engine.CurrentSessionMode() == Enum.eModes.MODE_CAMPAIGN and f3_local8.gametype == "coop" then
			local f3_local10 = Lobby.MapVote.mapsTable[f3_local0]
			if f3_local10 then
				f3_local10 = Lobby.MapVote.mapsTable[f3_local0].mapLocation
			end
