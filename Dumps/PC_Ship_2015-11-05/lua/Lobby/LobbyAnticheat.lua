...
Lobby.Anticheat.OnPushAnticheatMessageToUI = function ( f19_arg0 )
	...
	if Engine.GetModelValue( f19_local3 ) > 0 and Engine.GetModelValue( Engine.GetModel( f19_local3, "highestVersionViewed" ) ) < Engine.GetModelValue( Engine.GetModel( f19_local3, "highestVersionAvailable" ) ) then
		local f19_local4 = Engine.GetModelValue( Engine.GetModel( f19_local3, "currentIndex" ) )
		if f19_local4 <= Engine.GetModelValue( f19_local3 ) then
			local f19_local5 = Engine.GetModel( f19_local3, "index_" .. f19_local4 )
			if not Engine.GetModelValue( f19_local5, "viewed" ) then
				local f19_local6 = Engine.GetModel( f19_local2, "message" )
				local f19_local7 = Engine.GetModelValue( Engine.GetModel( f19_local5, "reference" ) )
				local f19_local8 = Engine.GetModelValue( Engine.GetModel( f19_local5, "url" ) )
				Engine.SetModelValue( Engine.GetModel( f19_local6, "string" ), f19_local7 )
				Engine.SetModelValue( Engine.GetModel( f19_local6, "url" ), f19_local8 )
				return true
			end
		end
	end
