...
Lobby.Hopper.Pump = function ()
	...
	elseif Lobby.Hopper.HOPPER_STATE.UNPARKED == Lobby.Hopper.hopperState then
		if Engine.GetLobbyClientCount( Enum.LobbyType.LOBBY_TYPE_GAME ) == 0 and not Engine.ProcessingJoin( Enum.LobbyType.LOBBY_TYPE_GAME ) and false == Dvar.sv_running:get() and Dvar.hopper_playlist:get() == 0 then
			local f3_local0 = Engine.GetPlaylistID()
			local f3_local1 = Engine.GetParkingPlaylistForRealPlaylist( f3_local0 )
			if f3_local1 ~= 0 then
				Engine.PrintWarning( Enum.consoleLabel.LABEL_LOBBYHOST, "No clients left and we're currently on playlist " .. f3_local0 .. " - parking on playlist " .. f3_local1 .. "\n" )
				...
				Engine.SwitchPlaylists( f3_local1 )
				Engine.SetPlaylistID( f3_local1 )
				Dvar.lobbyAdvertisePlaylistNumber:set( f3_local1 )
