	...
	elseif f2_local0 == Enum.LobbyModule.LOBBY_MODULE_HOST and f2_local1 == Enum.LobbyType.LOBBY_TYPE_GAME then
		if f2_local2 == Enum.LobbyMode.LOBBY_MODE_CUSTOM then
			if not CoDShared.IsGametypeTeamBased() then
				return false
			elseif Engine.IsInGame() == true then
				return true
			elseif Engine.GetGametypeSetting( "teamAssignment" ) == LuaEnums.TEAM_ASSIGNMENT.AUTO and Lobby.Timer.GetTimerStatus() ~= Lobby.Timer.LOBBY_STATUS.STARTING then
				return false
			else
				return true
			end
...
Lobby.TeamSelection.GetAllowedTeams = function ()
...
Lobby.TeamSelection.SwitchTeamClientAssignment = function ( f11_arg0 )
	...
	local f11_local5 = Lobby.TeamSelection.GetAllowedTeams()
	local f11_local6 = 0
	local f11_local7 = 1
...
Lobby.TeamSelection.SwitchTeamHostAssignment = function ( f12_arg0 )
	...
	local f12_local5 = Lobby.TeamSelection.GetAllowedTeams()
	local f12_local6 = 0
	local f12_local7 = 1
...
Lobby.TeamSelection.AddAsIndividualToPrivateLobbyList = function ( f17_arg0, f17_arg1, f17_arg2 )
	local f17_local0 = {
		...
		lobbySkill = f17_arg0.skillRating,
		memberList = {}
	}
...
Lobby.TeamSelection.CanSplitInProgress = function ( f19_arg0, f19_arg1, f19_arg2 )
	local f19_local0 = 0
	local f19_local1 = 0
	local f19_local2 = 0
	local f19_local3 = 0
	...
	for f19_local10, f19_local11 in ipairs( f19_arg1 ) do
		if f19_local1 < f19_local2 then
			f19_local1 = f19_local1 + f19_local11
		else
			f19_local2 = f19_local2 + f19_local11
		end
	end
...
Lobby.TeamSelection.CanFitLobbys = function ( f27_arg0 )
	local f27_local0 = f27_arg0.lobbyHostXuid
	local f27_local1 = f27_arg0.probedXuid
	local f27_local2 = Engine.LobbyGetSessionClients( Enum.LobbyModule.LOBBY_MODULE_HOST, Enum.LobbyType.LOBBY_TYPE_GAME )
	if f27_local0 ~= nil and f27_local0 ~= LuaEnums.INVALID_XUID and f27_local0 == Engine.GetLobbyHostXuid( Enum.LobbyType.LOBBY_TYPE_GAME ) then
		return true
	elseif Engine.IsMultiplayerGame() and f27_local1 ~= nil and f27_local1 ~= LuaEnums.INVALID_XUID then
		local f27_local3 = nil
		for f27_local7, f27_local8 in ipairs( f27_local2.sessionClients ) do
			if f27_local8.xuid == f27_local1 then
				f27_local3 = f27_local8.lobbyID
				break
			end
		end
		if f27_local3 ~= nil then
			f27_local4 = 0
			for f27_local8, f27_local9 in ipairs( f27_local2.sessionClients ) do
				if f27_local9.lobbyID == f27_local3 then
					f27_local4 = f27_local4 + 1
				end
			end
			if f27_local4 + #f27_arg0.members > Engine.GetPlaylistMaxPartySize( Engine.GetPlaylistID() ) then
				return false
			end
		end
	end
	if CoDShared.IsTeamBasedGame() == false then
		return true
	end
	local f27_local3 = {}
	local f27_local4 = Engine.GetLobbyMaxClients( Enum.LobbyType.LOBBY_TYPE_GAME )
	LuaUtils.LogQoS( "can_fit/input", f27_arg0 )
	if Engine.IsInGame() then
		local f27_local5 = {}
		for f27_local9, f27_local10 in ipairs( f27_arg0.lobbys ) do
			table.insert( f27_local5, f27_local10 )
		end
		f27_local6 = Lobby.TeamSelection.CanSplitInProgress( f27_local2.sessionClients, f27_local5, f27_local4 )
		LuaUtils.LogQoS( "can_fit/result", f27_local6 )
		return f27_local6
	end
	for f27_local8, f27_local9 in ipairs( f27_local2.sessionClients ) do
		Lobby.TeamSelection.AddToPrivateLobbyList( f27_local9, f27_local8, f27_local3 )
	end
	for f27_local8, f27_local9 in ipairs( f27_arg0.lobbys ) do
		Lobby.TeamSelection.AddDummyToPrivateLobbyList( f27_local9, f27_local3 )
	end
	if f27_arg0.members ~= nil and #f27_arg0.members > Dvar.lobbyAdvertiseTeamSizeMax:get() then
		LuaUtils.LogQoS( "can_fit/result", false )
		return false
	end
	f27_local5 = Lobby.TeamSelection.VladSplit( f27_local3, f27_local4 )
	f27_local6 = false
	if f27_local5.isBalanced then
		f27_local6 = true
	elseif f27_local5.canBalance then
		f27_local6 = true
	end
	LuaUtils.LogQoS( "can_fit/result", f27_local6 )
	return f27_local6
...
Lobby.TeamSelection.ClientJoined = function ( f28_arg0 )
	if not Lobby.TeamSelection.ShouldAssignToTeam( f28_arg0 ) then
		return 
	end
	local f28_local0 = f28_arg0.xuid
	local f28_local1 = Engine.LobbyGetSessionClients( Enum.LobbyModule.LOBBY_MODULE_HOST, Enum.LobbyType.LOBBY_TYPE_GAME )
	...
	Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "-----------------------------------------\n" )
	for f28_local5, f28_local6 in ipairs( f28_local1.sessionClients ) do
		if f28_local0 ~= f28_local6.xuid then
			Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "(" .. tostring( f28_local5 ) .. ") " .. f28_local6.gamertag .. " - lobbyID(" .. tostring( f28_local6.lobbyID ) .. ") -- simpleLobbyID: " .. tostring( f28_local6.simpleLobbyID ) .. " -- team: " .. Lobby.TeamSelection.TeamIdToString( f28_local6.team ) .. " -- score: " .. tostring( f28_local6.score ) .. ".\n" )
		end
	end
	Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "-----------------------------------------\n" )
	f28_local2 = nil
	for f28_local6, f28_local7 in ipairs( f28_local1.sessionClients ) do
		if f28_local0 == f28_local7.xuid then
			f28_local2 = f28_local7
			break
		end
	end
	if f28_local2 == nil then
		return 
	elseif f28_arg0.lobbyMode == Enum.LobbyMode.LOBBY_MODE_CUSTOM and Engine.GetGametypeSetting( "teamAssignment" ) ~= LuaEnums.TEAM_ASSIGNMENT.AUTO then
		if f28_local2.team == Enum.team_t.TEAM_FREE and CoDShared.IsGametypeTeamBased() then
			Engine.LobbyHostAssignTeamToClient( f28_local0, Enum.team_t.TEAM_ALLIES )
		end
		return 
	end
	for f28_local6, f28_local7 in ipairs( f28_local1.sessionClients ) do
		if f28_local2.xuid ~= f28_local7.xuid and f28_local2.lobbyID == f28_local7.lobbyID then
			Engine.LobbyHostAssignTeamToClient( f28_local2.xuid, f28_local7.team )
			Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "Joining client is part of a party.\n" )
			Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "Adding joining client " .. f28_local2.gamertag .. " - lobbyID(" .. tostring( f28_local2.lobbyID ) .. ") -- simpleLobbyID: " .. tostring( f28_local2.simpleLobbyID ) .. " -- team: " .. Lobby.TeamSelection.TeamIdToString( f28_local7.team ) .. " -- score: " .. tostring( f28_local7.score ) .. ".\n" )
	...
	f28_local3 = 0
	f28_local4 = 0
	f28_local5 = Enum.team_t.TEAM_FREE
	if CoDShared.IsGametypeTeamBased() then
		for f28_local9, f28_local10 in ipairs( f28_local1.sessionClients ) do
			if f28_local2.xuid ~= f28_local10.xuid then
				if f28_local10.team == Enum.team_t.TEAM_ALLIES then
					f28_local3 = f28_local3 + 1
				end
				if f28_local10.team == Enum.team_t.TEAM_AXIS then
					f28_local4 = f28_local4 + 1
				end
			end
		end
		f28_local5 = Enum.team_t.TEAM_ALLIES
		if f28_local4 < f28_local3 then
			f28_local5 = Enum.team_t.TEAM_AXIS
		end
	end
	Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "Clients in session: " .. #f28_local1.sessionClients .. ", teamAlliesCount: " .. f28_local3 .. ", teamAxisCount:" .. f28_local4 .. ".\n" )
	Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "Adding joining client " .. f28_local2.gamertag .. " - lobbyID(" .. tostring( f28_local2.lobbyID ) .. ") -- simpleLobbyID: " .. tostring( f28_local2.simpleLobbyID ) .. " -- team: " .. tostring( f28_local5 ) .. ".\n" )
	Engine.LobbyHostAssignTeamToClient( f28_local2.xuid, f28_local5 )
	Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "Lobby.TeamSelection.ClientJoined end.\n" )
end

Lobby.TeamSelection.GametypeSettingsChange = function ()
	local f29_local0 = CoDShared.IsGametypeTeamBased()
	local f29_local1 = Engine.GetGametypeSetting( "allowspectating" ) == 1
	local f29_local2 = Engine.GetGametypeSetting( "teamAssignment" )
	local f29_local3 = f29_local0 ~= Lobby.TeamSelection.gameSetting.teamBased
	local f29_local4 = f29_local1 ~= Lobby.TeamSelection.gameSetting.allowSpectating
	local f29_local5 = f29_local2 ~= Lobby.TeamSelection.gameSetting.teamAssignment
	if f29_local3 == false and f29_local4 == false and f29_local5 == false then
		return 
	end
	Engine.LuiVM_Event( "update_team_selection_buttons", {} )
	if f29_local3 and f29_local0 == false then
		Lobby.TeamSelection.ClearTeam( Enum.team_t.TEAM_ALLIES )
		Lobby.TeamSelection.ClearTeam( Enum.team_t.TEAM_AXIS )
	end
	if f29_local4 and f29_local1 == false then
		if f29_local2 == LuaEnums.TEAM_ASSIGNMENT.AUTO then
			Lobby.TeamSelection.ClearTeam( Enum.team_t.TEAM_SPECTATOR )
		else
			local f29_local6 = Engine.LobbyGetSessionClients( Enum.LobbyModule.LOBBY_MODULE_HOST, Enum.LobbyType.LOBBY_TYPE_GAME )
			for f29_local10, f29_local11 in ipairs( f29_local6.sessionClients ) do
				if f29_local11.team == Enum.team_t.TEAM_SPECTATOR then
					Engine.LobbyHostAssignTeamToClient( f29_local11.xuid, Enum.team_t.TEAM_ALLIES )
				end
			end
		end
	end
	if f29_local5 and f29_local0 == true then
		local f29_local6 = Engine.LobbyGetSessionClients( Enum.LobbyModule.LOBBY_MODULE_HOST, Enum.LobbyType.LOBBY_TYPE_GAME )
		if f29_local2 == LuaEnums.TEAM_ASSIGNMENT.AUTO then
			for f29_local10, f29_local11 in ipairs( f29_local6.sessionClients ) do
				if f29_local11.team ~= Enum.team_t.TEAM_SPECTATOR then
					Engine.LobbyHostAssignTeamToClient( f29_local11.xuid, Enum.team_t.TEAM_FREE )
				end
			end
		else
			for f29_local10, f29_local11 in ipairs( f29_local6.sessionClients ) do
				if f29_local11.team == Enum.team_t.TEAM_FREE then
					Engine.LobbyHostAssignTeamToClient( f29_local11.xuid, Enum.team_t.TEAM_ALLIES )
				end
			end
		end
...
Lobby.TeamSelection.Clear = function ()
	...
	for f31_local0 = 1, LuaEnums.MAX_CLIENTS, 1 do
		Lobby.TeamSelection.Clients[f31_local0] = {
			team = Enum.team_t.TEAM_FREE,
			switchTeam = Enum.team_t.TEAM_FREE,
			switchTeamTime = Lobby.TeamSelection.DONT_SWITCH
		}
...
function TeamSelectionTest()
	...
	local f33_local1 = function ( f34_arg0, f34_arg1 )
		...
		for f34_local1 = 1, f34_arg0, 1 do
			f34_local0[f34_local1] = 0
		end
		table.insert( f33_local0, {
			lobbySkill = f34_arg1,
			memberList = f34_local0
		} )
	end
	...
	f33_local1( 2, 2 )
	f33_local1( 3, 1 )
	local f33_local2 = Lobby.TeamSelection.VladSplit( f33_local0, 12 )
end
