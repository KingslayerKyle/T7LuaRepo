...
CoD.freeCursorButtonPrompt.new = function ( menu, controller )
	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if f22_local0 then
					...
					if f22_local0 then
						f22_local0 = IsSelfModelValueEnumFlagSet( element, controller, "flags", Enum.LUIButtonPromptFlags.BPF_CONTEXTUAL )
					end
				end
				return f22_local0
			end
		},
		{
			...
			condition = function ( menu, element, event )
				...
				if f23_local0 then
					...
					if f23_local0 then
						f23_local0 = IsSelfModelValueEnumFlagSet( element, controller, "flags", Enum.LUIButtonPromptFlags.BPF_CONTEXTUAL )
					end
				end
				return f23_local0
			end
		},
		{
			...
			condition = function ( menu, element, event )
				...
				if f24_local0 then
					if not IsGamepad( controller ) and not ShouldHideButtonPromptForPC( element, controller ) then
						f24_local0 = IsSelfModelValueEnumFlagSet( element, controller, "flags", Enum.LUIButtonPromptFlags.BPF_CONTEXTUAL )
						if f24_local0 then
							f24_local0 = IsPC()
						end
					else
		...
		},
		{
			...
			condition = function ( menu, element, event )
				...
				if f25_local0 then
					if not IsGamepad( controller ) and not ShouldHideButtonPromptForPC( element, controller ) then
						f25_local0 = IsSelfModelValueEnumFlagSet( element, controller, "flags", Enum.LUIButtonPromptFlags.BPF_CONTEXTUAL )
						if f25_local0 then
							f25_local0 = IsPC()
						end
					else
