...
CoD.ChooseCharacterLoadout_CardBack_TransmissionContactsList.new = function ( menu, controller )
	...
	self:setTopBottom( 0, 0, 0, 435 )
	self.anyChildUsesUpdateState = true
	
	local contactsList = LUI.GridLayout.new( menu, controller, false, 0, 0, 15, 0, nil, nil, false, false, 0, 0, false, false )
	contactsList:setLeftRight( 1, 1, -192, 0 )
	contactsList:setTopBottom( 0, 0, 0, 435 )
	contactsList:setWidgetType( CoD.ChooseCharacterLoadout_CardBack_ContactListEntry )
	contactsList:setVerticalCount( 3 )
	contactsList:setSpacing( 15 )
	contactsList:linkToElementModel( self, "contactsListDatasource", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			contactsList:setDataSource( modelValue )
		end
	end )
