...
CoD.paintshopTabWidget.new = function ( menu, controller )
	...
	GameSettingsChangedIndicator:setLeftRight( 0, 0, 5.5, 50.5 )
	GameSettingsChangedIndicator:setTopBottom( 0, 0, -22.5, 22.5 )
	...
	self.clipsPerState = {
		...
		NavButton = {
			DefaultClip = function ()
				...
				self.clipFinished( text, {} )
				textDark:completeAnimation()
				self.textDark:setLeftRight( 0.5, 0.5, 0, 0 )
				self.textDark:setTopBottom( 0.5, 0.5, -14, 18 )
				self.clipFinished( textDark, {} )
				buttonText:completeAnimation()
	...
	self:mergeStateConditions( {
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f80_local0 = ShouldDisplayButton( element, controller )
				if f80_local0 then
