...
CoD.WeaponGroupsTabWidget = InheritFrom( LUI.UIElement )
CoD.WeaponGroupsTabWidget.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.WeaponGroupsTabWidget )
	self.id = "WeaponGroupsTabWidget"
	...
	breadcrumbCount:setLeftRight( 1, 1, -33.5, -6.5 )
	breadcrumbCount:setTopBottom( 0, 0, -7.5, 19.5 )
	...
	self:mergeStateConditions( {
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f79_local0 = ShouldDisplayButton( element, controller )
				if f79_local0 then
