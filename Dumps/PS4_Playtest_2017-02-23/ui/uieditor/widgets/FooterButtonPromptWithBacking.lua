...
CoD.FooterButtonPromptWithBacking = InheritFrom( LUI.UIElement )
CoD.FooterButtonPromptWithBacking.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.FooterButtonPromptWithBacking )
	self.id = "FooterButtonPromptWithBacking"
	...
	label:setLeftRight( 0, 0, 74, 122 )
	label:setTopBottom( 0, 0, 10, 37 )
	label:setText( Engine.Localize( "MENU_SELECT" ) )
