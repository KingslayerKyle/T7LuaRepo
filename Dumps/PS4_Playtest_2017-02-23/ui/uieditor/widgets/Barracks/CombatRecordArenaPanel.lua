...
CoD.CombatRecordArenaPanel.new = function ( menu, controller )
	...
	SummaryTitle:setLeftRight( 0, 0, 20, 186 )
	...
	Emblem:setImage( RegisterImage( GetArenaRankIconLG( CombatRecordGetStat( controller, "arenaBest.points", "t7_icon_rank_arena_level_21_lrg" ) ) ) )
	self:addElement( Emblem )
	self.Emblem = Emblem
	
	local RankNameBackground = LUI.UIImage.new()
	RankNameBackground:setLeftRight( 0.5, 0.5, -644, -425 )
	...
	RankBackground:setLeftRight( 0.5, 0.5, -644, -425 )
	...
	RankName:setLeftRight( 0.5, 0.5, -644, -425 )
	RankName:setTopBottom( 0, 0, 121, 151 )
	RankName:setRGB( 0, 0, 0 )
	RankName:setText( Engine.Localize( SetValueIfEmptyString( "--", CombatRecordGetStat( controller, "arenaBestPlaylist", "-" ) ) ) )
	...
	Rank:setLeftRight( 0.5, 0.5, -644, -425 )
	Rank:setTopBottom( 0, 0, 157, 199 )
	...
	CombatRecordLineDivider2:setLeftRight( 0, 0, 550, 588 )
	...
	BestCareerRankLabel:setLeftRight( 0, 0, 268, 568 )
	BestCareerRankLabel:setTopBottom( 0, 0, 81, 111 )
	...
	StartMenuframenoBG00:setLeftRight( 0.5, 0.5, -647, -422 )
	StartMenuframenoBG00:setTopBottom( 0.5, 0.5, -297, -207 )
	...
	self.resetProperties = function ()
		...
		KDRatio:completeAnimation()
		Wins:completeAnimation()
		WLRatio:completeAnimation()
	...
	self.clipsPerState = {
		DefaultState = {
			DefaultClip = function ()
				...
				local KillsFrame2 = function ( Kills, event )
					if not event.interrupted then
						Kills:beginAnimation( "keyframe", 19, false, false, CoD.TweenType.Linear )
					end
					Kills:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( Kills, event )
					else
						Kills:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
				
				Kills:beginAnimation( "keyframe", 39, false, false, CoD.TweenType.Linear )
				Kills:setAlpha( 0 )
				Kills:registerEventHandler( "transition_complete_keyframe", KillsFrame2 )
				local WinsFrame2 = function ( Wins, event )
					if not event.interrupted then
						Wins:beginAnimation( "keyframe", 19, false, false, CoD.TweenType.Linear )
					end
					Wins:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( Wins, event )
					else
						Wins:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
				
				Wins:beginAnimation( "keyframe", 59, false, false, CoD.TweenType.Linear )
				Wins:setAlpha( 0 )
				Wins:registerEventHandler( "transition_complete_keyframe", WinsFrame2 )
				local ScoreFrame2 = function ( Score, event )
					if not event.interrupted then
						Score:beginAnimation( "keyframe", 20, false, false, CoD.TweenType.Linear )
					end
					Score:setAlpha( 1 )
					if event.interrupted then
				...
				Score:setAlpha( 0 )
				Score:registerEventHandler( "transition_complete_keyframe", ScoreFrame2 )
				local KDRatioFrame2 = function ( KDRatio, event )
					if not event.interrupted then
						KDRatio:beginAnimation( "keyframe", 19, false, false, CoD.TweenType.Linear )
					end
					KDRatio:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( KDRatio, event )
					else
						KDRatio:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
				
				KDRatio:beginAnimation( "keyframe", 50, false, false, CoD.TweenType.Linear )
				KDRatio:setAlpha( 0 )
				KDRatio:registerEventHandler( "transition_complete_keyframe", KDRatioFrame2 )
				local WLRatioFrame2 = function ( WLRatio, event )
					if not event.interrupted then
						WLRatio:beginAnimation( "keyframe", 20, false, false, CoD.TweenType.Linear )
					end
					WLRatio:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( WLRatio, event )
					else
						WLRatio:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
				
				WLRatio:beginAnimation( "keyframe", 70, false, false, CoD.TweenType.Linear )
				WLRatio:setAlpha( 0 )
				WLRatio:registerEventHandler( "transition_complete_keyframe", WLRatioFrame2 )
				local SPMFrame2 = function ( SPM, event )
					if not event.interrupted then
						SPM:beginAnimation( "keyframe", 19, false, false, CoD.TweenType.Linear )
					end
					SPM:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( SPM, event )
					else
						SPM:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
				
				SPM:beginAnimation( "keyframe", 90, false, false, CoD.TweenType.Linear )
				SPM:setAlpha( 0 )
				SPM:registerEventHandler( "transition_complete_keyframe", SPMFrame2 )
