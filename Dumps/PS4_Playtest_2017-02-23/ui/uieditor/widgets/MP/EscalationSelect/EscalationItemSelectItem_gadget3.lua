...
CoD.EscalationItemSelectItem_gadget3 = InheritFrom( LUI.UIElement )
CoD.EscalationItemSelectItem_gadget3.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.EscalationItemSelectItem_gadget3 )
	self.id = "EscalationItemSelectItem_gadget3"
	self.soundSet = "none"
