...
CoD.EscalationItemSelectItem_gadget1 = InheritFrom( LUI.UIElement )
CoD.EscalationItemSelectItem_gadget1.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.EscalationItemSelectItem_gadget1 )
	self.id = "EscalationItemSelectItem_gadget1"
	self.soundSet = "none"
