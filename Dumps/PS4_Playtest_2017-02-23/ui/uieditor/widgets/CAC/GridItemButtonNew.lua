...
CoD.GridItemButtonNew = InheritFrom( LUI.UIElement )
CoD.GridItemButtonNew.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.GridItemButtonNew )
	self.id = "GridItemButtonNew"
	...
	GridItemBGBGlow:setLeftRight( 0, 1, -32, 32 )
	GridItemBGBGlow:setTopBottom( 0, 1, -32, 32 )
	...
	tokenUnlock:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if not IsCampaign() then
					f6_local0 = ShouldShowTokenIconOnItem( menu, element, controller )
					if f6_local0 then
		...
		},
		{
			...
			condition = function ( menu, element, event )
				...
				if f7_local0 then
					f7_local0 = ShouldShowTokenIconOnItem( menu, element, controller )
					if f7_local0 then
	...
	LabelButton:mergeStateConditions( {
		{
			stateName = "Hidden",
			condition = function ( menu, element, event )
				return IsCACItemBubbleGum( menu, element, controller )
			end
		}
	} )
	LabelButton:linkToElementModel( LabelButton, "itemIndex", true, function ( model )
		menu:updateElementState( LabelButton, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
			modelName = "itemIndex"
	...
	LabelButton:setLeftRight( 0, 0, 3, 159 )
	LabelButton:setTopBottom( 1, 1, -33, -3 )
	...
	lockedIcon:setLeftRight( 0.5, 0.5, -23, 22 )
	...
	HeaderLabel:setLeftRight( 0, 0, 3, 159 )
	HeaderLabel:setTopBottom( 1, 1, -156, -126 )
	HeaderLabel:setAlpha( 0.6 )
	...
	HeaderLabel:linkToElementModel( self, "headerName", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			HeaderLabel.itemName:setText( Engine.Localize( modelValue ) )
		end
	end )
	...
	RestrictedItemWarning:mergeStateConditions( {
		{
			stateName = "Visible",
			condition = function ( menu, element, event )
				return ItemIsBanned( menu, element, controller ) and not ItemRestrictionVoteActive()
			end
		}
	} )
	...
	RestrictedItemWarning:setLeftRight( 0, 0, -1, 164 )
	RestrictedItemWarning:setTopBottom( 0, 0, 0, 142 )
	...
	GridItemUpgradeIconWidget:linkToElementModel( GridItemUpgradeIconWidget, "itemIndex", true, function ( model )
	...
	GridItemUpgradeIconWidget:setLeftRight( 0, 0, 2.5, 29.5 )
	GridItemUpgradeIconWidget:setTopBottom( 0, 0, 2.5, 29.5 )
	...
	local DLCLabel = CoD.DLCLabel.new( menu, controller )
	DLCLabel:setLeftRight( 1, 1, -70.5, 22.5 )
	DLCLabel:setTopBottom( 0.5, 0.5, 11, 89 )
	DLCLabel:setScale( 0.8 )
	DLCLabel:linkToElementModel( self, nil, false, function ( model )
		DLCLabel:setModel( model, controller )
	end )
	self:addElement( DLCLabel )
	self.DLCLabel = DLCLabel
	...
	self.clipsPerState = {
		...
		New = {
			...
			GainFocus = function ()
				...
				local GridItemBGBGlowFrame2 = function ( GridItemBGBGlow, event )
					if not event.interrupted then
						GridItemBGBGlow:beginAnimation( "keyframe", 70, false, false, CoD.TweenType.Linear )
					end
					GridItemBGBGlow:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( GridItemBGBGlow, event )
					else
						GridItemBGBGlow:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
				
				GridItemBGBGlow:completeAnimation()
				self.GridItemBGBGlow:setAlpha( 0 )
				GridItemBGBGlowFrame2( GridItemBGBGlow, {} )
				local FocusBarTFrame2 = function ( FocusBarT, event )
					local FocusBarTFrame3 = function ( FocusBarT, event )
						local FocusBarTFrame4 = function ( FocusBarT, event )
							local FocusBarTFrame5 = function ( FocusBarT, event )
		...
		NotAvailable = {
			DefaultClip = function ()
				...
				self.clipFinished( BoxButtonLrgIdle, {} )
				GridItemBGBGlow:completeAnimation()
				self.GridItemBGBGlow:setAlpha( 0.15 )
				self.clipFinished( GridItemBGBGlow, {} )
				itemImage:completeAnimation()
	...
	self:mergeStateConditions( {
		{
			stateName = "Equipped",
			condition = function ( menu, element, event )
				return IsCACItemEquipped( menu, element, controller ) and not IsInPermanentUnlockMenu( controller )
			end
		},
		{
			stateName = "New",
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f678_local0 = IsInPermanentUnlockMenu( controller )
				if f678_local0 then
					if not IsPermanentlyUnlocked( element, controller ) then
						f678_local0 = HavePermanentUnlockTokens( controller )
						if f678_local0 then
							f678_local0 = IsCACItemPurchased( element, controller )
						end
					else
						f678_local0 = false
					end
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( self )
		...
		self.GridItemUpgradeIconWidget:close()
