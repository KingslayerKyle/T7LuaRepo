...
CoD.CustomizationListButton_NB.new = function ( menu, controller )
	...
	LabelButton:setLeftRight( 0, 0, 109, 327 )
	LabelButton:setTopBottom( 0.5, 0.5, -14, 16 )
	...
	itemImage:linkToElementModel( itemImage, "paintjobSlot", true, function ( model )
		menu:updateElementState( itemImage, {
	...
	itemImage:linkToElementModel( itemImage, "paintjobIndex", true, function ( model )
		menu:updateElementState( itemImage, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
	...
	itemImage:setTopBottom( 0.5, 0.5, -48, 48 )
	...
	itemImage:linkToElementModel( self, "paintjobSlotAndIndex", true, function ( model )
		...
		if modelValue then
			itemImage.paintjobExtraCamRender:setupWCPaintjobIconExtraCamRender( GetPaintshopExtraCamParameters( controller, modelValue ) )
		end
	end )
	self:addElement( itemImage )
	self.itemImage = itemImage
	
	self.LabelButton:linkToElementModel( self, "displayText", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
