...
LUI.createMenu.MyShowcase_Clips = function ( controller )
	...
	RecentGamesList:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "FileshareRoot.PublishMode" ), function ( model )
	...
	RecentGamesList:linkToElementModel( RecentGamesList, "fileCategory", true, function ( model )
		local element = RecentGamesList
		local event = {
			controller = controller,
			name = "model_validation",
	...
	RecentGamesList:linkToElementModel( RecentGamesList, "isPublishNew", true, function ( model )
		local element = RecentGamesList
		local event = {
			controller = controller,
			name = "model_validation",
	...
	RecentGamesList:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "fileshareRoot.itemsCount" ), function ( model )
		local element = RecentGamesList
		local event = {
			controller = controller,
			name = "model_validation",
	...
	RecentGamesList:registerEventHandler( "list_item_lose_focus", function ( element, event )
		local retVal = nil
	...
	SelectedFilmInformation:subscribeToGlobalModel( controller, "FileshareRoot", "showFileDetails", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			SelectedFilmInformation:setAlpha( modelValue )
		end
	end )
	...
	FilesharePublishDetails:setLeftRight( 0, 0, 1155, 1644 )
	FilesharePublishDetails:setTopBottom( 0, 0, 206.5, 917.5 )
	...
	FileshareNoContent:setLeftRight( 0, 0, 372.5, 747.5 )
	FileshareNoContent:setTopBottom( 0, 0, 473, 563 )
