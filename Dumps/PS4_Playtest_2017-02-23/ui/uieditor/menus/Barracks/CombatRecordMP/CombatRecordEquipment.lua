...
LUI.createMenu.CombatRecordEquipment = function ( controller )
	...
	MenuFrame.cac3dTitleIntermediary0.FE3dTitleContainer0.MenuTitle.TextBox1.FeatureIcon.FeatureIcon:setImage( RegisterImage( "t7_mp_icon_equipment" ) )
	self:addElement( MenuFrame )
	self.MenuFrame = MenuFrame
	
	local ItemList = LUI.UIList.new( self, controller, 2, 0, nil, false, false, 0, 0, false, false )
	ItemList:makeFocusable()
	ItemList:setLeftRight( 0.5, 0.5, -844, -424 )
	...
	KillsStat:setLeftRight( 0.5, 0.5, -345, -143 )
	...
	KillsPerUseStat:setLeftRight( 0.5, 0.5, 43, 229 )
	KillsPerUseStat:setTopBottom( 0, 0, 268, 352 )
	...
	UsedStat:setLeftRight( 0.5, 0.5, -143, 43 )
	UsedStat:setTopBottom( 0, 0, 268, 352 )
	...
	WeaponNameTitle:setLeftRight( 0.5, 0.5, -346, 230 )
	...
	UsedStatLabel:setLeftRight( 1, 1, -1524, -1384 )
	UsedStatLabel:setTopBottom( 0, 0, 184, 222 )
	...
	EquipmentLabel:setLeftRight( 0, 0, 114, 218 )
	EquipmentLabel:setTopBottom( 0, 0, 184, 222 )
	...
	CombatRecordStatDescription:setLeftRight( 0, 0, 615, 1189 )
	CombatRecordStatDescription:setTopBottom( 0, 0, 352, 390 )
	...
	KillsStat:linkToElementModel( ItemList, "statComparison", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			KillsStat.ComparedStatValue:setText( Engine.Localize( modelValue ) )
		end
	end )
	KillsPerUseStat:linkToElementModel( ItemList, "lethalAlpha", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			KillsPerUseStat:setAlpha( modelValue )
		end
	end )
	KillsPerUseStat:linkToElementModel( ItemList, "itemIndex", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			KillsPerUseStat.StatValue:setText( Engine.Localize( CombatRecordGetTwoStatRatioForItemIndex( controller, "kills", "used", modelValue ) ) )
		end
	end )
	KillsPerUseStat:linkToElementModel( ItemList, "itemIndex", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			KillsPerUseStat.ComparedStatValue:setText( Engine.Localize( CombatRecordGetComparisonTwoStatRatioForItemIndex( controller, "kills", "used", modelValue ) ) )
		end
	end )
	...
	CombatRecordStatDescription:linkToElementModel( ItemList, "tacticalAlpha", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			CombatRecordStatDescription:setAlpha( modelValue )
		end
	end )
	CombatRecordStatDescription:linkToElementModel( ItemList, "name", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			CombatRecordStatDescription.StatDescription:setText( Engine.Localize( AppendString( "_STAT_DESC", modelValue ) ) )
		end
	end )
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( self )
		...
		self.CombatRecordStatDescription:close()
