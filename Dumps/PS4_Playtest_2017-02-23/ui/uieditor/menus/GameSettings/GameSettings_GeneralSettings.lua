...
local PrepareGeneralSettingOptions = function ( controller )
	local optionsTable = {}
	local settingInfo = nil
	for index, setting in ipairs( CoD.OptionsUtility.GeneralSettings ) do
		settingInfo = CoD.OptionsUtility.GameSettings[setting]
		table.insert( optionsTable, CoD.OptionsUtility.CreateListOptions( controller, settingInfo.name, settingInfo.hintText, setting, settingInfo, "GeneralSettingsOptionsList_" .. setting ) )
	end
	return optionsTable
end
...
DataSources.GeneralSettings = DataSourceHelpers.ListSetup( "GeneralSettings", PrepareGeneralSettingOptions, nil, nil, SettingsUpdate )
