...
local PreLoadFunc = function ( self, controller )
	...
	self.restoreState = function ( self )
		...
		if focusedWidget then
			...
			DataSources.Unlockables.setCurrentFilterItem( primaryGroupCategory[1].weapon_category )
...
local PostLoadFunc = function ( self, controller )
	if CoD.PrestigeUtility.isInPermanentUnlockMenu then
		self.weaponCategoryList.Tabs.grid:setDataSource( "GunsmithWeaponTabType" )
		self.selectMenuWidget.background.cac3dTitleIntermediary0.FE3dTitleContainer0.MenuTitle.TextBox1.Label0:setText( Engine.Localize( "MENU_WEAPONS_CAPS" ) )
	end
...
LUI.createMenu.PrimaryWeaponSelect = function ( controller )
	...
	LeftPanel:setTopBottom( 0, 1, 116, 36 )
	...
	CategoryListPanel:setTopBottom( 0, 0, 129, 187 )
	...
	self:AddButtonCallbackFunction( weaponList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( element, menu, controller, model )
		if not IsCACItemLocked( menu, element, controller ) and not IsCACItemPurchased( element, controller ) and not IsCACHaveTokens( controller ) and not IsInPermanentUnlockMenu( controller ) and not IsSelfModelValueTrue( element, controller, "isBMClassified" ) then
			SetUnlockConfirmationInfo( element, controller )
			OpenPopup( self, "OutOfUnlockTokens", controller, "", "" )
			return true
		elseif not IsCACItemLocked( menu, element, controller ) and not IsCACItemPurchased( element, controller ) and IsCACHaveTokens( controller ) and not IsInPermanentUnlockMenu( controller ) and not IsSelfModelValueTrue( element, controller, "isBMClassified" ) then
			OpenUnlockClassItemDialog( menu, element, controller )
			return true
		elseif not IsCACItemLocked( menu, element, controller ) and IsCACItemPurchased( element, controller ) and IsCACWeaponVariantAvailable( element, controller ) and not IsInPermanentUnlockMenu( controller ) and not IsSelfModelValueTrue( element, controller, "isBMClassified" ) then
			...
			return true
		elseif not IsCACItemLocked( menu, element, controller ) and IsCACItemPurchased( element, controller ) and not IsCACWeaponVariantAvailable( element, controller ) and not IsInPermanentUnlockMenu( controller ) and not IsSelfModelValueTrue( element, controller, "isBMClassified" ) then
			SetClassItem( self, element, controller )
			return true
		elseif IsInPermanentUnlockMenu( controller ) and not IsPermanentlyUnlocked( element, controller ) and HavePermanentUnlockTokens( controller ) and not IsSelfModelValueTrue( element, controller, "isBMClassified" ) then
	...
	weaponCategoryList:setLeftRight( 0, 1, 0, 1920 )
	weaponCategoryList:setTopBottom( 0, 0, 128, 189 )
	...
	local weaponListCounter = CoD.verticalCounter.new( self, controller )
	weaponListCounter:setLeftRight( 0, 0, 174, 474 )
	weaponListCounter:setTopBottom( 0, 0, 963, 1001 )
	...
	self.clipsPerState = {
		...
		ShowVariants = {
			...
			DefaultState = function ()
				...
				weaponListCounter:beginAnimation( "keyframe", 100, false, false, CoD.TweenType.Linear )
				weaponListCounter:setAlpha( 0 )
				weaponListCounter:registerEventHandler( "transition_complete_keyframe", weaponListCounterFrame2 )
			end
		}
	}
	...
	weaponListCounter.id = "weaponListCounter"
	self:processEvent( {
		name = "menu_loaded",
		controller = controller
	} )
