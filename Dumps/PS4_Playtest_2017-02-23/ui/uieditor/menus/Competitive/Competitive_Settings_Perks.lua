...
local PreparePerkOptions = function ( controller )
	local optionsTable = {}
	local items = {}
	...
	local PerkSort = function ( a, b )
		...
		if itemInfoA.loadoutSlot == itemInfoB.loadoutSlot then
			return a.models.name < b.models.name
		else
			return itemInfoA.loadoutSlot < itemInfoB.loadoutSlot
		end
	end
	
	table.sort( optionsTable, PerkSort )
	return optionsTable
end
...
DataSources.CompetitiveSettingsPerkOptions = DataSourceHelpers.ListSetup( "CompetitiveSettingsPerkOptions", PreparePerkOptions, nil, nil, ItemOptionsUpdate )
