...
local PrepareScorestreakOptions = function ( controller )
	local optionsTable = {}
	local items = {}
	for itemIndex = 0, 255, 1 do
		if Engine.ItemIndexValid( itemIndex ) then
			local itemInfo = Engine.GetUnlockableInfoByIndex( itemIndex )
			if 0 <= itemInfo.allocation and LUI.startswith( itemInfo.loadoutSlot, "killstreak" ) then
				local image = itemInfo.image .. "_menu_large"
	...
	local ScorestreakSort = function ( a, b )
		local itemInfoA = Engine.GetUnlockableInfoByIndex( a.models.itemIndex )
		local itemInfoB = Engine.GetUnlockableInfoByIndex( b.models.itemIndex )
		return itemInfoA.momentum < itemInfoB.momentum
	end
	
	table.sort( optionsTable, ScorestreakSort )
	return optionsTable
end
...
DataSources.CompetitiveSettingsScorestreakOptions = DataSourceHelpers.ListSetup( "CompetitiveSettingsScorestreakOptions", PrepareScorestreakOptions, nil, nil, SettingsUpdate )
local PreLoadFunc = function ( self, controller )
	self.disablePopupOpenCloseAnim = true
end

LUI.createMenu.Competitive_Settings_Scorestreaks = function ( controller )
	local self = CoD.Menu.NewForUIEditor( "Competitive_Settings_Scorestreaks" )
	if PreLoadFunc then
	...
	self.buttonModel = Engine.CreateModel( Engine.GetModelForController( controller ), "Competitive_Settings_Scorestreaks.buttonPrompts" )
	...
	GameSettingsBackground.MenuFrame.titleLabel:setText( Engine.Localize( "MENU_SCORESTREAKS_CAPS" ) )
	GameSettingsBackground.MenuFrame.cac3dTitleIntermediary0.FE3dTitleContainer0.MenuTitle.TextBox1.Label0:setText( Engine.Localize( "MENU_SCORESTREAKS_CAPS" ) )
	...
	Options:setDataSource( "CompetitiveSettingsScorestreakOptions" )
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( self )
		...
		Engine.UnsubscribeAndFreeModel( Engine.GetModel( Engine.GetModelForController( controller ), "Competitive_Settings_Scorestreaks.buttonPrompts" ) )
	end )
