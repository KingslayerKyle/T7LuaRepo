...
local PostLoadFunc = function ( self, controller )
	...
	if IsMaxPrestigeLevel( controller ) then
		self.EnterPrestige.navigation.down = self.FreshStartButton
		self.FreshStartButton.navigation.left = nil
	end
...
LUI.createMenu.Prestige_Main = function ( controller )
	...
	HiddenButtonBacking0:setLeftRight( 0, 0, 498, 858 )
	HiddenButtonBacking0:setTopBottom( 1, 1, -387, -87 )
	...
	HiddenButtonBacking1:setLeftRight( 0, 0, 880, 1240 )
	HiddenButtonBacking1:setTopBottom( 1, 1, -387, -87 )
	...
	EnterPrestige:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if not IsMaxPrestigeLevel( controller ) then
					f8_local0 = not IsAtXPCap( controller )
				else
					f8_local0 = false
				end
				return f8_local0
	...
	EnterPrestige:setLeftRight( 0, 0, 138, 1198 )
	EnterPrestige:setTopBottom( 0, 0, 166, 613 )
	EnterPrestige:subscribeToGlobalModel( controller, "PrestigeStats", "icon", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			EnterPrestige.PrestigeIcon:setImage( RegisterImage( GetRankIconLarge( modelValue ) ) )
		end
	end )
	EnterPrestige:subscribeToGlobalModel( controller, "PrestigeStats", "backgroundId", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			EnterPrestige.PrestigeCallingCard:setImage( RegisterImage( GetBackgroundByID( modelValue ) ) )
		end
	end )
	EnterPrestige:subscribeToGlobalModel( controller, "PrestigeStats", "plevel", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			EnterPrestige.PrestigeButtonTitle.ButtonTitle:setText( GetPrestigeTitleText( controller, modelValue ) )
		end
	end )
	...
	self:AddButtonCallbackFunction( EnterPrestige, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( element, menu, controller, model )
		if not IsMaxPrestigeLevel( controller ) and IsAtXPCap( controller ) and not IsElementInState( element, "PrestigeMasterLocked" ) then
			...
			return true
		elseif IsMultiplayerCombatRecordMode() and IsMaxPrestigeLevel( controller ) then
			OpenCustomizePrestigeMenu( self, controller, menu )
			return true
		elseif IsZombiesCombatRecordMode() and IsMaxPrestigeLevel( controller ) then
			OpenCustomizePrestigeMenu( self, controller, menu )
			return true
		else
	...
	PermanentUnlocksButton:setLeftRight( 0, 0, 118, 498 )
	PermanentUnlocksButton:setTopBottom( 1, 1, -411, -109 )
	PermanentUnlocksButton:setScale( 0.9 )
	PermanentUnlocksButton.ButtonIcon:setImage( RegisterImage( "t7_icons_prestige_mp_permanentunlock" ) )
	...
	RefundButton:setLeftRight( 0, 0, 478, 858 )
	RefundButton:setTopBottom( 1, 1, -411, -109 )
	RefundButton:setScale( 0.9 )
	RefundButton.ButtonIcon:setImage( RegisterImage( "t7_icons_prestige_mp_refund" ) )
	...
	FreshStartButton:setLeftRight( 0, 0, 838, 1218 )
	FreshStartButton:setTopBottom( 1, 1, -411, -109 )
	FreshStartButton:setScale( 0.9 )
	FreshStartButton.ButtonIcon:setImage( RegisterImage( "t7_icons_prestige_mp_freshstart" ) )
	...
	PermanentUnlockTokens:setLeftRight( 0, 0, 150, 406 )
	PermanentUnlockTokens:setTopBottom( 1, 1, -198, -168 )
	...
	PrestigeAwardsTitle:setLeftRight( 0, 0, 142, 670 )
	PrestigeAwardsTitle:setTopBottom( 0, 0, 637, 673 )
	...
	Title:setLeftRight( 0, 0, 155, 1201 )
	Title:setTopBottom( 1, 1, -543, -513 )
	...
	self.resetProperties = function ()
		...
		PermanentUnlocksButton:setLeftRight( 0, 0, 118, 498 )
		PermanentUnlocksButton:setTopBottom( 1, 1, -411, -109 )
		PermanentUnlocksButton:setAlpha( 1 )
		RefundButton:setAlpha( 1 )
		FreshStartButton:setLeftRight( 0, 0, 838, 1218 )
		FreshStartButton:setTopBottom( 1, 1, -411, -109 )
		HiddenButtonBacking0:setLeftRight( 0, 0, 498, 858 )
		HiddenButtonBacking0:setTopBottom( 1, 1, -387, -87 )
	...
	self.clipsPerState = {
		...
		PrestigeMaster = {
			DefaultClip = function ()
				...
				HiddenButtonBacking0:completeAnimation()
				self.HiddenButtonBacking0:setLeftRight( 0, 0, 501, 837 )
				self.HiddenButtonBacking0:setTopBottom( 1, 1, -393.5, -126.5 )
				self.HiddenButtonBacking0:setAlpha( 0.2 )
				self.clipFinished( HiddenButtonBacking0, {} )
				HiddenButtonBacking1:completeAnimation()
				self.HiddenButtonBacking1:setLeftRight( 0, 0, 860, 1196 )
				self.HiddenButtonBacking1:setTopBottom( 1, 1, -393.5, -126.5 )
				...
				FreshStartButton:completeAnimation()
				self.FreshStartButton:setLeftRight( 0, 0, 118, 498 )
				self.FreshStartButton:setTopBottom( 1, 1, -411, -109 )
		...
		DefaultStateZM = {
			DefaultClip = function ()
				...
				self:setupElementClipCounter( 5 )
				PermanentUnlocksButton:completeAnimation()
				self.PermanentUnlocksButton:setLeftRight( 0, 0, 100, 684 )
				self.PermanentUnlocksButton:setTopBottom( 1, 1, -391, -69 )
				self.clipFinished( PermanentUnlocksButton, {} )
				RefundButton:completeAnimation()
				self.RefundButton:setAlpha( 0 )
				self.clipFinished( RefundButton, {} )
				FreshStartButton:completeAnimation()
				self.FreshStartButton:setLeftRight( 0, 0, 649.5, 1264.5 )
				self.FreshStartButton:setTopBottom( 1, 1, -391, -69 )
				self.clipFinished( FreshStartButton, {} )
				PermanentUnlockTokens:completeAnimation()
				self.PermanentUnlockTokens:setLeftRight( 0, 0, 148, 404 )
				self.PermanentUnlockTokens:setTopBottom( 1, 1, -157, -127 )
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( self )
		...
		self.EnterPrestige:close()
		self.PermanentUnlocksButton:close()
		self.RefundButton:close()
		self.FreshStartButton:close()
		self.InfopaneTop:close()
		self.InfopaneBottom:close()
		self.PrestigeAwardsTitle:close()
