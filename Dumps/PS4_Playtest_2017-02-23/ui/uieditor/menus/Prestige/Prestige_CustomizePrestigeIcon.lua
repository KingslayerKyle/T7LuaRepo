...
DataSources.PrestigeIconButtonList = DataSourceHelpers.ListSetup( "PrestigeIconButtonList", function ( controller )
	local iconTable = {}
	local mode = CoD.PrestigeUtility.GetPrestigeGameMode()
	local paragonIcons = Engine.GetParagonIconTable( mode )
	local currentStats = Engine.GetPlayerStats( controller, CoD.STATS_LOCATION_NORMAL, mode )
	local paragonRank = currentStats.PlayerStatsList.PARAGON_RANK.StatValue:get()
	local currentParagonIconId = currentStats.PlayerStatsList.PARAGON_ICON_ID.StatValue:get()
	if currentParagonIconId == CoD.PrestigeUtility.INVALID_PARAGON_ICON_ID then
		currentParagonIconId = CoD.PrestigeUtility.DEFAULT_PARAGON_ICON_ID
	end
	for _, icon in pairs( paragonIcons ) do
		local f1_local3 = Engine.Localize( icon.description )
		local f1_local4 = paragonRank < icon.unlockParagonLvl
		if f1_local4 then
			f1_local3 = Engine.Localize( "PERKS_UNLOCKED_AT_LV", Engine.GetParagonRankDisplayLevel( icon.unlockParagonLvl, mode ) )
		end
		table.insert( iconTable, {
			models = {
				iconId = icon.id,
				buttonTitle = Engine.Localize( icon.iconTitle ),
				icon = icon.imageName,
				iconTitle = Engine.Localize( icon.iconTitle, icon.plevel ),
				gameTitle = Engine.Localize( icon.gameTitle ),
				description = f1_local3,
				isLocked = f1_local4
			},
			properties = {
				selectIndex = icon.id == currentParagonIconId
			}
		} )
	end
	return iconTable
...
LUI.createMenu.Prestige_CustomizePrestigeIcon = function ( controller )
	...
	PrestigeIconProfiler:subscribeToGlobalModel( controller, "StorageGlobal", "stats_mp", function ( model )
		...
		if modelValue then
			PrestigeIconProfiler.RankText:setRGB( SetToParagonColorIfPrestigeMasterFromStorage( controller, "mp", 148, 162, 165, modelValue ) )
		end
	end )
	PrestigeIconProfiler:subscribeToGlobalModel( controller, "StorageGlobal", "stats_mp", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			PrestigeIconProfiler.RankText:setText( LevelStringFromStorage( controller, "mp", modelValue ) )
		end
	end )
	PrestigeIconProfiler:subscribeToGlobalModel( controller, "StorageGlobal", "stats_mp", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			PrestigeIconProfiler.PrestigeMasterTierWidget.ParagonStars:setHorizontalCount( GetPrestigeMasterTierCountFromStorage( controller, "mp", modelValue ) )
		end
	end )
