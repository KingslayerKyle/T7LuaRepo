			...
			},
			{
				...
				waitForRepeatCondition = function ()
					...
					if activeWidget then
						if not IsHeroLocked( activeWidget, controller ) and activeWidget.heroIndex ~= Engine.GetEquippedHero( controller, Enum.eModes.MODE_MULTIPLAYER ) then
				...
				fn = function ()
					...
					if not activeWidget or activeWidget == lastActiveWidget then
						...
						Rat.SimulateButtonPress( menu, controller, Enum.LUIButton.LUI_KEY_RB )
					end
				end
			},
			{
				...
				fn = function ()
					...
					Rat.SimulateButtonPress( menu, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS )
					Rat.DisableGoBack = false
					if activeWidget.heroIndex ~= Engine.GetEquippedHero( controller, Enum.eModes.MODE_MULTIPLAYER ) then
