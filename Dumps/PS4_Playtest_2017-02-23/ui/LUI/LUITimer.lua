...
LUI.UITimer.create = function ( interval, disposable )
	local timer = LUI.UIElement.new( 0, 0, 0, 1, 0, 0, 0, 1 )
	timer.id = "LUITimer"
...
LUI.UITimer.Reset = function ( self )
	self:registerEventHandler( "transition_complete_keyframe", nil )
	self:beginAnimation( "keyframe", self.interval, false, false, CoD.TweenType.Linear )
	self:registerEventHandler( "transition_complete_keyframe", LUI.UITimer.Tick )
end
