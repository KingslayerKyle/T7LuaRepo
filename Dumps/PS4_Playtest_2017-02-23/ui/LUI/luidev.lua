...
LUI.DEV.InitEventHandlers = function ( root )
	if root ~= nil then
		root:registerEventHandler( "hide_button_prompts", LUI.DEV.HideButtonPrompts )
		root:registerEventHandler( "debug_print_element", LUI.DEV.DebugPrintElement )
		root:registerEventHandler( "debug_materials_and_images", LUI.DEV.DebugAllMaterials )
		root:registerEventHandler( "override_visibility_bit", LUI.DEV.OverrideVisibilityBit )
	end
end
...
LUI.DEV.ErrorIfModelsDoNotMatch = function ( model1, model2 )
	if Engine.GetModelPath( model1 ) ~= Engine.GetModelPath( model2 ) then
		error( "unsubscribing " .. Engine.GetModelPath( model1 ) .. ", expected " .. Engine.GetModelPath( model2 ) )
		return true
	else
		return false
	end
...
LUI.DEV.PrintCallstack = function ( message )
	local debugTrace = ""
...
LUI.DEV.AddDebugBackground = function ( element, color )
	local red = 1
	local green = 1
	local blue = 1
	local alpha = 0.25
