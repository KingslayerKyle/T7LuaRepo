...
LUI.splitString = function ( str, delimiters )
	local result = {}
	local pattern = "([^" .. delimiters .. "]+)"
	string.gsub( str, pattern, function ( val )
...
LUI.getTableFromPath = function ( stringPath, usingTable )
	local f9_local0 = usingTable
	local currentTable = _G
	local f9_local1 = f9_local0 or currentTable
	for tableString in string.gmatch( stringPath, "[^.]+" ) do
		f9_local1 = f9_local1[tableString]
...
LUI.IterateTableBySortedKeys = function ( tableToIterate, sortFn, filterFn )
	...
	local iter = function ()
		...
		if sortedKeyList[f10_local0] == nil then
			return nil
		else
			return sortedKeyList[f10_local0], tableToIterate[sortedKeyList[f10_local0]]
		end
	end
...
function CountFieldsHelper( data, seen )
	...
	for key, value in pairs( data ) do
		if type( key ) == "table" then
			sum = sum + CountFieldsHelper( key, seen )
		end
		sum = sum + 1
	end
	return sum
...
function DisableGlobals()
	...
	if Dvar.cg_development:get() then
		globalMetatable.__index = function ( t, key )
			if key == "PreLoadFunc" or key == "PostLoadFunc" or key == "LobbyVM" then
				return nil
			end
			local debugTrace = ""
