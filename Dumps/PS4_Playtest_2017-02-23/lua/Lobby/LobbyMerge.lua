...
Lobby.Merge.IsMergingAllowed = function ( lobbyType )
	...
	elseif Engine.GetSessionStatus and Engine.GetSessionStatus( lobbyType ) ~= Enum.SessionStatus.SESSION_STATUS_IDLE then
		...
		if timerStatus == Lobby.Timer.LOBBY_STATUS.STARTING or timerStatus == Lobby.Timer.LOBBY_STATUS.IDLE or timerStatus == Lobby.Timer.LOBBY_STATUS.NOT_RUNNING or timerStatus == Lobby.Timer.LOBBY_STATUS.POST_GAME or timerStatus == Lobby.Timer.LOBBY_STATUS.FIND_NEW_LOBBY or timerStatus == Lobby.Timer.MATCH_START_INVALID then
			return false
		else
			return true
		end
...
Lobby.Merge.Update = function ()
	...
	elseif Lobby.Merge.mergeData.merging == false then
		...
		if Engine.IsDedicatedServer() == true then
			local process = Lobby.Process.MergePublicDedicatedLobby( controller )
...
Lobby.Merge.Pump = function ()
	...
	local isActive = Engine.IsLobbyActive( Enum.LobbyModule.LOBBY_MODULE_HOST, Enum.LobbyType.LOBBY_TYPE_GAME )
	if isActive == false then
		Lobby.Merge.mergeData = nil
		return 
	end
	local timerType = Lobby.Timer.GetTimerType()
	if timerType ~= LuaEnum.TIMER_TYPE.AUTO_MP then
		Lobby.Merge.mergeData = nil
		return 
	end
	local curScreenId = Engine.GetLobbyUIScreen()
	local f4_local0 = LobbyData.GetLobbyMenuByName( LuaEnum.UI.ONLINE_MP_PUBLIC )
	if curScreenId ~= f4_local0.id then
		f4_local0 = LobbyData.GetLobbyMenuByName( LuaEnum.UI.ONLINE_MP_ARENA )
		if curScreenId ~= f4_local0.id then
			Lobby.Merge.mergeData = nil
			return 
		end
	end
	if Lobby.Merge.IsMergingAllowed( Enum.LobbyType.LOBBY_TYPE_GAME ) then
		if Lobby.Merge.mergeData == nil then
