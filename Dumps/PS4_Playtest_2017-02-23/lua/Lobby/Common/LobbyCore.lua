...
Lobby.Core.AddHandler = function ( fromTargetName, toTargetName, handlerType, handler )
	local fromTarget = LobbyData.GetLobbyMenuByName( fromTargetName )
	local toTarget = LobbyData.GetLobbyMenuByName( toTargetName )
...
Lobby.Core.AddProcessForward = function ( fromTargetName, toTargetName, handler )
	...
	Lobby.Core.AddHandler( fromTargetName, toTargetName, handlerType, handler )
end

Lobby.Core.AddProcessBack = function ( fromTargetName, toTargetName, handler )
	...
	if Lobby.Navigation[fromTarget.name] ~= nil then
		...
		if backNavTable ~= nil then
			...
			if backMenuName ~= nil then
				...
				return LobbyData.GetLobbyMenuByName( backMenuName ), handler
			end
		end
	end
	return nil, nil
