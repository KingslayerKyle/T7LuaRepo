-- a7fe672bfb469e7cab08f32a2d8cef2d
...
Lobby.GMLOC_OFF = string.char( 21 )
...
Lobby.Actions.ExecuteScriptWithReturn = function ( f11_arg0, f11_arg1, f11_arg2, f11_arg3, f11_arg4 )
	return {
		name = "ExecuteScript",
		inlineFunction = f11_arg0,
		startFuncPtr = function ( f12_arg0 )
			local f12_local0 = f12_arg0.inlineFunction( f11_arg4 )
	...
	return {
		...
		startFuncPtr = function ( f46_arg0 )
			...
			elseif f45_arg2 == true or Engine.GetLobbyUIScreen() == LobbyData.UITargets.UI_MAIN.id then
				LuaUtils.SetNetworkMode( f46_arg0.controller, Enum.LobbyNetworkMode.LOBBY_NETWORKMODE_LIVE )
			end
			Engine.LiveConnectEnableDemonwareConnect()
...
Lobby.Actions.ErrorPopup = function ( f57_arg0 )
	...
	return {
		...
		startFuncPtr = function ( f58_arg0 )
			if f57_arg0.joinResults then
				joinResult = f57_arg0.joinResults[#f57_arg0.joinResults]
			end
			if joinResult and joinResult.joinResult == Enum.JoinResult.JOIN_RESULT_INVALID_PASSWORD then
				LuaUtils.ShowPasswordInput( {
					controller = f57_arg0.controller,
					xuid = f57_arg0.infoProbe.info.xuid,
					joinType = f57_arg0.joinType
				} )
			else
	...
	return {
		...
		startFuncPtr = function ( f127_arg0 )
			if LuaUtils.Mods_IsUsingMods() then
				...
				return 
			elseif f127_arg0.info and f127_arg0.info.probeResult and f127_arg0.info.probeResult.networkMode ~= Enum.LobbyNetworkMode.LOBBY_NETWORKMODE_LIVE then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f127_arg0.actionId
				} )
				return 
			...
			if LuaUtils.IsStarterPack( f126_arg0 ) and f127_local0 ~= Enum.eModes.MODE_MULTIPLAYER then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f127_arg0.actionId
				} )
				return 
			elseif f127_local0 == nil or f127_local0 == Enum.eModes.MODE_ZOMBIES or Engine.IsCampaignModeZombies() or Engine.IsFirstTimeComplete( f126_arg0, f127_local0 ) then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f127_arg0.actionId
				} )
			else
...
Lobby.Actions.SubscribeUGC = function ( f128_arg0, f128_arg1 )
	return {
		name = "SubscribeUGC",
		controller = f128_arg0,
		checkMods = f128_arg1,
		startFuncPtr = function ( f129_arg0 )
			if not LuaUtils.Mods_IsUGC( f129_arg0.checkMods.ugcInfo ) then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f129_arg0.actionId
				} )
				return 
			elseif Engine.Mods_IsSubscribedItem( f129_arg0.checkMods.ugcInfo.ugcName ) then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f129_arg0.actionId
				} )
				return 
			elseif LuaUtils.Mods_Lists_IsInstalled( f129_arg0.checkMods.ugcInfo ) or f129_arg0.checkMods.ugcInfo.ugcName == LuaEnums.DEFAULT_MOD_NAME then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f129_arg0.actionId
				} )
				return 
			elseif not Engine.Mods_SubscribeUGC( f129_arg0.checkMods.ugcInfo.ugcName ) then
				f129_arg0.ErrorMsg = "PLATFORM_MODS_INVALID_WORKSHOP_ITEM"
				LobbyVM.ProcessCompleteError( {
					actionId = f129_arg0.actionId
				} )
				return 
			else
				Engine.LuiVM_Event( "ugc_activity", {
					type = "subscribe",
					modName = f129_arg0.checkMods.ugcInfo.ugcName,
					immediate = true
				} )
				f129_arg0.endTime = 5000 + Engine.milliseconds()
			end
		end
		,
		pumpFuncPtr = function ( f130_arg0 )
			if Engine.Mods_IsSubscribedItem( f130_arg0.checkMods.ugcInfo.ugcName ) then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f130_arg0.actionId
				} )
				return 
			elseif f130_arg0.endTime < Engine.milliseconds() then
				f130_arg0.ErrorMsg = "PLATFORM_MODS_SUBSCRIBE_TIMEOUT"
				LobbyVM.ProcessCompleteError( {
					actionId = f130_arg0.actionId
				} )
				return 
		...
		errorFuncPtr = function ( f131_arg0 )
			return Lobby.GMLOC_OFF .. Engine.Localize( f131_arg0.ErrorMsg, "" .. f131_arg0.checkMods.ugcInfo.ugcType .. "/" .. f131_arg0.checkMods.ugcInfo.ugcName )
		end
		
	}
end

Lobby.Actions.InstalledUGC = function ( f132_arg0, f132_arg1 )
	return {
		name = "InstalledUGC",
		controller = f132_arg0,
		checkMods = f132_arg1,
		startFuncPtr = function ( f133_arg0 )
			if not LuaUtils.Mods_IsUGC( f133_arg0.checkMods.ugcInfo ) then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f133_arg0.actionId
				} )
				return 
			elseif LuaUtils.Mods_Lists_IsInstalled( f133_arg0.checkMods.ugcInfo ) or f133_arg0.checkMods.ugcInfo.ugcName == LuaEnums.DEFAULT_MOD_NAME then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f133_arg0.actionId
				} )
				return 
			elseif f133_arg0.checkMods.ugcInfo.skipLoad == true then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f133_arg0.actionId
				} )
				return 
				...
				f133_arg0.endTime = 60000 + Engine.milliseconds()
				f133_arg0.pumpTime = Engine.milliseconds()
				Engine.LuiVM_Event( "ugc_activity", {
					type = "install_started",
					modName = f133_arg0.checkMods.ugcInfo.ugcName,
					immediate = true
				} )
			end
		end
		...
		pumpFuncPtr = function ( f134_arg0 )
			if LuaUtils.Mods_Lists_IsInstalled( f134_arg0.checkMods.ugcInfo ) then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f134_arg0.actionId
				} )
				Engine.LuiVM_Event( "ugc_activity", {
					type = "install_complete",
					modName = f134_arg0.checkMods.ugcInfo.ugcName,
					immediate = true
				} )
				return 
			elseif f134_arg0.pumpTime > Engine.milliseconds() then
				return 
			end
			f134_arg0.pumpTime = Engine.milliseconds() + 1000
			local f134_local0 = Engine.Mods_InstallProgressUGC( f134_arg0.checkMods.ugcInfo.ugcName )
			if f134_arg0.installProgress == nil then
				f134_arg0.installProgress = f134_local0
			end
			if f134_arg0.installProgress == 1 or f134_arg0.installProgress < f134_local0 then
				f134_arg0.installProgress = f134_local0
				if Engine.Mods_InstalledUGC( f134_arg0.checkMods.ugcInfo.ugcName ) then
					if f134_arg0.checkMods.ugcInfo.ugcType == LuaEnums.MODS_BASE_PATH then
						Engine.Mods_Lists_UpdateMods()
					elseif f134_arg0.checkMods.ugcInfo.ugcType == LuaEnums.USERMAP_BASE_PATH then
						Engine.Mods_Lists_UpdateUsermaps()
					end
					return 
				elseif f134_arg0.endTime < Engine.milliseconds() then
					f134_arg0.endTime = 5000 + Engine.milliseconds()
				end
			end
			if f134_arg0.endTime < Engine.milliseconds() then
				f134_arg0.ErrorMsg = "PLATFORM_MODS_INSTALL_TIMEOUT"
				LobbyVM.ProcessCompleteError( {
					actionId = f134_arg0.actionId
				} )
				return 
		...
		,
		errorFuncPtr = function ( f135_arg0 )
			return Lobby.GMLOC_OFF .. Engine.Localize( f135_arg0.ErrorMsg, "" .. f135_arg0.checkMods.ugcInfo.ugcType .. "/" .. f135_arg0.checkMods.ugcInfo.ugcName )
		end
		
	}
end

Lobby.Actions.LoadMod = function ( f136_arg0, f136_arg1 )
	return {
		name = "LoadMod",
		controller = f136_arg0,
		checkMods = f136_arg1,
		startFuncPtr = function ( f137_arg0 )
			if f137_arg0.checkMods.ugcInfo.ugcType ~= LuaEnums.MODS_BASE_PATH or f137_arg0.checkMods.ugcInfo.skipLoad == true then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f137_arg0.actionId
				} )
				return 
			elseif LuaUtils.Mods_IsModLoaded( f137_arg0.checkMods.ugcInfo ) then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f137_arg0.actionId
				} )
				return 
			elseif LuaUtils.Mods_IsUGC( f137_arg0.checkMods.ugcInfo ) then
				Engine.Mods_Lists_UpdateMods()
				if LuaUtils.Mods_Lists_IsInstalled( f137_arg0.checkMods.ugcInfo ) or f137_arg0.checkMods.ugcInfo.ugcName == LuaEnums.USERMAP_BASE_PATH then
					Engine.LuiVM_Event( "ugc_activity", {
						type = "load",
						modName = f137_arg0.checkMods.ugcInfo.ugcName,
						immediate = true
					} )
					f137_arg0.loadModName = f137_arg0.checkMods.ugcInfo.ugcName
					f137_arg0.startTime = 1000 + Engine.milliseconds()
					f137_arg0.endTime = 60000 + Engine.milliseconds()
					return 
				else
					f137_arg0.ErrorMsg = "PLATFORM_MODS_NOT_INSTALLED"
					LobbyVM.ProcessCompleteError( {
						actionId = f137_arg0.actionId
					} )
					return 
				end
			elseif not LuaUtils.Mods_IsUsingMods() then
				f137_arg0.ErrorMsg = "PLATFORM_MODS_NOT_LOADED"
				LobbyVM.ProcessCompleteError( {
					actionId = f137_arg0.actionId
				} )
				return 
			else
				f137_arg0.loadedModName = LuaUtils.Mods_UsingModsUgcName()
				Engine.LuiVM_Event( "ugc_activity", {
					type = "unload",
					modName = f137_arg0.loadedModName,
					immediate = true
				} )
				f137_arg0.loadModName = ""
				f137_arg0.startTime = 1000 + Engine.milliseconds()
				f137_arg0.endTime = 60000 + Engine.milliseconds()
			end
		end
		,
		pumpFuncPtr = function ( f138_arg0 )
			if LuaUtils.Mods_IsModLoaded( f138_arg0.checkMods.ugcInfo ) then
				if LuaUtils.Mods_IsUGC( f138_arg0.checkMods.ugcInfo ) then
					Engine.LuiVM_Event( "ugc_activity", {
						type = "loaded",
						modName = f138_arg0.checkMods.ugcInfo.ugcName,
						immediate = true
					} )
				else
					Engine.LuiVM_Event( "ugc_activity", {
						type = "unloaded",
						modName = f138_arg0.loadedModName,
						immediate = true
					} )
				end
				LobbyVM.ProcessCompleteSuccess( {
				...
				return 
			elseif f138_arg0.startTime < Engine.milliseconds() then
				LuaUtils.Mods_LoadMod( f138_arg0.loadModName )
			end
			if f138_arg0.endTime < Engine.milliseconds() then
				f138_arg0.ErrorMsg = "PLATFORM_MODS_LOAD_TIMEOUT"
				LobbyVM.ProcessCompleteError( {
					actionId = f138_arg0.actionId
				} )
			end
		...
		,
		errorFuncPtr = function ( f139_arg0 )
			return Lobby.GMLOC_OFF .. Engine.Localize( f139_arg0.ErrorMsg, "" .. f139_arg0.checkMods.ugcInfo.ugcType .. "/" .. f139_arg0.checkMods.ugcInfo.ugcName )
		end
		
	}
end

Lobby.Actions.CheckMods = function ( f140_arg0, f140_arg1, f140_arg2 )
	return {
		name = "CheckMods",
		...
		startFuncPtr = function ( f141_arg0 )
			f141_arg0.ugcInfo = f141_arg0.info.probeResult.ugcInfo
			if f141_arg0.ugcInfo == nil then
				f141_arg0.ugcInfo = {}
				f141_arg0.ugcInfo.ugcName = ""
				f141_arg0.ugcInfo.ugcVersion = 0
			end
			f140_arg1.probeResult.ugcInfo.skipLoad = f140_arg2
			if LuaUtils.Mods_IsUGC( f141_arg0.ugcInfo ) and LuaUtils.IsStarterPack( f140_arg0 ) then
				f141_arg0.ErrorMsg = "PLATFORM_MODS_STARTERPACK_RESTRICT"
				LobbyVM.ProcessCompleteError( {
		...
		,
		errorFuncPtr = function ( f142_arg0 )
			return f142_arg0.ErrorMsg
		end
		
	}
end

Lobby.Actions.CheckStarterPackRequirements = function ( f143_arg0, f143_arg1 )
	return {
		name = "CheckStarterPackRequirements",
		controller = f143_arg0,
		info = f143_arg1,
		mainMode = nil,
		startFuncPtr = function ( f144_arg0 )
			f144_arg0.mainMode = f144_arg0.info.probeResult.mainMode
			f144_arg0.uiScreen = f144_arg0.info.probeResult.uiScreen
			if f144_arg0.mainMode == Enum.LobbyMainMode.LOBBY_MAINMODE_INVALID then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f144_arg0.actionId
				} )
				return 
			elseif LuaUtils.IsStarterPackParty( f143_arg0 ) and (f144_arg0.mainMode == Enum.LobbyMainMode.LOBBY_MAINMODE_ZM or f144_arg0.mainMode == Enum.LobbyMainMode.LOBBY_MAINMODE_CP or LuaUtils.IsCustomGameTarget( f144_arg0.uiScreen )) then
				if LuaUtils.IsStarterPack( f143_arg0 ) then
					f144_arg0.CheckStarterPackRequirementsErrorMsg = "PLATFORM_STARTER_PACK_JOIN_ERROR"
				else
					f144_arg0.CheckStarterPackRequirementsErrorMsg = "PLATFORM_STARTER_PACK_MEMBER_MISSING_CONTENT"
				end
				LobbyVM.ProcessCompleteError( {
					actionId = f144_arg0.actionId
				} )
				return 
			elseif LuaUtils.IsPartyMemberMissingRequiredChunk( f143_arg0, f144_arg0.mainMode ) then
				f144_arg0.CheckStarterPackRequirementsErrorMsg = "PLATFORM_MEMBER_MISSING_CONTENT"
				LobbyVM.ProcessCompleteError( {
					actionId = f144_arg0.actionId
				} )
				return 
		...
		,
		errorFuncPtr = function ( f145_arg0 )
			return f145_arg0.CheckStarterPackRequirementsErrorMsg
		end
		
	}
end

Lobby.Actions.JoinHost = function ( f146_arg0, f146_arg1, f146_arg2, f146_arg3, f146_arg4 )
	return {
		name = "LobbyJoinHost",
		controller = f146_arg0,
		mainMode = f146_arg1,
		networkMode = f146_arg2,
		hostXuid = f146_arg3,
		hostInfo = f146_arg4,
		joinResults = {},
		startFuncPtr = function ( f147_arg0 )
			if Engine.IsLobbyHost( Enum.LobbyType.LOBBY_TYPE_GAME ) then
				Engine.JoinHost( f147_arg0.actionId, f147_arg0.controller, Enum.LobbyType.LOBBY_TYPE_GAME, Enum.LobbyType.LOBBY_TYPE_GAME, f147_arg0.hostXuid, f147_arg0.hostInfo )
			else
				Engine.JoinHost( f147_arg0.actionId, f147_arg0.controller, Enum.LobbyType.LOBBY_TYPE_PRIVATE, Enum.LobbyType.LOBBY_TYPE_GAME, f147_arg0.hostXuid, f147_arg0.hostInfo )
			end
		end
		,
		updateFuncPtr = function ( f148_arg0, f148_arg1 )
			f148_arg0.joinResults[#f148_arg0.joinResults + 1] = f148_arg1
		end
		,
		errorFuncPtr = function ( f149_arg0 )
			local f149_local0 = Engine.Localize( "MENU_JOIN_RESULT_NOT_JOINABLE" )
			if f149_arg0.joinResult == nil and #f149_arg0.joinResults == 0 then
				return f149_local0
			else
				local f149_local1 = f149_arg0.joinResults[#f149_arg0.joinResults].joinResult
				local f149_local2 = LobbyVM.JoinResultToString( f149_local1, true )
				f149_local0 = Engine.Localize( f149_local2.errorMsg )
				if not Engine.IsDevelopmentBuild() then
					return f149_local0
				else
					return f149_local0 .. "\n\nDEBUG INFO: JoinResult( " .. tostring( f149_local1 ) .. " )\n" .. f149_local2.debug
				end
			end
		end
...
Lobby.Actions.LobbyJoinXUID = function ( f150_arg0, f150_arg1, f150_arg2 )
	return {
		name = "LobbyJoinXUID",
		...
		startFuncPtr = function ( f151_arg0 )
			Engine.JoinXUID( f151_arg0.actionId, f151_arg0.controller, f151_arg0.info.xuid, f151_arg0.joinType )
		end
		,
		updateFuncPtr = function ( f152_arg0, f152_arg1 )
			f152_arg0.joinResults[#f152_arg0.joinResults + 1] = f152_arg1
		end
		,
		endFuncPtr = function ( f153_arg0 )
			
		end
		,
		errorFuncPtr = function ( f154_arg0 )
			local f154_local0 = Engine.Localize( "MENU_JOIN_RESULT_NOT_JOINABLE" )
			if #f154_arg0.joinResults == 0 then
				return f154_local0
			else
				local f154_local1 = f154_arg0.joinResults[#f154_arg0.joinResults].joinResult
				local f154_local2 = LobbyVM.JoinResultToString( f154_local1, true )
				f154_local0 = Engine.Localize( f154_local2.errorMsg )
				if not Engine.IsDevelopmentBuild() then
					return f154_local0
				else
					return f154_local0 .. "\n\nDEBUG INFO: JoinResult( " .. tostring( f154_local1 ) .. " )\n" .. f154_local2.debug
				end
			end
		end
...
Lobby.Actions.LobbyJoinXUIDExt = function ( f155_arg0, f155_arg1, f155_arg2, f155_arg3 )
	return {
		name = "LobbyJoinXUIDExt",
		controller = f155_arg0,
		joinType = f155_arg1,
		infoProbe = f155_arg2,
		destinationLobby = f155_arg3,
		joinResults = {},
		startFuncPtr = function ( f156_arg0 )
			local f156_local0 = f156_arg0.infoProbe.probeResult
			local f156_local1 = f156_local0.probedXuid
			local f156_local2 = f156_arg0.destinationLobby
			local f156_local3 = nil
			if f156_local2 == Enum.LobbyType.LOBBY_TYPE_COUNT then
				if f156_local0.gameLobby.isValid == true then
					f156_local2 = Enum.LobbyType.LOBBY_TYPE_GAME
					f156_local3 = f156_local0.gameLobby
				else
					f156_local2 = Enum.LobbyType.LOBBY_TYPE_PRIVATE
					f156_local3 = f156_local0.privateLobby
				end
			elseif f156_local2 == Enum.LobbyType.LOBBY_TYPE_GAME then
				f156_local3 = f156_local0.gameLobby
			else
				f156_local3 = f156_local0.privateLobby
			end
			if Engine.JoinXUIDExt( f156_arg0.actionId, f156_arg0.controller, f156_local3.isValid, f156_local3.hostXuid, f156_local3.hostName, f156_local2, f156_local3.secId, f156_local3.secKey, f156_local3.serializedAdr, f156_arg0.joinType, f156_local1 ) == false then
				LobbyVM.ProcessCompleteError( {
					actionId = f156_arg0.actionId
				} )
			end
		end
		,
		updateFuncPtr = function ( f157_arg0, f157_arg1 )
			f157_arg0.joinResults[#f157_arg0.joinResults + 1] = f157_arg1
		end
		,
		...
		,
		errorFuncPtr = function ( f159_arg0 )
			local f159_local0 = Enum.XONLINE_NAT_TYPE.XONLINE_NAT_OPEN
			if f159_arg0.infoProbe ~= nil and f159_arg0.infoProbe.probeResult ~= nil and f159_arg0.infoProbe.probeResult.natType ~= nil then
				f159_local0 = f159_arg0.infoProbe.probeResult.natType
			end
			local f159_local1 = LuaUtils.IsCompatibleNatConnection( f159_local0 )
			local f159_local2 = Engine.Localize( "MENU_JOIN_RESULT_NOT_JOINABLE" )
			if #f159_arg0.joinResults == 0 then
				if f159_local1 == false then
					f159_local2 = Engine.Localize( "MENU_JOIN_RESULT_NOT_JOINABLE_NAT_TYPE" )
				end
				return f159_local2
			end
			local f159_local3 = f159_arg0.joinResults[#f159_arg0.joinResults].joinResult
			local f159_local4 = LobbyVM.JoinResultToString( f159_local3, true )
			if f159_local1 == false and (f159_local3 == Enum.JoinResult.JOIN_RESULT_HANDSHAKE_WINDOW_EXPIRED or f159_local3 == Enum.JoinResult.JOIN_RESULT_INVALID) then
				f159_local2 = Engine.Localize( "MENU_JOIN_RESULT_NOT_JOINABLE_NAT_TYPE" )
			else
				f159_local2 = Engine.Localize( f159_local4.errorMsg )
			end
			if not Engine.IsDevelopmentBuild() then
				return f159_local2
			end
			return f159_local2 .. "\n\nDEBUG INFO: JoinResult( " .. tostring( f159_local3 ) .. " )\n" .. f159_local4.debug
		end
		
	}
end

Lobby.Actions.WaitForJoiningClients = function ( f160_arg0 )
	return {
		name = "WaitForJoiningClients",
		waitMilliseconds = f160_arg0,
		endWaitTime = nil,
		startFuncPtr = function ( f161_arg0 )
			f161_arg0.endWaitTime = f161_arg0.waitMilliseconds + Engine.milliseconds()
			local f161_local0 = Engine.LobbyJoinCount()
			if f161_local0 == 0 then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f161_arg0.actionId
				} )
			end
			Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "WaitForJoiningClients start, number of joining clients: " .. f161_local0 .. ". Waiting " .. f161_arg0.waitMilliseconds .. "ms for clients to join.\n" )
		end
		,
		pumpFuncPtr = function ( f162_arg0 )
			local f162_local0 = Engine.LobbyJoinCount()
			Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "WaitForJoiningClients pump, number of clients still joining: " .. f162_local0 .. ". Waiting for " .. f162_arg0.endWaitTime - Engine.milliseconds() .. "ms\n" )
			if f162_arg0.endWaitTime < Engine.milliseconds() then
				Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "WaitForJoiningClients pump stop, number of clients still joining: " .. f162_local0 .. ". Clients took to long to join.\n" )
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f162_arg0.actionId
				} )
				return 
			elseif f162_local0 == 0 then
		...
		,
		cancelFuncPtr = function ( f163_arg0 )
			LobbyVM.ProcessCompleteSuccess( {
				actionId = f163_arg0.actionId
			} )
		end
...
Lobby.Actions.LobbySettings = function ( f164_arg0, f164_arg1, f164_arg2, f164_arg3 )
	return {
		name = "LobbySettings",
		controller = f164_arg0,
		toTarget = f164_arg1,
		skipSwitchMode = f164_arg2,
		isDevMap = f164_arg3,
...
Lobby.Actions.SetQueueCancellable = function ( f166_arg0 )
	return {
		name = "SetQueueCancellable",
		startFuncPtr = function ( f167_arg0 )
			Lobby.ProcessQueue.SetCancellable( f166_arg0 )
			LobbyVM.ProcessCompleteSuccess( {
...
Lobby.Actions.UpdateUI = function ( f168_arg0, f168_arg1 )
	return {
		name = "UpdateUI",
		require = "LobbySettings",
...
Lobby.Actions.SetNetworkMode = function ( f170_arg0, f170_arg1 )
	return {
		name = "SetNetworkMode",
		controller = f170_arg0,
		networkMode = f170_arg1,
		startFuncPtr = function ( f171_arg0 )
			LuaUtils.SetNetworkMode( f171_arg0.controller, f171_arg0.networkMode )
			Engine.Exec( f171_arg0.controller, "savegameRestore" )
			Engine.ExecNow( f171_arg0.controller, "invalidateEmblemComponent" )
			LobbyVM.ProcessCompleteSuccess( {
...
Lobby.Actions.PromoteToHostDone = function ( f172_arg0, f172_arg1, f172_arg2, f172_arg3 )
	return {
		name = "promoteToHostDone",
		controller = f172_arg0,
		lobbyType = f172_arg1,
		migrateIndexBits = f172_arg2,
		isInGame = f172_arg3,
		startFuncPtr = function ( f173_arg0 )
			Engine.LobbyHostMigrateSetIndexBits( f173_arg0.lobbyType, f173_arg0.migrateIndexBits )
			Engine.Exec( f173_arg0.controller, "lobby_setmap " .. Dvar.ui_mapname:get() )
			Engine.Exec( f173_arg0.controller, "lobby_setgametype " .. Dvar.ui_gametype:get() )
			Engine.PromoteToHostDone( f173_arg0.controller, f173_arg0.lobbyType, f173_arg0.isInGame )
			LobbyVM.ProcessCompleteSuccess( {
...
Lobby.Actions.SignUserInToLive = function ( f174_arg0 )
	return {
		name = "SignUserInToLive",
		controller = f174_arg0,
		startFuncPtr = function ( f175_arg0 )
			Engine.SignUserIntoLive( f175_arg0.controller )
			LobbyVM.ProcessCompleteSuccess( {
...
Lobby.Actions.SignUserOutOfLive = function ( f176_arg0, f176_arg1 )
	return {
		name = "SignUserOutOfLive",
		controller = f176_arg0,
		networkMode = f176_arg1,
		startFuncPtr = function ( f177_arg0 )
			Engine.SignUserOutOfLive( f177_arg0.controller )
			LobbyVM.ProcessCompleteSuccess( {
...
Lobby.Actions.PlatformSessionGetSessionInfo = function ( f178_arg0, f178_arg1, f178_arg2 )
	return {
		...
		startFuncPtr = function ( f179_arg0 )
			if Lobby.Platform.PlatformSessionOrbisEnabled() == false or Engine.GetLobbyNetworkMode() ~= Enum.LobbyNetworkMode.LOBBY_NETWORKMODE_LIVE then
				...
				return 
			elseif Engine.PlatformSessionPS4GetSessionInfo( f179_arg0.actionId, f179_arg0.controller, f179_arg0.lobbyType, f179_arg0.sessionId ) == false then
				LobbyVM.ProcessCompleteFailure( {
					actionId = f179_arg0.actionId
				} )
				return 
		...
		,
		endFuncPtr = function ( f180_arg0 )
			if Lobby.Platform.PlatformSessionOrbisEnabled() == false or Engine.GetLobbyNetworkMode() ~= Enum.LobbyNetworkMode.LOBBY_NETWORKMODE_LIVE then
				return 
			elseif f180_arg0.retData.isError == true then
				return 
			end
			f180_arg0.info = {
				sessionId = f180_arg0.retData.sessionId,
				sessionName = f180_arg0.retData.sessionName,
				sessionStatus = f180_arg0.retData.sessionStatus,
				sessionType = f180_arg0.retData.sessionType,
				sessionPrivacy = f180_arg0.retData.sessionPrivacy,
				locked = f180_arg0.retData.locked,
				maxSlots = f180_arg0.retData.maxSlots,
				creationTime = f180_arg0.retData.creationTime,
				sessionCreatorGamertag = f180_arg0.retData.sessionCreatorGamertag,
				sessionCreatorPlatform = f180_arg0.retData.sessionCreatorPlatform,
				members = f180_arg0.retData.members
			}
			if f180_arg0.info.members ~= nil and #f180_arg0.info.members > 0 then
				f180_arg0.gamertag = f180_arg0.info.members[#f180_arg0.info.members].gamertag
				f180_arg0.gamertags = {}
				for f180_local0 = 1, #f180_arg0.info.members, 1 do
					f180_arg0.gamertags[f180_local0] = f180_arg0.info.members[f180_local0].gamertag
				end
			end
		end
...
Lobby.Actions.PlatformSessionGetInviteInfo = function ( f181_arg0, f181_arg1, f181_arg2, f181_arg3 )
	return {
		name = "PlatformSessionGetInviteInfo",
		controller = f181_arg0,
		lobbyType = f181_arg2,
		invitationId = f181_arg3,
		xuid = f181_arg1,
		info = {},
		gamertag = nil,
		gamertags = nil,
		startFuncPtr = function ( f182_arg0 )
			if Lobby.Platform.PlatformSessionOrbisEnabled() == false or Engine.IsDemonwareFetchingDone( f181_arg0 ) == false then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f182_arg0.actionId
				} )
				return 
			elseif Engine.PlatformSessionPS4GetInviteInfo( f182_arg0.actionId, f182_arg0.controller, f182_arg0.xuid, f182_arg0.lobbyType, f182_arg0.invitationId ) == false then
				LobbyVM.ProcessCompleteFailure( {
					actionId = f182_arg0.actionId
				} )
				return 
		...
		,
		endFuncPtr = function ( f183_arg0 )
			if Lobby.Platform.PlatformSessionOrbisEnabled() == false or Engine.IsDemonwareFetchingDone( f183_arg0.controller ) == false then
				return 
			elseif f183_arg0.retData.isError == true then
				return 
			else
				f183_arg0.info.receivedDate = f183_arg0.retData.receivedDate
				f183_arg0.info.usedFlag = f183_arg0.retData.usedFlag
				f183_arg0.info.expired = f183_arg0.retData.expired
				f183_arg0.info.message = f183_arg0.retData.message
				f183_arg0.info.fromGamertag = f183_arg0.retData.fromGamertag
				f183_arg0.info.invitationId = f183_arg0.retData.invitationId
				f183_arg0.info.sessionId = f183_arg0.retData.sessionId
				f183_arg0.gamertag = f183_arg0.retData.fromGamertag
				f183_arg0.gamertags = {}
				f183_arg0.gamertags[1] = f183_arg0.gamertag
			end
		end
...
Lobby.Actions.GamertagsToXuids = function ( f184_arg0, f184_arg1 )
	return {
		name = "PlatformSessionJoin",
		controller = f184_arg0,
		info = f184_arg1,
		startFuncPtr = function ( f185_arg0 )
			if f184_arg1.gamertags == nil or #f184_arg1.gamertags == 0 then
				LobbyVM.ProcessCompleteFailure( {
					actionId = f185_arg0.actionId
				} )
				return 
			elseif Engine.GamertagsToXuids( f185_arg0.actionId, f185_arg0.controller, f185_arg0.info.gamertags ) == false then
				LobbyVM.ProcessCompleteFailure( {
					actionId = f185_arg0.actionId
				} )
				return 
		...
		,
		endFuncPtr = function ( f186_arg0 )
			if f186_arg0.retData.isError == true then
				return 
			elseif f186_arg0.retData.userInfo == nil or #f186_arg0.retData.userInfo == 0 then
				f186_arg0.gamertag = ""
				f186_arg0.xuid = 0
				return 
			else
				f186_arg0.gamertag = f186_arg0.retData.userInfo[1].gamertag
				f186_arg0.xuid = f186_arg0.retData.userInfo[1].xuid
			end
		end
...
Lobby.Actions.SetGameAndTypeMapName = function ( f187_arg0, f187_arg1, f187_arg2, f187_arg3 )
	return {
		name = "SetGameAndTypeMapName",
		controller = f187_arg0,
		gameType = f187_arg2,
		mapName = f187_arg3,
		startFuncPtr = function ( f188_arg0 )
			Engine.Exec( f188_arg0.controller, "lobby_setmap " .. f188_arg0.mapName )
			Engine.Exec( f188_arg0.controller, "lobby_setgametype " .. f188_arg0.gameType )
			LobbyVM.ProcessCompleteSuccess( {
				actionId = f188_arg0.actionId
			} )
		end
...
Lobby.Actions.SetSavedOrDefaultMap = function ( f189_arg0, f189_arg1 )
	return {
		name = "SetSavedOrDefaultMap",
		controller = f189_arg0,
		startFuncPtr = function ( f190_arg0 )
			if Engine.IsCampaignGame() then
				local f190_local0 = Engine.GetSavedMap()
				if string.len( f190_local0 ) == 0 then
					f190_local0 = LuaUtils.GetDefaultMap( f189_arg1 )
				end
				Dvar.ui_mapname:set( f190_local0 )
				Dvar.cp_queued_level:set( Engine.GetSavedMapQueuedMap() )
				Engine.GameLobbySetMap( f190_local0 )
			end
			if f189_arg1 == LobbyData.UITargets.UI_MPLOBBYONLINECUSTOMGAME and Engine.LobbyHost_SetOfficialCustomGame then
				Engine.LobbyHost_SetOfficialCustomGame( true )
			end
			LobbyVM.ProcessCompleteSuccess( {
				actionId = f190_arg0.actionId
			} )
		end
...
Lobby.Actions.PlayTogetherBeginPlayWait = function ( f191_arg0 )
	return {
		...
		,
		pumpFuncPtr = function ( f193_arg0, f193_arg1 )
			if Engine.GetLobbyUIScreen() == LobbyData.UITargets.UI_MODESELECT.id then
				LobbyVM.ProcessCompleteSuccess( {
					actionId = f193_arg0.actionId
				} )
				return 
			elseif Engine.GetModel( Engine.GetModelForController( f193_arg0.controller ), "SystemOverlay_MessageDialog.buttonPrompts" ) ~= nil then
				LobbyVM.ProcessCompleteFailure( {
					actionId = f193_arg0.actionId
				} )
			end
		...
		,
		cancelFuncPtr = function ( f194_arg0 )
			LobbyVM.ProcessCompleteSuccess( {
				actionId = f194_arg0.actionId
			} )
		end
...
Lobby.Actions.PlayTogetherPSNSessionWait = function ( f195_arg0, f195_arg1 )
	return {
		name = "PlayTogetherPSNSessionWait( old session id: " .. f195_arg1 .. ")",
		nonPSNSessionBlocking = true,
		controller = f195_arg0,
		oldSessionId = f195_arg1,
		startFuncPtr = function ( f196_arg0 )
			
		end
		,
		pumpFuncPtr = function ( f197_arg0, f197_arg1 )
			if Lobby.Platform.PS4.taskInProgress == true then
				return 
			else
				local f197_local0 = Lobby.Platform.PS4GetSessionId( f195_arg0 )
				if string.len( f197_local0 ) > 0 and f197_arg0.oldSessionId ~= f197_local0 then
					LobbyVM.ProcessCompleteSuccess( {
		...
		,
		cancelFuncPtr = function ( f198_arg0 )
			LobbyVM.ProcessCompleteSuccess( {
				actionId = f198_arg0.actionId
			} )
		end
