	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if f18_local0 then
					f18_local0 = IsGamepad( controller )
				end
				return f18_local0
			end
		},
		{
			...
			condition = function ( menu, element, event )
				...
				if f19_local0 then
					f19_local0 = IsGamepad( controller )
				end
				return f19_local0
			end
		},
		{
			...
			condition = function ( menu, element, event )
				local f20_local0 = IsSelfModelValueEqualToEnum( element, controller, "", Enum.LUIButtonPromptStates.FLAG_DISABLE_PROMPTS )
				if f20_local0 then
					f20_local0 = not IsGamepad( controller )
				end
				return f20_local0
			end
		},
		{
			stateName = "EnabledPC",
			condition = function ( menu, element, event )
				local f21_local0 = IsSelfModelValueEqualToEnum( element, controller, "", Enum.LUIButtonPromptStates.FLAG_ENABLE_PROMPTS )
				if f21_local0 then
