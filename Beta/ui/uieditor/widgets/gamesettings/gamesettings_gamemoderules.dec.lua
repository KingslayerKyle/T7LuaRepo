...
local PrepareGameTypeOptions = function ( controller )
	...
	if globalTopLevelSettings and #globalTopLevelSettings > 0 then
		for index, setting in ipairs( globalTopLevelSettings ) do
			settingInfo = CoD.GameOptions.GameSettings[setting]
			table.insert( optionsTable, CoD.OptionsUtility.CreateListOptions( controller, settingInfo.name, settingInfo.hintText, setting, settingInfo, "GameTypeOptionsList_" .. REG10 ) )
		end
	end
	return optionsTable
end
	...
	for index, buttonData in ipairs( buttonOptions ) do
		table.insert( buttons, {
			models = {
				...
				customId = REG9.customId
			},
			properties = {
				action = REG9.action,
				actionParam = REG9.actionParam,
				isLastButtonInGroup = REG9.isLastButtonInGroup,
				spacing = REG9.spacing
			}
		} )
	end
	return buttons
