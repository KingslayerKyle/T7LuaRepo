	...
	choosePaintSide:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f3_local0 = IsPaintshop( controller )
				if f3_local0 then
					f3_local0 = not IsModelValueEqualToEnum( controller, "Emblem.EmblemProperties.editorMode", Enum.CustomizationEditorMode.CUSTOMIZATION_EDITOR_MODE_EDIT )
				end
				return f3_local0
			end
		}
