...
CoD.GunsmithPaintjobButton.new = function ( menu, controller )
	...
	removeButton:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f8_local0 = IsPC()
				if f8_local0 then
					if not IsGamepad( controller ) and not IsSelfModelValueEqualTo( element, controller, "paintjobIndex", 32 ) then
						f8_local0 = not IsSelfModelValueEqualTo( element, controller, "paintjobSlot", 64 )
					else
						f8_local0 = false
					end
				end
	...
	if removeButton.m_eventHandlers.input_source_changed then
		local currentEv = removeButton.m_eventHandlers.input_source_changed
		removeButton:registerEventHandler( "input_source_changed", function ( self, event )
	...
	removeButton:linkToElementModel( "paintjobIndex", true, function ( model )
	...
	removeButton:linkToElementModel( "paintjobSlot", true, function ( model )
		menu:updateElementState( removeButton, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
	...
	self.clipsPerState = {
		DefaultState = {
			...
			GainFocus = function ()
				...
				extraCamRender:beginAnimation( "keyframe", 300, false, false, CoD.TweenType.Linear )
				extraCamRender:setAlpha( 0 )
							...
							local FocusBorderFrame5 = function ( FocusBorder, event )
								local FocusBorderFrame6 = function ( FocusBorder, event )
									local FocusBorderFrame7 = function ( FocusBorder, event )
										local FocusBorderFrame8 = function ( FocusBorder, event )
											if not event.interrupted then
												FocusBorder:beginAnimation( "keyframe", 39, false, false, CoD.TweenType.Linear )
											end
											FocusBorder:setAlpha( 0 )
											if event.interrupted then
												self.clipFinished( FocusBorder, event )
							...
							local FocusBorderFrame5 = function ( FocusBorder, event )
								local FocusBorderFrame6 = function ( FocusBorder, event )
									local FocusBorderFrame7 = function ( FocusBorder, event )
										local FocusBorderFrame8 = function ( FocusBorder, event )
											if not event.interrupted then
												FocusBorder:beginAnimation( "keyframe", 59, false, false, CoD.TweenType.Linear )
											end
											FocusBorder:setAlpha( 0 )
											if event.interrupted then
												self.clipFinished( FocusBorder, event )
