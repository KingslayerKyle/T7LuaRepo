...
CoD.PaintshopChooseIconName.new = function ( menu, controller )
	...
	decalDesc:subscribeToGlobalModel( controller, "EmblemProperties", "selectedDecalID", function ( model )
		...
		if modelValue then
			decalDesc:setText( Engine.Localize( GetEmblemDecalDesc( modelValue ) ) )
		end
	end )
	LUI.OverrideFunction_CallOriginalFirst( decalDesc, "setText", function ( element, text )
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( self )
		self.PaintjobSlotsBack0:close()
		self.decalDesc:close()
	end )
