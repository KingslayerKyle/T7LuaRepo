...
local PostLoadFunc = function ( self, controller )
	CoD.ConsoleUtility.Init( self, controller, "console" )
end
	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f12_local0 = IsCodCaster( controller )
				if f12_local0 then
					f12_local0 = not IsCodCasterProfileValueEqualTo( controller, "shoutcaster_killfeed", 1 )
				end
				return f12_local0
			end
		}
