...
CoD.FirstTimeLanguageSelectionListWidget.new = function ( menu, controller )
	...
	LanguageImage:setLeftRight( true, false, 5, 275 )
	LanguageImage:setTopBottom( true, false, 6, 26.92 )
	...
	self.LanguageImage:linkToElementModel( self, "image", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			LanguageImage:setImage( RegisterImage( modelValue ) )
		end
	end )
	self.clipsPerState = {
		DefaultState = {
