...
local PreLoadFunc = function ( self, controller )
	...
	Engine.CreateModel( perControllerModel, "Gunsmith.DataDownloaded" )
	Engine.CreateModel( perControllerModel, "Paintshop.DataDownloaded" )
end
...
CoD.WeaponItemListButtonNew.new = function ( menu, controller )
	...
	LabelButtonCount:setLeftRight( false, true, -36, -6 )
	LabelButtonCount:setTopBottom( false, false, -9, 11 )
	LabelButtonCount:linkToElementModel( self, "itemIndex", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			LabelButtonCount.ButtonLabelCount.itemName:setText( Engine.Localize( GetVariantCountByWeaponIndex( modelValue ) ) )
		end
	end )
	LabelButtonCount:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f6_local0 = Gunsmith_IsEnabled( element, controller )
				if f6_local0 then
					f6_local0 = Gunsmith_IsVariantCountEmpty( element, controller )
				end
				return f6_local0
			end
		}
	} )
	LabelButtonCount:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "Gunsmith.DataDownloaded" ), function ( model )
		menu:updateElementState( LabelButtonCount, {
	...
	LabelButtonCount:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "Paintshop.DataDownloaded" ), function ( model )
		menu:updateElementState( LabelButtonCount, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
	...
	tokenUnlock:setLeftRight( false, true, -43, -11 )
	...
	IconNew:setLeftRight( true, false, 0, 65 )
	...
	self:mergeStateConditions( {
		...
		},
		{
			...
			condition = function ( menu, element, event )
				return IsCACItemClassified( menu, element, controller )
			end
		},
		{
			stateName = "Locked",
