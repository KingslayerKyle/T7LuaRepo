...
CoD.WildcardItemButton.new = function ( menu, controller )
	...
	hintArrow:linkToElementModel( nil, true, function ( model )
	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f37_local0 = IsCACItemEquipped( menu, element, controller )
				if f37_local0 then
					f37_local0 = IsCACContextualWildcardOpen( controller )
				end
				return f37_local0
			end
		},
		{
			...
			condition = function ( menu, element, event )
				local f38_local0 = IsCACContextualWildcardOpen( controller )
				if f38_local0 then
					f38_local0 = IsCACItemLocked( menu, element, controller )
				end
				return f38_local0
			end
		},
		{
			...
			condition = function ( menu, element, event )
				local f39_local0 = IsCACContextualWildcardOpen( controller )
				if f39_local0 then
					f39_local0 = not IsCACItemPurchased( element, controller )
				end
				return f39_local0
			end
		},
