	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f24_local0 = IsCurrentMenuWeaponMenu( menu )
				if f24_local0 then
					f24_local0 = IsGlobalModelValueEqualTo( element, controller, "WGSV_Category", CoD.WGSV_Category.GUNSMITH_VARIANT )
				end
				return f24_local0
			end
		},
		{
			...
			condition = function ( menu, element, event )
				local f25_local0 = IsCurrentMenuAttachmentMenu( menu )
				if f25_local0 then
					f25_local0 = IsCACItemLocked( menu, element, controller )
				end
				return f25_local0
			end
		},
		{
			...
			condition = function ( menu, element, event )
				local f26_local0 = IsCACItemLocked( menu, element, controller )
				if f26_local0 then
					f26_local0 = IsCurrentMenuEquipmentMenu( menu )
				end
				return f26_local0
			end
		},
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f28_local0 = IsCACItemLocked( menu, element, controller )
				if f28_local0 then
					f28_local0 = IsCurrentMenuCybercoreOrRigMenu( menu )
				end
				return f28_local0
			end
		},
