...
CoD.vhud_ms_ButtonWidget.new = function ( menu, controller )
	...
	backinglines:subscribeToGlobalModel( controller, "PerController", "vehicle.bindings.color", function ( model )
		...
		if modelValue then
			backinglines:setRGB( modelValue )
		end
	end )
	...
	bind:linkToElementModel( self, "bind", true, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			bind:setText( Engine.Localize( modelValue ) )
		end
	end )
	...
	self:linkToElementModel( "text", true, function ( model )
	...
	self:linkToElementModel( "bind", true, function ( model )
		menu:updateElementState( self, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( self )
		self.vhudbuttonHexBlur:close()
		self.backinglines:close()
		self.prompt:close()
		self.bind:close()
	end )
