...
CoD.StartMenu_Barracks.new = function ( menu, controller )
	...
	CP:subscribeToGlobalModel( controller, "StorageGlobal", "stats_cp_offline", function ( model )
		...
		if modelValue then
			CP.RankName:setText( Engine.Localize( RankToTitleString( "cp", StorageLookup( controller, "playerstatslist.rank.statvalue", modelValue ) ) ) )
		end
	end )
	CP:subscribeToGlobalModel( controller, "StorageGlobal", "stats_cp_offline", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			CP.Rank:setText( Engine.Localize( RankToLevelString( "cp", StorageLookup( controller, "playerstatslist.rank.statvalue", modelValue ) ) ) )
		end
	end )
	self:addElement( CP )
	self.CP = CP
	...
	MP:subscribeToGlobalModel( controller, "StorageGlobal", "stats_mp", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			MP.RankName:setText( Engine.Localize( RankToTitleString( "mp", StorageLookup( controller, "playerstatslist.rank.statvalue", modelValue ) ) ) )
		end
	end )
	MP:subscribeToGlobalModel( controller, "StorageGlobal", "stats_mp", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			MP.Rank:setText( Engine.Localize( RankToLevelString( "mp", StorageLookup( controller, "playerstatslist.rank.statvalue", modelValue ) ) ) )
		end
	end )
	self:addElement( MP )
	self.MP = MP
	...
	ZM:subscribeToGlobalModel( controller, "StorageGlobal", "stats_zm", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			ZM.RankName:setText( Engine.Localize( RankToTitleString( "zm", StorageLookup( controller, "playerstatslist.rank.statvalue", modelValue ) ) ) )
		end
	end )
	ZM:subscribeToGlobalModel( controller, "StorageGlobal", "stats_zm", function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue then
			ZM.Rank:setText( Engine.Localize( RankToLevelString( "zm", StorageLookup( controller, "playerstatslist.rank.statvalue", modelValue ) ) ) )
		end
	end )
	self:addElement( ZM )
	self.ZM = ZM
