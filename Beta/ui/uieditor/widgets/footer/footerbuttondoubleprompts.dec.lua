	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f9_local0 = IsSelfModelValueEqualToEnum( element, controller, "", Enum.LUIButtonPromptStates.FLAG_DISABLE_PROMPTS )
				if f9_local0 then
					f9_local0 = IsGamepad( controller )
				end
				return f9_local0
			end
		},
		{
			stateName = "Enabled",
			condition = function ( menu, element, event )
				local f10_local0 = IsSelfModelValueEqualToEnum( element, controller, "", Enum.LUIButtonPromptStates.FLAG_ENABLE_PROMPTS )
				if f10_local0 then
					f10_local0 = IsGamepad( controller )
				end
				return f10_local0
			end
		}
