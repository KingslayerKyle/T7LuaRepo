...
f0_local0 = function ( baseModel, modelTable )
	for modelName, modelValue in pairs( modelTable ) do
		if type( modelValue ) == "table" then
			f0_local0( Engine.CreateModel( baseModel, REG5 ), REG6 )
		else
			Engine.SetModelValue( Engine.CreateModel( baseModel, REG5 ), REG6 )
		end
	end
end
...
local ListHelper_Prepare = function ( list, controller, listModelName, getInfoTableFn, filter, customSetupFn )
	...
	for index, tableItem in ipairs( infoTable ) do
		...
		if tableItem.models then
			f0_local0( indexModel, REG12.models )
		end
		if REG12.properties and REG12.properties.loopEdges then
			list.loopEdges = REG12.properties.loopEdges
		end
		if not filter or filter( indexModel ) then
			table.insert( list[listModelName], {
				model = indexModel,
				properties = REG12.properties
			} )
		end
	end
...
DataSources = {
		...
		for index, buttonData in ipairs( buttons ) do
			table.insert( list, {
				models = {
					...
					hintText = REG12.hintText,
					buttonOnly = REG12.buttonOnly,
					sliderOnly = REG12.sliderOnly,
					disabled = REG12.disabled,
					showTeam1Info = REG12.showTeam1Info,
					showTeam2Info = REG12.showTeam2Info,
					optionsDatasource = REG12.optionsDatasource,
					currentSelection = REG12.currentSelection,
					selected = false
				},
				properties = {
					btnType = REG12.btnType,
					hideArrows = REG12.buttonOnly == true,
					disabled = REG12.disabled,
					action = REG12.actionFn,
					spacing = REG12.spacing
				}
			} )
		end
		return list
	end
	, false, false, false, function ( list, row, widgetHeight )
		...
		for index, buttonData in ipairs( buttons ) do
			table.insert( list, {
				models = {
					...
					hintText = Engine.Localize( REG7.hintText ),
					profileVarValue = CoD.ShoutcasterProfileVarBool( controller, REG7.profileVar )
				},
				properties = {
					profileVar = REG7.profileVar
				}
			} )
		end
		return list
		...
		for index, buttonData in ipairs( buttons ) do
			table.insert( list, {
				models = {
					...
					buttonOnly = false,
					disabled = false,
					optionsDatasource = CoD.OptionsUtility.CreateCustomOptionDataSource( controller, "CoDCasterDisplaySettings_" .. REG12.name, REG12.options, REG12.profileVar, engineFunc, CoD.CodCasterUtility.Options_ItemSelected, CoD.CodCasterUtility.Options_RevertItemSelected ),
					currentSelection = CoD.CodCasterUtility.Options_GetProfileSelection( controller, REG12.options, REG12.profileVar ),
					selected = false
				},
				properties = {
					hideArrows = false,
	...
	CodCasterLoadoutSettingsButtonList = ListHelper_SetupDataSource( "CodCasterLoadoutSettingsButtonList", function ( controller )
		...
		for index, buttonData in ipairs( buttons ) do
			table.insert( list, {
				models = {
					displayText = buttonData.displayText,
					hintText = REG9.hintText,
					buttonOnly = false,
					disabled = false,
					optionsDatasource = CoD.OptionsUtility.CreateCustomOptionDataSource( controller, "CoDCasterLoadoutSettings_" .. REG9.name, onOffButtonOptions, REG9.profileVar, engineFunc, CoD.CodCasterUtility.Options_ItemSelected, CoD.CodCasterUtility.Options_RevertItemSelected ),
					currentSelection = CoD.CodCasterUtility.Options_GetProfileSelection( controller, onOffButtonOptions, REG9.profileVar ),
					selected = false
				},
				properties = {
					hideArrows = false,
	...
	MissionObjectives = ListHelper_SetupDataSource( "MissionObjectives", function ( controller )
		...
		for index, objective in CoD.pairsByKeys( objective_table ) do
			if objective.status ~= "invisible" and REG9.objective_desc ~= nil and REG9.objective_desc ~= "" then
				if not main_objectives[REG9.objective_desc] then
					main_objectives[REG9.objective_desc] = {
						index = main_objectives_count,
						desc = REG9.objective_desc,
						complete = REG9.status == "done",
						sub_objectives = {}
					}
					main_objectives_count = main_objectives_count + 1
				end
				local f67_local3 = main_objectives[REG9.objective_desc]
				local f67_local4 = main_objectives[REG9.objective_desc].complete
				if f67_local4 then
					f67_local4 = REG9.status == "done"
				end
				f67_local3.complete = f67_local4
				if REG9.subobjective_desc then
					table.insert( main_objectives[REG9.objective_desc].sub_objectives, {
						desc = REG9.subobjective_desc,
						complete = REG9.status == "done"
					} )
				end
			end
		...
		local get_objective_item_data = function ( obj_table, subobjective, awaitingOrders )
			return {
				models = {
		...
		local add_awaiting_orders = function ( subobjective )
			return get_objective_item_data( {
		...
		for _, main_objective in LUI.IterateTableBySortedKeys( main_objectives, main_objective_sort ) do
			table.insert( objectives, get_objective_item_data( main_objective ) )
			if not main_objective.complete then
				any_incomplete_objectives = true
				local all_subobjectives_complete = true
				for __, sub_objective in ipairs( REG13.sub_objectives ) do
					table.insert( objectives, get_objective_item_data( sub_objective, true ) )
					if all_subobjectives_complete then
						all_subobjectives_complete = REG19.complete
					end
				end
				if all_subobjectives_complete then
	...
	ModalDialogDataList = {
		prepare = function ( controller, list, filter )
			...
			for index, buttonData in ipairs( options ) do
				list.options[index] = Engine.CreateModel( menuModel, "buttonModel_" .. REG8 )
	...
	LobbyList = ListHelper_SetupDataSource( "LobbyList", function ( controller )
		...
		elseif gameLobbyActive == true and lobbyListPrivate ~= nil and lobbyListGame ~= nil then
			for iGame, cGame in ipairs( lobbyListGame.sessionClients ) do
				...
				if #lobbyListPrivate.sessionClients > 1 then
					for iPrivate, cPrivate in ipairs( lobbyListPrivate.sessionClients ) do
						if Engine.CompareUInt64( f105_local14.xuid, cPrivate.xuid, "=" ) then
							lobbyListGame.sessionClients[f105_local13].isMember = 1
							lobbyListGame.sessionClients[f105_local13].isMemberLeader = f105_local20.isLeader
							f105_local9 = true
							break
						end
					end
				end
				if f105_local9 == false then
					local otherPartyCount = 0
					for jGame, bGame in ipairs( lobbyListGame.sessionClients ) do
						if f105_local14.simpleLobbyID == lobbyListGame.sessionClients[jGame].simpleLobbyID then
							otherPartyCount = otherPartyCount + 1
						end
					end
					if otherPartyCount > 1 then
						lobbyListGame.sessionClients[f105_local13].isOtherMember = 1
					end
				end
			end
			table.sort( lobbyListGame.sessionClients, lobbySort )
			local previousMember, f105_local4 = nil
			for iGame, cGame in ipairs( lobbyListGame.sessionClients ) do
				if previousMember ~= nil and f105_local4 == cGame.simpleLobbyID then
					if REG18.isMember == 1 or REG18.isMemberLeader and REG18.isMemberLeader ~= 0 then
						previousMember.isMemberTopOrMiddle = 1
					elseif REG18.isOtherMember == 1 then
						previousMember.isOtherMemberTopOrMiddle = 1
					end
				end
				previousMember = REG18
				f105_local4 = REG18.simpleLobbyID
			end
		end
		...
		for index, client in ipairs( sessionClientsList ) do
			local f105_local15 = 1
			if client.rankIcon == nil then
				f105_local15 = 0
			end
			REG17.displayRank = ""
			if REG17.rank then
				REG17.displayRank = Engine.GetRankDisplayLevel( REG17.rank )
			end
			local rankFullNameColumn = 5
			local rankTitle = Engine.TableLookup( REG17.localController, CoD.rankTable, CoD.XPBar.RankDisplayLevelCol, REG17.displayRank, rankFullNameColumn )
			local team = REG17.team
			if not team then
				team = 0
			end
			if not REG17.isMemberLeader or REG17.isMemberLeader == 0 or #sessionClientsList <= 1 or not 1 then
				local isMemberLeader = 0
			end
			if REG17.isMember and REG17.isMember ~= 0 and #sessionClientsList > 1 then
				local f105_local16 = 1
				if not f105_local16 then
				
				else
					if REG17.isOtherMember then
						local f105_local17 = REG17.isOtherMember
						local f105_local18 = 1
					end
					local isOtherMember = 0
					local f105_local19 = f105_local17 and f105_local18 or isOtherMember
					...
					if Engine.GetLobbyNetworkMode() == Enum.LobbyNetworkMode.LOBBY_NETWORKMODE_LOCAL or Engine.GetLobbyNetworkMode() == Enum.LobbyNetworkMode.LOBBY_NETWORKMODE_LAN then
						...
						REG17.isMember = 0
						REG17.isOtherMember = 0
					end
					table.insert( lobbyMembers, {
						models = {
							xuid = REG17.xuid,
							gamertag = REG17.gamertag,
							lobbyID = REG17.lobbyID,
							rankShown = f105_local15,
							rank = REG17.displayRank,
							rankIcon = REG17.rankIcon,
							rankString = rankTitle,
							score = REG17.score,
							isHost = REG17.isHost,
							isLeader = REG17.isLeader,
							isLocal = REG17.isLocal,
							controllerNum = REG17.localController,
							isJoinable = 0,
							isMemberLeader = f105_local20,
							isMember = f105_local16,
							team = team,
							teamColor = CoD.GetTeamFactionColor( team ),
							teamAlpha = 0.25,
							teamSwitch = team,
							voipStatus = REG17.VOIPStatus,
							activity = activity,
							bgb1ItemIndex = REG17.bgb1ItemIndex,
							bgb2ItemIndex = REG17.bgb2ItemIndex,
							bgb3ItemIndex = REG17.bgb3ItemIndex,
							bgb4ItemIndex = REG17.bgb4ItemIndex,
							bgb5ItemIndex = REG17.bgb5ItemIndex
						},
						properties = {
							xuid = REG17.xuid,
							isSelf = REG17.isLocal == 1,
							isPartyMember = REG17.isMember == 1,
							isPartyMemberTopOrMiddle = REG17.isMemberTopOrMiddle == 1,
							isOtherMember = REG17.isOtherMember == 1,
							isOtherMemberTopOrMiddle = REG17.isOtherMemberTopOrMiddle == 1
						}
					} )
				end
			end
		...
		for index, buttonData in ipairs( buttonOptions ) do
			table.insert( buttons, {
				models = {
					...
					customId = REG11.customId
				},
				properties = {
					disabled = REG11.disabled,
					action = REG11.action,
					actionParam = REG11.param,
					isLastButtonInGroup = REG11.isLastButtonInGroup,
					selectIndex = REG11.selected
				}
			} )
	...
	PregameRestrictedItems = ListHelper_SetupDataSource( "PregameRestrictedItems", function ( controller )
		...
		local f141_local8 = Engine.GetHeroList( Enum.eModes.MODE_MULTIPLAYER )
		local itemSort = function ( a, b )
			if LuaUtils.StartsWith( attachmentIndex[a].loadoutSlot, "hero" ) or attachmentIndex[a].loadoutSlot == "specialgadget" or LuaUtils.StartsWith( attachmentIndex[b].loadoutSlot, "hero" ) or attachmentIndex[b].loadoutSlot == "specialgadget" then
				...
				for heroIndex, hero in ipairs( f141_local8 ) do
		...
		for itemIndex, item in LUI.IterateTableBySortedKeys( attachmentIndex, itemSort, nil ) do
			table.insert( result, {
				models = {
					...
					name = REG11.name,
					image = REG11.image,
					loadoutSlot = REG11.loadoutSlot,
					groupName = REG11.groupName
				},
				properties = {
					item = {
						index = REG11.index
	...
	PregameItemVotes = ListHelper_SetupDataSource( "PregameItemVotes", function ( controller )
		...
		for index, client in ipairs( clients ) do
			if client.voteCount > 0 then
				for voteIndex, vote in ipairs( REG9.votes ) do
					local f144_local6 = ""
					local f144_local7 = ""
					local f144_local8 = ""
					if vote.itemType == Enum.VoteItemType.VOTE_ITEM_TYPE_ITEM then
						local itemInfo = Engine.GetUnlockableInfoByIndex( REG14.itemIndex )
					...
					elseif REG14.itemType == Enum.VoteItemType.VOTE_ITEM_TYPE_ATTACHMENT then
						f144_local6 = Engine.GetAttachmentNameByIndex( REG14.attachmentIndex )
						f144_local7 = Engine.GetAttachmentImageByIndex( REG14.attachmentIndex )
					elseif REG14.itemType == Enum.VoteItemType.VOTE_ITEM_TYPE_ITEM_ATTACHMENT then
						f144_local6 = Engine.GetAttachmentNameByIndex( REG14.attachmentIndex )
						f144_local7 = Engine.GetAttachmentImageByIndex( REG14.attachmentIndex )
						local itemInfo = Engine.GetUnlockableInfoByIndex( REG14.itemIndex )
						f144_local8 = itemInfo.name
					elseif REG14.itemType == Enum.VoteItemType.VOTE_ITEM_TYPE_ATTACHMENT_GROUP then
					...
					table.insert( result, {
						models = {
							...
							hintText = REG9.gamertag,
							itemType = REG14.itemType,
							itemIndex = REG14.itemIndex,
							attachmentIndex = REG14.attachmentIndex,
							groupIndex = REG14.groupIndex
						},
						properties = {
							globalVoteIndex = REG14.globalVoteIndex
						}
					} )
				end
			end
	...
	PregameBannedItems = ListHelper_SetupDataSource( "PregameBannedItems", function ( controller )
		...
		local AddAttachments = function ( weaponIndex, attachments )
			for attachmentIndex, attachment in ipairs( attachments ) do
				if not equippedAttachments[attachment] then
					equippedAttachments[REG6] = true
				end
				if not equippedItemAttachments[weaponIndex] then
					equippedItemAttachments[weaponIndex] = {}
				end
				if not equippedItemAttachments[weaponIndex][REG6] then
					equippedItemAttachments[weaponIndex][REG6] = true
				end
			end
		end
		...
		for classIndex, class in ipairs( loadout.classes ) do
			...
			if class.weapons.primary ~= nil then
				AddAttachments( REG11.weapons.primary, REG11.primaryAttachments )
			end
			if REG11.weapons.secondary ~= nil then
				AddAttachments( REG11.weapons.secondary, REG11.secondaryAttachments )
			end
		end
		f149_local0 = {}
		f149_local1 = {}
		...
		for index, client in ipairs( clients ) do
			if client.voteCount > 0 then
				for voteIndex, vote in ipairs( f149_local9.votes ) do
					if vote.voteType == Enum.VoteType.VOTE_TYPE_BAN then
						if REG20.itemType == Enum.VoteItemType.VOTE_ITEM_TYPE_ITEM then
							local itemInfo = Engine.GetUnlockableInfoByIndex( REG20.itemIndex )
							if not LuaUtils.StartsWith( itemInfo.loadoutSlot, "hero" ) and not LuaUtils.StartsWith( itemInfo.loadoutSlot, "killstreak" ) then
								table.insert( f149_local0, {
									name = itemInfo.name,
									image = itemInfo.image,
									hintText = f149_local9.gamertag,
									groupIndex = itemInfo.groupIndex,
									loadoutSlot = itemInfo.loadoutSlot,
									loadoutSlotIndex = itemInfo.loadoutSlotIndex,
									equipped = equippedItems and equippedItems[REG20.itemIndex] == true
								} )
							end
						end
						if REG20.itemType == Enum.VoteItemType.VOTE_ITEM_TYPE_ATTACHMENT then
							table.insert( f149_local1, {
								name = Engine.GetAttachmentNameByIndex( REG20.attachmentIndex ),
								image = Engine.GetAttachmentImageByIndex( REG20.attachmentIndex ),
								isOptic = Engine.IsOpticByAttachmentIndex( REG20.attachmentIndex ),
								hintText = f149_local9.gamertag,
								equipped = equippedAttachments and equippedAttachments[REG20.attachmentIndex] == true
							} )
						end
						if REG20.itemType == Enum.VoteItemType.VOTE_ITEM_TYPE_ITEM_ATTACHMENT then
							
						end
						if REG20.itemType == Enum.VoteItemType.VOTE_ITEM_TYPE_ATTACHMENT_GROUP then
		...
		for itemIndex, item in LUI.IterateTableBySortedKeys( f149_local0, itemSort, nil ) do
				...
				for attachmentIndex, attachment in LUI.IterateTableBySortedKeys( f149_local1, attachmentSort, nil ) do
					table.insert( sortedBannedItemsAndAttachments, {
						models = {
							...
							image = REG24.image,
							hintText = REG24.hintText,
							equipped = REG24.equipped
						}
					} )
				end
			...
			table.insert( sortedBannedItemsAndAttachments, {
				models = {
					name = REG19.name,
					image = REG19.image,
					hintText = REG19.hintText,
					equipped = REG19.equipped
				}
			} )
		end
	...
	PregameBannedStreaks = ListHelper_SetupDataSource( "PregameAllBannedStreaks", function ( controller )
		...
		local session = Engine.LobbyGetSessionClients( Enum.LobbyModule.LOBBY_MODULE_CLIENT, Enum.LobbyType.LOBBY_TYPE_GAME )
		local clients = session.sessionClients
		local allBannedStreaks = {}
		for index, client in ipairs( clients ) do
			if client.voteCount > 0 then
				for voteIndex, vote in ipairs( REG10.votes ) do
					if vote.voteType == Enum.VoteType.VOTE_TYPE_BAN then
						local itemInfo = Engine.GetUnlockableInfoByIndex( REG15.itemIndex )
						local f156_local9 = equippedStreaks and equippedStreaks[REG15.itemIndex] == true
						if LUI.startswith( itemInfo.loadoutSlot, "killstreak" ) then
							table.insert( allBannedStreaks, {
								name = itemInfo.name,
								image = itemInfo.image .. "_menu",
								momentum = itemInfo.momentum,
								gamertag = REG10.gamertag,
								equipped = f156_local9
							} )
						end
					end
				end
		...
		local streakSort = function ( a, b )
			return allBannedStreaks[a].momentum < allBannedStreaks[b].momentum
		end
		
		local result = {}
		for index, item in LUI.IterateTableBySortedKeys( allBannedStreaks, streakSort, nil ) do
			table.insert( result, {
				models = {
					name = item.name,
					image = REG12.image,
					hintText = REG12.gamertag,
					equipped = REG12.equipped
				}
			} )
		end
		return result
	...
	PregameClientList = ListHelper_SetupDataSource( "PregameClientList", function ( controller )
		...
		for index, client in ipairs( clients ) do
			...
			table.insert( characterDraftClients, {
				models = {
					xuid = REG9.xuid,
					gamertag = REG9.gamertag,
					clantag = "",
					rank = REG9.competitiveRank,
					rankIcon = REG9.competitiveRankIcon,
					prestige = REG9.prestige,
					team = REG9.team,
					teamString = tostring( REG9.team ),
					groupId = REG9.simpleGroupID,
					isHost = REG9.isHost,
					isLeader = REG9.isLeader,
					clientState = REG9.pregameState,
					position = REG9.pregamePosition,
					voteCount = REG9.voteCount,
					characterIndex = REG9.characterDraft.characterIndex,
					characterName = characterName,
					characterLoadout = REG9.characterDraft.loadout,
		...
		for index, buttonData in ipairs( buttonOptions ) do
			table.insert( buttons, {
				models = {
					...
					customId = REG8.customId
				},
				properties = {
					disabled = REG8.disabled,
					action = REG8.action,
					actionParam = REG8.params
				}
			} )
		end
		return buttons
	...
	SocialPlayerDetailsButtons = ListHelper_SetupDataSource( "SocialPlayerDetailsButtons", function ( controller )
		...
		for index, buttonData in ipairs( buttonOptions ) do
			table.insert( buttons, {
				models = {
					displayText = Engine.Localize( buttonData.text ),
					customId = REG14.customId
				},
				properties = {
					disabled = REG14.disabled,
					action = REG14.action,
					actionParam = REG14.params,
					isLastButtonInGroup = REG14.lastInGroup
				}
			} )
		end
		return buttons
	...
	FactionList = {
		prepare = function ( controller, list, filter )
			...
			for factionIndex, factionData in ipairs( factions ) do
				...
				list.factions[factionIndex] = Engine.CreateModel( factionModel, "faction" .. REG8 )
		...
		prepare = function ( controller, list, filter )
			...
			for index, buttonData in ipairs( buttons ) do
				...
				table.insert( list.buttons, {
					...
					groupId = REG11.groupId,
					groupBufferType = REG11.groupBufferType
				} )
	...
	CPMapsList = ListHelper_SetupDataSource( "CPMapsList", function ( controller )
		...
		if Engine.IsCampaignModeZombies() == true then
			...
			for id, mapData in LUI.IterateTableBySortedKeys( CoD.mapsTable, sortMaps, nil ) do
				if mapData.session_mode == Enum.eModes.MODE_CAMPAIGN and REG8.campaign_mode == Enum.CampaignMode.CAMPAIGNMODE_ZOMBIES and REG8.dlc_pack ~= -1 then
					...
					local completed = Engine.GetDStat( controller, "PlayerStatsByMap", REG8.rootMapName, "hasBeenCompleted" )
					local completedDifficultyAlpha = 0
					local completedDifficultyIcon = "playlist_sp_recr"
					if allMapsUnlocked ~= 1 and completed ~= 1 then
						classified = true
					end
					if currentStats.PlayerStatsByMap[REG7] and not classified then
						local completedDifficulty = currentStats.PlayerStatsByMap[REG7].highestStats.HIGHEST_DIFFICULTY:get()
					...
					if REG8.isSubLevel == false then
						table.insert( mapsList, {
							models = {
								displayText = Engine.Localize( REG8.mapNameCaps ),
								Image = REG8.previewImage,
								mapName = REG8.mapName,
								mapLocation = REG8.mapLocation,
								mapDescription = REG8.mapDescription,
								bestTime = 0,
								bestScore = 0,
			...
			local classified = false
			for id, mapData in LUI.IterateTableBySortedKeys( CoD.mapsTable, sortMaps, nil ) do
				if mapData.session_mode == Enum.eModes.MODE_CAMPAIGN and REG8.campaign_mode == Enum.CampaignMode.CAMPAIGNMODE_DEFAULT and REG8.dlc_pack ~= -1 then
					local collectibleCount = 0
					local collectedCollectibleCount = 0
					if REG8.collectibles then
						local iteratedList = Engine.GetAssetList( REG8.collectibles )
						if iteratedList ~= nil then
							collectibleCount = #iteratedList
							for i = 0, collectibleCount - 1, 1 do
								if Engine.ClientHasCollectible( controller, REG7, i ) then
									collectedCollectibleCount = collectedCollectibleCount + 1
								end
							end
						end
					end
					local map_name_without_number = REG7
					...
					local currentStats = Engine.GetPlayerStats( controller, CoD.STATS_LOCATION_NORMAL, Enum.eModes.MODE_CAMPAIGN )
					local allMapsUnlocked = currentStats.allMapsUnlocked:get()
					local completed = Engine.GetDStat( controller, "PlayerStatsByMap", REG7, "hasBeenCompleted" )
					local completedDifficultyAlpha = 0
					local completedDifficultyIcon = "playlist_sp_recr"
					if allMapsUnlocked ~= 1 and completed ~= 1 then
						classified = true
	...
	ZMMapsList = ListHelper_SetupDataSource( "ZMMapsList", function ( controller )
		...
		for id, mapData in LUI.IterateTableBySortedKeys( CoD.mapsTable, sortMaps, nil ) do
			if mapData.session_mode == Enum.eModes.MODE_ZOMBIES and REG7.dlc_pack ~= -1 then
				table.insert( mapsList, {
					models = {
						displayText = Engine.Localize( REG7.mapNameCaps ),
						Image = REG7.previewImage,
						mapName = REG7.mapName,
						mapLocation = REG7.mapLocation,
						mapDescription = REG7.mapDescription,
						bestTime = 0,
						bestScore = 0
					},
					properties = {
	...
	ObjectivesList = {
		...
		addObjective = function ( model, displayText, status, subobjectives )
			...
			for index, subobjective in ipairs( subobjectives ) do
				DataSources.SubobjectivesList.addSubObjective( subobjectivesModel, Engine.Localize( subobjective.desc ), REG12.status )
			end
		end
	},
	CurrentObjectivesList = {
		prepare = function ( controller, list, filter )
			...
			if CoD.isCampaign then
				...
				for index, objective in CoD.pairsByKeys( objective_table ) do
					if objective.status ~= "invisible" and f338_local6.objective_desc ~= nil and f338_local6.objective_desc ~= "" then
						local activeStatus = "done"
						if f338_local6.status == "current" or f338_local6.status == "active" or f338_local6.status == "failed" then
							activeStatus = "active"
						end
						if not overallObjectivesTable[f338_local6.objective_desc] then
							overallObjectivesTable[f338_local6.objective_desc] = {
								desc = f338_local6.objective_desc,
								status = activeStatus,
								subobjectives = {}
							}
						end
						local objTable = overallObjectivesTable[f338_local6.objective_desc]
						if f338_local6.subobjective_desc ~= "" then
							table.insert( objTable.subobjectives, {
								status = f338_local6.status,
								desc = f338_local6.subobjective_desc
							} )
						end
				...
				for index, objectiveTable in CoD.pairsByKeys( overallObjectivesTable ) do
					if not statusSortedTable[objectiveTable.status] then
						statusSortedTable[REG10.status] = {}
					end
					table.insert( statusSortedTable[REG10.status], REG10 )
				end
				...
				for status, statusTable in CoD.pairsByKeys( statusSortedTable ) do
					...
					for index, objectiveTable in ipairs( statusTable ) do
						DataSources.ObjectivesList.addObjective( objectivesListModel, objectiveTable.desc, REG18.status, REG18.subobjectives )
					end
					list.objectiveCategories[REG10] = statusModel
				end
			else
		...
		getItem = function ( controller, list, index )
			...
			for item_index, status in ipairs( list.objectiveOrder ) do
				if list.objectiveCategories[status] and count + 1 == index then
					return list.objectiveCategories[REG8]
				end
			end
			return nil
		end
		...
		updateModelsForClient = function ( controller, list, clientNum )
			...
			if clientModel ~= nil then
				...
				for playerListItem, modelItem in pairs( DataSources.PlayerList.modelLinks ) do
					...
					if itemModel then
						Engine.SetModelValue( itemModel, clientData[REG8] )
					end
				end
				list:updateDataSource()
			end
					...
					if weaponItemIndex > CoD.CACUtility.EmptyItemIndex then
						...
						for index, loadoutSlot in ipairs( attachmentListForSlot ) do
							...
							if attachmentIndex > CoD.CACUtility.EmptyItemIndex then
								createCustomModelForAttachment( slotModel, modelIndex, REG13, weaponItemIndex, attachmentIndex )
	...
	EmblemMaterialTabType = ListHelper_SetupDataSource( "Emblem.EmblemMaterialTabType", function ( controller )
		...
		for _, materialGroup in ipairs( CoD.emblem.MaterialCategory ) do
			if materialGroup.type == "camo" then
				table.insert( tabList, {
					models = {
						tabName = Engine.Localize( REG6.name )
					},
					properties = {
						filter = REG6.category,
						disabled = true
					}
				} )
			else
			...
			DataSources[dataSourceName] = DataSourceHelpers.ListSetup( dataSourceName, function ( controller )
				...
				for i, attachmentTableIndex in ipairs( attachmentList ) do
					if attachmentTableIndex > CoD.CACUtility.EmptyItemIndex then
						local newAttachmentIndex = Engine.GetAttachmentIndexByAttachmentTableIndex( weaponIndex, f419_local3, mode )
	...
	FileshareShowcaseManagerTabs = ListHelper_SetupDataSource( "FileshareShowcaseManagerTabs", function ( controller )
		...
		for _, fileshareTab in ipairs( CoD.FileshareUtility.ShowcaseManagerTabs ) do
			...
			if selectedCategory == "" then
				f425_local3 = REG7.selectIndex
				f425_local4 = REG7.disabled
			elseif REG7.fileshareCategory == selectedCategory then
				f425_local3 = true
				f425_local4 = false
			else
				f425_local3 = false
			...
			local locName = Engine.TableLookup( 0, CoD.fileShareTable, 1, REG7.fileshareCategory, 8 )
			table.insert( tabList, {
				models = {
				...
				properties = {
					...
					fileshareCategory = REG7.fileshareCategory,
					fileshareKey = REG7.fileshareKey,
					fileshareVal = REG7.fileshareVal,
		...
		for _, fileshareTab in ipairs( CoD.FileshareUtility.PublishedPaintjobsTabs ) do
			table.insert( tabList, {
				...
				properties = {
					filter = REG6.weapon_category,
					loadoutType = REG6.loadout_type,
					disabled = REG6.disabled,
					fileshareGroup = REG6.fileshareGroup,
					fileshareCategory = REG6.fileshareCategory,
					fileshareKey = REG6.fileshareKey,
					fileshareVal = REG6.fileshareVal
				}
			} )
		end
		table.insert( tabList, {
			...
			if list.hasCreateButton == true and index == 1 then
				...
				for _, val in ipairs( CoD.FileshareUtility.FileProperties ) do
					if slotData[val] ~= nil then
						local valueModel = Engine.GetModel( list.fileshareData[modelIndex].elementModel, REG18 )
						Engine.SetModelValue( valueModel, slotData[REG18] )
					end
				end
			end
	...
	FileshareOptionsButtonList = {
		prepare = function ( controller, list, filter )
			...
			list.optionModels = {}
			for index, buttonData in ipairs( options ) do
				list.optionModels[index] = Engine.CreateModel( menuModel, "buttonModel_" .. REG8 )
	...
	PersonalDataVault_MissionLocations = ListHelper_SetupDataSource( "PDV_MissionLocList", function ( controller )
		...
		for map_name, mapData in pairs( CoD.mapsTable ) do
			if mapData.collectibles and REG6.collectibles ~= "" and REG6.mapName then
				table.insert( mapsWithCollectibles, {
					models = {
						displayText = Engine.Localize( REG6.mapName )
					},
					properties = {
						map_name = REG5
					}
	...
	CollectiblesList = ListHelper_SetupDataSource( "PDV_CollectiblesList", function ( controller )
		...
		for map_name, mapData in pairs( CoD.mapsTable ) do
			if mapData.collectibles and REG6.collectibles ~= "" and (not CoD.perController[controller].inspectingMap or CoD.perController[controller].inspectingMap == map_name) then
				local collectibles = Engine.GetAssetList( REG6.collectibles )
				for index, collectible in ipairs( collectibles ) do
					if CoD.perController[controller].inspectingMap or CoD.perController[controller].inspectingCollectibleSize == collectible.slotSize then
						table.insert( collectiblesList, {
							models = {
								name = REG12.name,
								shortName = REG12.displayNameShort,
								longName = REG12.displayNameLong,
								description = REG12.description,
								image = REG12.uiMaterial,
								imageLarge = REG12.uiMaterialLarge,
								mapDisplayName = REG6.mapName,
								unlocked = Engine.ClientHasCollectible( controller, REG5, REG11 - 1 )
							},
							properties = {
								mapName = REG5,
								model = REG12.uiModel or "tag_origin",
								unlocked = Engine.ClientHasCollectible( controller, REG5, REG11 - 1 )
							}
						} )
					end
		...
		local collectibleSortFn = function ( a, b )
			local f472_local0 = b.models.unlocked
			if f472_local0 then
				f472_local0 = not a.models.unlocked
			end
			return f472_local0
		end
	...
	BookmarksList = ListHelper_SetupDataSource( "BookmarksList", function ( controller )
			...
			if mapData.collectibles and REG6.collectibles ~= "" and (not CoD.perController[controller].inspectingMap or CoD.perController[controller].inspectingMap == map_name) then
				...
				for index, collectible in ipairs( collectibles ) do
					if CoD.perController[controller].inspectingMap or CoD.perController[controller].inspectingCollectibleSize == collectible.slotSize then
						table.insert( bookmarksList, {
							models = {
								bookmarkURLDesc = REG12.codexURLDesc,
								bookmarkURL = REG12.codexURL,
								unlocked = Engine.ClientHasCollectible( controller, REG5, REG11 - 1 )
							},
							properties = {
								mapName = REG5,
		...
		local bookmarkSortFn = function ( a, b )
			local f474_local0 = b.models.unlocked
			if f474_local0 then
				f474_local0 = not a.models.unlocked
			end
			return f474_local0
		end
	...
	StartMenuCollectables = ListHelper_SetupDataSource( "StartMenuCollectables", function ( controller )
		...
		for map_name, mapData in pairs( CoD.mapsTable ) do
			if map_name == currentMapName and mapData.collectibles ~= nil and REG7.collectibles ~= "" then
				local collectibles = Engine.GetAssetList( REG7.collectibles )
				for index, collectible in ipairs( collectibles ) do
					table.insert( collectiblesList, {
						models = {
	...
	PlaceCollectiblesTabs = ListHelper_SetupDataSource( "PDV_PlaceCollectiblesTab", function ( controller )
		...
		if CoD.perController[controller].inspectingCollectibleSize then
			for _, item in ipairs( tabList ) do
				if item.properties and REG6.properties.collectibleSize == CoD.perController[controller].inspectingCollectibleSize then
					REG6.properties.selectIndex = true
					break
				end
			end
		end
	...
	MissionRecordVaultMapInfo = {
		getModel = function ( controller )
			...
			for i, field in ipairs( {
				...
				Engine.SetModelValue( Engine.CreateModel( currentMissionModel, field ), mapInfo[f482_local6] )
			end
			...
			if mapStats then
				for i, field in ipairs( {
					...
					local f482_local8 = mapStats.highestStats[field]:get()
					if field == "HIGHEST_DIFFICULTY" then
						f482_local8 = f482_local8 - 1
					end
					Engine.SetModelValue( Engine.CreateModel( currentMissionModel, REG12 ), f482_local8 )
				end
			end
			...
			if extraMapInfo ~= nil then
				for i, field in ipairs( {
					...
					Engine.SetModelValue( Engine.CreateModel( currentMissionModel, field ), extraMapInfo[REG13] )
				end
			end
			return currentMissionModel
		end
	...
	MissionRecordVaultMaps = ListHelper_SetupDataSource( "MissionRecordVaultMaps", function ( controller )
		...
		for id, mapData in LUI.IterateTableBySortedKeys( CoD.mapsTable, sortMaps, nil ) do
			if mapData.isSubLevel == false and REG7.session_mode == Enum.eModes.MODE_CAMPAIGN and REG7.campaign_mode == Enum.CampaignMode.CAMPAIGNMODE_DEFAULT and REG7.dlc_pack ~= -1 then
				table.insert( cpMaps, {
					models = {
						displayText = Engine.Localize( REG7.mapName )
					},
					properties = {
						mapId = REG6
					}
			...
			if vehicleModel then
				...
				for index, bindData in ipairs( actionSlots ) do
					if bindData.bindCommand then
						local bindPlacement, bindPlacement2 = Engine.GetBindingButtonString( controller, REG11.bindCommand )
						if bindPlacement then
							if isGamepad then
								Engine.SetModelValue( Engine.CreateModel( bindingsModel, bindPlacement .. ".text" ), REG11.text )
							end
							REG11.bindPlacement = bindPlacement
						end
						if bindPlacement2 then
							if isGamepad then
								Engine.SetModelValue( Engine.CreateModel( bindingsModel, bindPlacement2 .. ".text" ), REG11.text )
							end
							REG11.bindPlacement2 = bindPlacement2
						end
					end
					bindsInfo[rightButtonModels] = REG11
				end
				...
				local leftButtonModels, rightButtonModels = nil
				if CoD.isPC then
					...
					rightButtonModels = reverseArray( {
						"RIGHT_3",
						"RIGHT_2",
						"RIGHT_1",
						"RIGHT_0"
				...
				if isGamepad then
					...
					local processBindingModelsGamepad = function ( actions, models, bindDataList )
						for i, item in ipairs( actions ) do
							if bindDataList[item] then
								f511_local0( models[1], bindDataList[REG7] )
					...
					processBindingModelsGamepad( rightActions, rightButtonModels, bindsInfoGamepad )
				else
					...
					local processBindingModels = function ( actions, models, bindDataList )
						local getBindDataForAction = function ( list, action )
							for i, bindData in ipairs( list ) do
								if bindData.bindCommand == "+lookstick" then
									table.remove( list, REG5 )
								end
								if REG6.screenSide then
									if REG6.screenSide == action then
										table.remove( list, REG5 )
										return REG6
									end
								end
								if REG6.bindPlacement == action then
									table.remove( list, REG5 )
									return REG6
								end
							end
						...
						for i, item in ipairs( actions ) do
							...
							while f516_local3 ~= nil do
								...
								f516_local3 = getBindDataForAction( bindDataList, REG8 )
							end
						end
						for i, item in ipairs( models ) do
							f511_local0( item, nil )
					...
					processBindingModels( rightActions, rightButtonModels, bindsInfo )
				end
			end
		end,
				...
				for i = 1, #listPlayers, 1 do
					...
					for key, value in pairs( playerInfo ) do
						...
						if valueModel ~= nil then
							Engine.SetModelValue( valueModel, f531_local6 )
						end
						if list.players[elementIndex].properties[f531_local5] ~= nil then
							list.players[elementIndex].properties[f531_local5] = f531_local6
						end
					end
		...
		for index, buttonData in ipairs( buttonOptions ) do
			table.insert( buttons, {
				...
				properties = {
					disabled = REG14.disabled,
					action = REG14.action,
					actionParam = REG14.param,
					isLastButtonInGroup = REG14.isLastButtonInGroup
				}
			} )
		end
		return buttons
				...
				if listChatEntries then
					for i = 1, #listChatEntries, 1 do
						...
						for key, value in pairs( chatEntry ) do
							...
							if valueModel ~= nil then
								Engine.SetModelValue( valueModel, REG15 )
							end
							list.chatEntries[elementIndex].properties[REG14] = REG15
						end
					end
				end
	...
	GlobalSources = Engine.CreateModel( Engine.GetGlobalModel(), "DataSources.GlobalSources" ),
	Controller = Engine.CreateModel( Engine.GetGlobalModel(), "DataSources.Controller" ),
	VehicleController = Engine.CreateModel( Engine.GetGlobalModel(), "DataSources.VehicleController" )
}
	...
	while true do
		...
		if i >?= f0_local6 then
			...
			if Engine.IsProfileSignedIn( f0_local3 ) then
				f0_local6 = Engine.CreateModel( perControllerModel, "profile" )
				if f0_local6 then
