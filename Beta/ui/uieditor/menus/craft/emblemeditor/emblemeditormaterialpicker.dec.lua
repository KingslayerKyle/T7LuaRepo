...
LUI.createMenu.EmblemEditorMaterialPicker = function ( controller )
	...
	local emblemDrawWidget0 = CoD.EmblemDrawWidgetNew.new( menu, controller )
	...
	local materialSwatch = CoD.EmblemEditorMaterialSwatch.new( menu, controller )
	...
	local TitleEditLayer = CoD.cac_3dTitleIntermediary.new( menu, controller )
	...
	local materialInfo = CoD.EmblemEditorMaterialInfo.new( menu, controller )
	...
	local materialControls = CoD.EmblemEditorMaterialControls.new( menu, controller )
	...
	materialControls:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "Emblem.EmblemProperties.materialScaleMode" ), function ( model )
		menu:updateElementState( materialControls, {
	...
	local buttonBar = CoD.fe_FooterContainer_NOTLobby.new( menu, controller )
