				...
				if type( possibleSlot ) == "table" then
					for index, additionalPossibleSlot in ipairs( possibleSlot ) do
						if additionalPossibleSlot.slot then
							local possibleWidget = self.weaponSlotTable[REG13.slot].widget
							if possibleWidget and (not REG13.condition or REG13.condition( controller, self, possibleWidget, {
								possibleSlot = REG13.slot
							} )) then
								widget = possibleWidget
								possibleSlot = REG13.slot
								conditionFn = REG13.condition
							end
						end
					end
...
LUI.createMenu.CustomClass = function ( controller )
	...
	local previewWidget = CoD.InfopanePreviewWidget.new( menu, controller )
	...
	local CACBackground0 = CoD.CACBackgroundNew.new( menu, controller )
	...
	menu:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBY_PSTRIANGLE, "C", function ( element, menu, controller, model )
	...
	menu:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_PCKEY_0, "SPACE", function ( element, menu, controller, model )
