...
local CloseAllPopups = function ( self, controller )
	...
	while occludedMenu ~= nil do
		...
		Engine.SendClientScriptNotify( controller, "menu_change", "Pregame_Main", "closeToMenu" )
		...
		elseif clientState == Enum.LobbyClientPregameState.LOBBY_CLIENT_PREGAME_STATE_SELECTION_FORCED then
			if pregameState == Enum.LobbyPregameState.LOBBY_PREGAME_STATE_CHARACTER_DRAFT then
				...
				for index, client in ipairs( session.sessionClients ) do
					if Engine.CompareUInt64( client.xuid, xuid, "=" ) then
						local characterIndex = REG9.characterDraft.characterIndex
						local loadout = REG9.characterDraft.loadout
...
LUI.createMenu.Pregame_Main = function ( controller )
	...
	local PregameVsPanel0 = CoD.Pregame_VsPanel.new( menu, controller )
	...
	local PregameVsPanel = CoD.Pregame_VsPanel.new( menu, controller )
	...
	local Team1ListSmall = LUI.UIList.new( menu, controller, 0, 0, function ( model )
	...
	local Team1ListLarge = LUI.UIList.new( menu, controller, 10, 0, function ( model )
		return Engine.GetModelValue( Engine.GetModel( model, "teamString" ) ) == "1"
	end, false, false, 0, 0, false, true )
	...
	local Team2ListSmall = LUI.UIList.new( menu, controller, 0, 0, function ( model )
	...
	local Team2ListLarge = LUI.UIList.new( menu, controller, 11, 0, function ( model )
		return Engine.GetModelValue( Engine.GetModel( model, "teamString" ) ) == "2"
	end, false, false, 0, 0, false, true )
	...
	local PregameBanProtectSelections = CoD.Pregame_BanProtectSelections.new( menu, controller )
	...
	local PregameMainTimer = CoD.Pregame_MainTimer.new( menu, controller )
	...
	local PregameTitle = CoD.Pregame_Title.new( menu, controller )
	...
	local PreGameStates = CoD.PrematchCountdown_BeginsIn.new( menu, controller )
	...
	local Team1IconAndNameWidgetContainer = CoD.TeamIconAndNameWidgetContainer.new( menu, controller )
	...
	local Team2IconAndNameWidgetContainer = CoD.TeamIconAndNameWidgetContainer.new( menu, controller )
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( self )
		...
		self.PregameVsPanel0:close()
		self.PregameVsPanel:close()
