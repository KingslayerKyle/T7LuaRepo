...
LUI.createMenu.Store = function ( controller )
	...
	local productList = LUI.UIList.new( menu, controller, 8, 0, nil, true, false, 0, 0, false, false )
	...
	local MissionRecordVaultlines020 = CoD.MissionRecordVault_lines.new( menu, controller )
	...
	self:mergeStateConditions( {
		...
		},
		{
			stateName = "NoItems",
			condition = function ( menu, element, event )
				local f12_local0 = IsGlobalModelValueEqualTo( element, controller, "StoreRoot.ready", 1 )
				if f12_local0 then
					f12_local0 = not HasStoreItems( controller )
				end
				return f12_local0
			end
		},
