		...
		elseif clientState == Enum.LobbyClientPregameState.LOBBY_CLIENT_PREGAME_STATE_CHARACTER_SELECTION_FORCED then
			...
			for index, client in ipairs( session.sessionClients ) do
				if client.xuid == xuid then
					local characterIndex = REG8.characterDraft.characterIndex
					local loadout = REG8.characterDraft.loadout
...
LUI.createMenu.Competitive_CharacterDraft = function ( controller )
	...
	local GameModeBackground = CoD.FE_ButtonPanel.new( menu, controller )
	...
	local StartMenuWhiteGlow0 = CoD.StartMenu_WhiteGlow.new( menu, controller )
	...
	local Team1List = LUI.UIList.new( menu, controller, 4, 0, function ( model )
	...
	local Team2List = LUI.UIList.new( menu, controller, 4, 0, function ( model )
