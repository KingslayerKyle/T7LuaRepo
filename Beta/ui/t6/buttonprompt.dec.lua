...
CoD.ButtonPrompt.new = function ( button, text, eventOwner, eventName, isBlankButton, qualifier, DualButton, keyboardShortcutOverride, shortcutKey )
		...
		local buttonEventHandler = function ( self, event )
			if not self.disabled and event.down == true then
				if event.button == button and (qualifier == nil or event.qualifier == qualifier) then
					...
					return true
				elseif CoD.isPC and event.button == "key_shortcut" and event.key == shortcutKey then
					eventOwner:processEvent( {
						name = eventName,
						controller = event.controller
					} )
	...
	if not isBlankButton then
		...
		if buttonImage ~= nil then
			local prompt = CoD.buttonStrings[button]
			if CoD.isPC then
				if shortcutKey ~= nil and shortcutKey ~= "" then
				...
				if not Engine.LastInput_Gamepad() then
					if buttonImage.shortcutKey then
						prompt = shortcutKey
...
CoD.ButtonPrompt.addUIEditorButtonPrompt = function ( owner, button, text, shortcutKey )
	...
	if CoD.useMouse and (buttonImage ~= nil or text ~= "") then
		...
		buttonPrompt:setHandleMouseMove( true )
		buttonPrompt:registerEventHandler( "input_source_changed", CoD.ButtonPrompt.InputSourceChanged )
		buttonPrompt.clipsPerState = {
			DefaultState = {
				DefaultClip = function ()
					buttonPrompt:setupElementClipCounter( 1 )
					buttonText:completeAnimation()
					buttonText:setRGB( 1, 1, 1 )
					buttonPrompt.clipFinished( buttonText, {} )
				end,
				Over = function ()
					buttonPrompt:setupElementClipCounter( 1 )
					buttonText:completeAnimation()
					buttonText:setRGB( 0.87, 0.37, 0 )
					buttonPrompt.clipFinished( buttonText, {} )
				end
			}
		}
		buttonPrompt:setState( "DefaultState" )
	end
	CoD.ButtonPrompt.ResizeButtonPrompt( buttonPrompt )
	buttonPrompt.SetText = CoD.ButtonPrompt.SetText
	return buttonPrompt
...
CoD.ButtonPrompt.ResizeButtonPrompt = function ( buttonPrompt )
	...
	if CoD.isPC and not Engine.LastInput_Gamepad() then
		...
		buttonPrompt.label:setLeftRight( true, false, newWidth, newWidth + labelTextWidth )
...
CoD.ButtonPrompt.InputSourceChanged = function ( buttonPrompt, event )
	...
	elseif CoD.useMouse then
		...
		CoD.ButtonPrompt.ResizeButtonPrompt( buttonPrompt )
		if buttonPrompt.button == "primary" then
			if event.source == 0 then
				buttonPrompt:setAlpha( 1 )
				buttonPrompt:setPriority( -100 )
			else
				buttonPrompt:setAlpha( 0 )
				buttonPrompt:setPriority( 1000 )
			end
		end
	end
end
