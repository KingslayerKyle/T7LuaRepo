...
CoD.ChangeGameMode.CreateCategoryItemInfo = function ()
	...
	for index, categoryName in ipairs( availableCategories ) do
		if categoryName ~= "saved" or Engine.SessionModeIsMode( CoD.SESSIONMODE_ONLINE ) then
			...
			category.ref = REG6
			category.name = Engine.Localize( "MPUI_CATEGORY_" .. REG6 .. "_CAPS" )
			category.description = Engine.Localize( "MPUI_CATEGORY_" .. REG6 .. "_DESC" )
			category.iconMaterial = RegisterMaterial( CoD.ChangeGameMode.CategoryIconNames[REG5] )
...
CoD.ChangeGameMode.AddCategoryButtons = function ( self )
	...
	for index, category in ipairs( categories ) do
		...
		if REG9.ref == gametypeCategory then
			currentCategoryIndex = REG8
		end
	end
...
CoD.ChangeGameMode.AddGameModeButtons = function ( self, controller, gameModeCategory )
	...
	for index, gameMode in pairs( gameModes ) do
		...
		if Dvar.ui_execdemo:get() then
			f13_local3 = CoD.ChangeGameMode.DemoBuildHasGameType( REG11.gametype )
		end
		if REG11.category == gameModeCategory and f13_local3 then
			local gameModeInfo = CoD.ChangeGameMode.CreateGameModeInfo( controller, REG11 )
			...
			local gameModeButton = CoD.PlaylistButton.new( gameModeButtonDefaultAnimationState, "game_mode_button_action", "game_mode_button_gain_focus", "game_mode_button_lose_focus", gameModeInfo, REG10 )
			...
			if REG11.gametype == gametypeName then
