	...
	elseif card.fileData.description == nil or card.fileData.description == "" then
		...
		elseif card.fileData.category ~= "emblem" then
			if not CoD.isZombie then
				...
				card.fileData.description = Engine.Localize( "MENU_FILESHARE_GAMEONMAP", data.gameTypeName, Engine.Localize( data.mapName ) )
			else
	...
	if card.fileData.fileID ~= "0" then
		...
		if not CoD.isZombie then
			...
			elseif card.fileData.map ~= nil then
				thumbImage = RegisterImage( CoD.GetMapValue( card.fileData.map, "previewImage", "$black" ) )
			end
...
CoD.Codtv.ShowCustomFilePanel = function ( card )
	...
	if card.fileData.fileID ~= nil and card.fileData.fileID ~= "0" then
		local gameTypeString = card.fileData.gameTypeName
...
CoD.Codtv.SlotsAvailable = function ( codtv, controller )
	if codtv.m_loaded == true then
		return 
	end
	codtv.m_loaded = true
	codtv.spinner:setAlpha( 0 )
	if codtv.m_rootRef ~= nil and codtv.m_rootRef ~= "" then
		local folderIndex = Engine.GetCodtvRoot( codtv.m_rootRef )
		if folderIndex == nil then
			codtv.occludedMenu:openPopup( "FileshareManager_Error", controller )
			codtv:close()
			return 
		end
		local codtvRoot = Engine.GetCodtvContent( folderIndex )
...
LUI.createMenu.CODTv = function ( controller )
	...
	if codtvRootRef == "combatrecord" then
		...
		codtv.saveButtonPrompt = CoD.ButtonPrompt.new( "alt1", Engine.Localize( "MENU_FILESHARE_SAVE" ), codtv, "card_save", false, false, false, false, "S" )
		codtv.reportButtonPrompt = CoD.ButtonPrompt.new( "alt1", Engine.Localize( "MENU_FILESHARE_REPORT_EMBLEM" ), codtv, "card_report", false, false, false, false, "S" )
		codtv.voteButtonPrompt = CoD.ButtonPrompt.new( "alt2", Engine.Localize( "MENU_FILESHARE_LIKEDISLIKE" ), codtv, "card_vote", false, false, false, false, "R" )
		codtv.emblemOptionsPrompt = CoD.ButtonPrompt.new( "alt1", Engine.Localize( "MENU_EMBLEM_OPTIONS" ), codtv, "fileshare_emblem_options", false, false, false, false, "O" )
	end
	...
	CoD.Codtv.SlotsAvailable( codtv, controller )
	return codtv
end
