...
CreateRestrictWeaponsMenu = function ( menuName, controller )
	...
	if menuName == "RestrictPrimaryWeapons" then
		...
		AddWeaponCategory( self, "weapon_knife" )
		AddWeaponCategory( self, "weapon_special" )
	end
...
AddWeaponCategory = function ( self, groupName )
	...
	button:setActionEventNameNewStyle( self, self:getOwner(), groupName )
	...
	for index, itemIndex in ipairs( items ) do
		if Engine.GetLoadoutSlotForItem( itemIndex ) == loadoutSlot and Engine.IsItemIndexRestricted( REG9 ) ~= Engine.IsItemIndexRestricted( REG9, true ) then
