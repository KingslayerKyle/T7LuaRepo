...
CoD.PlaylistSelection.AddCategoryButtons = function ( self, controller )
	...
	for index, category in ipairs( categories ) do
		if category.filter == self.filter then
			self.totalCategoryPlayerCount = self.totalCategoryPlayerCount + REG10.playerCount
			...
			local categoryButton = CoD.CategoryButton.new( categoryButtonDefaultAnimationState, "category_button_action", "category_button_gain_focus", "category_button_lose_focus", REG10 )
			categoryButton:setLabel( Engine.ToUpper( REG10.name ) )
			...
			if REG10.locked == true then
				categoryButton:disable()
			end
			if REG10 == profile_category then
...
CoD.PlaylistSelection.AddPlaylistButtons = function ( self, controller, itemInfo )
	...
	for index, playlist in ipairs( playlists ) do
		...
		if playlist.index == profile_playlist_index then
			profile_playlist = REG9
		end
		if CoD.PlaylistSelection.PlaylistIsLocked( controller, REG10 ) then
...
CoD.PlaylistSelection.AddSearchPreferencesButtonPrompt = function ( self )
	self.searchPreferencesButton = CoD.ButtonPrompt.new( "alt1", Engine.Localize( "MPUI_SEARCH_PREFERENCES" ), self, "button_prompt_search_preferences", false, false, false, false, "S" )
