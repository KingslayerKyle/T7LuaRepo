...
LUI.createMenu.EditDefaultClasses = function ( controller )
	...
	self.classOptionsButtonPrompt = CoD.ButtonPrompt.new( "alt1", Engine.Localize( "MENU_CLASS_OPTIONS" ), self, "open_class_options", false, false, false, false, "O" )
...
RefreshClassButtons = function ( self )
	local customClassNames = CoD.CACUtility.GetLoadoutNames( self:getOwner() )
	for index, button in ipairs( self.classButtons ) do
		button.label.setText( REG6.label, Engine.Localize( customClassNames[REG6.classNum]:get() ) )
