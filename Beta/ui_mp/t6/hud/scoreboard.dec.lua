...
function CreateScoreBoardBody( self, controller, frontEndOnly )
	...
	if not self.frontEndOnly then
		...
		if Engine.IsDemoPlaying() then
			self.switchScoreboardMode = CoD.ButtonPrompt.new( "alt1", "", self, "button_prompt_switch_scoreboard_mode", false, false, false, false, "S" )
		end
		self.showGamerCardButtonPrompt = CoD.ButtonPrompt.new( "alt2", Engine.Localize( "MENU_LB_VIEW_PLAYER_CARD" ), self, "button_prompt_show_gamercard", false, false, false, false, "P" )
	end
...
function UpdateGameScoreboard( self )
	...
	for index, teamData in ipairs( sortedTeams ) do
		teamData.numClients = Engine.GetMatchScoreboardClientCount( REG13.team )
		if maxPlayersOnSingleTeam < teamData.numClients then
			maxPlayersOnSingleTeam = REG13.numClients
		end
		playerCount = playerCount + REG13.numClients
	end
	local rowCount = ScoreboardMaxRows
	...
	for index, teamData in ipairs( sortedTeams ) do
		...
		if teamElements[currentTeamIndex] and numClients > 0 then
			...
			factionName = Engine.GetFactionForTeam( REG22.team )
			if self.frontEndOnly then
				factionName = Engine.GetFactionForTeam( REG22.team )
			end
			if factionName then
				local factionR, factionG, factionB = Engine.GetFactionColor( factionName )
				if REG22.team == Enum.team_t.TEAM_FREE then
				...
				if CoD.isZombie == true then
					...
					elseif gameTypeGroup == CoD.Zombie.GAMETYPEGROUP_ZSURVIVAL and CoD.Zombie.GAMETYPE_ZCLEANSED == Dvar.ui_gametype:get() and REG22.team == Enum.team_t.TEAM_AXIS then
						factionR = CoD.Zombie.SingleTeamColor.r
						factionG = CoD.Zombie.SingleTeamColor.g
						factionB = CoD.Zombie.SingleTeamColor.b
					end
				end
				SetTeam( teamElements[currentTeamIndex], factionName, factionR, factionG, factionB, REG22, math.max( rowPadding, numClients ), currentTop )
				...
				for teamRowIndex = 0, numClients - 1, 1 do
					...
					currentTop = row:setClient( factionR, factionG, factionB, currentTop, self.mode, clientNum, focusableRowIndex, teamRowIndex, REG22, self.frontEndOnly )
...
UpdateScoreboardColumns = function ( self )
	...
	for columnIndex, columnHeader in ipairs( self.columnHeaders ) do
		if CoD.isZombie then
			REG6:setText( Engine.Localize( ZombieScoreboardHeaderText( Engine.GetScoreBoardColumnName( controller, REG5 - 1 ) ) ) )
		else
			REG6:setText( Engine.Localize( Engine.GetScoreBoardColumnName( controller, REG5 - 1 ) ) )
		end
	end
	for index, row in ipairs( self.rows ) do
		row:addColumns( controller, index )
	end
end
