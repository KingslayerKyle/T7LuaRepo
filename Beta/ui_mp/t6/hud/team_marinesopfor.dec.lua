...
CoD.ChooseTeam.AddButton = function ( self, buttonText, actionEventName, controller )
	local button = self.buttonList:addButton( buttonText )
	button:setActionEventNameNewStyle( self, controller, actionEventName )
end
...
CoD.ChooseTeam.PrepareButtonList = function ( controller, self )
	...
	self.buttonListScaleContainer = buttonListScaleContainer
	
	self.buttonListScaleContainer.id = "buttonListScaleContainer"
	...
	buttonListScaleContainer.buttonList = self.buttonList
	...
	if CoD.IsTeamChangeAllowed() then
		if teamName ~= Enum.team_t.TEAM_ALLIES then
			CoD.ChooseTeam.AddButton( self, CoD.GetTeamNameCaps( Enum.team_t.TEAM_ALLIES ), "alliesTeamSelected", controller )
		end
		if teamName ~= Enum.team_t.TEAM_AXIS then
			CoD.ChooseTeam.AddButton( self, CoD.GetTeamNameCaps( Enum.team_t.TEAM_AXIS ), "axisTeamSelected", controller )
		end
	end
	CoD.ChooseTeam.AddButton( self, Engine.Localize( "MPUI_AUTOASSIGN_CAPS" ), "autoAssignSelected", controller )
	if teamName ~= Enum.team_t.TEAM_SPECTATOR and f9_local0 == true then
		CoD.ChooseTeam.AddButton( self, CoD.GetTeamNameCaps( Enum.team_t.TEAM_SPECTATOR ), "spectatorSelected", controller )
	end
	if teamName == Enum.team_t.TEAM_SPECTATOR then
		if Engine.DvarBool( nil, "onlineunrankedgameandhost" ) then
			CoD.ChooseTeam.AddButton( self, Engine.Localize( "MPUI_END_GAME_CAPS" ), "open_endGamePopup", controller )
		else
			CoD.ChooseTeam.AddButton( self, Engine.Localize( "MPUI_LEAVE_GAME_CAPS" ), "open_endGamePopup", controller )
		end
	end
...
CoD.ChooseTeam.Close = function ( self, event )
	if event.menuName == CoD.ChangeClassMenuName then
		self:close()
	end
end
...
LUI.createMenu.team_marinesopfor = function ( controller )
	...
	ChooseTeam.buttonModel = Engine.CreateModel( Engine.GetModelForController( controller ), "team_marinesopfor.buttonPrompts" )
