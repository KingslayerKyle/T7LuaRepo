...
local updateVehicleHUD = function ( self, controller, vehicleType, recreateExistingHud )
	...
	if not f5_local1.hudName then
		local hudTable = nil
		for _, item in ipairs( f5_local1 ) do
			if not item.condition or f5_local7.condition( controller ) then
				hudTable = f5_local7
				break
			end
		end
...
local ShowLocationSelector = function ( controller )
	...
	if f64_local0 then
		f64_local0 = not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_IS_DEMO_PLAYING )
	end
	return f64_local0
end
