...
Lobby.Pregame.GetTeams = function ()
	...
	for index, client in pairs( session.sessionClients ) do
		if client.team ~= Enum.team_t.TEAM_SPECTATOR then
			if teams[REG6.team] == nil then
				teams[REG6.team] = {}
			end
			table.insert( teams[REG6.team], REG6 )
		end
	end
	...
	elseif shuffleMethod == Enum.PregameShuffleMethod.PREGAME_SHUFFLE_METHOD_REVERSE then
		for teamId, team in pairs( teams ) do
			...
			for f14_local4 = #team, 1, -1 do
				Engine.LobbyHost_SetClientPregamePosition( REG6[f14_local4].xuid, f14_local3 )
...
Lobby.Pregame.GetRandomUnpickedCharacter = function ( team )
	...
	for clientId, client in pairs( team ) do
		...
		if client.pregameState == Enum.LobbyClientPregameState.LOBBY_CLIENT_PREGAME_STATE_SELECTED or REG7.pregameState == Enum.LobbyClientPregameState.LOBBY_CLIENT_PREGAME_STATE_SELECTION_FORCED then
	...
	for heroIndex, hero in ipairs( heroTable ) do
		if hero.disabled == false then
			for i = 0, Enum.heroLoadoutTypes_e.HERO_LOADOUT_TYPE_COUNT - 1, 1 do
				local item = {
					heroIndex = REG6,
					loadoutIndex = i
				}
				local loadoutSlotInfo = Engine.GetLoadoutInfoForHero( Enum.eModes.MODE_MULTIPLAYER, REG6, i )
				if loadoutSlotInfo then
					local itemDisabled = loadoutSlotInfo.devOnly
					if itemDisabled == false and (REG7.loadouts == nil or REG7.loadouts[i] == nil) and Engine.GetItemVote( loadoutSlotInfo.itemIndex ) ~= Enum.VoteType.VOTE_TYPE_BAN then
	...
	for teamID, team in pairs( teams ) do
		...
		for clientIndex, client in pairs( team ) do
			if not f20_local3 and client.pregameState == Lobby.Pregame.ClientState.IDLE then
				Engine.LobbyHost_SetClientPregameState( REG12.xuid, Lobby.Pregame.ClientState.SELECTING )
				f20_local3 = true
		...
		for clientId, client in pairs( team ) do
			if client.pregameState == Lobby.Pregame.ClientState.SELECTING then
				if currentLobbyState == Lobby.Pregame.LobbyState.VOTE then
					Engine.LobbyHost_ForceClientRoundComplete( REG11.xuid )
				end
				if currentLobbyState == Lobby.Pregame.LobbyState.DRAFT then
					local characterIndex, loadout = Lobby.Pregame.GetRandomUnpickedCharacter( REG6 )
					Engine.LobbyHost_ForceClientCharacterSelection( REG11.xuid, characterIndex, loadout )
				end
				if currentLobbyState == Lobby.Pregame.LobbyState.CAC_EDIT or currentLobbyState == Lobby.Pregame.LobbyState.STREAK_EDIT then
					Engine.LobbyHost_ForceClientEditComplete( REG11.xuid )
				end
			end
		end
	end
...
Lobby.Pregame.GameLobbyClientDataUpdate = function ( data )
	for index, client in pairs( data.clients ) do
		if Engine.IsLocalClient( client.xuid ) then
			Engine.SetModelValue( Engine.CreateModel( Lobby.Pregame.GetRootModelForController( data.controller ), "clientState" ), REG5.pregameState )
			local maxVotes = Engine.GetGametypeSetting( Lobby.Pregame.Settings.MAX_VOTE_COUNT )
			if maxVotes > 0 then
				local votesRemaining = maxVotes - REG5.restrictedItemVoteCount
