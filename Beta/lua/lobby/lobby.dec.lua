...
LobbyVM = {
	AutoJoin = {},
	DevGui = {},
	DevGui = function ( mapname, gametype )
		if mapname ~= nil then
	...
	playSoundHistory = {},
	PlaySound = function ( sound, waitTimeBeforeNextPlay )
	...
	OnMatchLaunchClient = function ( data )
		local lobbyModule = data.lobbyModule
		local lobbyType = data.lobbyType
		local lobbyMode = data.lobbyMode
		LuaUtils.LogQoS( "match_launch_client", data )
	end,
	OnMatchChangeMap = function ( data )
		local lobbyModule = data.lobbyModule
		local lobbyType = data.lobbyType
		local lobbyMode = data.lobbyMode
	...
	OnMatchRecordStart = function ( data )
	...
	OnLobbyOnlineUpdate = function ( data )
		
	end,
	OnLeaveWithParty = function ( data )
		local lobbyModule = data.lobbyModule
		local lobbyType = data.lobbyType
		local lobbyMode = data.lobbyMode
		LobbyVM.LobbyLeaveWithParty( data )
	end,
	OnGametypeSettingsChange = function ( data )
		local lobbyModule = data.lobbyModule
		local lobbyType = data.lobbyType
		local lobbyMode = data.lobbyMode
	...
	OnPartyPrivacyChange = function ( data )
		local partyPrivacy = data.partyPrivacy
	end,
	OnPlatformJoin = function ( data )
		Lobby.PlatformSession.OnPlatformJoin( data )
	end,
	OnInviteAccepted = function ( data )
		
	end,
	OnComError = function ( data )
	...
	OnComErrorCleanup = function ( data )
		local controller = data.controller
		local errorCode = data.errorCode
		local errorMsg = data.errorMsg
	...
	OnErrorShutdown = function ( data )
		LobbyVM.ErrorShutdown( data )
	end,
	OnPlayerBanned = function ( data )
		Lobby.Anticheat.OnPlayerBanned( data )
	end,
	OnFeatureBan = function ( data )
		Lobby.Anticheat.OnFeatureBan( data )
	end,
	OnStorageRead = function ( data )
	...
	OnStorageWrite = function ( data )
		if data.result ~= Enum.StorageResult.STORAGE_SUCCESS then
			LuaUtils.LogQoS( "storage", data )
		end
	end,
	OnStorageOperationReadResult = function ( data )
		
	end,
	InitilizeGunsmithBuffer = function ( data )
		local controller = data.controller
	...
	OnDownloadMPOnlineStats = function ( controller )
	...
	OnInitializeLoadouts = function ( data )
		local controller = data.controller
	...
	LobbyHostLeftNoMigration = function ( data )
		local controller = data.controller
		local lobbyType = data.lobbyType
	...
	LobbyHostLeft_InGameMigrateFinished = function ( data )
	...
	LobbyClientLeftEvent = function ( data )
		local controller = data.controller
		local lobbyType = data.lobbyType
		local lobbyMainMode = data.lobbyMainMode
		local lobbyNetworkMode = data.lobbyNetworkMode
	...
	CreateDedicatedLobby = function ( data )
	...
	LobbyClientPromoteToHost = function ( data )
	...
	LobbyLeaveWithParty = function ( data )
		local lobbyModule = data.lobbyModule
		local lobbyType = data.lobbyType
		local lobbyMode = data.lobbyMode
	...
	LobbyLocalClientLeave = function ( data )
		local controller = data.controller
	...
	Leaderboard_CalculateLBColValue = function ( data )
		return Lobby.Leaderboard.CalculateLBColValue( data )
	end,
	Leaderboard_ShouldWriteLeaderboard = function ( data )
		return Lobby.Leaderboard.ShouldWriteLeaderboard( data )
	end,
	LaunchGameExec = function ( controller, lobbyType )
	...
	LaunchDemoExec = function ( controller, lobbyType )
		if lobbyType ~= Enum.LobbyType.LOBBY_TYPE_GAME then
	...
	clientLaunch = {},
	ClientLaunchInit = function ( justConnected )
	...
	SessionSQJRefreshInfo = function ( data )
		Lobby.Debug.SessionSQJRefreshInfo()
	end,
	StopLobbyTimer = function ( data )
		Lobby.Timer.HostingLobbyEnd( data )
	end,
	ChangeSigninState = function ( data )
		local process = nil
	...
	JoinResultToString = function ( joinResult, verbose )
	...
	LobbyStatusUpdate = function ( data )
		local lobbyRootModel = Engine.GetModel( Engine.GetGlobalModel(), "lobbyRoot" )
		local lobbyStatusString1Model = Engine.CreateModel( lobbyRootModel, "lobbyStatusString1" )
		local lobbyStatusString2Model = Engine.CreateModel( lobbyRootModel, "lobbyStatusString2" )
		local lobbyStatusString3Model = Engine.CreateModel( lobbyRootModel, "lobbyStatusString3" )
	...
	ProcessCompleteSuccess = function ( data )
		Lobby.ProcessQueue.Success( data )
	end,
	ProcessCompleteFailure = function ( data )
		Lobby.ProcessQueue.Failure( data )
	end,
	ProcessCompleteError = function ( data )
		Lobby.ProcessQueue.Error( data )
	end,
	ProcessUpdate = function ( data )
		Lobby.ProcessQueue.Update( data )
	end,
	GameLobbyGameServerDataUpdate = function ( data )
	...
	GameLobbyClientDataUpdate = function ( data )
	...
	HopperClientJoin = function ( data )
	...
	OnRecordComScoreEvent = function ( data )
		local controller = data.controller
