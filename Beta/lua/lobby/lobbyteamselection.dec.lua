...
Lobby.TeamSelection.GameLobbyClientDataUpdate = function ( data )
	...
	for clientNum = 1, LuaEnums.MAX_CLIENTS, 1 do
		...
		for key, value in pairs( clientList ) do
			if value.xuid ~= 0 and xuid == REG12.xuid then
				client.team = REG12.team
...
Lobby.TeamSelection.AddToPrivateLobbyList = function ( member, memberIndex, privateLobbyList )
	for index, privateLobby in ipairs( privateLobbyList ) do
		if member.lobbyID == privateLobby.lobbyID then
			REG7.lobbySkill = REG7.lobbySkill + member.skillRating
			table.insert( REG7.memberList, {
				index = memberIndex,
				xuid = member.xuid,
...
Lobby.TeamSelection.VladSplit = function ( privateLobbyList, maxClients )
	...
	local swapSolo = function ( a, b )
		...
		for i, g in ipairs( a ) do
			...
			if #g.memberList == 1 then
				table.insert( soloA, f22_local6 )
			end
		end
		for i, g in ipairs( b ) do
			skillB = skillB + g.lobbySkill
			if #g.memberList == 1 then
				table.insert( soloB, f22_local6 )
			end
		end
		...
		for _ia, idxA in ipairs( soloA ) do
			for _ib, idxB in ipairs( soloB ) do
				delta1 = delta0 - 2 * (a[REG13].lobbySkill - b[idxB].lobbySkill)
	...
	for attempt = 1, 2, 1 do
		...
		for i, lobby in ipairs( privateLobbyList ) do
			...
			if team == 1 then
				table.insert( teamA, REG21 )
				countA = countA + #REG21.memberList
			else
				table.insert( teamB, REG21 )
				countB = countB + #REG21.memberList
			end
			local k = x
			for j = 0, #REG21.memberList - 1, 1 do
				local f18_local10 = j
			end
			local f18_local10 = j
		end
...
Lobby.TeamSelection.AutoSplitTeam = function ( mustBalance, logOutput )
	...
	if logOutput then
		...
		for index, client in ipairs( gamelobby.sessionClients ) do
			Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "(" .. tostring( index ) .. ") " .. client.gamertag .. " - lobbyID(" .. tostring( REG11.lobbyID ) .. ") -- simpleLobbyID: " .. tostring( REG11.simpleLobbyID ) .. " -- skill: " .. tostring( REG11.skillRating ) .. "\n" )
		end
		Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "-----------------------------------------\n" )
	end
	...
	for i, privateLobby in ipairs( split.teams[1] ) do
		local team = Enum.team_t.TEAM_ALLIES
		for idx, member in ipairs( privateLobby.memberList ) do
			local client = gamelobby.sessionClients[member.index]
			if logOutput then
				Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "(" .. tostring( REG10 ) .. ") " .. client.gamertag .. " - lobbyID(" .. tostring( client.lobbyID ) .. ") -- simpleLobbyID: " .. tostring( client.simpleLobbyID ) .. " -- team: " .. Lobby.TeamSelection.TeamIdToString( team ) .. " -- lobbySkill: " .. tostring( REG11.lobbySkill ) .. " -- skill: " .. tostring( client.skillRating ) .. "\n" )
			end
			Engine.LobbyHostAssignTeamToClient( client.xuid, team )
		end
	end
	for i, privateLobby in ipairs( split.teams[2] ) do
...
Lobby.TeamSelection.ClientJoined = function ( data )
	...
	for index, client in ipairs( gamelobby.sessionClients ) do
		if xuid ~= client.xuid then
			Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "(" .. tostring( REG6 ) .. ") " .. REG7.gamertag .. " - lobbyID(" .. tostring( REG7.lobbyID ) .. ") -- simpleLobbyID: " .. tostring( REG7.simpleLobbyID ) .. " -- team: " .. Lobby.TeamSelection.TeamIdToString( REG7.team ) .. " -- score: " .. tostring( REG7.score ) .. ".\n" )
		end
	end
	Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "-----------------------------------------\n" )
	...
	for index, client in ipairs( gamelobby.sessionClients ) do
		if joinedClient.xuid ~= client.xuid and joinedClient.lobbyID == REG8.lobbyID then
			Engine.LobbyHostAssignTeamToClient( joinedClient.xuid, REG8.team )
			Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "Joining client is part of a party.\n" )
			Engine.PrintInfo( Enum.consoleLabel.LABEL_LOBBYHOST, "Adding joining client " .. joinedClient.gamertag .. " - lobbyID(" .. tostring( joinedClient.lobbyID ) .. ") -- simpleLobbyID: " .. tostring( joinedClient.simpleLobbyID ) .. " -- team: " .. Lobby.TeamSelection.TeamIdToString( REG8.team ) .. " -- score: " .. tostring( REG8.score ) .. ".\n" )
	...
	for index, client in ipairs( gamelobby.sessionClients ) do
		if joinedClient.xuid ~= client.xuid then
			if REG10.team == Enum.team_t.TEAM_ALLIES then
				teamAlliesCount = teamAlliesCount + 1
			end
			if REG10.team == Enum.team_t.TEAM_AXIS then
		...
		if teamAssignment == LuaEnums.TEAM_ASSIGNMENT.AUTO then
			for index, client in ipairs( gamelobby.sessionClients ) do
				if client.team ~= Enum.team_t.TEAM_SPECTATOR then
					Engine.LobbyHostAssignTeamToClient( REG11.xuid, Enum.team_t.TEAM_FREE )
				end
			end
		else
			for index, client in ipairs( gamelobby.sessionClients ) do
				if client.team == Enum.team_t.TEAM_FREE then
					Engine.LobbyHostAssignTeamToClient( REG11.xuid, Enum.team_t.TEAM_ALLIES )
				end
			end
		end
	end
