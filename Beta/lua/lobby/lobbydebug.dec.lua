...
Lobby.Debug.SessionSQJSearchResults = function ( data )
	...
	for index, result in ipairs( data.remoteHosts ) do
		...
		if Engine.XUIDToGamertag then
			f13_local3 = Engine.XUIDToGamertag( REG6.xuid ) or ""
		end
		sqjData[REG5].gamertag = f13_local3
		local hostAddress = Engine.SerializedAdrToLua( REG6.hostAddress )
		sqjData[REG5].publicIPAddress = hostAddress.publicIPAddress
		sqjData[REG5].privateIPAddress = hostAddress.privateIPAddress
		sqjData[REG5].natType = hostAddress.natType
	end
	Lobby.Debug.SessionSQJUpdateUIInfo()
end
	...
	for index, result in ipairs( Lobby.Debug.sessionSQJ.results[Lobby.Debug.sessionSQJ.searchStage].data ) do
		if Engine.CompareUInt64( result.xuid, data.xuid, "=" ) then
			if data.validResult then
				REG5.ping = tostring( math.floor( data.latency ) ) .. "ms"
			else
				REG5.ping = "PING-INVALID"
			end
			Lobby.Debug.SessionSQJUpdateUIInfo()
			return 
		end
...
Lobby.Debug.SessionSQJJoinInitiate = function ( xuid )
	...
	for index, result in ipairs( Lobby.Debug.sessionSQJ.results[Lobby.Debug.sessionSQJ.searchStage].data ) do
		if Engine.CompareUInt64( result.xuid, xuid, "=" ) then
			...
			REG5.status = "(" .. tostring( Lobby.Debug.sessionSQJ.joinOrder ) .. ") JOINING"
			Lobby.Debug.SessionSQJUpdateUIInfo()
			return 
		end
	end
...
Lobby.Debug.SessionSQJJoinResult = function ( data )
	...
	for index, result in ipairs( Lobby.Debug.sessionSQJ.results[Lobby.Debug.sessionSQJ.searchStage].data ) do
		if Engine.CompareUInt64( result.xuid, data.xuid, "=" ) then
			...
			REG5.status = joinResultInfo.debug
			Lobby.Debug.SessionSQJUpdateUIInfo()
			return 
		end
	end
