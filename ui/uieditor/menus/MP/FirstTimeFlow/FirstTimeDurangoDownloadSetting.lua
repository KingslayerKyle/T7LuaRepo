-- 5c2e5d2ff2a2b40d6e91f3f0df85d4df
...
LUI.createMenu.FirstTimeDurangoDownloadSetting = function ( controller )
	...
	f1_local1:AddButtonCallbackFunction( background, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE, nil, function ( f4_arg0, f4_arg1, f4_arg2, f4_arg3 )
		GoBack( self, f4_arg2 )
		return true
	...
	local Zombie = CoD.CACGenericButton.new( f1_local1, controller )
	...
	f1_local1:AddButtonCallbackFunction( Zombie, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( f8_arg0, f8_arg1, f8_arg2, f8_arg3 )
		if not IsGameModeInstalled( "", Enum.eModes.MODE_ZOMBIES ) then
			...
			SetDownloadPreference( f8_arg2, Enum.eModes.MODE_ZOMBIES )
			return true
		else
			OpenGenericSmallPopup( f8_arg1, f8_arg2, "MENU_DOWNLOAD_SETTING_TITLE", "MENU_DOWNLOAD_COMPLETE", "", "", "", "" )
			return true
		end
	...
	local Multiplayer = CoD.CACGenericButton.new( f1_local1, controller )
	...
	f1_local1:AddButtonCallbackFunction( Multiplayer, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( f12_arg0, f12_arg1, f12_arg2, f12_arg3 )
		if not IsGameModeInstalled( "", Enum.eModes.MODE_MULTIPLAYER ) then
			SetProfileVar( f12_arg2, "com_first_time", 0 )
			UpdateGamerprofile( self, f12_arg0, f12_arg2 )
			ForceNotifyGlobalModel( f12_arg2, "mainFirstTimeFlowComplete" )
			SetDownloadPreference( f12_arg2, Enum.eModes.MODE_MULTIPLAYER )
			return true
		else
			OpenGenericSmallPopup( f12_arg1, f12_arg2, "MENU_DOWNLOAD_SETTING_TITLE", "MENU_DOWNLOAD_COMPLETE", "", "", "", "" )
			return true
		end
		...
		if not IsGameModeInstalled( "", Enum.eModes.MODE_MULTIPLAYER ) then
			...
			CoD.Menu.SetButtonLabel( f13_arg1, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "" )
			return true
		end
	end, false )
	...
	local Campaign = CoD.CACGenericButton.new( f1_local1, controller )
	...
	f1_local1:AddButtonCallbackFunction( Campaign, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( f16_arg0, f16_arg1, f16_arg2, f16_arg3 )
		if not IsGameModeInstalled( "", Enum.eModes.MODE_CAMPAIGN ) then
			SetProfileVar( f16_arg2, "com_first_time", 0 )
			UpdateGamerprofile( self, f16_arg0, f16_arg2 )
			ForceNotifyGlobalModel( f16_arg2, "mainFirstTimeFlowComplete" )
			SetDownloadPreference( f16_arg2, Enum.eModes.MODE_CAMPAIGN )
			return true
		else
			OpenGenericSmallPopup( f16_arg1, f16_arg2, "MENU_DOWNLOAD_SETTING_TITLE", "MENU_DOWNLOAD_COMPLETE", "", "", "", "" )
			return true
		end
	...
	if self.m_eventHandlers.input_source_changed then
		...
		self:registerEventHandler( "input_source_changed", function ( element, event )
			...
			return f1_local17( element, event )
		end )
	else
	...
	f1_local1:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE, nil, function ( f23_arg0, f23_arg1, f23_arg2, f23_arg3 )
