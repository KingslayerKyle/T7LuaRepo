-- c4aeb76dbc13cae2b1519a1a9e4954f1
...
LUI.createMenu.Prestige_CustomizePrestigeIconZM = function ( controller )
	...
	local PrestigeIconList = LUI.UIList.new( f2_local1, controller, 5, 0, nil, true, false, 0, 0, false, true )
	...
	PrestigeIconList:linkToElementModel( PrestigeIconList, "isLocked", true, function ( model )
		local f6_local0 = PrestigeIconList
		local f6_local1 = {
	...
	f2_local1:AddButtonCallbackFunction( PrestigeIconList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( f9_arg0, f9_arg1, f9_arg2, f9_arg3 )
		if not IsSelfModelValueTrue( f9_arg0, f9_arg2, "isLocked" ) then
			SetParagonIcon( self, f9_arg0, f9_arg2 )
			GoBack( self, f9_arg2 )
			return true
		else
		...
		if not IsSelfModelValueTrue( f10_arg0, f10_arg2, "isLocked" ) then
	...
	local PrestigeIconProfiler = CoD.Prestige_PrestigeIconProfiler.new( f2_local1, controller )
	...
	PrestigeIconProfiler:subscribeToGlobalModel( controller, "StorageGlobal", "stats_zm", function ( model )
		...
		if statsZm then
			PrestigeIconProfiler.RankText:setRGB( SetToParagonColorIfPrestigeMasterFromStorage( controller, "zm", 148, 162, 165, statsZm ) )
		end
	end )
	PrestigeIconProfiler:subscribeToGlobalModel( controller, "StorageGlobal", "stats_zm", function ( model )
		local statsZm = Engine.GetModelValue( model )
		if statsZm then
			PrestigeIconProfiler.RankText:setText( LevelStringFromStorage( controller, "zm", statsZm ) )
		end
	end )
	PrestigeIconProfiler:subscribeToGlobalModel( controller, "StorageGlobal", "stats_zm", function ( model )
		local statsZm = Engine.GetModelValue( model )
		if statsZm then
			PrestigeIconProfiler.PrestigeMasterTierWidget.ParagonStars:setHorizontalCount( GetPrestigeMasterTierCountFromStorage( controller, "zm", statsZm ) )
		end
	end )
	...
	PrestigeIconProfiler:linkToElementModel( PrestigeIconProfiler, "isLocked", true, function ( model )
		f2_local1:updateElementState( PrestigeIconProfiler, {
			name = "model_validation",
			menu = f2_local1,
			modelValue = Engine.GetModelValue( model ),
			modelName = "isLocked"
	...
	f2_local1:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE, "ESCAPE", function ( f21_arg0, f21_arg1, f21_arg2, f21_arg3 )
