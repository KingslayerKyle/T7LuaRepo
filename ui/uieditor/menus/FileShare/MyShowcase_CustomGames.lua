-- 568158e493c13a7e963418c84c379209
...
LUI.createMenu.MyShowcase_CustomGames = function ( controller )
	...
	local FileshareCustomGamesList = LUI.UIList.new( f3_local1, controller, 10, 0, nil, true, false, 0, 0, false, false )
	...
	f3_local1:AddButtonCallbackFunction( FileshareCustomGamesList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, nil, function ( f15_arg0, f15_arg1, f15_arg2, f15_arg3 )
		if IsSelfModelValueTrue( f15_arg0, f15_arg2, "isPublishNew" ) and not FileshareShowcaseIsPublishMode( f15_arg0, f15_arg2 ) then
			FileshareSetShowcasePublishMode( f15_arg2, true )
			return true
		elseif not IsSelfModelValueTrue( f15_arg0, f15_arg2, "isPublishNew" ) then
		...
		if IsSelfModelValueTrue( f16_arg0, f16_arg2, "isPublishNew" ) and not FileshareShowcaseIsPublishMode( f16_arg0, f16_arg2 ) then
			CoD.Menu.SetButtonLabel( f16_arg1, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "MENU_PUBLISH" )
			return true
	...
	f3_local1:AddButtonCallbackFunction( FileshareCustomGamesList, controller, Enum.LUIButton.LUI_KEY_START, "O", function ( f17_arg0, f17_arg1, f17_arg2, f17_arg3 )
		if not IsSelfModelValueTrue( f17_arg0, f17_arg2, "isPublishNew" ) then
			OpenPopup( self, "FileshareOptions", f17_arg2, "", "" )
			return true
		else
	...
	local CustomGamesList = LUI.UIList.new( f3_local1, controller, 10, 0, nil, true, false, 0, 0, false, false )
	...
	f3_local1:AddButtonCallbackFunction( CustomGamesList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, nil, function ( f24_arg0, f24_arg1, f24_arg2, f24_arg3 )
		if FileshareCanPublish( f24_arg2 ) and FileshareCanPublishItem( self, f24_arg0, f24_arg2 ) then
			FileshareShowcasePublishSelectedItem( self, f24_arg0, f24_arg2 )
			return true
		else
			
		end
	end, function ( f25_arg0, f25_arg1, f25_arg2 )
		if FileshareCanPublish( f25_arg2 ) and FileshareCanPublishItem( self, f25_arg0, f25_arg2 ) then
			CoD.Menu.SetButtonLabel( f25_arg1, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "MENU_PUBLISH" )
			return true
		else
			return false
		end
	...
	local verticalCounter = CoD.verticalCounter.new( f3_local1, controller )
	...
	local GameName = CoD.WeaponNameWidget.new( f3_local1, controller )
	GameName:setLeftRight( true, false, 759, 1182 )
	GameName:setTopBottom( true, false, 414, 448 )
	self:addElement( GameName )
	self.GameName = GameName
	
	local NoContentLabel = CoD.FileshareNoContent.new( f3_local1, controller )
	...
	local PublishedCustomGameInfo = CoD.MyShowcase_PublishedCustomGameInfo.new( f3_local1, controller )
	...
	PublishedCustomGameInfo:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "fileshareRoot.itemsCount" ), function ( model )
		f3_local1:updateElementState( PublishedCustomGameInfo, {
			name = "model_validation",
			menu = f3_local1,
			modelValue = Engine.GetModelValue( model ),
	...
	local MyShowcaseCustomGameListInfo = CoD.MyShowcase_CustomGameListInfo.new( f3_local1, controller )
	...
	MyShowcaseCustomGameListInfo:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "fileshareRoot.itemsCount" ), function ( model )
		f3_local1:updateElementState( MyShowcaseCustomGameListInfo, {
			name = "model_validation",
			menu = f3_local1,
			modelValue = Engine.GetModelValue( model ),
			modelName = "fileshareRoot.itemsCount"
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "FileshareRoot.PublishMode" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f49_local0, f3_local1, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE )
	end )
	...
	f3_local1:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE, nil, function ( f51_arg0, f51_arg1, f51_arg2, f51_arg3 )
		if FileshareShowcaseIsPublishMode( f51_arg0, f51_arg2 ) then
			FileshareSetShowcasePublishMode( f51_arg2, false )
			return true
		else
			GoBack( self, f51_arg2 )
			ClearSavedState( self, f51_arg2 )
