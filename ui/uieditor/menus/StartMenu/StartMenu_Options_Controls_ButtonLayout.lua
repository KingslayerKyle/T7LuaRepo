-- 13336e65f57ab9a818252e672a54f4c9
...
if CoD.isPC then
	...
	DataSources.ButtonLayoutDropdownTriggerConfig = DataSourceHelpers.ListSetup( "PC.ButtonLayoutDropdownTriggerConfig", function ( f12_arg0 )
		local f12_local0 = {}
		...
		table.insert( f12_local0, {
			models = {
				value = CoD.TRIGGERS_FLIPPED,
				valueDisplay = "MENU_FLIPPED_CAPS"
			}
		} )
		return f12_local0
	end, true )
	DataSources.ButtonLayoutDropdown = DataSourceHelpers.ListSetup( "ButtonLayoutDropdown", function ( f13_arg0 )
		local f13_local0 = {}
		...
		table.insert( f13_local0, {
			models = {
				label = "PLATFORM_TRIGGER_CONFIG",
				profileVarName = "flipped_control_config",
				profileType = "user",
				datasource = "ButtonLayoutDropdownTriggerConfig",
				widgetType = "dropdown"
			},
			properties = {
				dropDownItemSelected = function ( f14_arg0, f14_arg1, f14_arg2 )
					local f14_local0 = CoD.PCUtil.GetElementModelValues( f14_arg2, "value" )
					if f14_local0 then
						f0_local3( f14_local0 )
					end
					return CoD.PCUtil.OptionsDropdownItemSelected( f14_arg0, f14_arg1, f14_arg2 )
				end,
	...
	DataSources.ButtonLayoutDropdown.getWidgetTypeForItem = function ( f15_arg0, f15_arg1, f15_arg2 )
		if f15_arg1 then
			local f15_local0 = Engine.GetModelValue( Engine.GetModel( f15_arg1, "widgetType" ) )
			if f15_local0 == "dropdown" then
				return CoD.OptionDropdown
			elseif f15_local0 == "checkbox" then
...
local f0_local7 = function ( f17_arg0, f17_arg1, f17_arg2 )
	...
	f17_arg0:registerEventHandler( "dropdown_triggered", function ( element, event )
		for f18_local0 = 1, element.buttonLayoutDropdownList.requestedRowCount, 1 do
			local f18_local3 = element.buttonLayoutDropdownList:getItemAtPosition( f18_local0, 1 )
			if event.inUse then
				if f18_local3 ~= event.widget then
					f18_local3.m_inputDisabled = true
				end
			end
			f18_local3.m_inputDisabled = nil
		end
	end )
	f17_arg0.buttonLayoutDropdownList.m_managedItemPriority = true
	f17_arg0.buttonLayoutDropdownList:registerEventHandler( "dropdown_triggered", function ( element, event )
		if not event.inUse then
			f0_local2( event.widget.dropDownCurrentValue( f17_arg1, element ) )
		end
		element:dispatchEventToParent( event )
	end )
	f17_arg0:registerEventHandler( "input_source_changed", function ( element, event )
		f17_arg0:updateState( event )
		f17_arg0.buttonLayoutButtonList:updateDataSource()
		f17_arg0.buttonLayoutDropdownList:updateDataSource()
		if Engine.LastInput_Gamepad() then
			f17_arg0.buttonLayoutButtonList:processEvent( {
				name = "gain_focus",
				controller = f17_arg1
			} )
			f17_arg0.buttonLayoutDropdownList:processEvent( {
				name = "lose_focus",
				controller = f17_arg1
			} )
		else
	...
	LUI.OverrideFunction_CallOriginalSecond( f17_arg0, "close", function ()
		CoD.PCUtil.RefreshAllOptions( f17_arg0, f17_arg1 )
	end )
end
...
LUI.createMenu.StartMenu_Options_Controls_ButtonLayout = function ( controller )
	...
	local buttonLayoutButtonList = LUI.UIList.new( f24_local1, controller, 2, 0, nil, false, false, 0, 0, false, true )
	...
	local leftStickText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local leftTriggerText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local leftBumperText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local rightStickText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local rightTriggerText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local rightBumperText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local XBAPSCrossText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local XBBPSCircleText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local XBXPSSquareText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local XBYPSTriangleText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local backButtonText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local startButtonText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local leftButtonText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local rightButtonText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local upButtonText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local downButtonText = CoD.StartMenu_Options_Controls_GenericControllerItem.new( f24_local1, controller )
	...
	local MenuFrame = CoD.GenericMenuFrame.new( f24_local1, controller )
	...
	local StartMenulineGraphicsOptions0 = CoD.StartMenu_lineGraphics_Options.new( f24_local1, controller )
	...
	local buttonLayoutDropdownList = LUI.UIList.new( f24_local1, controller, 0, 0, nil, false, false, 0, 0, false, false )
	...
	self.clipsPerState = {
		...
		Orbis = {
			DefaultClip = function ()
				...
				buttonLayoutButtonList:completeAnimation()
				self.buttonLayoutButtonList:setAlpha( 1 )
				self.clipFinished( buttonLayoutButtonList, {} )
				durangoControllerImage:completeAnimation()
				self.durangoControllerImage:setAlpha( 0 )
				self.clipFinished( durangoControllerImage, {} )
	...
	if self.m_eventHandlers.input_source_changed then
		local f24_local29 = self.m_eventHandlers.input_source_changed
		self:registerEventHandler( "input_source_changed", function ( element, event )
			event.menu = event.menu or f24_local1
			element:updateState( event )
			return f24_local29( element, event )
		end )
	else
	...
	CoD.Menu.AddNavigationHandler( f24_local1, self, controller )
	f24_local1:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE, nil, function ( f52_arg0, f52_arg1, f52_arg2, f52_arg3 )
		GoBack( self, f52_arg2 )
		ForceNotifyControllerModel( f52_arg2, "forceUpdateVehicleBindings" )
		UpdateGamerprofile( self, f52_arg0, f52_arg2 )
		ClearSavedState( self, f52_arg2 )
		return true
	end, function ( f53_arg0, f53_arg1, f53_arg2 )
		CoD.Menu.SetButtonLabel( f53_arg1, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE, "MENU_BACK" )
		return true
	end, false )
	LUI.OverrideFunction_CallOriginalFirst( self, "setState", function ( element, controller )
		if IsMenuInState( f24_local1, "PC" ) then
