-- 116d4e80dc60409cee785a06a2920c85
-- This hash is used for caching, delete to decompile the file again

require( "ui.uieditor.widgets.HUD.ZM_FX.ZmFx_Spark2" )

CoD.GasMaskPieceWidget = InheritFrom( LUI.UIElement )
CoD.GasMaskPieceWidget.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.GasMaskPieceWidget )
	self.id = "GasMaskPieceWidget"
		...
		},
		{
			...
			condition = function ( menu, element, event )
				...
				if not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_SCOREBOARD_OPEN ) then
					f26_local0 = ShowGasMaskPartsWidget( controller )
				else
					f26_local0 = false
				end
				return f26_local0
