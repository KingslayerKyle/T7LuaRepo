-- 741d345ddaa1d7c0712dec35dcc2287a
-- This hash is used for caching, delete to decompile the file again

require( "ui.uieditor.widgets.HUD.ZM_FX.ZmFx_Spark2" )

CoD.SeedPieceWidget = InheritFrom( LUI.UIElement )
CoD.SeedPieceWidget.new = function ( menu, controller )
	...
	self:setClass( CoD.SeedPieceWidget )
	self.id = "SeedPieceWidget"
	...
	ZmFxSpark20:setTopBottom( false, false, -57, 32 )
	...
	self:mergeStateConditions( {
		...
		},
		{
			...
			condition = function ( menu, element, event )
				...
				if not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_SCOREBOARD_OPEN ) then
					f27_local0 = ShowSeedPartsWidget( controller )
				else
					f27_local0 = false
				end
				return f27_local0
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f28_local0
				if not Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_SCOREBOARD_OPEN ) then
					f28_local0 = ShowBucketOrSeedWidget( controller )
				else
					f28_local0 = false
				end
				return f28_local0
