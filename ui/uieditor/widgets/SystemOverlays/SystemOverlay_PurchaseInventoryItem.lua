-- 53723d4d302bb1fa0400ccf624b07278
...
CoD.SystemOverlay_PurchaseInventoryItem.new = function ( menu, controller )
	...
	largeImage:setLeftRight( true, false, 50, 384 )
	largeImage:setTopBottom( true, false, 50, 272 )
	...
	PurchasingWidget.PurchasingText:setText( Engine.Localize( "MENU_PROCESSING_PURCHASE" ) )
	...
	menu:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, nil, function ( f28_arg0, f28_arg1, f28_arg2, f28_arg3 )
		if not IsMenuModelValueEqualTo( f28_arg1, "title", "MPUI_BM_NO_CURRENCY" ) and AreCodPointsEnabled( f28_arg2 ) and not IsPC() and not IsSelfInState( self, "Purchasing" ) then
			GoBackAndOpenStoreCodPointsOverlayOnParent( self, "InventoryItemCPStore", f28_arg2 )
			return true
		else
			
		end
	end, function ( f29_arg0, f29_arg1, f29_arg2 )
		if not IsMenuModelValueEqualTo( f29_arg1, "title", "MPUI_BM_NO_CURRENCY" ) and AreCodPointsEnabled( f29_arg2 ) and not IsPC() and not IsSelfInState( self, "Purchasing" ) then
			CoD.Menu.SetButtonLabel( f29_arg1, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, "MPUI_PURCHASE_CODPOINTS" )
			return true
		else
			return false
		end
