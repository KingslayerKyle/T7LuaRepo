-- 8bf724b5ed966ce03562536145eb8f9d
-- This hash is used for caching, delete to decompile the file again

CoD.systemOverlay_Connect_Lines = InheritFrom( LUI.UIElement )
CoD.systemOverlay_Connect_Lines.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.systemOverlay_Connect_Lines )
	self.id = "systemOverlay_Connect_Lines"
			...
			DefaultClip = function ()
				...
				local maskFrame2 = function ( mask, event )
					if not event.interrupted then
						mask:beginAnimation( "keyframe", 3009, false, true, CoD.TweenType.Linear )
					end
