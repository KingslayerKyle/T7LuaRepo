-- 9770123e90fd57c062db3dce16e794e5
-- This hash is used for caching, delete to decompile the file again

CoD.Promo_ZMDH_HeaderWidget = InheritFrom( LUI.UIElement )
CoD.Promo_ZMDH_HeaderWidget.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.Promo_ZMDH_HeaderWidget )
	self.id = "Promo_ZMDH_HeaderWidget"
	self.soundSet = "default"
	self:setLeftRight( true, false, 0, 535 )
	self:setTopBottom( true, false, 0, 121 )
	
	local left = LUI.UIImage.new()
	left:setLeftRight( true, false, 0, 176 )
	left:setTopBottom( true, false, 12.74, 108.74 )
	left:setImage( RegisterImage( "uie_t7_menu_community_bgb_header" ) )
