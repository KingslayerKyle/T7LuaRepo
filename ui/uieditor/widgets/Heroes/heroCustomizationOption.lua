-- 00f27b0f367d275a5f7bb5bcbb4c8d2a
...
CoD.heroCustomizationOption.new = function ( menu, controller )
	...
	classifiedIcon:setLeftRight( true, true, 1, -1 )
	classifiedIcon:setTopBottom( true, true, -1, 1 )
	...
	local Purchaseable = LUI.UIImage.new()
	Purchaseable:setLeftRight( true, false, 30, 78 )
	Purchaseable:setTopBottom( true, false, 30, 78 )
	Purchaseable:setAlpha( 0 )
	Purchaseable:setImage( RegisterImage( "uie_t7_icon_contextual_purchase" ) )
	self:addElement( Purchaseable )
	...
	local colorImage = LUI.UIImage.new()
	colorImage:setLeftRight( true, true, 0, 0 )
	colorImage:setTopBottom( true, false, 0, 85 )
	colorImage:setAlpha( 0 )
	colorImage:setupUIStreamedImage( 0 )
	colorImage:linkToElementModel( self, "color", true, function ( model )
		local color = Engine.GetModelValue( model )
		if color then
			colorImage:setRGB( color )
		end
	end )
	colorImage:linkToElementModel( self, "image", true, function ( model )
		local image = Engine.GetModelValue( model )
		if image then
			colorImage:setImage( RegisterImage( image ) )
		end
	end )
	self:addElement( colorImage )
	...
	LabelButton:setLeftRight( true, false, 0, 107 )
	LabelButton:setTopBottom( false, false, 31, 51 )
	...
	classifiedText:setLeftRight( true, true, 0, 0 )
	classifiedText:setTopBottom( false, false, -10, 10 )
	classifiedText:setRGB( 1, 0, 0 )
	classifiedText:setAlpha( 0 )
	classifiedText:setText( Engine.Localize( "MENU_CLASSIFIED_CAPS" ) )
	classifiedText:setTTF( "fonts/UnitedSansSmCdBd_0.ttf" )
	classifiedText:setAlignment( Enum.LUIAlignment.LUI_ALIGNMENT_CENTER )
	classifiedText:setAlignment( Enum.LUIAlignment.LUI_ALIGNMENT_MIDDLE )
	...
	self.itemImage:linkToElementModel( self, "itemExtraCamInfo", true, function ( model )
		local itemExtraCamInfo = Engine.GetModelValue( model )
		if itemExtraCamInfo then
			itemImage:setupCharacterItemRender( SplitString( ":", itemExtraCamInfo ) )
		end
	end )
	self.clipsPerState = {
		DefaultState = {
		...
		BMClassified = {
			...
			GainFocus = function ()
				...
				local classifiedIconFrame2 = function ( classifiedIcon, event )
					...
					classifiedIcon:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( classifiedIcon, event )
					else
						classifiedIcon:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				return IsSelfModelValueTrue( element, controller, "isBMPurchasable" )
			end
		},
		{
		...
		},
		{
			...
			condition = function ( menu, element, event )
				return IsCurrentItemClassified( element, controller )
			end
		},
		{
			stateName = "New",
			condition = function ( menu, element, event )
				return IsCurrentNewHeroOption( self, element, controller )
			end
		},
		{
			stateName = "Locked",
	...
	self:linkToElementModel( self, "isBMPurchasable", true, function ( model )
		menu:updateElementState( self, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
			modelName = "isBMPurchasable"
		} )
	end )
	...
	self:linkToElementModel( self, "variantIndex", true, function ( model )
		menu:updateElementState( self, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
			modelName = "variantIndex"
		} )
	end )
	self:linkToElementModel( self, "optionIndex", true, function ( model )
		menu:updateElementState( self, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
	...
	menu:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, nil, function ( f500_arg0, f500_arg1, f500_arg2, f500_arg3 )
		if IsSelfInState( self, "BMPurchasable" ) and IsElementInFocus( f500_arg0 ) then
			OpenOutfitStore( self, f500_arg0, f500_arg2, "OutfitStore", f500_arg1 )
			return true
		else
			
		end
	end, function ( f501_arg0, f501_arg1, f501_arg2 )
		if IsSelfInState( self, "BMPurchasable" ) and IsElementInFocus( f501_arg0 ) then
			CoD.Menu.SetButtonLabel( f501_arg1, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "MENU_VIEW_STORE_OUTFITS" )
			return true
		else
			return false
		end
