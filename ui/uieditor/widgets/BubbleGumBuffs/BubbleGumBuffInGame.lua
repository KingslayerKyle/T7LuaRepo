-- c82b4add6df6e9298ab0c015254f068a
...
local PostLoadFunc = function ( self, controller, menu )
	...
	self.ConsumableLabel:subscribeToGlobalModel( controller, "PerController", "scriptNotify", function ( model )
		if IsParamModelEqualToString( model, "zombie_bgb_used" ) then
...
CoD.BubbleGumBuffInGame.new = function ( menu, controller )
	...
	ConsumableLabel:linkToElementModel( ConsumableLabel, "itemIndex", true, function ( model )
	...
	ConsumableLabel:linkToElementModel( ConsumableLabel, "remaining", true, function ( model )
		menu:updateElementState( ConsumableLabel, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
	...
	self.ConsumableLabel:linkToElementModel( self, "remaining", true, function ( model )
	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				return IsSelfModelValueNilOrZero( element, controller, "remaining" ) and IsCACItemConsumable( menu, element, controller )
			end
		}
	} )
