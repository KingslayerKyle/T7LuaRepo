-- 62dea49d3813dd2cee8697bfe9fb3654
...
CoD.EquippedBubbleGumBuffButton.new = function ( menu, controller )
	...
	itemImage:linkToElementModel( self, "itemIndex", true, function ( model )
		local itemIndex = Engine.GetModelValue( model )
		if itemIndex then
			itemImage:setImage( RegisterImage( GetItemImageFromIndex( itemIndex ) ) )
		end
	end )
	...
	tokenUnlock:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if not IsCampaign() then
					f5_local0 = ShouldShowTokenIconOnItem( menu, element, controller )
				else
					f5_local0 = false
				end
				return f5_local0
	...
	LabelButton:linkToElementModel( self, "itemIndex", true, function ( model )
		local itemIndex = Engine.GetModelValue( model )
		if itemIndex then
			LabelButton.itemName:setText( Engine.Localize( GetItemNameFromIndex( itemIndex ) ) )
		end
	end )
	...
	LabelButton0:setLeftRight( true, false, 2, 106 )
	LabelButton0:setTopBottom( false, true, -22, -2 )
	LabelButton0:setAlpha( 0.6 )
	LabelButton0:linkToElementModel( self, nil, false, function ( model )
		LabelButton0:setModel( model, controller )
	end )
	LabelButton0:linkToElementModel( self, "name", true, function ( model )
		local name = Engine.GetModelValue( model )
		if name then
			LabelButton0.itemName:setText( Engine.Localize( name ) )
		end
	end )
	LabelButton0:mergeStateConditions( {
		{
			stateName = "Hidden",
			condition = function ( menu, element, event )
				return IsCACItemBubbleGum( menu, element, controller )
	...
	LabelButton0:linkToElementModel( LabelButton0, "itemIndex", true, function ( model )
		menu:updateElementState( LabelButton0, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
			modelName = "itemIndex"
