-- 79aeead47dae49139b4d213984b3b3cd
...
CoD.BubbleGumPackListItem.new = function ( menu, controller )
	...
	self.btnDisplayText:linkToElementModel( self, "bgbPackIndex", true, function ( model )
		...
		if bgbPackIndex then
			btnDisplayText:setText( GetBubbleGumPackNameFromPackIndex( controller, bgbPackIndex ) )
		end
	end )
	self.btnDisplayTextStroke:linkToElementModel( self, "bgbPackIndex", true, function ( model )
		local bgbPackIndex = Engine.GetModelValue( model )
		if bgbPackIndex then
			btnDisplayTextStroke:setText( GetBubbleGumPackNameFromPackIndex( controller, bgbPackIndex ) )
		end
	end )
	self.clipsPerState = {
		DefaultState = {
