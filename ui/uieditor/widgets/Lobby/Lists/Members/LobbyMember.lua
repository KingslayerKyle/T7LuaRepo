-- b449883e94d9d6b45a86f1aadee57045
	...
	if CoD.isPC then
		...
		LUI.OverrideFunction_CallOriginalFirst( self, "setState", function ( element, controller )
			if IsSelfInState( self, "AnonymousPlayer" ) then
				...
				local f5_local0 = Engine.GetModel( self:getModel(), "isStarterPack" )
				if f5_local0 then
					self.LobbyMemberMP45:setAlpha( Engine.GetModelValue( f5_local0 ) )
				end
			end
		end )
	end
...
CoD.LobbyMember.new = function ( menu, controller )
	...
	LobbyLeaderIcon0:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f23_local0 = IsSelfModelValueTrue( element, controller, "isPartyMemberTopOrMiddle" )
				if f23_local0 then
					f23_local0 = IsZM()
					if f23_local0 then
	...
	local LobbyMemberMP45 = CoD.LobbyMemberMP45.new( menu, controller )
	LobbyMemberMP45:setLeftRight( true, false, 384, 408 )
	LobbyMemberMP45:setTopBottom( true, false, 1, 26 )
	self:addElement( LobbyMemberMP45 )
	self.LobbyMemberMP45 = LobbyMemberMP45
	...
	self.LobbyMemberMP45:linkToElementModel( self, "isStarterPack", true, function ( model )
		local isStarterPack = Engine.GetModelValue( model )
		if isStarterPack then
			LobbyMemberMP45:setAlpha( isStarterPack )
		end
	end )
	self.clipsPerState = {
		DefaultState = {
	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f252_local0 = IsSelfItem( element, controller )
				if f252_local0 then
					f252_local0 = IsZM()
					if f252_local0 then
						f252_local0 = LobbyHas4PlayersOrLess()
					end
				end
				return f252_local0
			end
		...
		},
		{
			stateName = "IsDOA",
			condition = function ( menu, element, event )
				local f254_local0 = IsSelfModelValueTrue( element, controller, "validClient" )
				if f254_local0 then
					f254_local0 = IsGlobalModelValueEqualTo( element, controller, "lobbyRoot.lobbyNav", LobbyData.UITargets.UI_DOALOBBYONLINEPUBLICGAME.id )
					if f254_local0 then
						f254_local0 = IsSelfItem( element, controller )
						if f254_local0 then
							f254_local0 = IsGameTypeDOA()
						end
					end
				end
				return f254_local0
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f255_local0 = IsGlobalModelValueEqualTo( element, controller, "lobbyRoot.lobbyNav", LobbyData.UITargets.UI_DOALOBBYONLINEPUBLICGAME.id )
				if f255_local0 then
					f255_local0 = IsGameTypeDOA()
					if f255_local0 then
						f255_local0 = IsSelfModelValueTrue( element, controller, "validClient" )
					end
				end
				return f255_local0
			end
		...
		},
		{
			...
			condition = function ( menu, element, event )
				...
				if not IsSelfModelValueTrue( element, controller, "validClient" ) then
					f259_local0 = not IsCurrentLanguageReversed()
				else
					f259_local0 = false
				end
				return f259_local0
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f260_local0
				if not IsSelfModelValueTrue( element, controller, "validClient" ) then
					f260_local0 = IsCurrentLanguageReversed()
				else
					f260_local0 = false
				end
				return f260_local0
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f261_local0 = IsArenaMode()
				if f261_local0 then
					f261_local0 = IsSelfModelValueNilOrZero( element, controller, "isMember" )
					if f261_local0 then
						if not IsGlobalModelValueTrue( element, controller, "lobbyRoot.lobbyLockedIn" ) then
							f261_local0 = not MapVoteInState( "3" )
						else
							f261_local0 = false
						end
					end
	...
	if self.m_eventHandlers.on_client_added then
		local f6_local28 = self.m_eventHandlers.on_client_added
		self:registerEventHandler( "on_client_added", function ( element, event )
			event.menu = event.menu or menu
			element:updateState( event )
			return f6_local28( element, event )
		end )
	else
		self:registerEventHandler( "on_client_added", LUI.UIElement.updateState )
	end
	if self.m_eventHandlers.on_client_removed then
		local f6_local28 = self.m_eventHandlers.on_client_removed
		self:registerEventHandler( "on_client_removed", function ( element, event )
			event.menu = event.menu or menu
			element:updateState( event )
			return f6_local28( element, event )
		end )
	...
	self:registerEventHandler( "gain_focus", function ( element, event )
		if element.m_focusable and element.LobbyMemberReady:processEvent( event ) then
			return true
		else
