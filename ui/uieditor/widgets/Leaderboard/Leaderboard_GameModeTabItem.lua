-- 341e7e104a0ac987b399fe57711693ea
...
CoD.Leaderboard_GameModeTabItem.new = function ( menu, controller )
	...
	textDark:linkToElementModel( self, "tabName", true, function ( model )
		local tabName = Engine.GetModelValue( model )
		if tabName then
			textDark:setText( Engine.Localize( tabName ) )
		end
	end )
	...
	newBreadcrumb:setLeftRight( true, false, 206, 226 )
	newBreadcrumb:setTopBottom( true, false, -6, 14 )
	...
	if self.m_eventHandlers.input_source_changed then
		local f1_local10 = self.m_eventHandlers.input_source_changed
		self:registerEventHandler( "input_source_changed", function ( element, event )
			event.menu = event.menu or menu
			element:updateState( event )
			return f1_local10( element, event )
		end )
	else
