-- f9dfff905e7cc10c55d25e7a725c1299
...
local PostLoadFunc = function ( self, controller, menu )
	self.itemHintText:linkToElementModel( self, "name", true, function ( model )
		...
		if modelValue then
			...
			local f2_local2 = self.itemHintText.textCenterAlign:getTextWidth()
			self.itemHintText:setLeftRight( false, false, -f2_local2 / 2 - f2_local1, f2_local2 / 2 + f2_local1 )
		end
	end )
end
...
CoD.OverCapacityItem.new = function ( menu, controller )
	...
	sizeElement:linkToElementModel( self, "image", true, function ( model )
		...
		if image then
			sizeElement.itemImage:setImage( RegisterImage( image ) )
		end
	end )
	...
	itemHintText:linkToElementModel( self, "name", true, function ( model )
		...
		if name then
			itemHintText.textCenterAlign:setText( Engine.Localize( name ) )
		end
	end )
