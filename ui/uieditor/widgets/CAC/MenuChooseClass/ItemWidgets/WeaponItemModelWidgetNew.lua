-- 5f5e9f351c412bda8b48d4f54bc252a2
...
local f0_local0 = function ( f1_arg0, f1_arg1 )
	...
	f1_arg0:registerEventHandler( "button_action", function ( element, event )
		if not IsElementInState( element, "NotVisible" ) then
...
CoD.WeaponItemModelWidgetNew = InheritFrom( LUI.UIElement )
CoD.WeaponItemModelWidgetNew.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.WeaponItemModelWidgetNew )
	self.id = "WeaponItemModelWidgetNew"
	self.soundSet = "CAC"
	...
	local cacNewTag0 = CoD.cac_NewTag.new( menu, controller )
	cacNewTag0:setLeftRight( false, true, -19, 1 )
	cacNewTag0:setTopBottom( true, false, -1, 19 )
	self:addElement( cacNewTag0 )
	self.cacNewTag0 = cacNewTag0
	...
	local FocusBorder = LUI.UIImage.new()
	FocusBorder:setLeftRight( true, true, -1, 1 )
	FocusBorder:setTopBottom( true, true, -1.5, 1 )
	FocusBorder:setRGB( 1, 0.3, 0 )
	FocusBorder:setAlpha( 0.43 )
	FocusBorder:setImage( RegisterImage( "uie_t7_menu_frontend_titlenumbrdrfull" ) )
	FocusBorder:setMaterial( LUI.UIImage.GetCachedMaterial( "uie_nineslice_add" ) )
	FocusBorder:setShaderVector( 0, 0.03, 0.03, 0, 0 )
	FocusBorder:setupNineSliceShader( 4, 4 )
	...
	itemHintText:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				return IsHintTextRightAlign( element, controller ) and IsCACAnyPersonalizationOptionNew( menu, self, element, controller )
			end
		},
		{
			stateName = "CenterHasNew",
			condition = function ( menu, element, event )
				return IsCACAnyPersonalizationOptionNew( menu, self, element, controller )
			end
		}
	} )
	...
	lockedIcon:setLeftRight( false, false, -15.5, 14.5 )
	...
	if IsPC() then
		f5_local19 = CoD.RemoveItemButton.new( menu, controller )
	else
		f5_local19 = LUI.UIElement.createFake()
	end
	f5_local19:setLeftRight( false, true, -22, -3 )
	f5_local19:setTopBottom( true, false, 3, 23 )
	f5_local19:setAlpha( 0 )
	f5_local19:linkToElementModel( self, nil, false, function ( model )
		f5_local19:setModel( model, controller )
	end )
	if IsPC() then
		self:addElement( f5_local19 )
	end
	self.removeButton = f5_local19
	local f5_local20 = nil
	if IsPC() then
		f5_local20 = CoD.PersonalizeButton.new( menu, controller )
	else
		f5_local20 = LUI.UIElement.createFake()
	end
	f5_local20:setLeftRight( true, false, 3, 23 )
	f5_local20:setTopBottom( true, false, 3.75, 23.75 )
	f5_local20:setAlpha( 0 )
	f5_local20.itemHintText.textCenterAlign:setText( Engine.Localize( "HEROES_PERSONALIZE" ) )
	f5_local20:linkToElementModel( self, nil, false, function ( model )
		f5_local20:setModel( model, controller )
	end )
	f5_local20:mergeStateConditions( {
		{
			stateName = "Visible",
			condition = function ( menu, element, event )
				local f14_local0 = IsPC()
				if f14_local0 then
					f14_local0 = IsCACSlotEquipped( menu, element, controller )
					if f14_local0 then
						f14_local0 = IsCurrentMenu( menu, "CustomClass" )
						if f14_local0 then
							if not IsGamepad( controller ) then
								f14_local0 = HasHintText( element )
							else
								f14_local0 = false
							end
						end
	...
	if f5_local20.m_eventHandlers.input_source_changed then
		local RestrictedItemWarning = f5_local20.m_eventHandlers.input_source_changed
		f5_local20:registerEventHandler( "input_source_changed", function ( element, event )
			event.menu = event.menu or menu
			element:updateState( event )
			return RestrictedItemWarning( element, event )
		end )
	else
		f5_local20:registerEventHandler( "input_source_changed", LUI.UIElement.updateState )
	end
	f5_local20:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "LastInput" ), function ( model )
		menu:updateElementState( f5_local20, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
			modelName = "LastInput"
	...
	f5_local20:linkToElementModel( f5_local20, nil, true, function ( model )
		menu:updateElementState( f5_local20, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
	...
	self.personalizeButton = f5_local20
	
	local RestrictedItemWarning = CoD.RestrictedItemWarning.new( menu, controller )
	RestrictedItemWarning:setLeftRight( true, false, 0, 129 )
	RestrictedItemWarning:setTopBottom( true, false, 8.55, 119.95 )
	...
	local GridItemUpgradeIconWidget = CoD.GridItemUpgradeIconWidget.new( menu, controller )
	GridItemUpgradeIconWidget:setLeftRight( true, false, 3, 21 )
	GridItemUpgradeIconWidget:setTopBottom( true, false, 3, 21 )
	GridItemUpgradeIconWidget:linkToElementModel( self, nil, false, function ( model )
		GridItemUpgradeIconWidget:setModel( model, controller )
	end )
	GridItemUpgradeIconWidget:mergeStateConditions( {
		{
	...
	GridItemUpgradeIconWidget:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyNav" ), function ( model )
		menu:updateElementState( GridItemUpgradeIconWidget, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
	...
	GridItemUpgradeIconWidget:linkToElementModel( GridItemUpgradeIconWidget, nil, true, function ( model )
		menu:updateElementState( GridItemUpgradeIconWidget, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
	...
	self:addElement( GridItemUpgradeIconWidget )
	self.GridItemUpgradeIconWidget = GridItemUpgradeIconWidget
	...
	self.clipsPerState = {
		DefaultState = {
			DefaultClip = function ()
				...
				f5_local19:completeAnimation()
				self.removeButton:setAlpha( 0 )
				self.clipFinished( f5_local19, {} )
				f5_local20:completeAnimation()
				self.personalizeButton:setAlpha( 0 )
				self.clipFinished( f5_local20, {} )
			end,
			GainFocus = function ()
				...
				local itemImageFrame2 = function ( itemImage, event )
					if not event.interrupted then
						itemImage:beginAnimation( "keyframe", 79, false, false, CoD.TweenType.Linear )
					end
		...
		Equipped = {
			DefaultClip = function ()
				...
				self.clipFinished( FocusWhiteB, {} )
				f5_local19:completeAnimation()
				self.removeButton:setLeftRight( false, true, -16, -2 )
				self.removeButton:setTopBottom( true, false, 3, 17 )
				self.removeButton:setAlpha( 0 )
				self.clipFinished( f5_local19, {} )
				f5_local20:completeAnimation()
				self.personalizeButton:setAlpha( 0 )
			...
			GainFocus = function ()
				...
				self.glitch2:setAlpha( 1 )
				glitch2Frame2( glitch2, {} )
				local f5_local19Frame2 = function ( f5_local19, event )
					if not event.interrupted then
						f5_local19:beginAnimation( "keyframe", 140, false, false, CoD.TweenType.Linear )
					end
					f5_local19:setLeftRight( false, true, -16, -2 )
					f5_local19:setTopBottom( true, false, 3, 17 )
					f5_local19:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( f5_local19, event )
					else
						f5_local19:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
		...
		WeaponEquipped = {
			DefaultClip = function ()
				...
				self.clipFinished( FocusWhiteB, {} )
				f5_local19:completeAnimation()
				self.removeButton:setLeftRight( false, true, -23, -3 )
				self.removeButton:setTopBottom( true, false, 3, 23 )
				self.removeButton:setAlpha( 0 )
				self.clipFinished( f5_local19, {} )
				f5_local20:completeAnimation()
				self.personalizeButton:setAlpha( 0 )
			...
			GainFocus = function ()
				...
				local itemHintTextFrame2 = function ( itemHintText, event )
					if not event.interrupted then
						itemHintText:beginAnimation( "keyframe", 140, false, false, CoD.TweenType.Linear )
					end
				...
				local hintArrowFrame2 = function ( hintArrow, event )
					if not event.interrupted then
						hintArrow:beginAnimation( "keyframe", 140, false, false, CoD.TweenType.Linear )
					end
				...
				self.removeButton:setAlpha( 0 )
				f5_local19Frame2( f5_local19, {} )
				local f5_local20Frame2 = function ( f5_local20, event )
					if not event.interrupted then
						f5_local20:beginAnimation( "keyframe", 140, false, false, CoD.TweenType.Linear )
					end
					f5_local20:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( f5_local20, event )
					else
						f5_local20:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
				
				f5_local20:completeAnimation()
				self.personalizeButton:setAlpha( 0 )
				f5_local20Frame2( f5_local20, {} )
			end,
			Focus = function ()
				self:setupElementClipCounter( 18 )
				BoxButtonLrgInactiveStroke:completeAnimation()
				...
				local itemHintTextFrame2 = function ( itemHintText, event )
					local itemHintTextFrame3 = function ( itemHintText, event )
						if not event.interrupted then
							itemHintText:beginAnimation( "keyframe", 300, false, false, CoD.TweenType.Linear )
						end
						itemHintText:setAlpha( 0 )
						if event.interrupted then
							self.clipFinished( itemHintText, event )
				...
				itemHintTextFrame2( itemHintText, {} )
				local hintArrowFrame2 = function ( hintArrow, event )
					local hintArrowFrame3 = function ( hintArrow, event )
						if not event.interrupted then
							hintArrow:beginAnimation( "keyframe", 300, false, false, CoD.TweenType.Linear )
						end
						hintArrow:setAlpha( 0 )
						if event.interrupted then
							self.clipFinished( hintArrow, event )
					...
					if event.interrupted then
						hintArrowFrame3( hintArrow, event )
						return 
					else
						hintArrow:beginAnimation( "keyframe", 1299, false, false, CoD.TweenType.Linear )
						hintArrow:registerEventHandler( "transition_complete_keyframe", hintArrowFrame3 )
					end
				end
				...
				self.clipFinished( f5_local19, {} )
				f5_local20:completeAnimation()
				self.personalizeButton:setAlpha( 1 )
				self.clipFinished( f5_local20, {} )
			end,
			LoseFocus = function ()
				self:setupElementClipCounter( 11 )
				...
				local f5_local19Frame2 = function ( f5_local19, event )
					if not event.interrupted then
						f5_local19:beginAnimation( "keyframe", 159, false, false, CoD.TweenType.Linear )
					end
					f5_local19:setAlpha( 0 )
					if event.interrupted then
						self.clipFinished( f5_local19, event )
				...
				local f5_local20Frame2 = function ( f5_local20, event )
					if not event.interrupted then
						f5_local20:beginAnimation( "keyframe", 159, false, false, CoD.TweenType.Linear )
					end
					f5_local20:setAlpha( 0 )
					if event.interrupted then
						self.clipFinished( f5_local20, event )
		...
		Available = {
			...
			GainFocus = function ()
				...
				local itemHintTextFrame2 = function ( itemHintText, event )
					if not event.interrupted then
						itemHintText:beginAnimation( "keyframe", 109, false, false, CoD.TweenType.Linear )
					end
					itemHintText:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( itemHintText, event )
				...
				local hintArrowFrame2 = function ( hintArrow, event )
					if not event.interrupted then
						hintArrow:beginAnimation( "keyframe", 109, false, false, CoD.TweenType.Linear )
					end
					hintArrow:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( hintArrow, event )
			...
			Focus = function ()
				...
				local itemHintTextFrame2 = function ( itemHintText, event )
					local itemHintTextFrame3 = function ( itemHintText, event )
						if not event.interrupted then
							itemHintText:beginAnimation( "keyframe", 259, false, false, CoD.TweenType.Linear )
						end
						itemHintText:setAlpha( 0 )
						if event.interrupted then
							self.clipFinished( itemHintText, event )
				...
				local hintArrowFrame2 = function ( hintArrow, event )
					local hintArrowFrame3 = function ( hintArrow, event )
						if not event.interrupted then
							hintArrow:beginAnimation( "keyframe", 259, false, false, CoD.TweenType.Linear )
						end
						hintArrow:setAlpha( 0 )
						if event.interrupted then
							self.clipFinished( hintArrow, event )
			...
			LoseFocus = function ()
				...
				local plusIconFrame2 = function ( plusIcon, event )
					if not event.interrupted then
						plusIcon:beginAnimation( "keyframe", 100, false, false, CoD.TweenType.Linear )
					end
					plusIcon:setAlpha( 0 )
					if event.interrupted then
						self.clipFinished( plusIcon, event )
			...
			Focus = function ()
				...
				local itemHintTextFrame2 = function ( itemHintText, event )
					...
					if event.interrupted then
						...
						itemHintText:beginAnimation( "keyframe", 1279, false, false, CoD.TweenType.Linear )
						itemHintText:registerEventHandler( "transition_complete_keyframe", itemHintTextFrame3 )
					end
				end
				...
				local hintArrowFrame2 = function ( hintArrow, event )
					...
					if event.interrupted then
						...
						hintArrow:beginAnimation( "keyframe", 1279, false, false, CoD.TweenType.Linear )
						hintArrow:registerEventHandler( "transition_complete_keyframe", hintArrowFrame3 )
					end
				end
		...
		NotAvailable = {
			...
			GainFocus = function ()
				self:setupElementClipCounter( 9 )
				local BoxButtonLrgInactiveDiagFrame2 = function ( BoxButtonLrgInactiveDiag, event )
					if not event.interrupted then
						BoxButtonLrgInactiveDiag:beginAnimation( "keyframe", 59, false, false, CoD.TweenType.Linear )
					end
					BoxButtonLrgInactiveDiag:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( BoxButtonLrgInactiveDiag, event )
					else
						BoxButtonLrgInactiveDiag:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
				
				BoxButtonLrgInactiveDiag:completeAnimation()
				self.BoxButtonLrgInactiveDiag:setAlpha( 0 )
				BoxButtonLrgInactiveDiagFrame2( BoxButtonLrgInactiveDiag, {} )
				local itemImageFrame2 = function ( itemImage, event )
					if not event.interrupted then
				...
				local itemHintTextFrame2 = function ( itemHintText, event )
					if not event.interrupted then
						itemHintText:beginAnimation( "keyframe", 79, false, false, CoD.TweenType.Linear )
					end
					itemHintText:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( itemHintText, event )
				...
				local hintArrowFrame2 = function ( hintArrow, event )
					if not event.interrupted then
						hintArrow:beginAnimation( "keyframe", 79, false, false, CoD.TweenType.Linear )
					end
					hintArrow:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( hintArrow, event )
				...
				local FocusWhiteTFrame2 = function ( FocusWhiteT, event )
					...
					if event.interrupted then
						...
						FocusWhiteT:beginAnimation( "keyframe", 29, false, false, CoD.TweenType.Linear )
				...
				local FocusWhiteBFrame2 = function ( FocusWhiteB, event )
					...
					if event.interrupted then
						...
						FocusWhiteB:beginAnimation( "keyframe", 29, false, false, CoD.TweenType.Linear )
			...
			Focus = function ()
				...
				local itemHintTextFrame2 = function ( itemHintText, event )
					local itemHintTextFrame3 = function ( itemHintText, event )
						if not event.interrupted then
							itemHintText:beginAnimation( "keyframe", 330, false, false, CoD.TweenType.Linear )
						end
						itemHintText:setAlpha( 0 )
						if event.interrupted then
							self.clipFinished( itemHintText, event )
				...
				local hintArrowFrame2 = function ( hintArrow, event )
					local hintArrowFrame3 = function ( hintArrow, event )
						if not event.interrupted then
							hintArrow:beginAnimation( "keyframe", 330, false, false, CoD.TweenType.Linear )
						end
						hintArrow:setAlpha( 0 )
						if event.interrupted then
							self.clipFinished( hintArrow, event )
						...
						local FocusWhiteTFrame4 = function ( FocusWhiteT, event )
							local FocusWhiteTFrame5 = function ( FocusWhiteT, event )
								local FocusWhiteTFrame6 = function ( FocusWhiteT, event )
									local FocusWhiteTFrame7 = function ( FocusWhiteT, event )
										if not event.interrupted then
											FocusWhiteT:beginAnimation( "keyframe", 59, false, false, CoD.TweenType.Linear )
										end
										FocusWhiteT:setAlpha( 0 )
										if event.interrupted then
											self.clipFinished( FocusWhiteT, event )
						...
						local FocusWhiteBFrame4 = function ( FocusWhiteB, event )
							local FocusWhiteBFrame5 = function ( FocusWhiteB, event )
								local FocusWhiteBFrame6 = function ( FocusWhiteB, event )
									local FocusWhiteBFrame7 = function ( FocusWhiteB, event )
										if not event.interrupted then
											FocusWhiteB:beginAnimation( "keyframe", 59, false, false, CoD.TweenType.Linear )
										end
										FocusWhiteB:setAlpha( 0 )
										if event.interrupted then
											self.clipFinished( FocusWhiteB, event )
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( element )
		...
		element.personalizeButton:close()
		element.RestrictedItemWarning:close()
		element.GridItemUpgradeIconWidget:close()
