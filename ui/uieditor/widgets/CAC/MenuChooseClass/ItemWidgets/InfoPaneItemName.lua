-- 064db690f8a8f5ea5bef73033466126f
...
CoD.InfoPaneItemName.new = function ( menu, controller )
	...
	emptyOrUnavailableLabels:setLeftRight( true, false, 0, 275 )
	...
	self:mergeStateConditions( {
		...
		},
		{
			...
			condition = function ( menu, element, event )
				...
				if not IsCACSlotEquipped( menu, element, controller ) then
					f12_local0 = IsCACSlotCanEquipWildcard( menu, element, controller )
				else
					f12_local0 = false
				end
				return f12_local0
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f13_local0
				if not IsCACSlotEquipped( menu, element, controller ) then
					f13_local0 = IsCACSlotNeedWildcard( menu, element, controller )
				else
					f13_local0 = false
				end
				return f13_local0
