-- 112765f9b27c2a8f20273d8a6d7d54a9
...
CoD.PrimaryWeaponAttachmentsWidget.new = function ( menu, controller )
			...
			condition = function ( menu, element, event )
				...
				if not IsCACSlotEquipped( menu, element, controller ) then
					...
					if f23_local0 then
						...
						if f23_local0 then
							f23_local0 = IsCACPrimaryAttachmentSlotLockedByWildcard( controller )
						end
					end
				else
					f23_local0 = false
	...
	self:registerEventHandler( "gain_focus", function ( element, event )
		if element.m_focusable and element.attachment5:processEvent( event ) then
			return true
		else
