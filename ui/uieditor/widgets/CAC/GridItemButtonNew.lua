-- dcd9a2157f1e4454ac675aa30e3a1ca6
...
CoD.GridItemButtonNew.new = function ( menu, controller )
	...
	RestrictedItemWarning:mergeStateConditions( {
		{
			stateName = "Visible",
			condition = function ( menu, element, event )
				return ItemIsBannedLobby( menu, element, controller ) and not ItemRestrictionVoteActive()
			end
		}
	} )
	...
	GridItemUpgradeIconWidget:setLeftRight( true, false, 2, 20 )
	GridItemUpgradeIconWidget:setTopBottom( true, false, 2, 20 )
	...
	self.clipsPerState = {
		...
		RequiresDLC = {
			DefaultClip = function ()
				...
				self.ConsumableLabel:setAlpha( 0 )
				self.clipFinished( ConsumableLabel, {} )

				dlcDownloadImage:completeAnimation()
				self.dlcDownloadImage:setAlpha( 1 )
				self.clipFinished( dlcDownloadImage, {} )
			end,
			GainFocus = function ()
				self:setupElementClipCounter( 5 )
	...
	self:mergeStateConditions( {
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f746_local0 = IsInPermanentUnlockMenu( controller )
				if f746_local0 then
					if not IsPermanentlyUnlocked( element, controller ) then
						f746_local0 = HavePermanentUnlockTokens( controller )
						if f746_local0 then
							f746_local0 = IsCACItemPurchased( element, controller )
						end
					else
						f746_local0 = false
					end
