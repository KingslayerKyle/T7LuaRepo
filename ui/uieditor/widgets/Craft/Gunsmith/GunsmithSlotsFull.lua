-- f9748bdfab45f66e38b246c1875de821
-- This hash is used for caching, delete to decompile the file again

require( "ui.uieditor.widgets.BackgroundFrames.BackgroundPattern01" )

CoD.GunsmithSlotsFull = InheritFrom( LUI.UIElement )
CoD.GunsmithSlotsFull.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.GunsmithSlotsFull )
	self.id = "GunsmithSlotsFull"
	...
	count:subscribeToGlobalModel( controller, "PerController", "Gunsmith.UpdateDataSource", function ( model )
		UpdateSlotCountText( self, count, controller, Enum.StorageFileType.STORAGE_GUNSMITH )
	end )
	...
	VariantSlots:setText( Engine.Localize( "MENU_GUNSMITH_SLOTS" ) )
