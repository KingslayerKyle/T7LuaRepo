-- a7db6404852998799f4968aaf912de3a
...
CoD.WeaponBuildKitsCategoryButton = InheritFrom( LUI.UIElement )
CoD.WeaponBuildKitsCategoryButton.new = function ( menu, controller )
	...
	self:setClass( CoD.WeaponBuildKitsCategoryButton )
	self.id = "WeaponBuildKitsCategoryButton"
	...
	BoxButtonLrgIdle:setLeftRight( true, true, -1, 0 )
	BoxButtonLrgIdle:setTopBottom( true, true, -0.5, 0 )
	BoxButtonLrgIdle:setRFTMaterial( LUI.UIImage.GetCachedMaterial( "ui_add" ) )
	self:addElement( BoxButtonLrgIdle )
	self.BoxButtonLrgIdle = BoxButtonLrgIdle
	
	local ImageContainer = CoD.StartMenu_ImageContainer.new( menu, controller )
	ImageContainer:setLeftRight( false, false, -103, 103 )
	ImageContainer:setTopBottom( false, false, -103, 103 )
	...
	StartMenuIdentitySubTitle0:setLeftRight( true, false, 5, 257 )
	...
	tokenUnlock:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f2_local0
				if not IsCampaign() and not IsCACItemLocked( menu, element, controller ) then
					f2_local0 = not IsCACItemPurchased( element, controller )
				else
					f2_local0 = false
				end
				return f2_local0
		...
		},
		{
			stateName = "VisibleCP",
			condition = function ( menu, element, event )
				local f3_local0 = IsCampaign()
				if f3_local0 then
					if not IsCACItemLocked( menu, element, controller ) then
						f3_local0 = not IsCACItemPurchased( element, controller )
					else
						f3_local0 = false
					end
				end
	...
	IconNew:setLeftRight( true, false, 136.5, 201.5 )
	IconNew:setTopBottom( false, false, 10.5, 34.5 )
	...
	breadcrumbCount:setLeftRight( false, true, -132, -114 )
	breadcrumbCount:setTopBottom( true, false, 69, 87 )
	...
	self.ImageContainer:linkToElementModel( self, "categoryImage", true, function ( model )
		local categoryImage = Engine.GetModelValue( model )
		if categoryImage then
			ImageContainer.ImageContainer:setImage( RegisterImage( categoryImage ) )
		end
	end )
	self.StartMenuIdentitySubTitle0:linkToElementModel( self, "categoryName", true, function ( model )
		local categoryName = Engine.GetModelValue( model )
		if categoryName then
			StartMenuIdentitySubTitle0.SubTitle:setText( Engine.Localize( categoryName ) )
		end
	end )
	self.clipsPerState = {
		DefaultState = {
			DefaultClip = function ()
				...
				breadcrumbCount:completeAnimation()
				self.breadcrumbCount:setAlpha( 0 )
				self.clipFinished( breadcrumbCount, {} )
			end,
			GainFocus = function ()
			...
			Focus = function ()
				...
				BoxButtonLrgInactive:completeAnimation()
				self.BoxButtonLrgInactive:setLeftRight( true, true, -2, 0 )
		...
		New = {
			DefaultClip = function ()
				...
				breadcrumbCount:completeAnimation()
				self.breadcrumbCount:setAlpha( 1 )
				self.clipFinished( breadcrumbCount, {} )
			end,
			GainFocus = function ()
				self:setupElementClipCounter( 6 )
		...
		Locked = {
			...
			GainFocus = function ()
				...
				local BoxButtonLrgIdleFrame2 = function ( BoxButtonLrgIdle, event )
					if not event.interrupted then
						BoxButtonLrgIdle:beginAnimation( "keyframe", 79, false, false, CoD.TweenType.Linear )
					end
	...
	self:mergeStateConditions( {
		...
		},
		{
			stateName = "New",
			condition = function ( menu, element, event )
				return Gunsmith_AnyNewWeaponsOrAttachmentsForGroupButton( element, controller )
			end
		},
		{
			stateName = "Locked",
