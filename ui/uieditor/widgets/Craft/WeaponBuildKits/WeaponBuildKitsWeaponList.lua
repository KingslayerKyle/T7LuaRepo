-- 3d9ed796edee722fc642a10109742e13
...
CoD.WeaponBuildKitsWeaponList = InheritFrom( LUI.UIElement )
CoD.WeaponBuildKitsWeaponList.new = function ( menu, controller )
	...
	self:setClass( CoD.WeaponBuildKitsWeaponList )
	self.id = "WeaponBuildKitsWeaponList"
	...
	tokenUnlock:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if not IsCampaign() and not IsCACItemLocked( menu, element, controller ) then
					f4_local0 = not IsCACItemPurchased( element, controller )
				else
					f4_local0 = false
				end
				return f4_local0
		...
		},
		{
			stateName = "VisibleCP",
			condition = function ( menu, element, event )
				local f5_local0 = IsCampaign()
				if f5_local0 then
					if not IsCACItemLocked( menu, element, controller ) then
						f5_local0 = not IsCACItemPurchased( element, controller )
					else
						f5_local0 = false
					end
				end
	...
	LabelButton0:setLeftRight( true, false, 82, 168 )
	LabelButton0:setTopBottom( false, false, -9.5, 10.5 )
	...
	local NewBreadcrumb0 = CoD.NewBreadcrumb.new( menu, controller )
	NewBreadcrumb0:setLeftRight( true, false, 256, 276 )
	NewBreadcrumb0:setTopBottom( true, false, 23.5, 43.5 )
	NewBreadcrumb0:mergeStateConditions( {
		{
			stateName = "Visible",
			condition = function ( menu, element, event )
				return Gunsmith_AnyNewItemsForWeapon( element, controller )
			end
		}
	} )
	self:addElement( NewBreadcrumb0 )
	self.NewBreadcrumb0 = NewBreadcrumb0
