-- a9cd66caf15c08d6b2d53fad7b6e4109
...
CoD.PaintjobSelectorItem.new = function ( menu, controller )
	...
	FEPanelNoBlur0:setLeftRight( true, true, 8, -7 )
	FEPanelNoBlur0:setTopBottom( true, true, 84, -3 )
	FEPanelNoBlur0:setRGB( 0, 0, 0 )
	FEPanelNoBlur0:setAlpha( 0.4 )
	self:addElement( FEPanelNoBlur0 )
	self.FEPanelNoBlur0 = FEPanelNoBlur0
	
	local horizontalScrollingTextBox20pt0 = CoD.horizontalScrollingTextBox_20pt.new( menu, controller )
	horizontalScrollingTextBox20pt0:setLeftRight( true, false, 8, 101 )
	horizontalScrollingTextBox20pt0:setTopBottom( true, false, 84, 105 )
	horizontalScrollingTextBox20pt0.textBox:setTTF( "fonts/default.ttf" )
	horizontalScrollingTextBox20pt0.textBox:setAlignment( Enum.LUIAlignment.LUI_ALIGNMENT_LEFT )
	horizontalScrollingTextBox20pt0:linkToElementModel( self, "paintjobName", true, function ( model )
		local paintjobName = Engine.GetModelValue( model )
		if paintjobName then
			horizontalScrollingTextBox20pt0.textBox:setText( GetPaintjobName( paintjobName ) )
		end
	end )
	self:addElement( horizontalScrollingTextBox20pt0 )
	self.horizontalScrollingTextBox20pt0 = horizontalScrollingTextBox20pt0
	
	local UnavailableText = LUI.UIText.new()
	UnavailableText:setLeftRight( true, false, 8, 101 )
	UnavailableText:setTopBottom( true, false, 84, 105 )
	UnavailableText:setAlpha( 0 )
	UnavailableText:setText( Engine.Localize( "MENU_UNAVAILABLE_CAPS" ) )
	UnavailableText:setTTF( "fonts/default.ttf" )
	UnavailableText:setAlignment( Enum.LUIAlignment.LUI_ALIGNMENT_CENTER )
	UnavailableText:setAlignment( Enum.LUIAlignment.LUI_ALIGNMENT_TOP )
	self:addElement( UnavailableText )
	...
	self.clipsPerState = {
		DefaultState = {
			...
			GainFocus = function ()
				...
				local extraCamRenderFrame2 = function ( extraCamRender, event )
					if not event.interrupted then
						extraCamRender:beginAnimation( "keyframe", 90, false, false, CoD.TweenType.Linear )
					end
					extraCamRender:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( extraCamRender, event )
					else
						extraCamRender:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
				
				extraCamRender:completeAnimation()
				self.extraCamRender:setAlpha( 0.8 )
				extraCamRenderFrame2( extraCamRender, {} )
			...
			LoseFocus = function ()
				...
				local extraCamRenderFrame2 = function ( extraCamRender, event )
					if not event.interrupted then
						extraCamRender:beginAnimation( "keyframe", 140, false, false, CoD.TweenType.Linear )
					end
					extraCamRender:setAlpha( 0.8 )
					if event.interrupted then
						self.clipFinished( extraCamRender, event )
					else
						extraCamRender:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
		...
		EmptySlot = {
			...
			LoseFocus = function ()
				...
				local plusIconFrame2 = function ( plusIcon, event )
					if not event.interrupted then
						plusIcon:beginAnimation( "keyframe", 119, false, false, CoD.TweenType.Linear )
					end
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( element )
		...
		element.FEPanelNoBlur0:close()
		element.horizontalScrollingTextBox20pt0:close()
