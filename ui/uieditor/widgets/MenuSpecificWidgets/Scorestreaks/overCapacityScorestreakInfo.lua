-- d5a9ad301545ebcd336763c945642177
...
CoD.overCapacityScorestreakInfo.new = function ( menu, controller )
	...
	descText:linkToElementModel( self, "itemIndex", true, function ( model )
		...
		if itemIndex then
			descText:setText( Engine.Localize( LocalizeIntoString( "MENU_SCORE_REQUIRED", GetScorestreakMomentumFromIndex( itemIndex ) ) ) )
		end
	end )
	...
	nameLabel:linkToElementModel( self, "itemIndex", true, function ( model )
		local itemIndex = Engine.GetModelValue( model )
		if itemIndex then
			nameLabel:setText( Engine.Localize( GetItemNameFromIndex( itemIndex ) ) )
		end
	end )
