-- 4013b3fccb475926dd33d746b6eef979
-- This hash is used for caching, delete to decompile the file again

require( "ui.uieditor.widgets.StartMenu.StartMenu_frame_noBG" )
require( "ui.uieditor.widgets.Lobby.Common.FE_FocusBarContainer" )

CoD.MonthItem = InheritFrom( LUI.UIElement )
CoD.MonthItem.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.MonthItem )
	self.id = "MonthItem"
	...
	self.Text:linkToElementModel( self, "month", true, function ( model )
		local month = Engine.GetModelValue( model )
		if month then
			Text:setText( month )
		end
	end )
