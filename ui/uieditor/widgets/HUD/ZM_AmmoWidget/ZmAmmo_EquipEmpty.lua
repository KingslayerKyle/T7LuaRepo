-- 7ca43930e326bb49108aea37ff8a87c8
-- This hash is used for caching, delete to decompile the file again

CoD.ZmAmmo_EquipEmpty = InheritFrom( LUI.UIElement )
CoD.ZmAmmo_EquipEmpty.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.ZmAmmo_EquipEmpty )
	self.id = "ZmAmmo_EquipEmpty"
	...
	EmptyFlash:setLeftRight( false, false, -18, 18 )
	EmptyFlash:setTopBottom( false, false, -17.5, 18.5 )
	EmptyFlash:setRGB( 1, 0.01, 0 )
	EmptyFlash:setImage( RegisterImage( "uie_t7_zm_hud_ammo_elminvcrc" ) )
