-- 562ab32aecaf6ff744ae10f78e454add
...
CoD.AmmoWidgetMP_ClipInfo.new = function ( menu, controller )
	...
	TotalAmmo:mergeStateConditions( {
		{
			stateName = "Hidden",
			condition = function ( menu, element, event )
				local f36_local0 = IsGameTypeEqualToString( "gun" )
				if not f36_local0 then
					f36_local0 = IsGameTypeEqualToString( "prop" )
				end
				return f36_local0
			end
		},
	...
	self:mergeStateConditions( {
		...
		},
		{
			stateName = "WeaponNoReserve",
			condition = function ( menu, element, event )
				local f134_local0 = IsCurrentWeaponReference( controller, "lmg_infinite" )
				if f134_local0 then
					if not IsGameTypeEqualToString( "gun" ) then
						f134_local0 = not IsGameTypeEqualToString( "prop" )
					else
						f134_local0 = false
					end
				end
