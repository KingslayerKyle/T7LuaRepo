-- c4b7c60c3fd70eed781cd0ea48f6b542
...
local PreLoadFunc = function ( self, controller )
	...
	Engine.SetModelValue( Engine.CreateModel( f1_local0, "rouletteStatus" ), 0 )
	Engine.SetModelValue( Engine.CreateModel( f1_local0, "cardImage" ), "blacktransparent" )
	Engine.SetModelValue( Engine.CreateModel( f1_local0, "thiefStatus" ), 0 )
	Engine.SetModelValue( Engine.CreateModel( f1_local0, "thiefWeaponStatus" ), 0 )
	Engine.SetModelValue( Engine.CreateModel( f1_local0, "thiefLastWeapon" ), CoD.SafeGetModelValue( f1_local0, "imageAvailable" ) or "" )
end
	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if not IsHeroWeaponGadgetAmmoEmpty( menu, controller ) and not IsHeroWeaponGadgetCurrentWeapon( menu, controller ) then
					f77_local0 = IsHeroWeaponOrSpecialGadgetAvailable( menu, controller )
					if f77_local0 then
		...
		},
		{
			stateName = "Charge",
			condition = function ( menu, element, event )
				local f78_local0 = IsSpecialGadgetAvailable( controller )
				if f78_local0 then
					if not IsHeroWeaponOrGadgetInUse( menu, controller ) then
						f78_local0 = IsHeroAbilityGadget( controller )
					else
						f78_local0 = false
					end
				end
