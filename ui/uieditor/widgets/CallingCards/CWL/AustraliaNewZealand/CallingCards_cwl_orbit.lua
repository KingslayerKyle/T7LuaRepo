-- 5319283f04a2c9a014280ab121536809
-- This hash is used for caching, delete to decompile the file again

require( "ui.uieditor.widgets.CallingCards.CWL.CallingCards_cwl_worldmap_widget" )

CoD.CallingCards_cwl_orbit = InheritFrom( LUI.UIElement )
CoD.CallingCards_cwl_orbit.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( true )
	self:setClass( CoD.CallingCards_cwl_orbit )
	self.id = "CallingCards_cwl_orbit"
	...
	LOGO:setLeftRight( true, false, 11.43, 480 )
	LOGO:setTopBottom( true, false, 0, 120 )
	LOGO:setImage( RegisterImage( "uie_callingcards_cwl_orbit" ) )
