-- 27e40bdce5e61c6a0fbd31de11cbd2b0
-- This hash is used for caching, delete to decompile the file again

require( "ui.uieditor.widgets.CallingCards.CWL.CallingCards_cwl_worldmap_widget" )

CoD.CallingCards_cwl_nv = InheritFrom( LUI.UIElement )
CoD.CallingCards_cwl_nv.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( true )
	self:setClass( CoD.CallingCards_cwl_nv )
	self.id = "CallingCards_cwl_nv"
	...
	LOGO:setLeftRight( true, false, 2, 478.38 )
	LOGO:setTopBottom( true, false, -2, 120 )
	LOGO:setImage( RegisterImage( "uie_callingcards_cwl_nv" ) )
