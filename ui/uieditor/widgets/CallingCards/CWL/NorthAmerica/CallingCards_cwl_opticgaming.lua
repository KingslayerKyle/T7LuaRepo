-- 6cf6cd7860f696b4740f4613c935d4be
-- This hash is used for caching, delete to decompile the file again

require( "ui.uieditor.widgets.CallingCards.CWL.CallingCards_cwl_worldmap_widget" )

CoD.CallingCards_cwl_opticgaming = InheritFrom( LUI.UIElement )
CoD.CallingCards_cwl_opticgaming.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( true )
	self:setClass( CoD.CallingCards_cwl_opticgaming )
	self.id = "CallingCards_cwl_opticgaming"
	...
	LOGO:setTopBottom( true, false, -1.93, 121 )
	LOGO:setImage( RegisterImage( "uie_callingcards_cwl_opticgaming" ) )
