-- 89329528dd561ff99639b2aab26c5024
-- This hash is used for caching, delete to decompile the file again

require( "ui.uieditor.widgets.Lobby.Common.FE_ButtonPanelShaderContainer" )
require( "ui.uieditor.widgets.Lobby.Common.FE_HelpItemsLabel" )

CoD.MOTD_Button = InheritFrom( LUI.UIElement )
CoD.MOTD_Button.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.MOTD_Button )
	self.id = "MOTD_Button"
	...
	btnDisplayText:linkToElementModel( self, "text", true, function ( model )
		local text = Engine.GetModelValue( model )
		if text then
			btnDisplayText:setText( Engine.Localize( text ) )
		end
	end )
	...
	btnDisplayTextStroke:linkToElementModel( self, "text", true, function ( model )
		local text = Engine.GetModelValue( model )
		if text then
			btnDisplayTextStroke:setText( Engine.Localize( text ) )
		end
	end )
