-- b6dc539c2fd227da73cde1574c07e363
...
CoD.CombatRecordButton.new = function ( menu, controller )
	...
	Emblem:subscribeToGlobalModel( controller, "StorageGlobal", "stats_mp", function ( model )
		...
		if statsMp then
			Emblem:setImage( RegisterImage( GetRankIconLarge( GetRankOrParagonIcon( controller, "playerstatslist.rank.statvalue", "playerstatslist.plevel.statvalue", "playerstatslist.paragon_icon_id.statvalue", "mp", statsMp ) ) ) )
		end
	end )
	...
	RankName:setLeftRight( false, false, -330.5, -136 )
	...
	Rank:subscribeToGlobalModel( controller, "StorageGlobal", "stats_mp", function ( model )
		local statsMp = Engine.GetModelValue( model )
		if statsMp then
			Rank:setRGB( SetToParagonColorIfPrestigeMasterFromStorage( controller, "mp", 255, 255, 255, statsMp ) )
		end
	end )
	Rank:subscribeToGlobalModel( controller, "StorageGlobal", "stats_mp", function ( model )
		local statsMp = Engine.GetModelValue( model )
		if statsMp then
			Rank:setText( Engine.Localize( LevelStringFromStorage( controller, "mp", statsMp ) ) )
		end
	end )
	...
	PrestigeMasterTierWidget:subscribeToGlobalModel( controller, "StorageGlobal", "stats_mp", function ( model )
		local statsMp = Engine.GetModelValue( model )
		if statsMp then
			PrestigeMasterTierWidget.ParagonStars:setHorizontalCount( GetPrestigeMasterTierCountFromStorage( controller, "mp", statsMp ) )
		end
	end )
