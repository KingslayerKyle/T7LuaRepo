-- 8c16295ef7c63473c419cd29d76f859a
...
CoD.GameSettings_Button.new = function ( menu, controller )
	...
	btnDisplayText:setTopBottom( true, false, 7, 27 )
	...
	btnDisplayTextStroke:setTopBottom( true, false, 7, 27 )
	...
	GameSettingsChangedIndicator:setLeftRight( true, false, -30, 0 )
	GameSettingsChangedIndicator:setTopBottom( true, false, 1.92, 31.92 )
	...
	self.btnDisplayText:linkToElementModel( self, "displayText", true, function ( model )
		...
		if displayText then
			btnDisplayText:setText( Engine.Localize( LocalizeToUpperString( displayText ) ) )
		end
	end )
	self.btnDisplayTextStroke:linkToElementModel( self, "displayText", true, function ( model )
		local displayText = Engine.GetModelValue( model )
		if displayText then
			btnDisplayTextStroke:setText( Engine.Localize( LocalizeToUpperString( displayText ) ) )
		end
	end )
	self.clipsPerState = {
		DefaultState = {
			...
			Intro = function ()
				...
				self.Glow2:setAlpha( 0 )
				self.clipFinished( Glow2, {} )
				local GameSettingsChangedIndicatorFrame2 = function ( GameSettingsChangedIndicator, event )
					if not event.interrupted then
						GameSettingsChangedIndicator:beginAnimation( "keyframe", 300, false, false, CoD.TweenType.Bounce )
					end
					GameSettingsChangedIndicator:setAlpha( 1 )
					if event.interrupted then
						self.clipFinished( GameSettingsChangedIndicator, event )
					else
						GameSettingsChangedIndicator:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
				
				GameSettingsChangedIndicator:completeAnimation()
				self.GameSettingsChangedIndicator:setAlpha( 0 )
				GameSettingsChangedIndicatorFrame2( GameSettingsChangedIndicator, {} )
			end
		},
		Disabled = {
			DefaultClip = function ()
			...
			Intro = function ()
				...
				local GameSettingsChangedIndicatorFrame2 = function ( GameSettingsChangedIndicator, event )
					if not event.interrupted then
						GameSettingsChangedIndicator:beginAnimation( "keyframe", 400, false, false, CoD.TweenType.Bounce )
					end
					GameSettingsChangedIndicator:setAlpha( 0.2 )
					if event.interrupted then
						self.clipFinished( GameSettingsChangedIndicator, event )
					else
						GameSettingsChangedIndicator:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
