-- c8877124bc4fe4db2ded8a8481b62e29
...
CoD.GridItemTacticalRig = InheritFrom( LUI.UIElement )
CoD.GridItemTacticalRig.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.GridItemTacticalRig )
	self.id = "GridItemTacticalRig"
	...
	BoxButtonLrgInactiveDiag:setLeftRight( true, true, -2, -215 )
	...
	itemImageSolid:linkToElementModel( self, "image", true, function ( model )
		...
		if image then
			itemImageSolid:setImage( RegisterImage( image ) )
		end
	end )
	...
	itemImage:setLeftRight( true, true, 0, -215 )
	...
	tokenUnlock:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if not IsCampaign() then
					f4_local0 = ShouldShowTokenIconOnItem( menu, element, controller )
				else
					f4_local0 = false
				end
				return f4_local0
		...
		},
		{
			stateName = "VisibleCP",
			condition = function ( menu, element, event )
				return IsCampaign() and ShouldShowTokenIconOnItem( menu, element, controller )
			end
		}
	} )
	...
	LabelButton:setLeftRight( true, false, 74, 178 )
	LabelButton:setTopBottom( false, true, -42, -22 )
	...
	LabelButton:linkToElementModel( self, "name", true, function ( model )
		...
		if name then
			LabelButton.itemName:setText( Engine.Localize( AppendStringIfRefExists( "_SHORT", name ) ) )
		end
	end )
	...
	equippedIcon:setLeftRight( false, true, -81, -67 )
	...
	alertIcon:setLeftRight( false, true, -83, -69 )
	...
	lockedIcon:setLeftRight( false, false, -126, -96 )
	lockedIcon:setTopBottom( false, false, -16, 16 )
	lockedIcon:setAlpha( 0.8 )
	...
	ConsumableLabel:setLeftRight( true, false, 3, 25 )
	ConsumableLabel:setTopBottom( true, false, 5, 23 )
	ConsumableLabel.ComsumableCountLabel:setText( Engine.Localize( "0" ) )
	...
	local HeaderLabel = CoD.cac_ListButtonLabel.new( menu, controller )
	HeaderLabel:setLeftRight( true, false, 2, 106 )
	HeaderLabel:setTopBottom( false, true, -104.25, -84.25 )
	HeaderLabel:setAlpha( 0 )
	HeaderLabel:linkToElementModel( self, nil, false, function ( model )
		HeaderLabel:setModel( model, controller )
	end )
	HeaderLabel:linkToElementModel( self, "headerName", true, function ( model )
		local headerName = Engine.GetModelValue( model )
		if headerName then
			HeaderLabel.itemName:setText( Engine.Localize( headerName ) )
		end
	end )
	HeaderLabel:mergeStateConditions( {
		{
			stateName = "Hidden",
			condition = function ( menu, element, event )
				return not IsSelfModelValueNonEmptyString( element, controller, "headerName" )
			end
		}
	} )
	HeaderLabel:linkToElementModel( HeaderLabel, "headerName", true, function ( model )
		menu:updateElementState( HeaderLabel, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
	...
	RestrictedItemWarning:setLeftRight( true, false, -1, 109 )
	RestrictedItemWarning:setTopBottom( true, false, 0, 95 )
	...
	RestrictedItemWarning:mergeStateConditions( {
		{
			stateName = "Visible",
			condition = function ( menu, element, event )
				local f16_local0 = ItemIsBanned( menu, element, controller )
				if f16_local0 then
					f16_local0 = PregameActive()
					if f16_local0 then
						f16_local0 = not ItemRestrictionVoteActive()
					end
				end
				return f16_local0
			end
	...
	RestrictedItemWarning:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "lobbyRoot.Pregame.state" ), function ( model )
		menu:updateElementState( RestrictedItemWarning, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
	...
	local GridItemUpgradeIconTacticalRig = CoD.GridItemUpgradeIconTacticalRig.new( menu, controller )
	GridItemUpgradeIconTacticalRig:setLeftRight( true, false, 216, 280 )
	GridItemUpgradeIconTacticalRig:setTopBottom( true, false, 0.5, 64.5 )
	GridItemUpgradeIconTacticalRig:mergeStateConditions( {
		{
			stateName = "Upgradable",
			condition = function ( menu, element, event )
				return IsCACItemUpgradable( menu, element, controller ) and IsCampaign()
			end
		},
		{
			stateName = "Upgraded",
			condition = function ( menu, element, event )
				return IsCACItemUpgraded( menu, element, controller ) and IsCampaign()
			end
		}
	} )
	GridItemUpgradeIconTacticalRig:linkToElementModel( GridItemUpgradeIconTacticalRig, "itemIndex", true, function ( model )
		menu:updateElementState( GridItemUpgradeIconTacticalRig, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
			modelName = "itemIndex"
	...
	GridItemUpgradeIconTacticalRig:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyNav" ), function ( model )
		menu:updateElementState( GridItemUpgradeIconTacticalRig, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
	...
	self:addElement( GridItemUpgradeIconTacticalRig )
	self.GridItemUpgradeIconTacticalRig = GridItemUpgradeIconTacticalRig
	
	local itemImageLine = LUI.UIImage.new()
	itemImageLine:setLeftRight( true, false, 213, 217 )
	...
	local itemImageLine1 = LUI.UIImage.new()
	itemImageLine1:setLeftRight( true, false, 64, 68 )
	itemImageLine1:setTopBottom( false, false, -33, 34 )
	itemImageLine1:setAlpha( 0.3 )
	itemImageLine1:setImage( RegisterImage( "uie_t7_menu_frontend_featuredemblemline" ) )
	itemImageLine1:setMaterial( LUI.UIImage.GetCachedMaterial( "ui_add" ) )
	self:addElement( itemImageLine1 )
	self.itemImageLine1 = itemImageLine1
	
	local tokenUnlockable = CoD.cac_IconTokenStatic.new( menu, controller )
	tokenUnlockable:setLeftRight( false, true, -94, -66 )
	tokenUnlockable:setTopBottom( true, false, -2, 26 )
	tokenUnlockable:setAlpha( 0 )
	tokenUnlockable:mergeStateConditions( {
		{
			stateName = "Visible",
			condition = function ( menu, element, event )
				local f23_local0
				if not IsCampaign() and not IsCACItemLocked( menu, element, controller ) and not IsCACItemPurchased( element, controller ) then
					f23_local0 = not IsCurrentMenuAttachmentMenu( menu )
				else
					f23_local0 = false
				end
				return f23_local0
	...
	tokenUnlockable:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyNav" ), function ( model )
		menu:updateElementState( tokenUnlockable, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
			modelName = "lobbyRoot.lobbyNav"
	...
	tokenUnlockable:linkToElementModel( tokenUnlockable, "itemIndex", true, function ( model )
		menu:updateElementState( tokenUnlockable, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
	...
	self:addElement( tokenUnlockable )
	self.tokenUnlockable = tokenUnlockable
	
	local equippedIconFully = LUI.UIImage.new()
	equippedIconFully:setLeftRight( false, true, -16, -2 )
	equippedIconFully:setTopBottom( true, false, 2, 16 )
	equippedIconFully:setRGB( 0.56, 0.68, 0.27 )
	equippedIconFully:setAlpha( 0 )
	equippedIconFully:setImage( RegisterImage( "uie_t7_hud_cac_equipped_16" ) )
	self:addElement( equippedIconFully )
	self.equippedIconFully = equippedIconFully
	
	local UpgradableIconLock = LUI.UIImage.new()
	UpgradableIconLock:setLeftRight( true, true, 215, 0 )
	UpgradableIconLock:setTopBottom( true, true, -0.25, -0.25 )
	UpgradableIconLock:setAlpha( 0.25 )
	UpgradableIconLock:setImage( RegisterImage( "uie_t7_cac_cp_upgrade0_icon" ) )
	self:addElement( UpgradableIconLock )
	self.UpgradableIconLock = UpgradableIconLock
	...
	self.clipsPerState = {
		DefaultState = {
			DefaultClip = function ()
				...
				local tokenUnlockFrame2 = function ( tokenUnlock, event )
					if not event.interrupted then
						tokenUnlock:beginAnimation( "keyframe", 300, false, false, CoD.TweenType.Linear )
					end
					tokenUnlock:setAlpha( 0 )
					if event.interrupted then
						self.clipFinished( tokenUnlock, event )
					else
						tokenUnlock:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
					end
				end
				
				tokenUnlock:completeAnimation()
				self.tokenUnlock:setAlpha( 0 )
				tokenUnlockFrame2( tokenUnlock, {} )
				...
				tokenUnlockable:beginAnimation( "keyframe", 300, false, false, CoD.TweenType.Linear )
				tokenUnlockable:setAlpha( 0 )
				tokenUnlockable:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
				UpgradableIconLock:completeAnimation()
				self.UpgradableIconLock:setAlpha( 0 )
				self.clipFinished( UpgradableIconLock, {} )
			end,
			GainFocus = function ()
		...
		ItemBanned = {
			DefaultClip = function ()
				...
				self.itemImageLine:setAlpha( 0.5 )
				self.clipFinished( itemImageLine, {} )
				tokenUnlockable:completeAnimation()
				self.tokenUnlockable:setAlpha( 1 )
				self.clipFinished( tokenUnlockable, {} )
				UpgradableIconLock:completeAnimation()
				self.UpgradableIconLock:setAlpha( 0.25 )
				self.UpgradableIconLock:setScale( 0.8 )
				self.clipFinished( UpgradableIconLock, {} )
			end,
			GainFocus = function ()
				self:setupElementClipCounter( 7 )
		...
		Equipped = {
			DefaultClip = function ()
				...
				self.clipFinished( LabelButton, {} )
				equippedIcon:completeAnimation()
				self.equippedIcon:setLeftRight( false, true, -83, -69 )
				self.equippedIcon:setTopBottom( true, false, 2, 16 )
				...
				self.clipFinished( glitch2, {} )
				itemImageLine:completeAnimation()
				self.itemImageLine:setAlpha( 1 )
				self.clipFinished( itemImageLine, {} )
				equippedIconFully:completeAnimation()
				self.equippedIconFully:setAlpha( 0 )
				self.clipFinished( equippedIconFully, {} )
				UpgradableIconLock:completeAnimation()
				self.UpgradableIconLock:setAlpha( 0 )
				self.clipFinished( UpgradableIconLock, {} )
			end,
		...
		Locked = {
			DefaultClip = function ()
				...
				self.clipFinished( itemImageLine, {} )
				UpgradableIconLock:completeAnimation()
				self.UpgradableIconLock:setLeftRight( true, true, 215, 0 )
				self.UpgradableIconLock:setTopBottom( true, true, -0.25, -0.25 )
				self.UpgradableIconLock:setAlpha( 0.25 )
				self.UpgradableIconLock:setScale( 0.8 )
				self.clipFinished( UpgradableIconLock, {} )
			end,
		...
		MutuallyExclusive = {
			...
			LoseFocus = function ()
				...
				local itemImageFrame2 = function ( itemImage, event )
					if not event.interrupted then
						itemImage:beginAnimation( "keyframe", 140, false, false, CoD.TweenType.Linear )
					end
					itemImage:setAlpha( 0.8 )
					if event.interrupted then
						self.clipFinished( itemImage, event )
		...
		EquippedFullyUpgraded = {
			DefaultClip = function ()
				...
				self.clipFinished( itemImageLine, {} )
				equippedIconFully:completeAnimation()
				self.equippedIconFully:setLeftRight( false, true, -16, -2 )
				self.equippedIconFully:setTopBottom( true, false, 2, 16 )
				self.equippedIconFully:setAlpha( 1 )
				self.clipFinished( equippedIconFully, {} )
				UpgradableIconLock:completeAnimation()
				self.UpgradableIconLock:setAlpha( 0 )
				self.clipFinished( UpgradableIconLock, {} )
	...
	self:mergeStateConditions( {
		{
			stateName = "ItemBanned",
			condition = function ( menu, element, event )
				return IsCACItemBanned( menu, element, controller )
			end
		},
		{
			stateName = "Equipped",
			condition = function ( menu, element, event )
				return IsCACItemEquipped( menu, element, controller ) and not IsCACItemUpgraded( menu, element, controller )
			end
		},
		{
			stateName = "New",
		...
		{
			stateName = "EquippedFullyUpgraded",
			condition = function ( menu, element, event )
				return IsCACItemEquipped( menu, element, controller ) and IsCACItemUpgraded( menu, element, controller )
			end
		}
	} )
	...
	ConsumableLabel.id = "ConsumableLabel"
	self:registerEventHandler( "gain_focus", function ( element, event )
		if element.m_focusable and element.ConsumableLabel:processEvent( event ) then
			return true
		else
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( element )
		...
		element.HeaderLabel:close()
		element.RestrictedItemWarning:close()
		element.GridItemUpgradeIconTacticalRig:close()
		element.tokenUnlockable:close()
