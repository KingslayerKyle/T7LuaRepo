-- 79a8a3fb56e7f313d99572729b64f6cb
...
CoD.MediaManager_CustomGameListItem = InheritFrom( LUI.UIElement )
CoD.MediaManager_CustomGameListItem.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.MediaManager_CustomGameListItem )
	self.id = "MediaManager_CustomGameListItem"
	...
	gameTypeImage:setLeftRight( true, false, 0, 68 )
	gameTypeImage:setTopBottom( true, false, 1, 65 )
	gameTypeImage:linkToElementModel( self, "showBuyImage", true, function ( model )
		local showBuyImage = Engine.GetModelValue( model )
		if showBuyImage then
			gameTypeImage:setAlpha( Negate( showBuyImage ) )
		end
	end )
	...
	PlusImage:linkToElementModel( self, "showBuyImage", true, function ( model )
		local showBuyImage = Engine.GetModelValue( model )
		if showBuyImage then
