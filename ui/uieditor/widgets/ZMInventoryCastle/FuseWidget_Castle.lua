-- 98df311a8ca5282459a9cdc063be45b3
-- This hash is used for caching, delete to decompile the file again

require( "ui.uieditor.widgets.HUD.ZM_FX.ZmFx_Spark2" )

CoD.FuseWidget_Castle = InheritFrom( LUI.UIElement )
CoD.FuseWidget_Castle.new = function ( menu, controller )
	...
	self:setClass( CoD.FuseWidget_Castle )
	self.id = "FuseWidget_Castle"
	...
	FuseImage:setImage( RegisterImage( "uie_t7_icon_inventory_dlc_fuse" ) )
	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f34_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_SCOREBOARD_OPEN )
				if f34_local0 then
					if not ShowSprayerWidget( controller ) then
						f34_local0 = SprayerInUse( controller )
					else
						f34_local0 = false
					end
				end
		...
		},
		{
			stateName = "Found",
			condition = function ( menu, element, event )
				return ShowSprayerWidget( controller ) and SprayerInUse( controller )
			end
		},
		{
			stateName = "Missing",
