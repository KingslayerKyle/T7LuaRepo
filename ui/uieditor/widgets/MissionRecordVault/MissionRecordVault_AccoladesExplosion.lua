-- 6143c5cb488fd344af8e5a2177de3ae0
-- This hash is used for caching, delete to decompile the file again

CoD.MissionRecordVault_AccoladesExplosion = InheritFrom( LUI.UIElement )
CoD.MissionRecordVault_AccoladesExplosion.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.MissionRecordVault_AccoladesExplosion )
	self.id = "MissionRecordVault_AccoladesExplosion"
	...
	image:setRGB( 1, 0.98, 0.47 )
	image:setImage( RegisterImage( "uie_explosion" ) )
	self:addElement( image )
	self.image = image
	
	local centerText = LUI.UIText.new()
	centerText:setLeftRight( false, true, -35.5, -10.5 )
	centerText:setTopBottom( true, false, 2, 32 )
	centerText:setRGB( 0.96, 1, 0 )
