-- 9d08917c8a32aa0ddc4b61924373ddc1
-- This hash is used for caching, delete to decompile the file again

CoD.MissionRecordVault_AccoladesTrickShot = InheritFrom( LUI.UIElement )
CoD.MissionRecordVault_AccoladesTrickShot.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.MissionRecordVault_AccoladesTrickShot )
	self.id = "MissionRecordVault_AccoladesTrickShot"
	...
	image:setRGB( 1, 0.98, 0.62 )
	image:setImage( RegisterImage( "uie_trickshot" ) )
	self:addElement( image )
	self.image = image
	
	local centerText = LUI.UIText.new()
	centerText:setLeftRight( true, true, 0, 0 )
	centerText:setTopBottom( false, false, -7, 12 )
