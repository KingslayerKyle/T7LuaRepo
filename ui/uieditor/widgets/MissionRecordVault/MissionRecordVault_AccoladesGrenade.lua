-- d35822aba7feabdbde5d44a2656a3451
-- This hash is used for caching, delete to decompile the file again

CoD.MissionRecordVault_AccoladesGrenade = InheritFrom( LUI.UIElement )
CoD.MissionRecordVault_AccoladesGrenade.new = function ( menu, controller )
	local self = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc( self, controller )
	end
	self:setUseStencil( false )
	self:setClass( CoD.MissionRecordVault_AccoladesGrenade )
	self.id = "MissionRecordVault_AccoladesGrenade"
	...
	image:setImage( RegisterImage( "uie_grenade" ) )
