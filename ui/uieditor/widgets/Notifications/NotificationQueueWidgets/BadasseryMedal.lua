-- 8aabe31ac820f2a1c066febf510b2934
...
CoD.BadasseryMedal.new = function ( menu, controller )
	...
	SubText:setTopBottom( true, false, 145, 187 )
	...
	Notification2xpReward:setLeftRight( true, false, 40, 88 )
	Notification2xpReward:setTopBottom( true, false, 86, 134 )
	Notification2xpReward:setAlpha( 0 )
	Notification2xpReward:setScale( 0.9 )
	...
	self.clipsPerState = {
		DefaultState = {
			...
			Combined = function ()
				...
				local SubTextFrame2 = function ( SubText, event )
					...
					SubText:setLeftRight( false, false, -640, 640 )
					SubText:setTopBottom( true, false, 147, 177 )
					if event.interrupted then
						self.clipFinished( SubText, event )
					else
						SubText:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
				...
				self.SubText:setTopBottom( true, false, 146, 187 )
				SubTextFrame2( SubText, {} )
				local FlashFrame2 = function ( Flash, event )
					if not event.interrupted then
		...
		DoubleXP = {
			DefaultClip = function ()
				...
				local Notification2xpRewardFrame2 = function ( Notification2xpReward, event )
					local Notification2xpRewardFrame3 = function ( Notification2xpReward, event )
						if not event.interrupted then
							Notification2xpReward:beginAnimation( "keyframe", 129, false, true, CoD.TweenType.Linear )
						end
					...
					if event.interrupted then
						...
						Notification2xpReward:beginAnimation( "keyframe", 129, false, false, CoD.TweenType.Linear )
			...
			Combined = function ()
				...
				local Notification2xpRewardFrame2 = function ( Notification2xpReward, event )
					if not event.interrupted then
						Notification2xpReward:beginAnimation( "keyframe", 349, false, true, CoD.TweenType.Linear )
					end
					Notification2xpReward:setAlpha( 1 )
					Notification2xpReward:setScale( 1 )
					if event.interrupted then
						self.clipFinished( Notification2xpReward, event )
					else
						Notification2xpReward:registerEventHandler( "transition_complete_keyframe", self.clipFinished )
				...
				Notification2xpReward:completeAnimation()
				self.Notification2xpReward:setAlpha( 1 )
				self.Notification2xpReward:setScale( 1.05 )
				Notification2xpRewardFrame2( Notification2xpReward, {} )
			end
		}
	}
