-- f3173ac55b9a14bb84c86956139f9936
...
require( "ui.uieditor.widgets.Safehouse.SafeHouse_ListButtonLabel" )
require( "ui.uieditor.widgets.CAC.cac_lock" )

CoD.medalCaseListItem = InheritFrom( LUI.UIElement )
CoD.medalCaseListItem.new = function ( menu, controller )
	...
	self:setClass( CoD.medalCaseListItem )
	self.id = "medalCaseListItem"
	self.soundSet = "none"
	...
	image:setLeftRight( false, false, -57, 57 )
	image:setTopBottom( false, false, -57, 57 )
	image:setupUIStreamedImage( 0 )
	image:linkToElementModel( self, "smallIcon", true, function ( model )
		local smallIcon = Engine.GetModelValue( model )
		if smallIcon then
			image:setImage( RegisterImage( smallIcon ) )
		end
	end )
	...
	local SafeHouseListButtonLabel0 = CoD.SafeHouse_ListButtonLabel.new( menu, controller )
	SafeHouseListButtonLabel0:setLeftRight( true, false, 4, 174 )
	SafeHouseListButtonLabel0:setTopBottom( false, false, 39.5, 59.5 )
	SafeHouseListButtonLabel0:linkToElementModel( self, "name", true, function ( model )
		local name = Engine.GetModelValue( model )
		if name then
			SafeHouseListButtonLabel0.itemName:setText( Engine.Localize( name ) )
		end
	end )
	self:addElement( SafeHouseListButtonLabel0 )
	self.SafeHouseListButtonLabel0 = SafeHouseListButtonLabel0
	
	local caclock0 = CoD.cac_lock.new( menu, controller )
	caclock0:setLeftRight( true, false, 74, 106 )
	caclock0:setTopBottom( true, false, 46.5, 78.5 )
	...
	self.clipsPerState = {
		...
		NotUnlocked = {
			DefaultClip = function ()
				...
				caclock0:completeAnimation()
				self.caclock0:setLeftRight( true, false, 74, 106 )
				self.caclock0:setTopBottom( true, false, 46.5, 78.5 )
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( element )
		...
		element.SafeHouseListButtonLabel0:close()
