-- 8b6fd553d91096d05a93d6762440cd9f
...
CoD.GroupsFeaturedItemLarge.new = function ( menu, controller )
	...
	WCFileshareIconExtraCamRender0:setLeftRight( true, false, -4.5, 647.5 )
	WCFileshareIconExtraCamRender0:setTopBottom( false, true, -461.25, 83.75 )
	...
	local Screenshot = LUI.UIElement.new()
	Screenshot:setLeftRight( false, false, -321.5, 321.5 )
	Screenshot:setTopBottom( false, false, -174, 173 )
	Screenshot:linkToElementModel( self, "fileId", true, function ( model )
		local fileId = Engine.GetModelValue( model )
		if fileId then
			Screenshot:setupImageViewer( GetImageViewerParams( "UI_SCREENSHOT_TYPE_SCREENSHOT", fileId ) )
		end
	end )
	...
	GameTypeImage:setLeftRight( false, false, -118.5, 125.5 )
	GameTypeImage:setTopBottom( false, false, -135.75, 108.25 )
	...
	TimeText:setLeftRight( false, true, -206.13, -5 )
	...
	ErrorImage:setLeftRight( false, false, -105.38, 115.38 )
	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				...
				if not IsSelfModelValueTrue( element, controller, "isValid" ) then
					f48_local0 = FileshareIsElementNormalSizeIcon( self, element, controller )
				else
					f48_local0 = false
				end
				return f48_local0
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f49_local0
				if not IsSelfModelValueTrue( element, controller, "isValid" ) then
					f49_local0 = not FileshareIsElementNormalSizeIcon( self, element, controller )
				else
					f49_local0 = false
				end
				return f49_local0
		...
		},
		{
			...
			condition = function ( menu, element, event )
				...
				if not FileshareIsElementNormalSizeIcon( self, element, controller ) and not IsFileshareCategoryFilm( self, element, controller ) and not IsFileshareCategoryClip( self, element, controller ) then
					f50_local0 = not IsFileshareCategoryScreenshot( self, element, controller )
				else
					f50_local0 = false
				end
				return f50_local0
		...
		},
		{
			...
			condition = function ( menu, element, event )
				...
				if not FileshareIsElementNormalSizeIcon( self, element, controller ) then
					f51_local0 = IsFileshareCategoryFilm( self, element, controller )
				else
					f51_local0 = false
				end
				return f51_local0
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f52_local0
				if not FileshareIsElementNormalSizeIcon( self, element, controller ) then
					f52_local0 = IsFileshareCategoryClip( self, element, controller )
				else
					f52_local0 = false
				end
				return f52_local0
		...
		},
		{
			...
			condition = function ( menu, element, event )
				local f53_local0
				if not FileshareIsElementNormalSizeIcon( self, element, controller ) then
					f53_local0 = IsFileshareCategoryScreenshot( self, element, controller )
				else
					f53_local0 = false
				end
				return f53_local0
