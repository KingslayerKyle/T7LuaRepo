-- 4c5fd8a871ed0b4ec4f4463c278b0eee
...
CoD.GroupsSummaryForSearch.new = function ( menu, controller )
	...
	local GroupsSummaryStatusWithJoinApproval = CoD.GroupsSummaryStatusWithJoinApproval.new( menu, controller )
	GroupsSummaryStatusWithJoinApproval:setLeftRight( true, false, 19, 339 )
	GroupsSummaryStatusWithJoinApproval:setTopBottom( true, false, 253.5, 273.5 )
	GroupsSummaryStatusWithJoinApproval:setAlpha( 0 )
	GroupsSummaryStatusWithJoinApproval:linkToElementModel( self, nil, false, function ( model )
		GroupsSummaryStatusWithJoinApproval:setModel( model, controller )
	end )
	self:addElement( GroupsSummaryStatusWithJoinApproval )
	self.GroupsSummaryStatusWithJoinApproval = GroupsSummaryStatusWithJoinApproval
	
	local buttonprompt = CoD.buttonprompt.new( menu, controller )
	buttonprompt:setLeftRight( true, false, 0, 214 )
	buttonprompt:setTopBottom( true, false, 460, 491 )
	buttonprompt:setAlpha( 0 )
	buttonprompt.label:setText( Engine.Localize( "GROUPS_VIEW_GROUP_FRIENDS" ) )
	...
	self:mergeStateConditions( {
		...
		},
		{
			...
			condition = function ( menu, element, event )
				return not IsModelValueTrue( controller, "groups.skipViewFriendsInGroup" )
			end
		}
	} )
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "groups.skipViewFriendsInGroup" ), function ( model )
		menu:updateElementState( self, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( element )
		...
		element.GroupsSummaryStatusWithJoinApproval:close()
