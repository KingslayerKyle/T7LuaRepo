-- 336dc5d1a7c71df33a005c3a5ceb6211
...
DataSources.RoadToCWLRewardList = {
	getModel = function ( f1_arg0 )
		local f1_local0 = Engine.CreateModel( Engine.GetGlobalModel(), "RoadToCWLRewardList" )
		local f1_local1 = 0
		for f1_local5, f1_local6 in ipairs( Engine.GetBackgroundsForCategoryName( f1_arg0, "default" ) ) do
			local f1_local7 = Engine.Localize( f1_local6.description .. "_TITLE" )
			local f1_local8 = ""
			if f1_local1 == 3 then
				break
			elseif f1_local6.isContractBg and IsLive() then
				ListHelper_CreateModelsFromTable( Engine.CreateModel( f1_local0, "tier" .. f1_local1 + 1 ), {
					title = f1_local7,
					description = f1_local8,
					iconId = f1_local6.id,
...
CoD.Promo_ThermometerAndRewards.new = function ( menu, controller )
	...
	LargeGlow:setTopBottom( true, false, -96.67, 469 )
	...
	Chip:setImage( RegisterImage( "uie_t7_blackmarket_promo_meter_logochip" ) )
	...
	RewardWidget0:linkToElementModel( self, "tier1", false, function ( model )
		RewardWidget0:setModel( model, controller )
	end )
	RewardWidget0:linkToElementModel( self, "tier1", false, function ( model )
		RewardWidget0.PromoRewardImage.CallingCardsFrameWidget:setModel( model, controller )
	end )
	RewardWidget0:linkToElementModel( self, "tier1.title", true, function ( model )
		local tier1Title = Engine.GetModelValue( model )
		if tier1Title then
			RewardWidget0.TextBox0:setText( Engine.Localize( tier1Title ) )
		end
	end )
	...
	RewardWidget2:linkToElementModel( self, "tier3.title", true, function ( model )
		local tier3Title = Engine.GetModelValue( model )
		if tier3Title then
			RewardWidget2.TextBox0:setText( Engine.Localize( tier3Title ) )
		end
	end )
	...
	RewardWidget3.PromoRewardImage.Image:setImage( RegisterImage( "uie_t7_icon_community_reward_boxing" ) )
	RewardWidget3.TextBox0:setText( Engine.Localize( AppendAndLocalizeString( "_TITLE", "WEAPON_MELEE_BOXING" ) ) )
	...
	RewardWidget1:linkToElementModel( self, "tier2", false, function ( model )
		RewardWidget1:setModel( model, controller )
	end )
	RewardWidget1:linkToElementModel( self, "tier2", false, function ( model )
		RewardWidget1.PromoRewardImage.CallingCardsFrameWidget:setModel( model, controller )
	end )
	RewardWidget1:linkToElementModel( self, "tier2.title", true, function ( model )
		local tier2Title = Engine.GetModelValue( model )
		if tier2Title then
			RewardWidget1.TextBox0:setText( Engine.Localize( tier2Title ) )
		end
	end )
