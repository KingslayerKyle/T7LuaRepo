-- b745db68c75394123519d05e397bcf8c
...
local f0_local0 = function ( f1_arg0, f1_arg1, f1_arg2 )
	...
	f1_local0.contested = f1_local1 > 1
	local f1_local4 = CoD.IsShoutcaster( f1_arg1 )
	local f1_local5 = Engine.CreateModel( Engine.GetModelForController( f1_arg1 ), "Hardpoint" )
	local f1_local6 = Engine.GetModel( f1_local5, "state" )
	local f1_local7 = Engine.GetModel( f1_local5, "objId" )
	local f1_local8 = Engine.GetModel( f1_local5, "team" )
	if Engine.GetModelValue( f1_local7 ) ~= f1_arg2 then
		Engine.SetModelValue( f1_local7, f1_arg2 )
	end
	if Engine.GetModelValue( f1_local8 ) ~= f1_local2 then
		Engine.SetModelValue( f1_local8, f1_local2 )
	end
	if f1_local2 == Enum.team_t.TEAM_FREE or f1_local2 == Enum.team_t.TEAM_NEUTRAL then
		...
		if f1_local0.contested then
			Engine.SetModelValue( f1_local6, "contested" )
		else
			Engine.SetModelValue( f1_local6, "neutral" )
		end
	elseif f1_local3:isOwnedByMyTeam( f1_arg1 ) then
		f1_local0.imagename = f1_arg0.waypoint_image_defend
		f1_local0.imagecolor = CoD.GetColorSetFriendlyColor( f1_arg1, f1_local3:getTeam( f1_arg1 ) )
		if f1_local0.contested then
			...
			if f1_local4 then
				Engine.SetModelValue( f1_local6, "anyteam" )
			else
				Engine.SetModelValue( f1_local6, "friendly" )
			end
		end
	else
		f1_local0.imagename = f1_arg0.waypoint_image_capture
		f1_local0.imagecolor = CoD.GetColorSetEnemyColor( f1_arg1, f1_local3:getTeam( f1_arg1 ) )
		if f1_local0.contested then
			f1_local0.waypointType = f1_arg0.waypoint_label_contested
			f1_local0.imagecolor = ColorSet.WaypointNeutral
			Engine.SetModelValue( f1_local6, "contested" )
			...
			if f1_local4 then
				...
				Engine.SetModelValue( f1_local6, "enemy" )
			end
		end
	end
	return f1_local0
...
local f0_local1 = function ( f2_arg0, f2_arg1 )
	...
	if f2_arg0.visible == true then
		...
		if CoD.IsShoutcaster( f2_local2 ) and f2_local3.contested then
			f2_local0:setCompassObjectiveIcon( f2_local2, f2_local1, "t7_hud_waypoints_contested_koth", f2_local3.imagecolor )
		else
