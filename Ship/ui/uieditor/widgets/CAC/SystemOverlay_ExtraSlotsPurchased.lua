-- 5616fb0217ef7a78c263730c27b1d3d7
...
CoD.SystemOverlay_ExtraSlotsPurchased.new = function ( menu, controller )
	...
	bg:setTopBottom( true, false, 0, 336 )
	bg:setAlpha( 0 )
	...
	largeImage:setLeftRight( true, false, 75, 291 )
	largeImage:setTopBottom( true, false, -14, 202 )
	...
	purchaseCompleteDesc:setLeftRight( true, false, 421, 920.6 )
	purchaseCompleteDesc:setTopBottom( true, false, 98, 123 )
	...
	self:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				return IsSelfModelValueEqualTo( element, controller, "listDatasource", "PurchaseInventoryItemCompleteMenuList" )
			end
		}
	} )
	self:linkToElementModel( self, "listDatasource", true, function ( model )
		menu:updateElementState( self, {
			name = "model_validation",
			menu = menu,
			modelValue = Engine.GetModelValue( model ),
			modelName = "listDatasource"
		} )
	end )
	...
	menu:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, nil, function ( f25_arg0, f25_arg1, f25_arg2, f25_arg3 )
		if not IsMenuModelValueEqualTo( f25_arg1, "title", "MPUI_BM_NO_CURRENCY" ) and AreCodPointsEnabled( f25_arg2 ) and not IsPC() then
			GoBackAndOpenStoreCodPointsOverlayOnParent( self, "ExtraSlotsCPStore", f25_arg2 )
			return true
		else
			
		end
	end, function ( f26_arg0, f26_arg1, f26_arg2 )
		if not IsMenuModelValueEqualTo( f26_arg1, "title", "MPUI_BM_NO_CURRENCY" ) and AreCodPointsEnabled( f26_arg2 ) and not IsPC() then
			CoD.Menu.SetButtonLabel( f26_arg1, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, "MPUI_PURCHASE_CODPOINTS" )
			return true
		else
			return false
		end
