-- 963ad168e838e4697c2184341699e2e0
...
LUI.createMenu.T7Hud_CP = function ( controller )
	...
	local PanelVignetteRight = CoD.MapWidget_PanelVignetteRight.new( f10_local1, controller )
	...
	local DamageWidgetContainer = CoD.DamageWidgetContainer.new( f10_local1, controller )
	...
	local AmmoWidgetContainer = CoD.AmmoWidgetContainer.new( f10_local1, controller )
	...
	local EMPWeaponInfo = CoD.AmmoWidgetEMP.new( f10_local1, controller )
	...
	local EMPScoreInfo = CoD.DamageWidgetEMP.new( f10_local1, controller )
	...
	local CommsSystemWidget = CoD.CommsSystemWidget.new( f10_local1, controller )
	...
	local fullscreenContainer = CoD.DynamicContainerWidget.new( f10_local1, controller )
	...
	local PartyListContainerParent0 = CoD.PartyListContainerParent.new( f10_local1, controller )
	...
	local RepulsorIndicator = CoD.RepulsorIndicator.new( f10_local1, controller )
	...
	RepulsorIndicator:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "playerAbilities.repulsorIndicatorIntensity" ), function ( model )
		f10_local1:updateElementState( RepulsorIndicator, {
	...
	local ProximityIndicator = CoD.RepulsorIndicator.new( f10_local1, controller )
	...
	ProximityIndicator:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "playerAbilities.proximityIndicatorIntensity" ), function ( model )
		f10_local1:updateElementState( ProximityIndicator, {
			name = "model_validation",
			menu = f10_local1,
			modelValue = Engine.GetModelValue( model ),
	...
	local ScoreboardWidgetCP = CoD.ScoreboardWidgetCP.new( f10_local1, controller )
	...
	local IngameChatClientContainer = CoD.IngameChatClientContainer.new( f10_local1, controller )
