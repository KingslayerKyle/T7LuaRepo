-- 5f024b2ed99d0e0569b6c24bba488f60
...
LUI.createMenu.Challenges_NearCompletion = function ( controller )
	local self = CoD.Menu.NewForUIEditor( "Challenges_NearCompletion" )
	if PreLoadFunc then
	...
	self.buttonModel = Engine.CreateModel( Engine.GetModelForController( controller ), "Challenges_NearCompletion.buttonPrompts" )
	...
	local NearCompletionFrame = CoD.Challenges_FrameWidget_NearCompletion.new( f1_local1, controller )
	NearCompletionFrame:setLeftRight( false, false, -574, 576 )
	NearCompletionFrame:setTopBottom( false, false, -236, 285 )
	...
	MenuFrame.titleLabel:setText( Engine.Localize( "MENU_NEAR_COMPLETION_CAPS" ) )
	MenuFrame.cac3dTitleIntermediary0.FE3dTitleContainer0.MenuTitle.TextBox1.Label0:setText( Engine.Localize( "MENU_NEAR_COMPLETION_CAPS" ) )
	...
	if not self:restoreState() then
		self.NearCompletionFrame:processEvent( {
			name = "gain_focus",
			controller = controller
		} )
	end
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( element )
		...
		element.NearCompletionFrame:close()
		element.MenuFrame:close()
		element.Tabs:close()
		element.FEMenuLeftGraphics:close()
		Engine.UnsubscribeAndFreeModel( Engine.GetModel( Engine.GetModelForController( controller ), "Challenges_NearCompletion.buttonPrompts" ) )
	end )
