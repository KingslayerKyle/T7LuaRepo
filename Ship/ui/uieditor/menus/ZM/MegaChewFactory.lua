-- 51223e4b18984cbf897ce80b08e0c962
...
local f0_local3 = function ( f63_arg0, f63_arg1 )
	f63_arg0:addElement( LUI.UITimer.newElementTimer( 100, true, function ()
		Engine.SetupUI3DWindow( f63_arg1, 0, 360, 135 )
		Engine.SetupUI3DWindow( f63_arg1, 1, 360, 135 )
		Engine.SetupUI3DWindow( f63_arg1, 2, 360, 135 )
		Engine.SetupUI3DWindow( f63_arg1, 3, 480, 90 )
	end ) )
end

local PostLoadFunc = function ( f65_arg0, f65_arg1 )
	local f65_local0 = Engine.CreateModel( Engine.GetModelForController( f65_arg1 ), "MegaChewFactory" )
	local f65_local1 = Engine.GetPlayerStats( f65_arg1, CoD.STATS_LOCATION_NORMAL, Enum.eModes.MODE_ZOMBIES )
	f65_local1.showMegaChewFactoryBreadcrumb:set( 0 )
	local f65_local2 = Engine.CreateModel( Engine.GetGlobalModel(), "MegaChewFactoryVialDisplay" )
	local f65_local3 = f65_arg0.Label0
	local f65_local4 = f65_local3
	f65_local3 = f65_local3.subscribeToGlobalModel
	local f65_local5 = f65_arg1
	local f65_local6 = "MegaChewTokens"
	f65_local3( f65_local4, f65_local5, f65_local6, "remainingTokens", function ( f66_arg0 )
		local modelValue = Engine.GetModelValue( f66_arg0 )
		if modelValue then
			Engine.SetModelValue( f65_local2, modelValue )
			Engine.SendClientScriptNotify( f65_arg1, "mega_chew_remaining_tokens", modelValue )
		end
	end )
	f65_local3 = 3000
	f65_local4 = 32
	f65_local5 = 0
	f65_local6 = function ()
		if Engine.LootResultsReady( f65_arg1 ) then
			local f67_local0 = Engine.LootFailureReason( f65_arg1 )
			if f67_local0 == Enum.LootResultType.LOOT_RESULT_SUCCESS then
				local f67_local1 = Engine.GetLootResults( f65_arg1 )
				Engine.SetModelValue( Engine.CreateModel( Engine.CreateModel( Engine.GetModelForController( f65_arg1 ), "MegaChewTokens" ), "remainingTokens" ), Engine.GetZMVials( f65_arg1 ) )
				f0_local2( f65_arg1, true )
				Engine.SendClientScriptNotify( f65_arg1, "mega_chew_results", true, f67_local1[1], f67_local1[2], f67_local1[3] )
				for f67_local2 = 4, 6, 1 do
					local f67_local5 = tonumber( Engine.TableLookup( nil, "gamedata/stats/zm/zm_statsTable.csv", 4, f67_local1[f67_local2], 0 ) )
					if f67_local5 then
						f65_local1.itemStats[f67_local5].markedOld:set( 0 )
					end
				end
			else
				Engine.PlaySound( "uin_bm_denied" )
				Engine.SendClientScriptNotify( f65_arg1, "mega_chew_results", false, "", "", "" )
				f0_local2( f65_arg1, false )
				LuaUtils.UI_ShowErrorMessageDialog( f65_arg1, f0_local1( f67_local0 ) )
			end
		else
			f65_local5 = f65_local5 + 1
			if f65_local5 < f65_local4 then
				f65_arg0:addElement( LUI.UITimer.newElementTimer( 250, true, f65_local6 ) )
			else
				Engine.SendClientScriptNotify( f65_arg1, "mega_chew_results", false )
			end
		end
	...
	local f65_local7 = Engine.CreateModel( f65_local0, "getResultsOrKeepWaiting" )
	f65_arg0:subscribeToModel( f65_local7, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue and modelValue ~= 0 then
			f65_local5 = 0
			f65_arg0:addElement( LUI.UITimer.newElementTimer( f65_local3, true, f65_local6 ) )
		end
		Engine.SetModelValue( model, 0 )
	end )
	local f65_local8 = Engine.CreateModel( f65_local0, "queryLoot" )
	local f65_local9 = Engine.CreateModel( f65_local0, "lootQueryResult" )
	Engine.SetModelValue( f65_local8, 0 )
	f65_arg0:subscribeToModel( f65_local8, function ( model )
		local modelValue = Engine.GetModelValue( model )
		if not modelValue or modelValue == 0 then
			return 
		else
			CoD.perController[f65_arg1].MegaChewFactory_PendingLootRewardTask = {}
			CoD.perController[f65_arg1].MegaChewFactory_PendingLootRewardTask.vialsSpent = modelValue
			CoD.perController[f65_arg1].MegaChewFactory_PendingLootRewardTask.vialsBefore = Engine.GetZMVials( f65_arg1 )
			if not Engine.SpendZMVials( f65_arg1, modelValue ) then
				PlaySoundAlias( "uin_bm_denied" )
				Engine.SetModelValue( f65_local8, 0 )
				Engine.SetModelValue( f65_local9, 0 )
				Engine.SendClientScriptNotify( f65_arg1, "loot_query_result_ready" )
				f0_local2( f65_arg1, false )
				LuaUtils.UI_ShowErrorMessageDialog( f65_arg1, Engine.Localize( "MENU_BGB_FACTORY_ERR_BAD_RESPONSE" ) )
				return 
			else
				Engine.SetModelValue( f65_local7, 1 )
				Engine.SetModelValue( f65_local9, 1 )
				Engine.SendClientScriptNotify( f65_arg1, "loot_query_result_ready" )
				Engine.SetModelValue( f65_local8, 0 )
			end
		end
	end, false )
	f65_arg0:registerEventHandler( "bm_autoevents_tick", function ( element, event )
		Engine.SetDvar( "live_autoEventPumpTime", 0 )
	end )
	f65_arg0:addElement( LUI.UITimer.new( 100, "bm_autoevents_tick", false, f65_arg0 ) )
	if CoD.isPC then
		f0_local3( f65_arg0, f65_arg1 )
	end
end
...
LUI.createMenu.MegaChewFactory = function ( controller )
	...
	local f71_local1 = self
	self.anyChildUsesUpdateState = true
	
	local GenericMenuFrame0 = CoD.GenericMenuFrame.new( f71_local1, controller )
	...
	local MegaChewMachine0 = CoD.MegaChewMachine.new( f71_local1, controller )
	...
	MegaChewMachine0:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "MegaChewFactory.disableInput" ), function ( model )
		local f73_local0 = MegaChewMachine0
		local f73_local1 = {
		...
		CoD.Menu.UpdateButtonShownState( f73_local0, f71_local1, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS )
	end )
	MegaChewMachine0:registerEventHandler( "gain_focus", function ( element, event )
		...
		CoD.Menu.UpdateButtonShownState( element, f71_local1, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS )
		return f74_local0
	end )
	MegaChewMachine0:registerEventHandler( "lose_focus", function ( element, event )
		local f75_local0 = nil
		if element.loseFocus then
			f75_local0 = element:loseFocus( event )
		elseif element.super.loseFocus then
			f75_local0 = element.super:loseFocus( event )
		end
		return f75_local0
	end )
	f71_local1:AddButtonCallbackFunction( MegaChewMachine0, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( f76_arg0, f76_arg1, f76_arg2, f76_arg3 )
		if HasBGBTokensRemaining( f76_arg0, f76_arg2 ) and IsModelValueEqualTo( f76_arg2, "MegaChewFactory.disableInput", 0 ) then
			MegaChewFactoryPurchase( self, f76_arg0, f76_arg2 )
			return true
		elseif IsModelValueEqualTo( f76_arg2, "MegaChewFactory.disableInput", 0 ) then
			OpenSystemOverlay( self, f76_arg1, f76_arg2, "NotEnoughVials", "" )
			return true
		else
			
		end
	end, function ( f77_arg0, f77_arg1, f77_arg2 )
		if HasBGBTokensRemaining( f77_arg0, f77_arg2 ) and IsModelValueEqualTo( f77_arg2, "MegaChewFactory.disableInput", 0 ) then
			CoD.Menu.SetButtonLabel( f77_arg1, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "MENU_SELECT" )
			return true
		elseif IsModelValueEqualTo( f77_arg2, "MegaChewFactory.disableInput", 0 ) then
			CoD.Menu.SetButtonLabel( f77_arg1, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "MENU_SELECT" )
			return true
		else
			return false
	...
	local MegaChewMachine1 = CoD.MegaChewMachine.new( f71_local1, controller )
	...
	MegaChewMachine1:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "MegaChewFactory.disableInput" ), function ( model )
		local f79_local0 = MegaChewMachine1
		local f79_local1 = {
			controller = controller,
			name = "model_validation",
			modelValue = Engine.GetModelValue( model ),
		...
		CoD.Menu.UpdateButtonShownState( f79_local0, f71_local1, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS )
	end )
	...
	MegaChewMachine1:registerEventHandler( "lose_focus", function ( element, event )
		...
		if element.loseFocus then
			f81_local0 = element:loseFocus( event )
		elseif element.super.loseFocus then
			f81_local0 = element.super:loseFocus( event )
		end
		return f81_local0
	end )
	f71_local1:AddButtonCallbackFunction( MegaChewMachine1, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( f82_arg0, f82_arg1, f82_arg2, f82_arg3 )
		if HasBGBTokensRemaining( f82_arg0, f82_arg2 ) and IsModelValueEqualTo( f82_arg2, "MegaChewFactory.disableInput", 0 ) then
			MegaChewFactoryPurchase( self, f82_arg0, f82_arg2 )
			return true
		elseif IsModelValueEqualTo( f82_arg2, "MegaChewFactory.disableInput", 0 ) then
			OpenSystemOverlay( self, f82_arg1, f82_arg2, "NotEnoughVials", "" )
			return true
		else
			
		end
	end, function ( f83_arg0, f83_arg1, f83_arg2 )
		if HasBGBTokensRemaining( f83_arg0, f83_arg2 ) and IsModelValueEqualTo( f83_arg2, "MegaChewFactory.disableInput", 0 ) then
			CoD.Menu.SetButtonLabel( f83_arg1, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "MENU_SELECT" )
			return true
		elseif IsModelValueEqualTo( f83_arg2, "MegaChewFactory.disableInput", 0 ) then
			CoD.Menu.SetButtonLabel( f83_arg1, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "MENU_SELECT" )
			return true
		else
			return false
	...
	local MegaChewMachine2 = CoD.MegaChewMachine.new( f71_local1, controller )
	...
	MegaChewMachine2:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "MegaChewFactory.disableInput" ), function ( model )
		local f85_local0 = MegaChewMachine2
		local f85_local1 = {
			controller = controller,
			name = "model_validation",
			modelValue = Engine.GetModelValue( model ),
		...
		CoD.Menu.UpdateButtonShownState( f85_local0, f71_local1, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS )
	end )
	...
	MegaChewMachine2:registerEventHandler( "lose_focus", function ( element, event )
		...
		if element.loseFocus then
			f87_local0 = element:loseFocus( event )
		elseif element.super.loseFocus then
			f87_local0 = element.super:loseFocus( event )
		end
		return f87_local0
	end )
	f71_local1:AddButtonCallbackFunction( MegaChewMachine2, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( f88_arg0, f88_arg1, f88_arg2, f88_arg3 )
		if HasBGBTokensRemaining( f88_arg0, f88_arg2 ) and IsModelValueEqualTo( f88_arg2, "MegaChewFactory.disableInput", 0 ) then
			MegaChewFactoryPurchase( self, f88_arg0, f88_arg2 )
			return true
		elseif IsModelValueEqualTo( f88_arg2, "MegaChewFactory.disableInput", 0 ) then
			OpenSystemOverlay( self, f88_arg1, f88_arg2, "NotEnoughVials", "" )
			return true
		else
			
		end
	end, function ( f89_arg0, f89_arg1, f89_arg2 )
		if HasBGBTokensRemaining( f89_arg0, f89_arg2 ) and IsModelValueEqualTo( f89_arg2, "MegaChewFactory.disableInput", 0 ) then
			CoD.Menu.SetButtonLabel( f89_arg1, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "MENU_SELECT" )
			return true
		elseif IsModelValueEqualTo( f89_arg2, "MegaChewFactory.disableInput", 0 ) then
			CoD.Menu.SetButtonLabel( f89_arg1, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "MENU_SELECT" )
			return true
		else
			return false
	...
	local console = CoD.CenterConsole.new( f71_local1, controller )
	console:setLeftRight( false, false, -370, 370 )
	console:setTopBottom( true, false, 123, 221 )
	self:addElement( console )
	self.console = console
	
	local MegaChewFactoryVatLeft = CoD.MegaChewFactoryVat.new( f71_local1, controller )
	...
	local MegaChewFactoryVatMiddle = CoD.MegaChewFactoryVat.new( f71_local1, controller )
	...
	local MegaChewFactoryVatRight = CoD.MegaChewFactoryVat.new( f71_local1, controller )
	...
	local MegaChewFactoryBottomLeftLabel = CoD.MegaChewFactoryBottomLeftLabel.new( f71_local1, controller )
	...
	local MegaChewFactoryBuyDivinium0 = CoD.MegaChewFactoryBuyDivinium.new( f71_local1, controller )
	...
	MegaChewFactoryBuyDivinium0.clickButton.label:setText( Engine.Localize( "ZMUI_BUY_LIQUID_DIVINIUM" ) )
	MegaChewFactoryBuyDivinium0.clickButton.keyshortcut:setText( Engine.Localize( "[^3B^7]" ) )
	MegaChewFactoryBuyDivinium0:mergeStateConditions( {
		...
		},
		{
			stateName = "VisiblePC",
			condition = function ( menu, element, event )
				local f92_local0 = IsModelValueEqualTo( controller, "MegaChewFactory.disableInput", 0 )
				if f92_local0 then
					f92_local0 = AreCodPointsEnabled( controller )
					if f92_local0 then
						f92_local0 = not IsGamepad( controller )
					end
				end
				return f92_local0
			end
	...
	MegaChewFactoryBuyDivinium0:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "MegaChewFactory.disableInput" ), function ( model )
		f71_local1:updateElementState( MegaChewFactoryBuyDivinium0, {
			name = "model_validation",
			menu = f71_local1,
			modelValue = Engine.GetModelValue( model ),
			modelName = "MegaChewFactory.disableInput"
	...
	if MegaChewFactoryBuyDivinium0.m_eventHandlers.input_source_changed then
		local f71_local13 = MegaChewFactoryBuyDivinium0.m_eventHandlers.input_source_changed
		MegaChewFactoryBuyDivinium0:registerEventHandler( "input_source_changed", function ( element, event )
			event.menu = event.menu or f71_local1
			element:updateState( event )
			return f71_local13( element, event )
		end )
	else
		MegaChewFactoryBuyDivinium0:registerEventHandler( "input_source_changed", LUI.UIElement.updateState )
	end
	MegaChewFactoryBuyDivinium0:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "LastInput" ), function ( model )
		f71_local1:updateElementState( MegaChewFactoryBuyDivinium0, {
			name = "model_validation",
			menu = f71_local1,
			modelValue = Engine.GetModelValue( model ),
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "MegaChewFactory.disableInput" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f98_local0, f71_local1, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE )
		CoD.Menu.UpdateButtonShownState( f98_local0, f71_local1, controller, Enum.LUIButton.LUI_KEY_LSTICK_PRESSED )
		CoD.Menu.UpdateButtonShownState( f98_local0, f71_local1, controller, Enum.LUIButton.LUI_KEY_XBY_PSTRIANGLE )
		CoD.Menu.UpdateButtonShownState( f98_local0, f71_local1, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	self:registerEventHandler( "input_source_changed", function ( element, event )
		CoD.Menu.UpdateButtonShownState( element, f71_local1, controller, Enum.LUIButton.LUI_KEY_LSTICK_PRESSED )
	end )
	self:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "LastInput" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f100_local0, f71_local1, controller, Enum.LUIButton.LUI_KEY_LSTICK_PRESSED )
	end )
	self:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "lobbyRoot.lobbyNetworkMode" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f101_local0, f71_local1, controller, Enum.LUIButton.LUI_KEY_XBY_PSTRIANGLE )
		CoD.Menu.UpdateButtonShownState( f101_local0, f71_local1, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	self:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "CryptoKeyProgress.codPoints" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f102_local0, f71_local1, controller, Enum.LUIButton.LUI_KEY_XBY_PSTRIANGLE )
		CoD.Menu.UpdateButtonShownState( f102_local0, f71_local1, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE )
	end )
	CoD.Menu.AddNavigationHandler( f71_local1, self, controller )
	self:registerEventHandler( "menu_loaded", function ( element, event )
		local f103_local0 = nil
		SendClientScriptMenuChangeNotify( controller, f71_local1, true )
		if not f103_local0 then
			f103_local0 = element:dispatchEventToChildren( event )
		end
		return f103_local0
	end )
	self:registerEventHandler( "menu_opened", function ( element, event )
		...
		if IsPerControllerTablePropertyValue( controller, "cameFromFeaturedCard", true ) and not HasSeenMegaChewFactoryFirstTimePopup( controller ) then
			OpenSystemOverlay( self, f71_local1, controller, "MegaChewFactoryWelcomePopup", "" )
		else
			OpenBlackMarketIncentivePopup( self, controller )
		end
		if not f104_local0 then
			f104_local0 = element:dispatchEventToChildren( event )
		end
		return f104_local0
	end )
	f71_local1:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE, nil, function ( f105_arg0, f105_arg1, f105_arg2, f105_arg3 )
		if IsModelValueEqualTo( f105_arg2, "MegaChewFactory.disableInput", 0 ) then
			SendClientScriptMenuChangeNotify( f105_arg2, f105_arg1, false )
			GoBack( self, f105_arg2 )
			ClearMenuSavedState( f105_arg1 )
			UploadStats( self, f105_arg2 )
			Close( self, f105_arg2 )
			HandleGoBackFromMegaChew( self, f105_arg0, f105_arg2, "", f105_arg1 )
			return true
		else
			
		end
	end, function ( f106_arg0, f106_arg1, f106_arg2 )
		if IsModelValueEqualTo( f106_arg2, "MegaChewFactory.disableInput", 0 ) then
			CoD.Menu.SetButtonLabel( f106_arg1, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE, "MENU_BACK" )
			return true
		else
			return false
		end
	end, false )
	f71_local1:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_LSTICK_PRESSED, nil, function ( f107_arg0, f107_arg1, f107_arg2, f107_arg3 )
		if IsModelValueEqualTo( f107_arg2, "MegaChewFactory.disableInput", 0 ) and IsGamepad( f107_arg2 ) then
			return true
		else
			
		end
	end, function ( f108_arg0, f108_arg1, f108_arg2 )
		if IsModelValueEqualTo( f108_arg2, "MegaChewFactory.disableInput", 0 ) and IsGamepad( f108_arg2 ) then
			CoD.Menu.SetButtonLabel( f108_arg1, Enum.LUIButton.LUI_KEY_LSTICK_PRESSED, "MENU_NAVIGATE" )
			return true
		else
			return false
		end
	end, false )
	f71_local1:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBY_PSTRIANGLE, "B", function ( f109_arg0, f109_arg1, f109_arg2, f109_arg3 )
		if AreCodPointsEnabled( f109_arg2 ) and not IsInventoryBusy( f109_arg2 ) and IsModelValueEqualTo( f109_arg2, "MegaChewFactory.disableInput", 0 ) then
			OpenSystemOverlay( self, f109_arg1, f109_arg2, "PurchaseVials", "" )
			return true
		else
			
		end
	end, function ( f110_arg0, f110_arg1, f110_arg2 )
		if AreCodPointsEnabled( f110_arg2 ) and not IsInventoryBusy( f110_arg2 ) and IsModelValueEqualTo( f110_arg2, "MegaChewFactory.disableInput", 0 ) then
			CoD.Menu.SetButtonLabel( f110_arg1, Enum.LUIButton.LUI_KEY_XBY_PSTRIANGLE, "" )
			return false
		else
			return false
		end
	end, false )
	f71_local1:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, "P", function ( f111_arg0, f111_arg1, f111_arg2, f111_arg3 )
		if AreCodPointsEnabled( f111_arg2 ) and not IsInventoryBusy( f111_arg2 ) and IsModelValueEqualTo( f111_arg2, "MegaChewFactory.disableInput", 0 ) then
			OpenCodPointsStore( self, f111_arg0, f111_arg2, "MegaChewFactory", f111_arg1 )
			return true
		else
			
		end
	end, function ( f112_arg0, f112_arg1, f112_arg2 )
		if AreCodPointsEnabled( f112_arg2 ) and not IsInventoryBusy( f112_arg2 ) and IsModelValueEqualTo( f112_arg2, "MegaChewFactory.disableInput", 0 ) then
			CoD.Menu.SetButtonLabel( f112_arg1, Enum.LUIButton.LUI_KEY_XBX_PSSQUARE, "MPUI_PURCHASE_CODPOINTS" )
			return true
		else
			return false
		end
