-- cc7c05430db9c95703447f6a6ec29ba3
...
LUI.createMenu.GunsmithReticleSelect = function ( controller )
	...
	local GridItemButtonBackdropContainer3x4 = CoD.GridItemButtonBackdropContainer3x4.new( f4_local1, controller )
	...
	selectionList:linkToElementModel( selectionList, "isChallengeClassified", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f7_local0, f4_local1, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS )
	end )
	selectionList:registerEventHandler( "list_item_gain_focus", function ( element, event )
		local f8_local0 = nil
		if Gunsmith_IsReticleNew( f4_local1, element, controller ) and not IsSelfModelValueTrue( element, controller, "isBMClassified" ) and not IsSelfModelValueTrue( element, controller, "isChallengeClassified" ) then
			Gunsmith_SetWeaponOptionAsOld( f4_local1, element, controller )
	...
	f4_local1:AddButtonCallbackFunction( selectionList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( f11_arg0, f11_arg1, f11_arg2, f11_arg3 )
		if not IsGunsmithItemWeaponOptionLocked( f11_arg1, f11_arg0, f11_arg2 ) and not IsSelfModelValueTrue( f11_arg0, f11_arg2, "isBMClassified" ) and not IsSelfModelValueTrue( f11_arg0, f11_arg2, "isChallengeClassified" ) then
			Gunsmith_SelectReticle( self, f11_arg0, f11_arg2 )
		...
		if not IsGunsmithItemWeaponOptionLocked( f12_arg1, f12_arg0, f12_arg2 ) and not IsSelfModelValueTrue( f12_arg0, f12_arg2, "isBMClassified" ) and not IsSelfModelValueTrue( f12_arg0, f12_arg2, "isChallengeClassified" ) then
	...
	local GridItemButtonBackdropSymbols = CoD.GridItemButtonBackdropSymbols.new( f4_local1, controller )
	...
	local ElemsSideList = CoD.cac_ElemsSideList.new( f4_local1, controller )
	ElemsSideList:setLeftRight( true, false, 4.13, 72 )
	ElemsSideList:setTopBottom( true, false, -11, 659 )
	self:addElement( ElemsSideList )
	self.ElemsSideList = ElemsSideList
	
	local WeaponNameWidget0 = CoD.WeaponNameWidget.new( f4_local1, controller )
	...
	if IsPC() then
		f4_local15 = CoD.XCamMouseControl.new( f4_local1, controller )
	else
	...
	f4_local1:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE, nil, function ( f40_arg0, f40_arg1, f40_arg2, f40_arg3 )
		GoBack( self, f40_arg2 )
		ClearMenuSavedState( f40_arg1 )
		SetPerControllerTableProperty( f40_arg2, "customizationType", nil )
		return true
	end, function ( f41_arg0, f41_arg1, f41_arg2 )
		CoD.Menu.SetButtonLabel( f41_arg1, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE, "MENU_BACK" )
		return true
	end, false )
	f4_local1:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_RSTICK_PRESSED, nil, function ( f42_arg0, f42_arg1, f42_arg2, f42_arg3 )
		if CACShowRotatePrompt( self, f42_arg0, f42_arg2 ) then
			return true
		else
			
		end
	end, function ( f43_arg0, f43_arg1, f43_arg2 )
		if CACShowRotatePrompt( self, f43_arg0, f43_arg2 ) then
			CoD.Menu.SetButtonLabel( f43_arg1, Enum.LUIButton.LUI_KEY_RSTICK_PRESSED, "PLATFORM_EMBLEM_ROTATE_LAYER" )
			return true
		else
			return false
		end
