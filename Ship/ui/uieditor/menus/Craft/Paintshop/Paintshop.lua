-- c1d43d99b4e5e67bf271fd79a63b4ac3
...
LUI.createMenu.Paintshop = function ( controller )
	...
	local TitleDotsWidget0 = CoD.TitleDotsWidget.new( f10_local1, controller )
	...
	local MenuFrame = CoD.GenericMenuFrameNoFooter.new( f10_local1, controller )
	...
	local LeftPanel = CoD.FE_ButtonPanelShaderContainer.new( f10_local1, controller )
	...
	local weaponList = LUI.UIList.new( f10_local1, controller, 5, 0, nil, false, false, 0, 0, false, false )
	...
	weaponList:linkToElementModel( weaponList, "isBMClassified", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f11_local0, f10_local1, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS )
	end )
	weaponList:linkToElementModel( weaponList, "isContractClassified", true, function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f12_local0, f10_local1, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS )
	end )
	...
	f10_local1:AddButtonCallbackFunction( weaponList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER", function ( f16_arg0, f16_arg1, f16_arg2, f16_arg3 )
		if not IsSelfModelValueTrue( f16_arg0, f16_arg2, "isBMClassified" ) and not IsSelfModelValueTrue( f16_arg0, f16_arg2, "isContractClassified" ) then
			PaintshopWeaponList_ChooseWeapon( self, f16_arg0, f16_arg2 )
	...
	local paintjobSelector = CoD.PaintjobSelector.new( f10_local1, controller )
	...
	local itemName = CoD.InfoPaneItemName.new( f10_local1, controller )
	...
	local feFooterContainerNOTLobby = CoD.fe_FooterContainer_NOTLobby.new( f10_local1, controller )
	...
	local slotsFull = CoD.PaintshopSlotsFull.new( f10_local1, controller )
	...
	slotsFull:subscribeToGlobalModel( controller, "PerController", "Paintshop.UpdateDataSource", function ( model )
		UpdateSelfElementState( f10_local1, slotsFull, controller )
	end )
	...
	local tabList = CoD.FE_TabBar.new( f10_local1, controller )
	...
	local weaponVerticalCounter = CoD.verticalCounter.new( f10_local1, controller )
	...
	local xcamMouse = CoD.XCamMouseControl.new( f10_local1, controller )
	...
	local FEMenuLeftGraphics = CoD.FE_Menu_LeftGraphics.new( f10_local1, controller )
	FEMenuLeftGraphics:setLeftRight( true, false, 19, 71 )
	FEMenuLeftGraphics:setTopBottom( true, true, 86, -16.75 )
	self:addElement( FEMenuLeftGraphics )
	self.FEMenuLeftGraphics = FEMenuLeftGraphics
	
	local LockIcon = CoD.cac_LockBig.new( f10_local1, controller )
	...
	LockIcon:linkToElementModel( LockIcon, "isBMClassified", true, function ( model )
		f10_local1:updateElementState( LockIcon, {
			name = "model_validation",
			menu = f10_local1,
			modelValue = Engine.GetModelValue( model ),
			modelName = "isBMClassified"
	...
	self:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "Paintshop.Mode" ), function ( model )
		...
		CoD.Menu.UpdateButtonShownState( f62_local0, f10_local1, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE )
	end )
	self:registerEventHandler( "menu_loaded", function ( element, event )
		local f63_local0 = nil
		SendClientScriptMenuChangeNotify( controller, f10_local1, true )
	...
	f10_local1:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE, nil, function ( f64_arg0, f64_arg1, f64_arg2, f64_arg3 )
		if IsModelValueEqualToEnum( f64_arg2, "Paintshop.Mode", Enum.PaintshopMode.PAINTSHOPMODE_WEAPONLIST ) then
			PaintshopWeaponList_Back( self, f64_arg0, f64_arg2 )
			ClearSavedState( self, f64_arg2 )
			SendClientScriptMenuChangeNotify( f64_arg2, f64_arg1, false )
			GoBack( self, f64_arg2 )
			SendClientScriptNotifyForAdjustedClient( f64_arg2, "CustomClass_closed", "" )
			return true
		elseif IsModelValueEqualToEnum( f64_arg2, "Paintshop.Mode", Enum.PaintshopMode.PAINTSHOPMODE_PAINTJOBS ) then
			PaintshopWeaponList_PressBackOnPaintjobsSelector( self, f64_arg0, f64_arg2 )
		...
		if IsModelValueEqualToEnum( f65_arg2, "Paintshop.Mode", Enum.PaintshopMode.PAINTSHOPMODE_WEAPONLIST ) then
			CoD.Menu.SetButtonLabel( f65_arg1, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE, "MENU_BACK" )
			return true
		elseif IsModelValueEqualToEnum( f65_arg2, "Paintshop.Mode", Enum.PaintshopMode.PAINTSHOPMODE_PAINTJOBS ) then
			CoD.Menu.SetButtonLabel( f65_arg1, Enum.LUIButton.LUI_KEY_XBB_PSCIRCLE, "MENU_BACK" )
			return true
		else
			return false
	...
	f10_local1:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, nil, function ( f66_arg0, f66_arg1, f66_arg2, f66_arg3 )
		return true
	end, function ( f67_arg0, f67_arg1, f67_arg2 )
		CoD.Menu.SetButtonLabel( f67_arg1, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "MENU_SELECT" )
		return true
	end, false )
	f10_local1:AddButtonCallbackFunction( self, controller, Enum.LUIButton.LUI_KEY_RSTICK_PRESSED, nil, function ( f68_arg0, f68_arg1, f68_arg2, f68_arg3 )
		return true
	end, function ( f69_arg0, f69_arg1, f69_arg2 )
		CoD.Menu.SetButtonLabel( f69_arg1, Enum.LUIButton.LUI_KEY_RSTICK_PRESSED, "PLATFORM_EMBLEM_ROTATE_LAYER" )
		return true
	end, false )
