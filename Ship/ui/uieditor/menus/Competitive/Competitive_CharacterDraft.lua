-- d27a52852380c911b867ea26850ff73c
...
local PostLoadFunc = function ( f1_arg0, f1_arg1 )
	f1_arg0:subscribeToModel( Engine.CreateModel( Engine.CreateModel( Engine.GetModelForController( f1_arg1 ), "Pregame" ), "clientState" ), function ( model )
		local modelValue = Engine.GetModelValue( model )
		if modelValue == Enum.LobbyClientPregameState.LOBBY_CLIENT_PREGAME_STATE_SELECTING_CHARACTER then
			OpenChooseCharacterLoadout( nil, nil, f1_arg1, LuaEnums.CHOOSE_CHARACTER_OPENED_FROM.DRAFT, f1_arg0 )
		elseif modelValue == Enum.LobbyClientPregameState.LOBBY_CLIENT_PREGAME_STATE_CHARACTER_SELECTION_FORCED then
			local f2_local1 = Engine.GetXUID( f1_arg1 )
			local f2_local2 = Engine.LobbyGetSessionClients( Enum.LobbyModule.LOBBY_MODULE_HOST, Enum.LobbyType.LOBBY_TYPE_GAME )
			for f2_local8, f2_local9 in ipairs( f2_local2.sessionClients ) do
				if f2_local9.xuid == f2_local1 and f2_local9.characterDraft.loadout ~= Enum.heroLoadoutTypes_e.HERO_LOADOUT_TYPE_INVALID then
					local f2_local6 = f2_local9.characterDraft.characterIndex
					local f2_local7 = f2_local9.characterDraft.loadout
					Engine.SetHero( f1_arg1, CoD.CCUtility.customizationMode, f2_local6 )
					Engine.SetHeroLoadoutItem( f1_arg1, CoD.CCUtility.customizationMode, f2_local6, f2_local7 )
					return 
				end
			end
		end
...
LUI.createMenu.Competitive_CharacterDraft = function ( controller )
	...
	Background:subscribeToGlobalModel( controller, "MapInfo", "mapImage", function ( model )
		...
		if mapImage then
			Background:setImage( RegisterImage( mapImage ) )
		end
	end )
	...
	local StartMenuWhiteGlow = CoD.StartMenu_WhiteGlow.new( f3_local1, controller )
	...
	local VSpanel = CoD.FE_ButtonPanel.new( f3_local1, controller )
	...
	local Team2IconAndNameWidgetContainer = CoD.TeamIconAndNameWidgetContainer.new( f3_local1, controller )
	...
	Team2IconAndNameWidgetContainer:subscribeToGlobalModel( controller, "LoadingScreenTeamInfo", "team2FactionName", function ( model )
		...
		if team2FactionName then
			Team2IconAndNameWidgetContainer.TeamIconAndNameWidget.FactionName:setText( Engine.Localize( team2FactionName ) )
		end
	end )
	Team2IconAndNameWidgetContainer:subscribeToGlobalModel( controller, "LoadingScreenTeamInfo", "team2FactionIcon", function ( model )
		...
		if team2FactionIcon then
			Team2IconAndNameWidgetContainer.TeamIconAndNameWidget.FactionIconSolid:setImage( RegisterImage( team2FactionIcon ) )
		end
	end )
	Team2IconAndNameWidgetContainer:subscribeToGlobalModel( controller, "LoadingScreenTeamInfo", "team2FactionIcon", function ( model )
		local team2FactionIcon = Engine.GetModelValue( model )
		if team2FactionIcon then
			Team2IconAndNameWidgetContainer.TeamIconAndNameWidget.FactionIcon:setImage( RegisterImage( team2FactionIcon ) )
		end
	end )
	Team2IconAndNameWidgetContainer:subscribeToGlobalModel( controller, "LoadingScreenTeamInfo", "team2FactionName", function ( model )
		local team2FactionName = Engine.GetModelValue( model )
		if team2FactionName then
			Team2IconAndNameWidgetContainer.TeamIconAndNameWidgetForCodCaster.FactionName:setText( Engine.Localize( team2FactionName ) )
		end
	end )
	Team2IconAndNameWidgetContainer:subscribeToGlobalModel( controller, "LoadingScreenTeamInfo", "team2FactionIcon", function ( model )
		local team2FactionIcon = Engine.GetModelValue( model )
		if team2FactionIcon then
			Team2IconAndNameWidgetContainer.TeamIconAndNameWidgetForCodCaster.FactionIconSolid:setImage( RegisterImage( team2FactionIcon ) )
		end
	end )
	Team2IconAndNameWidgetContainer:subscribeToGlobalModel( controller, "LoadingScreenTeamInfo", "team2FactionIcon", function ( model )
		local team2FactionIcon = Engine.GetModelValue( model )
		if team2FactionIcon then
			Team2IconAndNameWidgetContainer.TeamIconAndNameWidgetForCodCaster.FactionIcon:setImage( RegisterImage( team2FactionIcon ) )
		end
	end )
	...
	local Team1IconAndNameWidgetContainer = CoD.TeamIconAndNameWidgetContainer.new( f3_local1, controller )
	...
	Team1IconAndNameWidgetContainer:subscribeToGlobalModel( controller, "LoadingScreenTeamInfo", "team1FactionName", function ( model )
		...
		if team1FactionName then
			Team1IconAndNameWidgetContainer.TeamIconAndNameWidget.FactionName:setText( Engine.Localize( team1FactionName ) )
		end
	end )
	Team1IconAndNameWidgetContainer:subscribeToGlobalModel( controller, "LoadingScreenTeamInfo", "team1FactionIcon", function ( model )
		...
		if team1FactionIcon then
			Team1IconAndNameWidgetContainer.TeamIconAndNameWidget.FactionIconSolid:setImage( RegisterImage( team1FactionIcon ) )
		end
	end )
	Team1IconAndNameWidgetContainer:subscribeToGlobalModel( controller, "LoadingScreenTeamInfo", "team1FactionIcon", function ( model )
		local team1FactionIcon = Engine.GetModelValue( model )
		if team1FactionIcon then
			Team1IconAndNameWidgetContainer.TeamIconAndNameWidget.FactionIcon:setImage( RegisterImage( team1FactionIcon ) )
		end
	end )
	Team1IconAndNameWidgetContainer:subscribeToGlobalModel( controller, "LoadingScreenTeamInfo", "team1FactionName", function ( model )
		local team1FactionName = Engine.GetModelValue( model )
		if team1FactionName then
			Team1IconAndNameWidgetContainer.TeamIconAndNameWidgetForCodCaster.FactionName:setText( Engine.Localize( team1FactionName ) )
		end
	end )
	Team1IconAndNameWidgetContainer:subscribeToGlobalModel( controller, "LoadingScreenTeamInfo", "team1FactionIcon", function ( model )
		local team1FactionIcon = Engine.GetModelValue( model )
		if team1FactionIcon then
			Team1IconAndNameWidgetContainer.TeamIconAndNameWidgetForCodCaster.FactionIconSolid:setImage( RegisterImage( team1FactionIcon ) )
		end
	end )
	Team1IconAndNameWidgetContainer:subscribeToGlobalModel( controller, "LoadingScreenTeamInfo", "team1FactionIcon", function ( model )
		local team1FactionIcon = Engine.GetModelValue( model )
		if team1FactionIcon then
			Team1IconAndNameWidgetContainer.TeamIconAndNameWidgetForCodCaster.FactionIcon:setImage( RegisterImage( team1FactionIcon ) )
		end
	end )
	...
	local GameModeBackground = CoD.FE_ButtonPanel.new( f3_local1, controller )
	...
	local FEButtonPanel0 = CoD.FE_ButtonPanel.new( f3_local1, controller )
	...
	local StartMenuWhiteGlow0 = CoD.StartMenu_WhiteGlow.new( f3_local1, controller )
	...
	Timer:subscribeToGlobalModel( controller, "PregameGlobal", "timeleft", function ( model )
		...
		if timeleft then
			Timer:setText( Engine.Localize( timeleft ) )
		end
	end )
	...
	TimerStatus:subscribeToGlobalModel( controller, "PregameGlobal", "status", function ( model )
		...
		if status then
			TimerStatus:setText( Engine.Localize( status ) )
		end
	end )
	self:addElement( TimerStatus )
	self.TimerStatus = TimerStatus
	
	local Team1List = LUI.UIList.new( f3_local1, controller, 4, 0, function ( f20_arg0 )
		return Engine.GetModelValue( Engine.GetModel( f20_arg0, "teamString" ) ) == "1"
	...
	local Team2List = LUI.UIList.new( f3_local1, controller, 4, 0, function ( f21_arg0 )
		return Engine.GetModelValue( Engine.GetModel( f21_arg0, "teamString" ) ) == "2"
	end, false, false, 0, 0, false, true )
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( element )
		...
		element.GameModeBackground:close()
		element.FEButtonPanel0:close()
		element.StartMenuWhiteGlow0:close()
		element.Team1List:close()
		element.Team2List:close()
		element.Background:close()
		element.GamemodeIcon:close()
		element.Timer:close()
		element.TimerStatus:close()
