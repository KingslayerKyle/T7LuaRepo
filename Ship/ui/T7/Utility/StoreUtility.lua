-- f4e3ab5cfaf5995715ba70b522bb2929
...
CoD.StoreUtility.GetDWItemsByCategory = function ( f10_arg0, f10_arg1, f10_arg2 )
	...
	for f10_local1 = 1, #f10_local0, 1 do
		local f10_local4 = Engine.TableLookupGetColumnValueForRow( CoD.StoreUtility.InventoryItems.table, f10_local0[f10_local1], CoD.StoreUtility.InventoryItems.productDescColumn )
		if CoD.StoreUtility.IsInventoryItemVisible( f10_arg0, f10_local4 ) then
			local f10_local5 = Enum.StoreProductPurchaseStatus.STORE_PRODUCT_NOT_PURCHASED
			if Engine.GetInventoryItemQuantity( f10_arg0, Engine.TableLookupGetColumnValueForRow( CoD.StoreUtility.InventoryItems.table, f10_local0[f10_local1], CoD.StoreUtility.InventoryItems.itemIdColumn ) ) > 0 then
				f10_local5 = Enum.StoreProductPurchaseStatus.STORE_PRODUCT_PURCHASED
			end
			local f10_local6 = ""
			local f10_local7 = ""
			local f10_local8 = true
			for f10_local12, f10_local13 in ipairs( CoD.BlackMarketUtility.UniqueSpecialistOutfits ) do
				if string.find( f10_local4, f10_local13.specialist ) then
					if Engine.GetInventoryItemQuantity( f10_arg0, f10_local13.bodyId ) > 0 or Engine.GetInventoryItemQuantity( f10_arg0, f10_local13.contractId ) > 0 then
			...
			f10_local9 = Engine.TableLookupGetColumnValueForRow( CoD.StoreUtility.InventoryItems.table, f10_local0[f10_local1], CoD.StoreUtility.InventoryItems.previewImageColumn )
			if f10_local9 == "t7_store_preview_cwl" and CoD.isPS4 then
				f10_local9 = "t7_store_preview_cwl_ps4"
			end
			f10_local10 = Engine.TableLookupGetColumnValueForRow( CoD.StoreUtility.InventoryItems.table, f10_local0[f10_local1], CoD.StoreUtility.InventoryItems.productImageColumn )
			if f10_local10 == "t7_store_product_cwl" and CoD.isPS4 then
				f10_local10 = "t7_store_product_cwl_ps4"
			end
			f10_local11 = Engine.TableLookupGetColumnValueForRow( CoD.StoreUtility.InventoryItems.table, f10_local0[f10_local1], CoD.StoreUtility.InventoryItems.descColumn )
			if CoD.isPS4 and Engine.LocalizeRefExists( f10_local11 .. "_PS4" ) then
				f10_local11 = f10_local11 .. "_PS4"
			end
			if f10_local8 then
				table.insert( f10_arg2, {
					models = {
						...
						previewImage = f10_local9,
						productImage = f10_local10,
						videoHighResRef = f10_local6,
						videoLowResRef = f10_local7,
						purchasestatus = f10_local5,
						serviceLabel = 0,
...
CoD.StoreUtility.IsInventoryItemVisible = function ( f22_arg0, f22_arg1 )
	...
	elseif Dvar.store_hideOutfitContracts:exists() and Dvar.store_hideOutfitContracts:get() == "1" and string.find( f22_arg1, "outfit_contract" ) then
		return false
	elseif Dvar.store_hideOutfitPurchase:exists() and Dvar.store_hideOutfitPurchase:get() == "1" and string.find( f22_arg1, "outfit" ) and not string.find( f22_arg1, "outfit_contract" ) then
		return false
	else
		return f22_local1
	end
		...
		DataSources.PurchaseMapPackMenuList = DataSourceHelpers.ListSetup( "PurchaseMapPackMenuList", function ( f29_arg0, f29_arg1 )
			...
			if f28_arg1.dlcPack == "DLC6" then
				table.insert( f29_local0, f29_local1( Engine.ToUpper( Engine.Localize( "MENU_GO_TO_STORE" ) ), function ( f31_arg0, f31_arg1, f31_arg2, f31_arg3, f31_arg4 )
					GoBackAndOpenStoreBO4OverlayOnParent( f31_arg4, f28_arg1.actionSource, f31_arg2 )
				end, true, false ) )
			else
				table.insert( f29_local0, f29_local1( Engine.ToUpper( Engine.Localize( "MENU_GO_TO_STORE" ) ), function ( f32_arg0, f32_arg1, f32_arg2, f32_arg3, f32_arg4 )
					if CoD.isPC then
						local f32_local0 = GoBack( f32_arg0, f32_arg2 )
						OpenSteamStore( f32_arg0, f32_arg1, f32_arg2, f32_arg3, f32_arg4 )
					else
						GoBackAndOpenStoreDLCOverlayOnParent( f32_arg4, f28_arg1.actionSource, f32_arg2 )
					end
				end, true, false ) )
			end
			table.insert( f29_local0, f29_local1( "MENU_CANCEL_CAPS", function ( f33_arg0, f33_arg1, f33_arg2, f33_arg3, f33_arg4 )
